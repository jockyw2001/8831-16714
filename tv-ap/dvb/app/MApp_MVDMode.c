...
BOOLEAN MApp_MVD_SetMode ( SCALER_WIN eWindow )
{
    ...
    printf("[TIME] MApp_MVD_SetMode start = %ld\n", gU32TmpTime);
#endif
...
#if (ENABLE_CHCHANGETIME)
    gU32TmpTime = msAPI_Timer_DiffTimeFromNow(gU32ChChangeTime);
    printf("[TIME] MApp_MVD_SetMode end = %ld\n", gU32TmpTime);
#endif
...
void MApp_AVCH264_TimingMonitor( SCALER_WIN eWindow )
{
    ...
    if(E_VDEC_RET_NOT_RUNNING == MApi_VDEC_IsAlive())
    {
        u16VdecNotRunCnt++;
    }
    else if (E_VDEC_OK == MApi_VDEC_IsAlive())
    {
        u16VdecNotRunCnt = 0;
    }

    if (u16VdecNotRunCnt == 30000) //Remark: VDEC 状态异常约 15秒后，做切台动作，恢复图像
    {
        printf("\r\n [MST]...... H.264 Vdec Reset Action :%d,u16VdecNotRunCnt:%d\n",MApi_VDEC_IsAlive(),u16VdecNotRunCnt);
        u16VdecNotRunCnt = 0;
...
#if (ENABLE_CHCHANGETIME)
    gU32TmpTime = msAPI_Timer_DiffTimeFromNow(gU32ChChangeTime);
    printf("[TIME] MApp_AVCH264_SetMode start = %ld\n", gU32TmpTime);
#endif
...
//    msAPI_VID_AVCH264GetDispInfoDone();
    ...
    #if (ENABLE_CHCHANGETIME)
    gU32TmpTime = msAPI_Timer_DiffTimeFromNow(gU32ChChangeTime);
    printf("[TIME] MApp_AVCH264_SetMode end = %ld\n", gU32TmpTime);
    #endif
	u32HvdPicCountTimer = msAPI_Timer_GetTime0();
    enMVDVideoStatus = MVD_GOOD_VIDEO;
    g_bSetModeDone = TRUE;
    DEBUG_BOOT_TIME(DEBUG_FUNC_TIME_END(););
    return TRUE;
