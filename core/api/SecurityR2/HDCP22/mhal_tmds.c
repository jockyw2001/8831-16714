...
//PRINT_CURRENT_LINE();
  ...
  #if 0 // U11 only has HDMI 2.0 P0 and HDMI 2.0 P1
    else if((enInputPort == MSCombo_TMDS_PORT_2) || (enInputPort == MSCombo_TMDS_PORT_3))
    {
        if(MDrv_ReadByte(REG_1610CC) & BIT2)     // IRQ status: 66
        {
            MDrv_WriteByteMask(REG_1610CC, 0x04, 0x04); //clear wirte done status
            MDrv_WriteByteMask(REG_1610CC, 0x10, 0x10); //clear write start status
...
//printf("ucDataSize=%d \n", ucDataSize);
    ...
    switch(enInputPort)
    {
        ...
        case MSCombo_TMDS_PORT_3:
            ...
            for ( i = 0; i < ucDataSize; i++ )
            {
                pHDCPRxData[i] = MDrv_ReadByte(REG_101520); // memory to xiu read data (port0): 10
            }
        break;
      #endif
        default:
...
void mhal_tmds_HDCP2GetRxData(MSCombo_TMDS_PORT_INDEX enInputPort, BYTE *pHDCPRxData)
{
    if(enInputPort == MSCombo_TMDS_PORT_0)
        ...
        mhal_tmds_HDCP2GetDataInfo(enInputPort, MDrv_Read2Byte(REG_1610C8) & 0x03FF, pHDCPRxData);  // message length receive: 64
  #endif
    else
    {
        TMDS_PRINT("Port: 0x%x, no need\r\n", enInputPort);
...
BOOL mhal_tmds_HDCP2CheckWriteStart(MSCombo_TMDS_PORT_INDEX enInputPort)
{
    ...
    {
        if(MDrv_ReadByte(REG_1610CC) & BIT4)     // IRQ status: 66
        {
            MDrv_WriteByteMask(REG_1610CC, 0x10, 0x10);    // IRQ write start clear: 66
            MDrv_WriteByteMask(REG_1610CC, 0x00, 0x10);
    ...
    switch(enInputPort)
    {
        case MSCombo_TMDS_PORT_0:
            ...
            if(MessageID == TMDS_HDCP2_MSG_ID_AKE_SEND_CERT)
            {
                ...
                {
//                    U8 u8data;
                    RIU[REG_101508] = i + 1; //set address
...
//                    printf("%x " , *(BYTE*)(pHDCPTxData + i)  ) ;
        ...
        case MSCombo_TMDS_PORT_1:
            ...
            if(MessageID == TMDS_HDCP2_MSG_ID_AKE_SEND_CERT)
            {
				...
				MDrv_WriteByte(REG_100BC6, 0x16);
				MDrv_WriteByteMask(REG_100BC7, 0x2, 0x03);

				//polling read status
				while (MDrv_ReadByte(REG_100B33) & BIT2);
        ...
        case MSCombo_TMDS_PORT_3:
            ...
            if(MessageID == TMDS_HDCP2_MSG_ID_AKE_SEND_CERT)
            {
                MDrv_WriteByte(REG_101512, MessageID);  // xiu to memory wirte data: 09
                for(i = 0; i< 522; i++)
                {
                    MDrv_WriteByte(REG_101512, MIU8((ULONG)pHDCPTxData_CertRx + i));  // xiu to memory wirte data: 09
                }
                for(i = 0; i< DataSize; i++)
                {
                    MDrv_WriteByte(REG_101512, *(BYTE*)(pHDCPTxData + i));
                }
				MDrv_WriteByte(REG_1610C6, 0x16);
				MDrv_WriteByteMask(REG_1610C7, 0x02, 0x03);

				//polling read status
				while (MDrv_ReadByte(REG_161033) & BIT2);

                MDrv_WriteByteMask(REG_1610C9, 0x80, 0x80); // mdssage data length update pulse (sink)
            }
            else
            {
                MDrv_WriteByte(REG_101512, MessageID);  // xiu to memory wirte data: 09
                ...
                MDrv_Write2Byte(REG_1610C6, 1 + DataSize);  //  message datalength wrote by sink: 63 (1 + DataSize)
                MDrv_WriteByteMask(REG_1610C9, 0x80, 0x80); // mdssage data length update pulse (sink)
