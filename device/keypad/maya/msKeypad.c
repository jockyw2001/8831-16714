...
static BOOLEAN msKeypad_CH_GetMultiKey(void)
{
	U8 u8Temp[2];
  U16 i, KEY_LV[ADC_KEY_LEVEL];
  U8 j, Key_Value, *Keymapping=NULL;
  ...
  for (Channel=0; Channel<ADC_KEY_CHANNEL_NUM; Channel++)
  {
    ...
    for (j=0;j<u8CHLVLs[Channel];j++)
    {
      if(abs(Key_Value - tADCKeyLevel[u8ChIdx][j]) < ADCKeyValueOffest)
      {
				KEY_LV[j] = KEYPAD_STABLE_NUM_MIN;
        break;
      }
    }
    ...
    if(u8Temp[0]!=IRKEY_RIGHT)
    	return FALSE;
	}
	if(u8Temp[1]==IRKEY_UP&&u8Temp[0]==IRKEY_RIGHT)
		return TRUE;
	else
		return FALSE;
}
BOOLEAN msKeypad_CheckFactoryKey(void)
{
  if(msKeypad_CH_GetMultiKey()==TRUE)
  {
     return TRUE;
	}
	else
...
//Ray DPW 2017.05.24: Check if MENU key is pressed
//Output: TRUE: MENU key is pressed.  FALSE: no
BOOLEAN msKeypad_CheckMenuKeyPress(void)
{
  U8 u8Temp[2];
  U16 i, KEY_LV[ADC_KEY_LEVEL];
  U8 j, Key_Value, *Keymapping=NULL;
