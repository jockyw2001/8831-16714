...
void Mst_PwmPortInit( void )
{
    ...
    if ( PADS_PWM1_MODE != Unknown_pad_mux )
    {
        PWMPort = E_PWM_CH1;
				...
				{
		      //0x74:400Hz , 0xC2:240Hz , 0x104:180Hz , 0x186:120Hz(For Dimming)
  		    MDrv_PWM_Period(PWMPort, 0x104);//133);	
    		  MDrv_PWM_DutyCycle(PWMPort, 0x00);//INIT_PWM1_DUTY);
					MDrv_PWM_Div(PWMPort, 255);//170);
        }
        else
        {
            //Ray BKL 2017.02.21: Comment the original backlight init code
            /*
    	    MDrv_PWM_Period(PWMPort, PWM1_PERIOD);
  	      MDrv_PWM_DutyCycle(PWMPort, INIT_PWM1_DUTY);
	        MDrv_PWM_Div(PWMPort, 0);
	        */
	        MDrv_PWM_Div(PWMPort, 255);					//Ray BKL 2017.02.21: Divide the PWM freq by 255 as original "0" DIV is too high
	        dv_AdjustBacklightPWMFreq(GET_BACKLIGHT_PWM_FREQ());		//Ray BKL 2017.02.21: Adjust PWM frequency and duty
