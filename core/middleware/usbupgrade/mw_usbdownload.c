...
//#define USB_DL_ASSERT(x)        x	//Ray FWD 2017.01.25: Enable USB firmware download debug
...
static U8 _pu8FileName[25] ={0};		//Ray FWD 2017.02.23: Change USB download file name buffer size from 20 to 25 to accommodate file name "SP4096_E0.00.00.01.BIN"
    ...
    while(msAPI_FCtrl_EntryNext(_u8USBEnvironmentID,pFileEntry)==ENTRY_NEXT_RESULT_SUCCESS)
    {
        ...
        if(lenToCompare==len)
        {
		...
		#endif
			{
	            ...
	            {
	                U16 c1, c2;
	                //Ray FWD 2017.02.23: In firmware file name "SP4096_E0.00.00.01.BIN", we don't check the version no. E0.00.00.01. So we skip i from 7 to 17.
	                if((i<7)||(i>17)){
			  c1 = pu8FileName[i];
			  c2 = pFileNameToCompare[i];
...
static BOOLEAN _MW_UsbDownload_Copy(U32 u32DRAMAddr, U32 u32DRAMLen, U32 u32FlashSize)
{
    ...
    printf("File size: 0x%08lx; Flash size: 0x%08lx; Download Buffer Addr: 0x%08lx; Download Buffer Size: 0x%08lx\n", u32FileSize, u32FlashSize, u32DRAMAddr, u32DRAMLen);
...
#if (ENABLE_SZ_FACTORY_USB_SAVE_DATABASE_FUNCTION == ENABLE || ENABLE_UPGRADE_CIPLUSKEY_BY_USB == ENABLE || ENABLE_UPGRADE_HDCPKEY_BY_USB == ENABLE   || ENABLE_KEY_LOGGER)
