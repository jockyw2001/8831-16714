...
void MApp_PQ_Set_ColorTemp( SCALER_WIN eWindow)
{
    ...
    U8 u8ColorTempIdx = ST_VIDEO.eColorTemp;
...
void MApp_Picture_Setting_SetColor( INPUT_SOURCE_TYPE_t enInputSourceType, SCALER_WIN eWindow )
{
    ...
    GEN_SYS_COLOR_RANGE = EN_COLOR_RANGE_AUTO;		//Ray HDM 2017.03.07: Fix color range as auto color range such that the color range is set according to HMDI AVI packet value

		if(GEN_SYS_COLOR_RANGE == EN_COLOR_RANGE_AUTO)
    {
    ...
    {
				if(GEN_SYS_COLOR_RANGE == EN_COLOR_RANGE_16_235)
        {
            MDrv_PQ_SetColorRange(ePQWin, FALSE);
        }     
				else if(GEN_SYS_COLOR_RANGE == EN_COLOR_RANGE_0_255)
        {
            MDrv_PQ_SetColorRange(ePQWin, TRUE);
        }
    }
