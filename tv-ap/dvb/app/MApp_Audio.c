...
#ifdef sAudioCurveFineTune	
																																																		   0x7F00,    
		0x5E9C,    0x5D38,    0x5BD4,    0x5A71,    0x590D,    0x57A9,    0x5645,    0x54E1,    0x537D,    0x521A,    
		0x50B6,    0x4F52,    0x4DEE,    0x4C8A,    0x4B26,    0x49C3,    0x485F,    0x46FB,    0x4597,    0x4433,    
		0x42CF,    0x416C,    0x4008,    0x3EA4,    0x3D40,    0x3BDC,    0x3A78,    0x3914,    0x37B1,    0x364D,    
		0x34E9,    0x3385,    0x3221,    0x30BD,    0x2F5A,    0x2DF6,    0x2C92,    0x2B2E,    0x29CA,    0x2866,    
		0x2703,    0x259F,    0x243B,    0x22D7,    0x2173,    0x200F,    0x1EAC,    0x1D48,    0x1BE4,    0x1A80,              
		0x1A5F,    0x1A3D,    0x1A1C,    0x19FB,    0x19DA,    0x19B8,    0x1997,    0x1976,    0x1954,    0x1933,    
		0x1912,    0x18F1,    0x18CF,    0x18AE,    0x188D,    0x186C,    0x184A,    0x1829,    0x1808,    0x17E6,    
		0x17C5,    0x17A4,    0x1783,    0x1761,    0x1740,    0x171F,    0x16FD,    0x16DC,    0x16BB,    0x169A,    
		0x1678,    0x1657,    0x1636,    0x1614,    0x15F3,    0x15D2,    0x15B1,    0x158F,    0x156E,    0x154D,    
		0x152C,    0x150A,    0x14E9,    0x14C8,    0x14A6,    0x1485,    0x1464,    0x1443,    0x1421,    0x1400,    		   
#else                                                                                                       
                                                                                                       0x7F00,    //  00
...
BOOLEAN MApp_Audio_SetEWSAudio(MS_EWSAUDIOINFO mEwsAudioinfo)
{
    ...
    stAudioStreamInfo.wAudType = mEwsAudioinfo.u16EWSAudioType;
    stAudioStreamInfo.wAudPID = mEwsAudioinfo.u16EWSAudioPID;

    printf("\r\n>>>MApp_Audio_SetEWSAudio  Type=%x  PID=%x",stAudioStreamInfo.wAudType,stAudioStreamInfo.wAudPID);
...
//******************************************************************************
void MApp_Audio_AdjustMainVolume(BYTE VolumePercent)	//MingYuan Volume
{
    BYTE value1,value2;
    ...
    if(!EAR_PHONE_DETECT())
    {
  	  if(VolumePercent>0)	//Turn un-mute
    	{
  	  	MApp_Audio_MuteOnOff(0);  	
	    }	
  	  else //Turn mute
    	{
	    	MApp_Audio_MuteOnOff(1);
  	  }	
  	}  
  	else
  	{
...
void MApp_Aud_SetTVSpeaker(void)
{
    ...
    {
        if(DB_HDMI_SETTING.g_bHdmiCecDeviceControl == DISABLE)
        {
    ...
    else if (stGenSetting.g_SoundSetting.TVspeakerMode == TVspeaker_Mode_ExtAmp)
    {
      ...
      #if 0   //TV will mute after receive <Set System Audio Mode>["On"]
...
void MApp_Audio_Task_Slow(void)
{
    ...
    s_Audio_Task_Slow_u32LastTime = MsOS_GetSystemTime();
      ...
      if (GetEarphoneState() != PreEarphoneState)
      {
          ...
          {
              EarPhone_OFF();
              MUTE_Off(); //Audio_Amplifier_ON();
          }
          else
          {
              EarPhone_ON();
              MUTE_On(); //Audio_Amplifier_OFF();
          }
      }
    #endif
...
BOOLEAN MApp_Aud_SetMtsMode(void)
{
    ...
    if ( IsDTVInUse() )
    {
        ...
        //20120806 Max add for MTS switch mpeg/ac3
        if(u16AudStreamType == AUD_STREAM_TYPE_MPEG)
        {
            MApi_AUDIO_SetSystem(MSAPI_AUD_DVB_MPEG);
        }
        else if(u16AudStreamType == AUD_STREAM_TYPE_AAC)
            MApi_AUDIO_SetSystem(MSAPI_AUD_DVB_AAC);
        else
        {
            MApi_AUDIO_SetSystem(MSAPI_AUD_DVB_AC3);
...
void MApp_AudioStreamMonitor(void)
{
    if(msAPI_Timer_DiffTimeFromNow(u32MonitorAudioTrackTimer) > AUDIO_SOUNDTRACK_MONITOR_TIMES)
    {
        ...
        if(bReloadAudiotrack)
        {
            ...
            //u16AudioPID = MApp_SetPreferAudioSoundTrackPIDNew();
            u16TempStreamType = MApp_GetAudioStreamType();
            if(fTableAudioNumChanged)
            {
                /* set audio PID & start filter */
            ...
            if(u16TempStreamType == AUD_STREAM_TYPE_MPEG)
            {
                 MApi_AUDIO_SetSystem(MSAPI_AUD_DVB_MPEG);
            }
            else if(u16TempStreamType == AUD_STREAM_TYPE_AAC)
            {
                 MApi_AUDIO_SetSystem(MSAPI_AUD_DVB_AAC);
            }
            else
            ...
            if ( u16TempStreamType != AUD_STREAM_TYPE_AC3)
            {
                MApi_AUDIO_SPDIF_SetMode(MSAPI_AUD_SPDIF_PCM);
            }
            else
        ...
        if (msAPI_Timer_DiffTimeFromNow(start_decode_audio_time) > AUDIO_TRACK_CHECK_WAIT_TIME)
        {
            ...
            if (MApp_GetAudioStreamType() == AUD_STREAM_TYPE_AC3)
            {
                ...
                if (!msAPI_AUD_DECODER_GetMAD_LOCK() && (msAPI_AUD_DECODER_GetMAD_PCMBufLevel(E_AUDIOSTREAM_AC3_)==0))
                {
                    if ( ++u8AudioType > (PreferSoundTrack_Number+1) )
                    {
                        MTS_DBINFO(printf("%d has NO AC3\n", g_u8AudLangSelected));
                        g_u8AudLangSelected++;
                        g_u8AudLangSelected %= MApp_ChanProc_GetCurChannelPointer()->u8AudLangNum;
                        u8AudioType = 0;
                    }
