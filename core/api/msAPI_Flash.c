...
/*                                 Local                                      */
/******************************************************************************/
#define MSIF_FLASH_BUILD_VERSION        0x08
...
#define PRINT_STATUS_REG()  //do{ printf("%u: %s: BP:%X\n", __LINE__, __FILE__, msAPI_Flash_Get_StatusRegister()); } while(0)
...
void msAPI_Flash_Set_WriteProtectDisableStatus(U16 u16Client, BOOL bDisableWP)
{
    ...
    DEBUG_FLASH_WP( printf(" ==> s_Flash_WP_u16WriteProtectDisableStatus=%X\n", s_Flash_WP_u16WriteProtectDisableStatus); );
}
...
void msAPI_Flash_WriteProtect_SetBPStatus(void)
{
    ...
    msAPI_Flash_Set_WriteProtect_Disable_Range(SYSTEM_BANK_SIZE * QUICK_DB_GENSETTING_BANK, (FLASH_SIZE - QUICK_DB_GENSETTING_BANK*SYSTEM_BANK_SIZE));
...
void msAPI_Flash_Set_WP_DB_CH(BOOL bEnable )
{
    ...
    {
        if( g_bFlashWriteProtectMethodUseM12 )
        {
        }
        else
        {
...
void MApp_Printf_Flash_Variable(void)
{
    ...
    //printf(" g_bFlashWriteProtectMethodUseM12: %d\n",g_bFlashWriteProtectMethodUseM12);
...
{
    //printf("msAPI_Flash_Set_WriteProtect_Disable_Range(u32DisableLowerBound=%X, u32DisableSize=%X\n)\n", u32DisableLowerBound, u32DisableSize);
    ...
    //printf(" => Flash Status=0x%X\n", msAPI_Flash_Get_StatusRegU16());

    return TRUE;
#else
    return MDrv_FLASH_WriteProtect_Disable_Range_Set(u32DisableLowerBound, u32DisableSize);
#endif
}
