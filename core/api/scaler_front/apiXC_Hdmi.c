...
BOOLEAN MApi_XC_HDMI_CheckSumAndBCH(INPUT_SOURCE_TYPE_t enInputSourceType)
{
    ...
    {
        printf("\n ChecksumErrOrBCHParityErr: Count=%u at %u\n", g_HdmiPollingStatus.u8LostHDMICounter, MsOS_GetSystemTime());
...
void MApi_XC_HDMI_Packet_State(MS_HDMI_PACKET_STATE_t u8state)
{
    U16 u16MuteFlag;
    U8 u8ACPType;
    static U8 u8UnMuteStableCount=0,u8MuteStableCount=0;
...
void MApi_XC_HDMI_SetHotPlug(MS_BOOL bEnable, E_MUX_INPUTPORT enInputPortType)
{
    ...
    DEBUG_HDMI_HOT_PLUG(printf("MApi_XC_HDMI_SetHotPlug(en=%u, port=%u)\n", bEnable, enInputPortType););
    ...
    DEBUG_HDMI_HOT_PLUG(printf(" => call MDrv_HDMI_pullhpd( bEnable=%u, enInputPortType=%u, bInverse=%u)\n", bEnable, enInputPortType, bInverse););
...
XC_HDMI_Result MApi_XC_HDMI_Init(XC_HDMI_InitData *pstXC_HDMI_InitData, MS_U32 u32InitDataLen)
{
  ...
  #if ((MS_BOARD_TYPE_SEL == BD_MST222B_D01A_S)||(MS_BOARD_TYPE_SEL == BD_MST084B_10AHB_15043))
    Mdrv_HDMI_SetPackageType(E_PACKAGE_TYPE_QFP156_UHD);
  #elif (MS_BOARD_TYPE_SEL == BD_MST222A_D01A_S) // Maya
    Mdrv_HDMI_SetPackageType(E_PACKAGE_TYPE_QFP176_UHD);
  #else
...
BOOLEAN mApi_XC_HDMI_DetectEdidVersion(INPUT_SOURCE_TYPE_t enInputSourceType)
{
    ...
    //if(enInputSourceType == INPUT_SOURCE_HDMI)
    {
        ...
        if(u8ret == EDID_14_ver)
        {
            printf("=> EDID_14_ver\n");
            /*
            stGenSetting.g_SysSetting.u8EDID_select = EDID_1p4;
            */
            MApi_XC_Sys_Reload_Edid(enInputSourceType, EDID_1p4);
        }
        ...
        {
            DEBUG_EDID_SWITCH( printf("=> UNKNOW\n"); );
            // Do nothing.
        }
    }
