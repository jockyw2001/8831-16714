...
#define MEM_CHECK(addr) do  \
                        {   \
                            if ((addr < ((PVR_BROWSER_BUFFER_MEMORY_TYPE & MIU1) ? (PvrGetBrowBufAddr() | MIU_INTERVAL) : PvrGetBrowBufAddr())) || ((addr+sizeof(PVR_BROWSER_ENTRY)))>(((PVR_BROWSER_BUFFER_MEMORY_TYPE & MIU1) ? (PvrGetBrowBufAddr() | MIU_INTERVAL) : PvrGetBrowBufAddr())+PVR_BROWSER_BUFFER_LEN) )\
                            {\
                                __ASSERT(0); \
...
static S8 _PB_CompareEntry( PVR_BROWSER_ENTRY *pNewEntry, U16 u16SlotItem, SORT_TYPE sortType)
{
   ...
   switch(sortType)
   {
    ...
    #ifdef PVR_UTOPIA
        #if OBA2
            memcpy((void *)(tmpBuf.u16FileName), (void *)(addr), sizeof(tmpBuf.u16FileName));
        #else
    ...
    #ifdef PVR_UTOPIA
        #if OBA2
        memcpy((void *)(tmpBuf.u16ProgramName), (void *)(addr), sizeof(tmpBuf.u16ProgramName));
        #else
    ...
    #ifdef PVR_UTOPIA
        #if OBA2
        memcpy((void *)(tmpBuf.u16ChannelName), (void *)(addr), sizeof(tmpBuf.u16ChannelName));
        #else
    ...
    #ifdef PVR_UTOPIA
        #if OBA2
        memcpy((void *)(&tmpBuf.u64FileLength), (void *)(addr), sizeof(tmpBuf.u64FileLength));
        #else
...
#ifdef PVR_UTOPIA
    #if OBA2
    *(U16 *)addr = *(U16 *)pu16NextSlot;
//    memcpy((void *)addr, (void *)pu16NextSlot, sizeof(U16));
    #else
...
#ifdef PVR_UTOPIA
    #if OBA2
    memcpy((void *)addr, (void *)(pu16PrevSlot), sizeof(U16));
    #else
...
static U16 _PB_AddEntryToDB(PVR_BROWSER_ENTRY *pNewEntry)
{
    ...
    #if OBA2
    memcpy((void *)addr, (void *)pNewEntry, sizeof(PVR_BROWSER_ENTRY));
    #else
...
static E_PVR_RET _PB_PrintAllEntryData(SORT_TYPE sortList)
{
    ...
    while (idx != PVR_BROWSER_RESERVED_ENTRY_INDEX)
    {
        ...
        MEM_CHECK(addr);
        #if OBA2
        memcpy((void *)(&entry), (void *)addr, sizeof(PVR_BROWSER_ENTRY));
        #else
...
static E_PVR_RET _PB_BrowserInit(void)
{
    ...
    U8 i,j;
#ifdef MIPS_CHAKRA //[mips]
    _PB_ADDR_RESERVE = (U32)PvrGetBrowBufAddr() + (U32)PVR_BROWSER_RESERVED_ENTRY_INDEX * sizeof(PVR_BROWSER_ENTRY);
#endif
...
static BOOLEAN _PB_SaveFAT32FileInfo(U16 *FileName, U32 EntryAddr)
{
    ...
    else
    {
        ...
        #if OBA2
            memcpy((void *)(PvrGetWriteMetadataBufAddr() + PVR_TS_PACKET_SIZE*(u8Segment+RecordPathSeg) + PVR_TS_PACKET_HEADER_SIZE),  (void *)(EntryAddr+184*u8Segment), u8WriteLength);
        #else
        ...
        if (msAPI_PVRFS_GetFirstPVRFile(u8EnvironmentID, &fileEntry))
        {
            do
            {
                ...
                else
                {
                    ...
                    #ifdef PVR_UTOPIA
                    #if OBA2
                        memcpy((void *)((U32)&E+184*u8Segment), (void *)(PvrGetReadMetadataBufAddr()+PVR_TS_PACKET_SIZE*(u8Segment+RecordPathSeg) + PVR_TS_PACKET_HEADER_SIZE), u8ReadLength);
                    #else
                    ...
                    if (E.u32Duration == 0)
                    {
                        ...
                        E.u32Duration = msAPI_PVR_PlaybackGetTotalTimeByTimeStamp(u8fHandler)/10;
                    #endif
                    }
...
static E_PVR_RET _PB_GetFileAttribute(PVR_FILE_ATTRIBUTE_MEMBER AttribType, U16 PhysicalIndex, BYTE *pBuffer, const U16 BufLen)
{
    ...
    MEM_CHECK(addr);
    #if OBA2
    memcpy((void *)(&E), (void *)addr, sizeof(PVR_BROWSER_ENTRY));
    #else
...
E_PVR_RET MApp_PvrBrowser_GetFileAttributes(SORT_TYPE sortType, U16 LogicalIndex, PVR_UI_BROWSER_ENTRY *E)
{
    ...
    MEM_CHECK(addr);
    #if OBA2
    memcpy((void *)(&pvrEntry), (void *)addr ,sizeof(PVR_BROWSER_ENTRY));
    #else
...
U16 MApp_PvrBrowser_SetAttributeByFileName(PVR_FILE_ATTRIBUTE_MEMBER AttribType, U16 *FileName, void *para)
{
 ...
 //it should rewrite the search algorithm later to speed up the performance
    ...
    switch(AttribType)
    {
        ...
        #ifdef PVR_UTOPIA
            #if OBA2
            memcpy((void *)addr, (void *)(&pvrEntry), sizeof(PVR_BROWSER_ENTRY));
            #else
