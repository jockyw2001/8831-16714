...
static code TimeZone2Country stTimeZone2Country[]=
{
    ...
    {SI_TIMEZONE_BUCURESTI,    SI_COUNTRY_RUMANIA},
...
static code ST_ISO639_LANG iso639_LanguageCode[] =
{
    ...
    {{'q','a','a'}, SI_LANGUAGE_QAA},   /* qaa :: Original Sound Track */
...
static code U8 iso3166_CountryCode[][4]=
{
    ...
    {'R','U','M',SI_COUNTRY_RUMANIA},      //Rumania
...
U8 msAPI_SI_RemoveQuickInstallMismatchedProgram(SI_SHORT_DTV_CHANNEL_INFO *pastVirtualCh,U8 u8NumOfVch,BYTE bIDIndex, MEMBER_SI_SERVICETYPE eServiceType)
{
    ...
    U8 u8i;
    U8 u8Result=0;
    wServiceID=(WORD*)msAPI_Memory_Allocate(sizeof(WORD)*SI_MAX_VC_PER_PHYSICAL,(EN_BUFFER_ID)0);
    if(wServiceID)
    {
    ...
    if(wServiceID)msAPI_Memory_Free(wServiceID,(EN_BUFFER_ID)0);
    return u8Result;
}
#endif
...
BOOLEAN msAPI_SI_FillProgramDataWithDefault(SI_DTV_CHANNEL_INFO *pstDtvPgmData)
{
    ...
    if(pstDtvPgmData)
    {
        ...
        {
            pstDtvPgmData->bChannelName[i] = NULL;
        }
        return TRUE;
    }
    return FALSE;
...
BOOLEAN msAPI_SI_AddProgramIDTable(SI_DTVPROGRAMID *pstSI_DtvIDTable, WORD *u8IDIdex)
{
    ...
    if (NULL == pstSI_DtvIDTable || NULL == u8IDIdex)
        return FALSE;
...
#if ENABLE_DVBC
    if((FALSE == msAPI_SI_GetOpMode()) && (SI_UNCONFIRMED_PHYSICAL_CHANNEL_NUMBER!=stDtvIDTable->cRFChannelNumber)&&(stDtvIDTable->u32Frequency==0))
    {
        U8 u8PhNum=0;
    ...
    switch(eMember)
    {
        ...
        case E_SI_DATA_ALL:
            ...
            for(i=0;i<maxAudio;i++)
            {
                ...
                printf("\n[[In1_%x  out_%x",stDtvPgmData.stAudInfo[i].wAudType,ChannelInfo.stAudInfo[i].wAudType);
...
{
    return msAPI_Memory_Allocate(u16Number_of_bytes,(EN_BUFFER_ID)0);
}
U8 msAPI_SI_Memory_Free(void *pFree)
{
    return msAPI_Memory_Free(pFree,(EN_BUFFER_ID)0);
}
...
BOOLEAN msAPI_SI_IsSpecificCountry(EN_SI_COUNTRY_SETTING eCountry, E_SI_SPECIFIC_COUNTRY eSpecificCountry)
{
    switch(eSpecificCountry)
    {
        ...
        case SI_MALAYSIA_SPECIFIC_COUNTRY:
            ...
            return FALSE;
        case SI_GERMANY_SPECIFIC_COUNTRY:
        ...
        case SI_EUROPE_SPECIFIC_COUNTRY:
            if ((eCountry == SI_COUNTRY_AUSTRIA) || (eCountry == SI_COUNTRY_BELGIUM)
                ...
                || (eCountry == SI_COUNTRY_PORTUGAL) || (eCountry == SI_COUNTRY_RUMANIA)
...
BOOLEAN msAPI_SI_Action_CheckServiceInfo_DVB( SI_DTV_CHANNEL_INFO *pstSI_DTV_CHANNEL_INFO )
{
    ...
    #if (ENABLE_CI_PLUS)
    if((FALSE == msAPI_SI_GetOpMode()) && (!msAPI_SI_IsSpecificCountry( MApp_GetSICountry(OSD_COUNTRY_SETTING), SI_DTG_SPECIFIC_COUNTRY )) )
    #else
    if((!msAPI_SI_IsSpecificCountry( MApp_GetSICountry(OSD_COUNTRY_SETTING), SI_DTG_SPECIFIC_COUNTRY )) )
    #endif
    {
...
#if(ASTRA_HD_ENABLE)
BOOLEAN msAPI_SI_SaveServiceListInfo(const U8 *pu8Dest, U16 u16ServiceListID, BOOLEAN bListInFlag)
{
    return msAPI_CM_SaveServiceListInfo(pu8Dest, u16ServiceListID, bListInFlag);
}
...
{
    return msAPI_SI_GetCurrentServiceListID();
}
#endif //#if(ASTRA_HD_ENABLE)
