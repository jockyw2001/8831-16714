...
U8 sIsPollingSwitch = 0;
#endif*/
extern THR_TBL_TYPE code AuSifInitThreshold[];
extern void MApp_PCMode_SetFirstNoSignalSource(E_UI_INPUT_SOURCE eUiInputSource);	//Ray DMP 2017.04.07

#if(ENABLE_S2)
extern BOOL  msAPI_Get_CurrentTuningState(void);
#endif
...
#elif ((CHIP_FAMILY_TYPE == CHIP_FAMILY_MILAN)&&(ENABLE_VE))
    ...
    //CAL_TIME_FUNC_START();
        ...
        switch (enInputSourceType)
        {
           ...
           case INPUT_SOURCE_DVI:
                    ...
                    //MApi_AUDIO_SetSourceInfo(E_AUDIO_INFO_DTV_IN);
                    //msAPI_AUD_AdjustAudioFactor(E_ADJUST_CHANGE_AUDIOSOURCE, E_AUDIOSOURCE_DVI, 0);
                    msAPI_AUD_AdjustAudioFactor(E_ADJUST_CHANGE_AUDIOSOURCE, E_AUDIOSOURCE_PC, 0);		//Ray DVI 2017.04.06: Change audio source to line in
...
#else
    else if( IsDTVInUse()||IsStorageInUse())
#endif
    {
    }
    else
...
//        msAPI_VID_SetVOPClk( DISABLE );
    ...
    if( IsSrcTypeDTV(SYS_INPUT_SOURCE_TYPE(eWindow)) )
    {
    #if (ENABLE_T_C_COMBO )//TODO need add DVB-C case
      #if (DVB_T_C_DIFF_DB)
        //MApp_ZUI_ACT_SetFrontEndType(UI_INPUT_SOURCE_TYPE);
      #endif
...
static void MApp_InputSource_InitModeVariables( SCALER_WIN eWindow )
{
      ...
      )
    {
        ...
        {
            MApi_XC_Sys_ResetDVIPort(SYS_INPUT_SOURCE_TYPE(eWindow));
        }
    }
...
void _MApp_ChangeSrc_Step_02_DestroyPreSource( E_UI_INPUT_SOURCE enUiInputSourceType,
                                                               SCALER_WIN eWindow )
{
    INPUTSOURCE_DBG( printf("%s(UiSrcTyp=%u, win=%u)\n", __FUNCTION__,enUiInputSourceType, eWindow); );
    ...
    if(g_stFactorySetting.stFacMiscSetting.eFactoryExecuteScript== EN_FACTORY_EXECUTE_SCRIPT_ON)
    {
        ...
        {
            msUS_ScriptExecute(EN_SCRIPT_EVENT_INPUTSOURCE_SWITCHSOURCE);
        }
    }
#endif
    ...
    {
        MApp_DMP_Exit();
        MApp_PCMode_SetFirstNoSignalSource(UI_INPUT_SOURCE_NONE);	//Ray DMP 2017.04.07: Reset no signal source as NONE when exiting DMP
    }
#endif
...
#if ENABLE_DTV
    //if (eOldUiInputSourceType <= UI_INPUT_SOURCE_DTMB)
...
#else
  #if (ENABLE_DVB)
    msAPI_DTV_Comm_Set_DVBTorCType((enUiInputSourceType == UI_INPUT_SOURCE_DVBC));
  #endif
#endif
                                               ...
                                               SCALER_WIN eWindow )
{
    ...
    S16 s16PathId;
		...
		//printf("MingYuan Presource MApp_InputSource_SwitchSource %d\n",enUiInputSourceType);
    ...
    //printf(" >> MApp_InputSource_SwitchSource(%u[%s]) \n", enUiInputSourceType, MApp_InputSrc_Get_UiInputSrcName(enUiInputSourceType));
...
#if (ENABLE_ATSC)
    if( enUiInputSourceType == UI_INPUT_SOURCE_ATSC )
    {
  ...
  #if 0//ENABLE_S2
    else if(enUiInputSourceType != UI_INPUT_SOURCE_S2)
    {
        if(IsS2InUse())
        {
            stGenSetting.g_SysSetting.fPreSrcIsDvbs = TRUE;
        }
        else if(IsAtscInUse() || IsATVInUse())
        {
            stGenSetting.g_SysSetting.fPreSrcIsDvbs = FALSE;
        }
    }
  #endif
#endif
    ...
    _MApp_ChangeSrc_Step_02_DestroyPreSource(enUiInputSourceType, eWindow);


    DEBUG_CHG_SRC_TIME2();

// move to  _MApp_ChangeSrc_Step_03_SetupNewSource()
#if 0//ENABLE_PVR
    if( (enUiInputSourceType == UI_INPUT_SOURCE_DTV)
      #if(ENABLE_S2)
      ||(enUiInputSourceType == UI_INPUT_SOURCE_S2)
      #endif
      )
    {
        if ( !IsDTVInUse() ) // non-DTV to DTV
        {
            _MApp_ChangeSrc_Init_PVR();
        }
    }
    ...
    /////////////////////////////////////////
    {
        ...
        {
            SIGNAL_PATH_DBG(printf("\r\n Delete path success src =%d dst=%d ",SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW),OUTPUT_SCALER_MAIN_WINDOW ));
        }
        ...
        {
            u8PreCVBS1OutSource = _MAPP_GET_CVBSOUT_VIDEO_SOURCE(CVBS1_OUTPUT_Tbl, SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW));
            #if (ENABLE_CVBSOUT_2)
            u8PreCVBS2OutSource = _MAPP_GET_CVBSOUT_VIDEO_SOURCE(CVBS2_OUTPUT_Tbl, SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW));
            #endif
        }
      #endif
    #else
        ...
        ******************************************************************************/
        DEBUG_CVBS_OUT( printf("PWS.DEL.CurSrc(0x%X)\n", _MAPP_InputSource_Source2PWSSource( SYS_INPUT_SOURCE_TYPE(eWindow) ) ); );
    ...
    DEBUG_CHG_SRC_TIME2();
      ...
      if (IsSrcTypeDTV(SYS_INPUT_SOURCE_TYPE(eWindow)))
      {
        ...
        //if (IsSrcTypeATV(SYS_INPUT_SOURCE_TYPE(eWindow)))
        {
            ...
            DEBUG_CHG_SRC_TIME2();
        		#if (ENABLE_PIP)
            if((eWindow == MAIN_WINDOW &&(IsSrcTypeDTV(SYS_INPUT_SOURCE_TYPE(SUB_WINDOW))))
            ||(eWindow == SUB_WINDOW &&(IsSrcTypeDTV(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW))||IsSrcTypeStorage(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)))))
            {
                //printf("\r\n main or sub window is DTV/Storage  source ");
            }
            else
        		#endif
            {
...
*/
    ...
    DEBUG_CVBS_OUT( printf("PWS.Add. CurSrc(0x%X)\n", _MAPP_InputSource_Source2PWSSource( stSystemInfo[eWindow].enInputSourceType )); );
    ...
    {
        if ( eWindow == MAIN_WINDOW )
        {    printf("\nRay:path_thread=MApp_PC_MainWin_Handler()\n");		//Ray DBG 2017.03.14
...
******************************************************************************/
    ...
    if(IsSrcTypeStorage(stSystemInfo[eWindow].enInputSourceType))
    {
        ...
        if (s16PathId == -1)
        {
            printf(" Create DMP path fail src = %d  dest = %d \n",PathInfo.src ,PathInfo.dest );
        }
        else
        {
            printf(" Create CVBS1 path success src = %d  dest = %d \n",PathInfo.src ,PathInfo.dest );
            MApi_XC_Mux_EnablePath( (U16)s16PathId );
        }
    }
    */
...
#if( (CHIP_FAMILY_TYPE == CHIP_FAMILY_A7)&&((MEMORY_MAP == MMAP_64MB) || (defined(SCALER_DNR_BUF_PREV_ADR)))) //Brazil 64MB model
    if ( IsSrcTypeStorage(stSystemInfo[eWindow].enInputSourceType) )
    {
...
#if 0
    //SCART out ATV even if no signal 20100421EL
  #if( (CHIP_FAMILY_TYPE == CHIP_FAMILY_EDEN)  || \
        (CHIP_FAMILY_TYPE == CHIP_FAMILY_NASA)  || \
        (CHIP_FAMILY_TYPE == CHIP_FAMILY_EULER))
    if(IsSrcTypeAnalog(SYS_INPUT_SOURCE_TYPE(eWindow))||IsSrcTypeHDMI(SYS_INPUT_SOURCE_TYPE(eWindow)))  //Component, HDMI, and VGA
    {
...
void MApp_InputSource_ChangeInputSource_2(SCALER_WIN eWindow)
{
    ...
    INPUTSOURCE_DBG( printf(" SYS_INPUT_SOURCE_TYPE(eWindow)=0x%X\n", SYS_INPUT_SOURCE_TYPE(eWindow)); );
  ...
  #if 0//(ENABLE_SW_CH_FREEZE_SCREEN)
    if(stGenSetting.stTvSetting.u8SwitchMode == ATV_SWITCH_CH_FREEZE_SCREEN)
        msAPI_Scaler_SetFreezeScreen(DISABLE, 0, eWindow);
  #endif
    ...
    if(MApp_Get_PIPMode() != EN_PIP_MODE_OFF)
    {
        //printf("\r\n PIP Mode ,Source Switch According to the respective window ");
    }
    else
   #endif
    {
    ...
    if(stGenSetting.stTvSetting.u8SwitchMode == ATV_SWITCH_CH_FREEZE_SCREEN)
        msAPI_Scaler_SetFreezeScreen(DISABLE, 0, eWindow);
   #endif
    printf("\nRay:MApp_InputSource_ChangeInputSource_2\n");		//Ray DBG 2017.03.14
    MApp_InputSource_SwitchSource( source_type, eWindow);
   ...
   //Ray DMP 2017.04.07: If we change input source to DMP, we need to joint down last source before entering into DMP
      if(source_type == UI_INPUT_SOURCE_DMP){
  	UI_INPUT_DMP_PRE_SOURCE = MApp_InputSource_GetRecordSource();
      }
      else{
  	UI_INPUT_DMP_PRE_SOURCE = UI_INPUT_SOURCE_NONE;	//Else, if we just switch to other source, clear last source to none
      }
...
void MApp_InputSource_ScartIOMonitor(void)
{
...
#endif
#ifdef MTC_FIX_BUG
    switch (MApp_TopStateMachine_GetTopState())
    {
    ...
    if(msAPI_Get_CurrentTuningState()==FALSE)
    {
        return;
    }
#endif
...
void MApp_AISDebug(U8 bSrcCnt)
{
    if((bSrcCnt == UI_INPUT_SOURCE_ATV) ||(bSrcCnt == UI_INPUT_SOURCE_DTV))
        AIS_Debug(printf("*TV*"));
#if (ENABLE_ATSC)
    else if(bSrcCnt ==UI_INPUT_SOURCE_TV)
        AIS_Debug(printf("*TV*"));
#endif
#if (INPUT_YPBPR_VIDEO_COUNT>=1)
...
#if (ENABLE_ATSC)
         || (i==UI_INPUT_SOURCE_TV||i==UI_INPUT_SOURCE_DTV)
#endif
     #if (INPUT_AV_VIDEO_COUNT >= 1)
