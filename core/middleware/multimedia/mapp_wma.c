...
#ifndef WMA_INFO_READ_LEN
#define WMA_INFO_READ_LEN MP3_INFO_READ_LEN
#endif

#ifndef WMA_INFO_READ_ADR
#define WMA_INFO_READ_ADR MP3_INFO_READ_ADR
#endif


#ifndef WMA_INFO_READ_MEMORY_TYPE
#define WMA_INFO_READ_MEMORY_TYPE MP3_INFO_READ_MEMORY_TYPE
#endif
            ...
            do
            {
                if(_u32CurrentPayload%stWmaInfo.u8NumPayloads == 0) //Handle packet
                {
                    ...
                    do
                    {
                        ...
                        //re-calculate packet header
                        if(0x100 == msAPI_DataStreamIO_Read(u32WmaFileHandle, (void*)((WMA_INFO_READ_MEMORY_TYPE & MIU1) ? (WMA_INFO_READ_ADR | MIU_INTERVAL) : (WMA_INFO_READ_ADR)), 0x100))
                        {
                            //packet header maximum size: 0x100 byte
                            WMA_DBG(printf("No data to read.\n"));
                            return u32OriReqBytes;;
                        }
                        stWmaInfo.u16PacketOffset = MApp_WMA_ParsingASFPacketHeader(_PA2VA(((WMA_INFO_READ_MEMORY_TYPE & MIU1) ? (WMA_INFO_READ_ADR | MIU_INTERVAL) : (WMA_INFO_READ_ADR))),&stWmaInfo);
#endif
                ...
                u32SeekOffset = msAPI_DataStreamIO_Tell(u32WmaFileHandle);
                //re-calculate payload header , payload size.
                msAPI_DataStreamIO_Read(u32WmaFileHandle, (void*)((WMA_INFO_READ_MEMORY_TYPE & MIU1) ? (WMA_INFO_READ_ADR | MIU_INTERVAL) : (WMA_INFO_READ_ADR)), 0x100);  //payload header maximum size: 0x100
                stWmaInfo.u16PayloadOffset = MApp_WMA_ParsingASFPayloadHeader(_PA2VA(((WMA_INFO_READ_MEMORY_TYPE & MIU1) ? (WMA_INFO_READ_ADR | MIU_INTERVAL) : (WMA_INFO_READ_ADR))),&stWmaInfo);
#endif
                if(stWmaInfo.u16PayloadOffset == (U16)0xFFFFFFFF)
                {
                    WMA_INFO(printf("payload error! exit play.\n"));
...
static BOOLEAN MApp_WMA_CollectInfo(ST_WMA_INFO *pWmaInfo)
{
    U32 u32Index = 0;
    U32 i,j;
    U16 u16TitleLength, u16AuthorLength;
        ...
        for(u32Index=0;u32Index<(pWmaInfo->u32HeaderSize+0x100);u32Index++,i++)     //u32HeaderSize + 0x100 to parsing Data Object
        {
                ...
                (pu8Data[i+12]==0x0C)&&(pu8Data[i+13]==0x20)&&(pu8Data[i+14]==0x53)&&(pu8Data[i+15]==0x65))
            {
                ...
                (pu8Data[i+36] == 0x5F) && (pu8Data[i+37] == 0x5C) && (pu8Data[i+38] == 0x44) && (pu8Data[i+39] == 0x2B))
                {
                    ...
                    if(pWmaInfo->u32SampleRate > 48000)
                    {
                        //Don't support  >48000 sampling rate.
                        _enWMA_ErrCode = E_WMA_ERR_NOT_SUPPORT_SAMPLE_RATE;
                        return FALSE;
                    }
            ...
            if(!pWmaInfo->bContentFound)
            {
                ...
                )
                {
                    ...
                    memcpy(&u16TitleLength,&pu8Data[i+0x18],2);
                    memcpy(&u16AuthorLength,&pu8Data[i+0x1a],2);
                    WMA_INFO(printf("u16TitleLength = %u\n",u16TitleLength));
                    WMA_INFO(printf("u16AuthorLength = %u\n",u16AuthorLength));
                    if((i+u16TitleLength+u16AuthorLength+0x23) > (WMA_HEADER_READ_LEN-1))
                    {
                    #if ENABLE_FILE_BM
                        msAPI_DataStreamIO_MID_FS_Seek(u32Index);
                        msAPI_DataStreamIO_MID_FS_Read(u32WmaFileHandle, WMA_HEADER_READ_LEN, &u32DestAddr);
                    ...
                    for(j=0;j<u16TitleLength;j=j+2)
                    {
                    ...
                    for(j=0;j<u16AuthorLength;j=j+2)
                    {
                        pWmaInfo->u8Author[j+1] = pu8Data[i+j+0x22+u16TitleLength];
                        pWmaInfo->u8Author[j+2] = pu8Data[i+j+0x23+u16TitleLength];
                    }
                    ...
                    for(u32Skip=0x10;u32Skip<u32HeaderSize;u32Skip++)
                    {
                        ...
                        if(!pWmaInfo->bHE_MetadataObjectFound)
                        {
                            ...
                            )
                            {
                                ...
                                memcpy(&u16Attribute,&pu8Data[i+u32Skip+0x18],2);
                                ...
                                if(u16Attribute==6)
                                {
                                    ...
                                    u32Skip += 0xC;
                                    u32Skip += (u16NameLength+u32DataLength);
                                    ...
                                    memcpy(&pWmaInfo->u32WMADRCPeakReference,&pu8Data[i+u32Skip],4);
                                    ...
                                    u32Skip += 0xC;
                                    u32Skip += u16NameLength;
                                    memcpy(&pWmaInfo->u32WMADRCPeakTarget,&pu8Data[i+u32Skip],4);
                                    u32Skip += u32DataLength;
                                    ...
                                    u32Skip += 0xC;
                                    u32Skip += u16NameLength;
                                    memcpy(&pWmaInfo->u32WMADRCAverageReference,&pu8Data[i+u32Skip],4);
                                    u32Skip += u32DataLength;
                                    ...
                                    u32Skip += 0xC;
                                    u32Skip += u16NameLength;
                                    memcpy(&pWmaInfo->u32WMADRCAverageTarget,&pu8Data[i+u32Skip],4);
                                    u32Skip += u32DataLength;
...
////////////////////////////////////////////////////////
    ...
    for(_u32CurrentPacket=0;_u32CurrentPacket< stWmaInfo.u32TotalPacket;_u32CurrentPacket++)
    {
        ...
        //re-calculate packet header
        if(0 != msAPI_DataStreamIO_Read(u32WmaFileHandle, (void*)((WMA_INFO_READ_MEMORY_TYPE & MIU1) ? (WMA_INFO_READ_ADR | MIU_INTERVAL) : (WMA_INFO_READ_ADR)), 0x100))
        {
                //Packet header maximum size: 0x100 byte
        WMA_DBG(printf("No data to read.\n"));
        goto CreateIndexErr;
        ...
        stWmaInfo.u16PacketOffset = MApp_WMA_ParsingASFPacketSCR(_PA2VA(((WMA_INFO_READ_MEMORY_TYPE & MIU1) ? (WMA_INFO_READ_ADR | MIU_INTERVAL) : (WMA_INFO_READ_ADR))),&stWmaInfo);
#endif
...
#else
            msAPI_Steam_Seek(u32WmaFileHandle, u32SeekOffset);
            msAPI_DataStreamIO_Read(u32WmaFileHandle, (void*)((WMA_INFO_READ_MEMORY_TYPE & MIU1) ? (WMA_INFO_READ_ADR | MIU_INTERVAL) : (WMA_INFO_READ_ADR)), 0x100);   //payload header maximum size is 0x100
            stWmaInfo.u16PayloadOffset = MApp_WMA_ParsingASFPayloadPTime(_PA2VA(((WMA_INFO_READ_MEMORY_TYPE & MIU1) ? (WMA_INFO_READ_ADR | MIU_INTERVAL) : (WMA_INFO_READ_ADR))),&stWmaInfo, pWmaInfo->u32TotalPayloads-pWmaInfo->u8NumPayloads+_u32CurrentPayload, u32SeekOffset);
#endif
            if(stWmaInfo.u16PayloadOffset == (U16)0xFFFFFFFF)
            {
                WMA_INFO(printf("payload error! exit play.\n"));
...
BOOLEAN MApp_WMA_Initial(void)
{
    ...
    msAPI_WMA_SetASFParm(WMA_PARAMTYPE_MAX_PACKET_SIZE, stWmaInfo.u32PacketSize);
#endif
...
U8 *MApp_WMA_GetInfoString(EN_WMA_INFO enInfo)
{
    U16 u16Tmp;
    U8* u8Ptr = NULL;
...
BOOLEAN MApp_WMA_Probe(U32 u32Hdl, ST_WMA_PROBE_INFO *pstWmaProbeInfo, U32 u32InfoAttr)
{
    ...
    if(stWmaInfoLocal.bHeaderFound)
    {
        ...
        pstWmaProbeInfo->u16SampleRate = (U16)stWmaInfoLocal.u32SampleRate;
