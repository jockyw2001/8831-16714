...
//Ray OSD 2017.03.07: Define title window handle in relationship with Focus title ball icon
#define NO_OF_TITLE 	3		//Ray OSD 2017.04.19: total 3 titles by removing APP page
#define TITLE_HWND	0		//Position of title handle in Title_icon array
#define ICON_HWND	1		//Position of title icon handle in Title_icon array
static HWND _ZUI_TBLSEG Title_icon[NO_OF_TITLE][2] =
{
      { HWND_MENU_PICTURE_TITLE, HWND_MENU_BOTTOM_BALL_FOCUS_PICTURE},
      { HWND_MENU_SOUND_TITLE, HWND_MENU_BOTTOM_BALL_FOCUS_SOUND},
      { HWND_MENU_OPTION_TITLE, HWND_MENU_BOTTOM_BALL_FOCUS_OPTION},
      //{ HWND_MENU_APP_TITLE, HWND_MENU_BOTTOM_BALL_FOCUS_APP},		//Ray OSD 2017.04.19: Remove APP page

};

//Ray OSD 2017.03.07: Define page list in relationship with Focus title ball icon
#define PAGE_LIST_HWND		0		//Position of page list handle in PageList_Icon array
static HWND _ZUI_TBLSEG PageList_Icon[NO_OF_TITLE][2] =
{
    { HWND_MENU_PICTURE_PAGE_LIST, HWND_MENU_BOTTOM_BALL_FOCUS_PICTURE},
    { HWND_MENU_SOUND_PAGE_LIST, HWND_MENU_BOTTOM_BALL_FOCUS_SOUND},
    { HWND_MENU_OPTION_PAGE_LIST, HWND_MENU_BOTTOM_BALL_FOCUS_OPTION},
    //{ HWND_MENU_APP_PAGE_LIST, HWND_MENU_BOTTOM_BALL_FOCUS_APP},		//Ray OSD 2017.04.19: Remove APP page
...
BOOLEAN MApp_ZUI_API_SetTimer (HWND hWnd, U32 u32id, U32 period)
{
    ...
    //printf("MingYuan [WndSetTimer]  wnd = %d, id = %d, period = %ld \n", hWnd, u32id, period);
...
BOOLEAN MApp_ZUI_API_ResetTimer (HWND hWnd, U32 id)
{
    ...
    //printf("MingYuan MApp_ZUI_API_ResetTimer %d,%d\n",hWnd,id);
    //printf("MingYuan [MApp_ZUI_API_ResetTimer]  wnd = %d, id = %d \n", hWnd, id);
...
BOOLEAN _MApp_ZUI_API_WindowProcOnTimer(void)
{
    ...
    */
    ...
    for (timer = _MenuMsgQueue.Timers; timer < _MenuMsgQueue.Timers+ZUI_MAX_TIMER_COUNT; timer++)
    {
        ...
        {
        	  //printf("MingYuan timer->u32PassTime %ld,%ld,%ld \n",timer->u32PassTime,timer->u32Period,timer);
...
BOOLEAN MApp_ZUI_API_SetFocus(HWND hwnd)
{
    ...
    //printf("[setfocus] = %u\n", hwnd);		//Ray DBG 2017.05.19

    if (prev == hwnd){
	//printf("Ray: Set focus prev==hwnd = %u\n", hwnd);	//Ray DBG 2017.05.19
    ...
    if (hwnd != HWND_INVALID)
    {
        ...
        if ((GETWNDINITSTATE(hwnd)&WS_FOCUSABLE) == 0 ||
            ...
            (!GETWNDVISIBLE(hwnd)) )
        {
            //printf("Ray: set focus GETWNDINITSTATE  = %u\n", (GETWNDINITSTATE(hwnd)&WS_FOCUSABLE));	//Ray DBG 2017.05.19
            //printf("Ray: set focus GETWNDDISABLE  = %u\n", GETWNDDISABLE(hwnd));	//Ray DBG 2017.05.19
            //printf("Ray: set focus GETWNDVISIBLE  = %u\n", GETWNDVISIBLE(hwnd));	//Ray DBG 2017.05.19
            return FALSE; //nothing happened...
        }
    }
...
void MApp_ZUI_API_SetFocusByNav(HWND hwnd, U8 nav)
{
    ...
    for (test = 0; test < 100; test++)
    {
            ...
            !GETWNDDISABLE(next))
        {
            ...
            MApp_ZUI_API_CheckShowFocusTitleIcon(next);	//Ray OSD 2017.03.07: Check if it's needed to show or hide focus title icon
            return;
        }
        hwnd = next;
    }
...
S16 MApp_ZUI_API_GetChildIndex(HWND hwnd)
{
    ...
    for (child=hParent+1; child<=last_succ; child++)
    {
        if (MApp_ZUI_API_GetParent(child) != hParent)		//Ray OSD 2017.03.02: it is used to skip non menu item stuff like text or option.  So this routine only counts menu item
            continue;
        if ( child == hwnd )
            return s16Index;
        s16Index++;
...
//Ray 2017.04.13: This routine is to move the child items (such as MENU_PICTURE_LS_TEXT and MENU_PICTURE_LS_OPTION) according to
//		  parent item (such as MENU_PICTURE_LS) movement offset made by _MApp_ZUI_CTL_DynamicListInitPage. So this routine is called "move successors"
...
BOOLEAN MApp_ZUI_API_MoveAllSuccessors (HWND hWnd, S16 x, S16 y)
{
    ...
    { //2007/12/26: invalide all visible children....
        ...
        //printf("\nRay Before move hWnd:%d, x:%d, y:%d",hWnd, x,y);		//Ray DBG 2017.04.12
        //printf("\nRay last successor hWnd:%d",last_succ);		//Ray DBG 2017.04.12
        for (child = hWnd; child <= last_succ; child++) //for (child = hwnd+1; child < HWND_MAX; child++)
        {
            if (GETWNDVISIBLE(child))
                MApp_ZUI_API_InvalidateWindow(child);
	...
	//Ray 2017.04.13: The for loop below is just to move the child items (such as TEXT and OPTION) according to parent item (such as MENU_PICTURE_LS) movement made
        //                by _MApp_ZUI_CTL_DynamicListInitPage
        for (child = hWnd; child <= last_succ && child < ZUI_MAX_MOVABLE_HWND; child++) //for (child = hwnd+1; child < HWND_MAX; child++)
        {
            ...
            //printf("\nRay After move hWnd:%d, x:%d, y:%d",child, _VarDataMovableRect[child].left,_VarDataMovableRect[child].top);		//Ray DBG 2017.04.12
        }
...
///////////////////////////////////////////////////////////////////////////////
///  public  MApp_ZUI_API_ShowFocusTitleIcon
///  Show or hide focus title icon on Info Bar
///
///   @param [in]       hWnd HWND     	title handle
///   @param [in]       iCmdShow U32    SW_SHOW or SW_HIDE
///
///   @return BOOLEAN  TRUE for success, FALSE for fail
///
///Ray OSD 2017.03.07
///////////////////////////////////////////////////////////////////////////////
BOOLEAN MApp_ZUI_API_ShowFocusTitleIcon(HWND hWnd,U32 iCmdShow)
{
  BYTE i;

  for(i=0;i<NO_OF_TITLE;i++){
      //Find out title handle and then shown or hide the title icon
      if(Title_icon[i][TITLE_HWND]==hWnd){
	  if(iCmdShow==SW_HIDE){
	      //Check if the title icon is already hidden
	      if(MApp_ZUI_API_IsWindowVisible(hWnd)==FALSE){
		  return FAIL;		//If already hidden, there is no need to hide the icon again at below.
	      }
	  }
	  return(MApp_ZUI_API_ShowWindow(Title_icon[i][ICON_HWND], iCmdShow));
...
///////////////////////////////////////////////////////////////////////////////
///  public  MApp_ZUI_API_CheckShowFocusTitleIcon
///  Check if it's needed to show or hide focus title icon on Info Bar
///  according to the input menu item position.
///
///   @param [in]       hWnd HWND     	current focus menu item handle
///
///   @return BOOLEAN  TRUE for success, FALSE for fail
///
///Ray OSD 2017.03.07
///////////////////////////////////////////////////////////////////////////////
BOOLEAN MApp_ZUI_API_CheckShowFocusTitleIcon(HWND hWnd)
{
  BYTE i;
  GUI_DATA_DYNAMIC_LIST * windata;

  //Check if it is in main menu and title info bar is shown before proceeding
  if(MApp_ZUI_API_IsWindowVisible(HWND_MENU_MAIN_BOTTON_INFO_BAR)==TRUE){
    for(i=0;i<NO_OF_TITLE;i++){
	//Get the winddata
	windata = (GUI_DATA_DYNAMIC_LIST*)GETWNDDATA(PageList_Icon[i][PAGE_LIST_HWND]);

	//Ray OSD 2017.03.07: Check if current focus is first item of menu. If yes, show focus title icon
	if(windata->pVarData->hwndFirstShown == hWnd){
	    return(MApp_ZUI_API_ShowFocusTitleIcon(windata->pVarData->hwndFirstShown,SW_SHOW));	//show the title icon
	}
	else{
	    //If current focus is not first item of menu, hide the title icon
	    return(MApp_ZUI_API_ShowFocusTitleIcon(windata->pVarData->hwndFirstShown,SW_HIDE));	//hide the title icon
	}

	/*
	//Ray OSD 2017.03.07: Check if current foucs is second item of menu. If yes, hidden the focus title icon
	if((GETWNDNAVI(windata->pVarData->hwndFirstShown)[NAV_DOWN]) == hWnd) {
	    return(MApp_ZUI_API_ShowFocusTitleIcon(windata->pVarData->hwndFirstShown,SW_HIDE));	//hide the title icon
	}
	*/
