...
#define IS_NETWORK_UPDARE_COUNTRY(c)            ( (c==OSD_COUNTRY_MALAYSIA)      ||  \
                                                  (c==OSD_COUNTRY_KUWAIT)      ||  \
                                                  (c==OSD_COUNTRY_OMAN)      ||  \
                                                  (c==OSD_COUNTRY_QATAR)      ||  \
                                                  (c==OSD_COUNTRY_SAUDI_ARABIA)      ||  \
                                                  (c==OSD_COUNTRY_UNITED_ARAB_EMIRATES))

#define IS_DONOT_REMOVE_COUNTRY(c)              ( (c==OSD_COUNTRY_MALAYSIA)      ||  \
                                                  (c==OSD_COUNTRY_KUWAIT)      ||  \
                                                  (c==OSD_COUNTRY_OMAN)      ||  \
                                                  (c==OSD_COUNTRY_QATAR)      ||  \
                                                  (c==OSD_COUNTRY_SAUDI_ARABIA)      ||  \
...
#if(CHIP_FAMILY_TYPE != CHIP_FAMILY_S7LD && CHIP_FAMILY_TYPE != CHIP_FAMILY_A7&&CHIP_FAMILY_TYPE != CHIP_FAMILY_MAYA && CHIP_FAMILY_TYPE != CHIP_FAMILY_MILAN&& CHIP_FAMILY_TYPE != CHIP_FAMILY_MULAN)
...
static U8 MApp_S2_Scan_AddOneDTVPchVchs( U16 u16PhysicalChIdx,SI_SHORT_DTV_CHANNEL_INFO *pastVirtualCh,U8 u8NumOfVch,BOOLEAN bCheckServiceInfo ,BOOLEAN *bDBfull)
{
    ...
    for(u8Loop_1=0; u8Loop_1<u8NumOfVch; u8Loop_1++)
    {
        ...
        if( g_enScanType == S2_SCAN_TYPE_AUTO || g_enScanType == S2_SCAN_TYPE_BLIND|| g_enScanType == S2_SCAN_TYPE_NETWORK||S2_SCAN_TYPE_MANUAL== g_enScanType)
        {
            eResult = msAPI_SI_AddProgram(&stDtvIDTable, &stDtvPgmData,&bFull, FALSE);
            if(TRUE == eResult)
            {
    ...
    switch( enScanState )
    {
        case STATE_SCAN_INIT:
            ...
            if( MApp_DTV_Scan_Init() == TRUE )
            {
                ...
                else if(S2_SCAN_TYPE_AUTO== g_enScanType||S2_SCAN_TYPE_MANUAL== g_enScanType||S2_SCAN_TYPE_NETWORK== g_enScanType )
                {
...
*/
        ...
        case STATE_SCAN_SEARCH_RF_CHANNEL:
            ...
            else // Not Lock
            {
                ...
                #endif
                    {
                        if((msAPI_CM_GetCountry() == E_NORWAY) && (msAPI_DFT_VHF_CrossBWFreq_GetFinishScanStatus() == FALSE)
                            && (u8RFCh >= 5 && u8RFCh <= 120))
                        {
                            msAPI_DFT_GetTSSetting(stGenSetting.stScanMenuSetting.u8RFChannelNumber, &stTempTP);
                            ...
                            stTempTP.enBandWidth = (RF_CHANNEL_BANDWIDTH)msAPI_DFT_GetBandwidth();
        ...
        case STATE_FIND_NIT_FREQ:
        {
            ...
            while (u16FreqCnt)
            {
                if(MApp_SI_GetNextSatParameter(&siCDSInfo, &u16FreqCnt))
                {
        ...
        #if(ASTRA_HD_ENABLE)
            if( MApp_Dmx_AstraHD_GetScanTables(&_stScanConfig, &u8NumOfVchFound ) == FALSE )
            {
                u16GetProgramsCnt++;
        ...
        case STATE_SCAN_WAIT:
                ...
                _stNetworkChangInfo.bMuxRemove || _stNetworkChangInfo.bMuxAdd || _stNetworkChangInfo.bCellRemove))
            {
                ...
                else if(_stNetworkChangInfo.bMuxAdd)
                {
                    ...
                    {
                    #if 0//(ENABLE_UPDATE_MULTIPLEX_VIA_NIT) //TODO, wait UI
                        MApp_ZUI_ACT_StartupOSD(E_OSD_MESSAGE_BOX);
                    ...
                    //else if ((OSD_COUNTRY_SETTING == OSD_COUNTRY_NORWAY)||(OSD_COUNTRY_SETTING == OSD_COUNTRY_MALAYSIA)||(OSD_COUNTRY_SETTING == OSD_COUNTRY_UNITED_ARAB_EMIRATES))
                    {
...
static BOOLEAN _MApp_DTV_Scan_SetCableTPSetting(_DVBC_SCAN_FLAG *stScanFlag, MS_TP_SETTING *pstTPSetting)
{
    ...
    else if (g_enScanType == SCAN_TYPE_AUTO)
    {
        ...
        #if(CHIP_FAMILY_TYPE == CHIP_FAMILY_S7LD || CHIP_FAMILY_TYPE == CHIP_FAMILY_A7 || CHIP_FAMILY_TYPE == CHIP_FAMILY_MILAN|| CHIP_FAMILY_TYPE == CHIP_FAMILY_MAYA|| CHIP_FAMILY_TYPE == CHIP_FAMILY_MULAN)  // T4 support auto qam     //verity 0115
            bTuenOtherQamMode = FALSE;
        #else
    ...
    else if (g_enScanType == SCAN_TYPE_NETWORK)
    {
        ...
        {   //tune the other qam mode when fornt-end unlock
        #if(CHIP_FAMILY_TYPE == CHIP_FAMILY_S7LD || CHIP_FAMILY_TYPE == CHIP_FAMILY_A7 ||CHIP_FAMILY_TYPE == CHIP_FAMILY_MAYA|| CHIP_FAMILY_TYPE == CHIP_FAMILY_MILAN|| CHIP_FAMILY_TYPE == CHIP_FAMILY_MULAN)  // T4 support auto qam
            bTuenOtherQamMode = FALSE;
        #else
            bTuenOtherQamMode = _MApp_DTV_Scan_IncreaseModulationMode();
        #endif
