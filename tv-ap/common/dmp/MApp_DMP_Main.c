...
EN_RET MApp_DMP_Main(void)
{
    ...
    switch(m_enDmpVar.enDmpState)
    {
        ...
        case DMP_STATE_WAIT:
            ...
            if((u8KeyCode == 0xFF)&&(m_enDmpVar.enDmpUiState == DMP_UI_STATE_PLAYING_STAGE) &&(!MApp_DMP_IsOSDVisible())&&(!MApp_MPlayer_IsCurSubtitleText())
                )
            {
            ...
            //Ray DMP 2017.04.11: When Ui State is DMP_UI_STATE_NO_MEDIA_FILE, as there is no media file in USB, we need to exit DMP and back to previous input source
            if(m_enDmpVar.enDmpUiState == DMP_UI_STATE_NO_MEDIA_FILE)
            {
        	ucUSBNoMediaFile = 1;		//Denote there is no media file in USB
        	MsOS_DelayTask(2000);		//Delay for 2 sec to continuous showing "all files unsupported" message
        	MApp_DMP_GotoPreSrc();
            }
#endif
