...
#include "debug.h"
#ifndef ATSC_SYSTEM //do not support ATSC now, sicne subtitle issue needs to be fixed
#include "datatype.h"
            ...
            FileEntry *pFileDirEntry)
{
    ...
    if(pFileDirEntry == NULL)
          u8Handle = FAT_FileCreate(u8EnvironmentID, fileName, fileLen);
    else
...
static BOOLEAN BKFS_Show_Hide_File(U8 u8Handle, BOOLEAN bShow)
{
    FAT_FileHandle *pFileHandle;
    FAT_FileSysStruct* pFAT_FileSys;
...
static void BKFS_Show_Hide_Bulk_Folder(BOOLEAN bShow)
{
    ...
    BKFS_Show_Hide_File(u8Handle, bShow);
...
BOOLEAN BKFS_CreateRecordPathFiles(U16 *u16RecordPathFileName)
{
  ...
  //  u16TotalBulkNumInUsed = u16TotalBulkNumInUsed>u16MaxRecordBulkCnt?u16MaxRecordBulkCnt:u16TotalBulkNumInUsed;
    ...
    while(!LL_iszero(&llBulkFileLength))
    {
        ...
        u8Handle = FAT_FileCreate(u8EnvironmentID, g_segFilename, FS_strlen_U(g_segFilename));
        if(INVALID_FILE_HANDLE == u8Handle)
        {
            msAPI_FSEnv_UnRegister(u8EnvironmentID);
           __ASSERT(0);
...
extern U8 FAT_FileCreateDirEntry(U8 u8EnvironmentID, U16 *pu16LongName, U8 charLen, FileEntry *pEntryToSet, BOOLEAN bIsFile);
...
BOOLEAN msAPI_BKFS_CreateBulks(EN_CREATE_BULKS_STATE enCreateBulksState, U8 u8EnvironmentID, U16* u16BulkNum)
{
    ...
    switch(enCreateBulksState)
    {
        ...
        case EN_CREATE_BULKS_CREATEFOLDER:
        {
            ...
            for index file.
            **********************************************/
            bResult = FAT_FileCreateDirEntry(u8EnvironmentID, name8_3_idx, 12, &stIndexFileEntry, TRUE);
        ...
        case EN_CREATE_BULKS_CREATEEPG:
        {
            ...
            u8Handle = FAT_FileCreate(u8EnvironmentID, name8_3_epg, LENGTH_OF_BKFS_EPG_FILE);
            if(INVALID_FILE_HANDLE == u8Handle)
            {
                _enCreateBulksState = EN_CREATE_BULKS_INIT;
                __ASSERT(0);
