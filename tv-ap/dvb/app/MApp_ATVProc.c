...
BOOLEAN MApp_ATVProc_IsThisATVOptionEnabled(THISATV_OPTION eOption)
{
    ...
    case E_ENABLE_SCART:
        #if(ENABLE_DTMB_CHINA_APP || ENABLE_ATV_CHINA_APP || ENABLE_DVBC_PLUS_DTMB_CHINA_APP || ENABLE_SBTVD_BRAZIL_APP)
            bEnabled = FALSE;
        #else
            bEnabled = TRUE;
        #endif
...
/**
 FUNCTION    : void MApp_ATVProc_Initialize_Others(void)

 USAGE    : To initialize TV/AV module except Audio part. This function should be called, if you want to use TV/AV module.
...
*/
void MApp_ATVProc_Initialize_Others(void)
{
#if (ENABLE_BOOTTIME)
    ...
    debugTVAVHandlerPrint("init atvav",0);

    msAPI_AVD_InitVideoSystem();   // use 51ms

#if (ENABLE_BOOTTIME)
    gU32TmpTime = msAPI_Timer_DiffTimeFromNow(gU32BootStepTime);
    printf("[TIME] msAPI_AVD_InitVideoSystem = %ld\n", gU32TmpTime);
    gU32BootStepTime = msAPI_Timer_GetTime0();
#endif

    msAPI_Tuner_Init();     // use 169ms
...
/*       || g_u8MenuPageIndex ==   MENU_BLOCK_EDIT_LIST*/ )
    ...
    if( FALSE == bIsTuningProcessorBusy && msAPI_AVD_GetScart1SrcType() != E_SCART_SRC_TYPE_UNKNOWN)
    {
