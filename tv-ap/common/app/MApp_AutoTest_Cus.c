...
{
    DEBUG_AUTOTEST_CUS( printf("enAutoTestInputSrc=%d\n", enAutoTestInputSrc) );
    ...
    DEBUG_AUTOTEST_CUS( printf("eTargetUIInputSource=%d\n",eTargetUIInputSource) );
...
BOOL MApp_AT_IsSrcHasSignal(SCALER_WIN eWin)
{
    ...
    {
        return (MApp_GetSignalStatus()==SIGNAL_LOCK)?TRUE:FALSE;
    }
    else
#endif
    {
...
BOOL MApp_AT_Cmd_DtvAutoScan(EN_AT_MEMBER_COUNTRY enCountry)
{
    ...
    MApp_UiMenuFunc_SetAutoScanDigital();
    MApp_UiMenuFunc_ExecuteAutoScan();

    MApp_ZUI_ACT_Show_AutoScanMenu();
...
BOOL MApp_AT_Cmd_AtvAutoScan(EN_AT_MEMBER_COUNTRY enCountry)
{
    ...
    MApp_TopStateMachine_SetTopState(STATE_TOP_MENU);
    MApp_UiMenuFunc_SetAutoScanAnalog();
    MApp_UiMenuFunc_ExecuteAutoScan();
