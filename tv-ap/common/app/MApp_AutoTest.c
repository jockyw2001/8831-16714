...
static BOOLEAN _MApp_AT_CmdParser(AT_CMD *ATCmd)
{
    ...
    DEBUG_AUTOTEST( printf("DEBUG_AUTOTEST>> iCmdType= %d \n", ATCmd->u8CmdType) );
    DEBUG_AUTOTEST( printf("DEBUG_AUTOTEST>> iCmdSubType= %d \n", ATCmd->u16CmdSubType) );
    DEBUG_AUTOTEST( printf("DEBUG_AUTOTEST>> iCmdPara[0]= %d \n", ATCmd->iCmdPara[0]) );
...
static EN_AT_RESULT _MApp_AT_DirectCmdHandler(AT_CMD ATCmd)
{
    switch(ATCmd.u16CmdSubType)
    {
        case EN_AUTO_TEST_DIRECT_CMD_SET_INPUT_SOURCE: // 1
        {
            ...
            g_AT_u32ChangeSourceTime = MsOS_GetSystemTime();
            //printf("g_AT_u32ChangeSourceTime = %u\n", g_AT_u32ChangeSourceTime);
...
void MApp_AT_Task(void)
{
    ...
    if( g_AT_bChangeSource )
    {
        ...
        if( msAPI_Timer_DiffTime_2(g_AT_u32ChangeSourceTime, u32CurTime ) > u32TimeoutTime )
        {
