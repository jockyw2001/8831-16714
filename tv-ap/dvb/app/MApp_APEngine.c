...
static BOOLEAN bAPStartBG = FALSE;
#endif
...
#if OBA2
static E_OBAMA_APP_TYPE enObamaAppType = E_OBAMA_APP_TYPE_TV_NONE;
static E_OBAMA_APP_TYPE pre_enObamaAppType = E_OBAMA_APP_TYPE_TV_NONE;
#endif
...
#if OBA2
BOOLEAN MApp_APEngine_RegisterByName(char *AP_Name)
{
    msAPI_APEngine_Obama_ExecuteApp(AP_Name);
    return TRUE;
}
#endif
...
static void _MApp_APEngine_ProcessKey_Cowork(void)
{
    ...
    if(bGoCowork && (!g_bGoBack_AP && !g_bHideAP))
    {
        switch ( u8KeyCode )
        {
            ...
            case KEY_INPUT_SOURCE:
                ...
                g_bGoBack_AP = TRUE;
                MApi_GOP_GWIN_Set_STRETCHWIN(E_GOP_OSD, E_GOP_DST_OP0, 0, 0,1360, 768);
                msAPI_APEngine_TransmitKey(KEY_EXIT);  // not terminated, need hide
                u8KeyCode = KEY_NULL;
            break;

            case KEY_MENU:
                enAPEngineState = STATE_APENGINE_WITH_OSD;
                enAPEngineRetVal = EXIT_GOTO_MENU;
                g_bGoBack_AP = TRUE;
                MApi_GOP_GWIN_Set_STRETCHWIN(E_GOP_OSD, E_GOP_DST_OP0, 0, 0, 1360, 768);
                msAPI_APEngine_TransmitKey(KEY_EXIT);  // not terminated, need hide
                u8KeyCode = KEY_NULL;
          ...
          #if (OBA2 && defined(GADGET))
            case KEY_REVEAL:
                if(MApp_APEngine_ActivateGadget())
                {
                    if(g_bGoBack_AP == TRUE)
                    {
                        g_bGoBack_AP = FALSE;
                    }
                }
    ...
    {
      #if ((OBA2 == 1) && (ENABLE_BABAO_PLAYER == ENABLE))
        u8KeyCode = msAPI_NPlayer_KeyHandle(u8KeyCode);
      #endif
        switch ( u8KeyCode )
        {
            case KEY_EXIT:
                ...
                else
                {
                    ...
                    if(MApp_APEngine_IsGadgetActived())
                    {
                        // if current ap is GADGET, just hide it.
                        msAPI_APEngine_TransmitKey(KEY_EXIT);
                        enAPEngineState = STATE_APENGINE_WITH_OSD;
                        enAPEngineRetVal = EXIT_APENGINE_EXIT;
                        g_bGoBack_AP = TRUE;
                    ...
                    {
                        msAPI_APEngine_TransmitKey(u8KeyCode);//trans key tp AP,don't stop ap directly
                    }
                  #else
                    msAPI_APEngine_Obama_StopCurrentApp();
                    msAPI_APEngine_TransmitKey(u8KeyCode);
                  #endif
                  #endif
                    u8KeyCode = KEY_NULL;
...
char* MApp_APEngine_GetAPEngineStateName(void)
{
    char* APEStateName[]=
    {
        "LISTEN",
    ...
    return APEStateName[enAPEngineState];
}

char* MApp_APEngine_GetAPTypeName(void)
{
    char* APTypeName[]=
    {
        "TV_NONE",
        ...
        "TV_APP",
      #if (OBA2 && defined(GADGET))
        // AP register after it executed, and need time.
        // when AP Executed, and check the OBAMA_APP_TYPE,
        // it's default value would be E_OBAMA_APP_TYPE_ISOLATED_APP.
        // but it's not correct. so add this.
        "NOREG",
      #endif
    };
    return APTypeName[enObamaAppType];
}
void MApp_APEngine_Debug(void)
{
    printf("Top=%d:%s,APEState=%d:%s,AppType=%d:%s,Pre_AppType=%d,bGoCowork=%d,enHandleType=%d,g_bGoBack_AP=%d,g_bHideAP=%d,g_bPassTV=%d\n",
            MApp_TopStateMachine_GetTopState(),
            MApp_TopStateMachine_GetTopStateName(),
            enAPEngineState,
            MApp_APEngine_GetAPEngineStateName(),
            enObamaAppType,
            MApp_APEngine_GetAPTypeName(),
            pre_enObamaAppType,
            bGoCowork,  enHandleType,  g_bGoBack_AP, g_bHideAP,  g_bPassTV);
}

BOOLEAN MApp_APEngine_ActivateGadget(void)
{
    BOOLEAN bRet = FALSE;
    APP_STATUS appStatus = MAdp_APMNG_GetAppStatus(GADGETAP_NAME);
    DBG_APSTATE(printf("MApp_APEngine_ActivateGadget>>appStatus=%x\n", appStatus);)
    switch(appStatus)
    {
        case APP_STATUS_ACTIVATED:
            DBG_APSTATE(printf("MApp_APEngine_ActivateGadget>>msAPI_APEngine_TransmitKey\n");)
            msAPI_APEngine_TransmitKey(KEY_MENU); // show/hide reverse
            bRet = TRUE;
            break;
        case APP_STATUS_SUSPENDING:
            DBG_APSTATE(printf("MApp_APEngine_ActivateGadget>>MAdp_APMNG_ResumeApp\n");)
            bRet = MAdp_APMNG_ResumeApp(GADGETAP_NAME);
            break;
        case APP_STATUS_NOT_RUN:
            if(enObamaAppType==E_OBAMA_APP_TYPE_TV_NONE)
            {
                DBG_APSTATE(printf("MApp_APEngine_ActivateGadget>>MAdp_APMNG_ExecuteApp\n");)
                bRet = msAPI_APEngine_Obama_ExecuteApp(GADGETAP_NAME);
            }
            break;
         case APP_STATUS_UNKNOWN:
         default:
             DBG_APSTATE(printf("MApp_APEngine_ActivateGadget>>null\n");)
            break;
    }
    DBG_APSTATE(printf("MApp_APEngine_ActivateGadget(%s)\n", bRet?"Success!":"Failed!");)
    return bRet;
}

BOOLEAN MApp_APEngine_GadgetAutoLaunch(void)
{
    BOOLEAN bRet = FALSE;
    // after another AP killed
    if((enObamaAppType == E_OBAMA_APP_TYPE_TV_NONE)
        && (pre_enObamaAppType != E_OBAMA_APP_TYPE_TV_NONE))
    {
        MApp_APEngine_Debug();
        bRet = msAPI_APEngine_Obama_ExecuteApp2(GADGETAP_NAME, "BG");
        bAPStartBG = TRUE;
    }
...
BOOLEAN MApp_APEngine_IsGadgetActived(void)
{
    BOOLEAN bRet = FALSE;
    if(APP_STATUS_ACTIVATED == MAdp_APMNG_GetAppStatus(GADGETAP_NAME))
    {
        bRet = TRUE;
    }
    return bRet;
...
#if OBA2
static void MApp_APEngine_AP_Reg_Check(void)
{
    static U32 u32Timer=0;

    if(MAdp_APMNG_IsAppRegister(MAdp_APMNG_GetActivatedAppPid())== FALSE)
    {
    // AP register after it executed, and need time.
    // when AP Executed, and check the OBAMA_APP_TYPE,
    // it's default value would be E_OBAMA_APP_TYPE_ISOLATED_APP.
    // but it's not correct. so add this.
        ...
        if(msAPI_APEngine_Obama_ApRegstingFlag()==0)// none ap registing
        {
            u32Timer=0;
            enObamaAppType = E_OBAMA_APP_TYPE_TV_NONE;
            return;
        }
        else
        {
            if(msAPI_Timer_DiffTimeFromNow(u32Timer) > 10000) // 10 Sec
            {
                 // fixme: the APP type must be remembered first, cause after calling MApp_APEngine_Exit(), its value will be reset.
                 E_OBAMA_APP_TYPE eAppType = enObamaAppType;
                 MApp_APEngine_Exit();

                 // only ISOLATED_APP) needs to restore input source
                 if(eAppType==E_OBAMA_APP_TYPE_ISOLATED_APP)
                 {
                     MDrv_AUDIO_Open();
                 ...
                 {
                     enAPEngineRetVal = EXIT_GOTO_DMP;
                 }

                 pre_enObamaAppType = enObamaAppType= E_OBAMA_APP_TYPE_TV_NONE;
                 u32Timer=0;
                 msAPI_APEngine_Obama_StopApRegsting();
                 DBG_APSTATE(printf("\n ===========AP Register timeout,exit AP Engin State=====\n"));
            }
            else
                enObamaAppType = E_OBAMA_APP_TYPE_TV_NONE;
        }
    ...
    {
        if(enObamaAppType==E_OBAMA_APP_TYPE_TV_NONE)
            enObamaAppType = msAPI_APEngine_Obama_CurrentAppType();
        msAPI_APEngine_Obama_StopApRegsting();
    }
...
void MApp_APEngine_Process(void)
{
    ...
    if (MAdp_IR_IsEnableDaemon())
    {
        if (enObamaAppType != pre_enObamaAppType)
        {
            if (enObamaAppType == E_OBAMA_APP_TYPE_ISOLATED_APP)
                MAdp_MSGCH_SetPollingEventTimeout(500);
            else
                MAdp_MSGCH_SetPollingEventTimeout(0);
        }
    }
...
#if OBA2
    switch(enObamaAppType)
    {
        case E_OBAMA_APP_TYPE_TV_NONE:
            bGoCowork = FALSE;
             if(pre_enObamaAppType != E_OBAMA_APP_TYPE_TV_NONE)
             {
                 MApp_APEngine_Exit();
               #if (OBA2 && defined(GADGET))
                 if(APP_STATUS_SUSPENDING==MAdp_APMNG_GetAppStatus(GADGETAP_NAME))
                 {
                    printf("source do not change before, No need to restore Source\n");
                 }
                 else
               #endif
                 {
        ...
        case E_OBAMA_APP_TYPE_ISOLATED_APP:
            ...
            bGoCowork = FALSE;
            enHandleType = FULL_HANDLE;
        break;

        case E_OBAMA_APP_TYPE_TV_APP:
        default:
            if(pre_enObamaAppType == E_OBAMA_APP_TYPE_TV_NONE)
            {
                ...
                if(MApp_APEngine_IsGadgetActived())
                {
                    printf("active the Gadget, do not need to change the source\n");
                }
                else
              #endif
                {
    ...
    {
        if(enAPEngineState == STATE_APENGINE_LISTEN)
            MApp_APEngine_GadgetAutoLaunch();
    }
    #endif
    pre_enObamaAppType = enObamaAppType;
#endif
        ...
        case STATE_APENGINE_WAIT:
            {
                ...
                msAPI_APEngine_ProcessMailBox();
#elif (OBA2==1)
                // removed: msAPI_APEngine_Obama_Zodiac_VDPlayer_ProcessMailBox();
#endif
                if (msAPI_APEngine_APP_Finalize_Check()
