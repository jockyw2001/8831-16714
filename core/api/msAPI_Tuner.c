...
{
#if 1 // Use new method to check demod lock
    ...
    *CheckLockResult = FE_NOT_LOCK;
    ...
    if ( fScan )
    {
        U16 timeout = msAPI_Demod_Get_CheckLockTimeout();
        ...
        for ( count = 0; count < timeout; count++ )
        {
            ...
            if ( FALSE == devCOFDM_GetLockStatus(COFDM_ICFO_CH_EXIST_FLAG) )
            {
                if (count > 11) // 12*50ms = 600ms.
                    count = timeout;
            }
            else if ( TRUE == devCOFDM_GetLockStatus(COFDM_FEC_LOCK) )
            {
                *CheckLockResult = FE_LOCK;
                return TRUE;
            }
        #elif (FRONTEND_DEMOD_TYPE == EXTERNAL_DTMB_DEMOD || FRONTEND_DEMOD_TYPE == EMBEDDED_DTMB_DEMOD)
            if ( FALSE == devCOFDM_GetLockStatus(COFDM_PSYNC_LOCK) )
            {
                if (count > 22) //modify  PN Timeout//VT TEMP: PN Needs 500ms //10*50ms = 500ms.
                    count = timeout;
            }
            ...
            if ( FALSE == devCOFDM_GetLockStatus(COFDM_PSYNC_LOCK) )
            {
                if (count > 11) //VT TEMP: PN Needs 500ms //10*50ms = 500ms.
                    count = timeout;
            }
            else if ( TRUE == devCOFDM_GetLockStatus(COFDM_FEC_LOCK) )
            {
          ...
          #if(ENABLE_S2)
            if( msAPI_Demod_GetCurrentDemodType() == E_DEMODULATOR_TYPE_DVBS_S2 )
            {
                if( msAPI_Demod_DVBS_Get_LockStatus() == E_API_DEMOD_STATUS_LOCKED )
                {
                    DEBUG_DEMOD_S(printf(" DVBS Demod LOCK\n"););
                    *CheckLockResult = FE_LOCK;
                    return TRUE;
                }
                else
                {
                    if( count > 20 )
                    {
                        DEBUG_DEMOD_S( printf(" DVBS Demod UNLOCK\n"); );
                        break;
                    }
                }
            }
            ...
            if(msAPI_Demod_GetCurrentDemodType()==E_DEMODULATOR_TYPE_DVBT2)
            {
                ...
                if (mdev_CofdmGetLockStatus(COFDM_P1_LOCK_HISTORY_123X) == FALSE)
                {
                    //printf("%s,%d\r\n",__FUNCTION__,__LINE__);
                    if(count > 30)
                    {
            ...
            {
                 timeout = 120;
                 TUNER_API_DBG_ONELINE(printf("%s,%d\r\n",__FUNCTION__,__LINE__));
                 //printf("\n%s,%d\r\n",__FUNCTION__,__LINE__);
                 if ( TRUE == devQAM_GetLockStatus(QAM_FEC_LOCK) )
                 {
                     //printf("\n000 tdevQAM_GetLockStatus =true %s,%d\r\n",__LINE__);
                     TUNER_API_DBG_ONELINE(printf("%s,%d\r\n",__FUNCTION__,__LINE__));
                 ...
                 if ( FALSE == devQAM_GetLockStatus(QAM_TR_LOCK) )
                 {
                     ...
                     {
                         TUNER_API_DBG_ONELINE(printf("%s,%d\r\n",__FUNCTION__,__LINE__));
                         TUNER_API_DBG_ONELINE(printf("DVBC TR UNLOCK "));
                         break;
                     }
                 }
            ...
            {// DVB-T
                ...
                {
                    //printf("%s,%d\r\n",__FUNCTION__,__LINE__);
                    if(count > 10) // 20*50ms = 1 sec.
                    {
                        //printf("\r\n !!Check T Signal...No TPS info\n");
                        TUNER_API_DBG(printf("No TPS "));
    ...
    if( IsS2InUse() )
    {
        if( msAPI_Demod_DVBS_Get_LockStatus() == E_API_DEMOD_STATUS_LOCKED )
        {
            //printf(" 2. DVBS Demod LOCK\n");
            *CheckLockResult = FE_LOCK;
        }
        else
        {
            //printf(" 2. DVBS Demod UNLOCK\n");
            *CheckLockResult = FE_UNLOCKED;
        }
    }
    else
#endif
    ...
    {
        //MS_U8    u8plp_id = 0;
        //mdev_CofdmGetCurrentPlpID(&u8plp_id);
...
void msAPI_Tuner_Serial_Control(BOOLEAN bParallelMode, BOOLEAN bPVRMode)
{
    ...
    else if( IsS2InUse()/*ISDVBS2Source()*/ )
    {
        //MDrv_SYS_SetPadMux(E_TS0_PAD_SET, E_SERIAL_IN);
        MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_EXT_INPUT0, FALSE, TRUE, FALSE);
    }

  #endif
#endif

    TUNER_SERIAL_CONTROL_DBG(printf("\r\n @@@@ msAPI_Tuner_Serial_Control....bParallelMode = %d|bPVRMode = %d|bEnable = %d|bScanMode = %d\n",bParallelMode,bPVRMode,bEnable,bScanMode));
    if (bEnable == TRUE) // Serial
    {
    #if (  FRONTEND_DEMOD_TYPE == MSTAR_MSB1400_DEMOD       \
        || FRONTEND_DEMOD_TYPE == ALTOBEAM_883X   \
        || FRONTEND_DEMOD_TYPE == ALTOBEAM_884X    \
        || FRONTEND_DEMOD_TYPE == ALTOBEAM_885X    \
        || FRONTEND_DEMOD_TYPE == ALTOBEAM_886X \
        || FRONTEND_DEMOD_TYPE == HDIC_2311 \
        || FRONTEND_DEMOD_TYPE == LEGEND_9x_DEMOD \
        || FRONTEND_DEMOD_TYPE == ALTOBEAM_887X \
        || FRONTEND_DEMOD_TYPE == ALTOBEAM_888X \
        || FRONTEND_DEMOD_TYPE == EXTERNAL_DTMB_DEMOD \
        )
        {
        #if (MS_BOARD_TYPE_SEL == BD_MST157E_C02A_EMERALD_CUS21SH_DTMB || FRONTEND_DEMOD_TYPE == EXTERNAL_DTMB_DEMOD)
            MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_EXT_INPUT0, FALSE, TRUE, FALSE);
        #else
            MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_EXT_INPUT1, FALSE, TRUE, FALSE);
        #endif
        #if (FRONTEND_DEMOD_TYPE == HDIC_2311 || FRONTEND_DEMOD_TYPE == LEGEND_9x_DEMOD)
            ...
            MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_EXT_INPUT0, FALSE, TRUE, FALSE);
    #elif (FRONTEND_DEMOD_TYPE == MSTAR_MSB124X_DVBT)
            printf("FRONTEND_DEMOD_TYPE == MSTAR_MSB124X  serial\n");
            MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_EXT_INPUT0, FALSE, TRUE, FALSE);
    #elif(FRONTEND_DEMOD_TYPE == EMBEDDED_DVBT_DEMOD || FRONTEND_DEMOD_TYPE == EMBEDDED_DVBC_DEMOD \
        || FRONTEND_DEMOD_TYPE == EMBEDDED_ISDBT_DEMOD || FRONTEND_DEMOD_TYPE == EMBEDDED_DTMB_DEMOD \
        || FRONTEND_S2_DEMOD_TYPE  == EMBEDDED_DVBS_DEMOD || FRONTEND_DEMOD_T2_TYPE  == EMBEDDED_DVBT2_DEMOD)
        {
        #if (ENABLE_CI)
        ...
        #endif
            {
                ...
                || FRONTEND_DEMOD_TYPE == EMBEDDED_ISDBT_DEMOD || FRONTEND_DEMOD_TYPE == EMBEDDED_DTMB_DEMOD\
                || FRONTEND_S2_DEMOD_TYPE  == EMBEDDED_DVBS_DEMOD || FRONTEND_DEMOD_T2_TYPE  == EMBEDDED_DVBT2_DEMOD)
                MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_DEMOD, FALSE, TRUE, FALSE);
            #else
                MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_EXT_INPUT1, FALSE, TRUE, FALSE);
            #endif
            }
    ...
    //------------------------
        ...
        {
        #if (ENABLE_PVR) && (!BLOADER)
            if ((ParallelInterfaceFlag != E_QAM_SERIAL_INTERFACE) && MApp_PVR_IsRecording())
            {
                devQAM_SwitchTransportSteamInferface(E_QAM_SERIAL_INTERFACE);
                ParallelInterfaceFlag = E_QAM_SERIAL_INTERFACE;
            }
        #else
            devQAM_SwitchTransportSteamInferface(E_QAM_SERIAL_INTERFACE);
        #endif
        ...
        {
        #if (ENABLE_PVR) && (!BLOADER)
            if ((ParallelInterfaceFlag != E_SERIAL_INTERFACE) && MApp_PVR_IsRecording())
            {
                devCOFDM_SwitchTransportSteamInferface(E_SERIAL_INTERFACE);
                ParallelInterfaceFlag = E_SERIAL_INTERFACE;
            }
        #else
            devCOFDM_SwitchTransportSteamInferface(E_SERIAL_INTERFACE);
        #endif
    ...
    else
    {
        ...
        || FRONTEND_DEMOD_TYPE == LEGEND_8G42_DEMOD     \
        || FRONTEND_DEMOD_TYPE == LEGEND_8G52_DEMOD     \
        || FRONTEND_DEMOD_TYPE == LEGEND_8G80_DEMOD     \
        ...
        {
        #if (MS_BOARD_TYPE_SEL == BD_MST157E_C02A_EMERALD_CUS21SH_DTMB)
            MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_EXT_INPUT0, FALSE, TRUE, TRUE);
        #else
            MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_EXT_INPUT0, FALSE, TRUE, TRUE);
        #endif
        #if (FRONTEND_DEMOD_TYPE == HDIC_2311)
            MApi_DMX_Serial_Ext_Sync_1T(TRUE);
        #endif
        }
    #elif(FRONTEND_DEMOD_TYPE == EMBEDDED_DVBT_DEMOD || FRONTEND_DEMOD_TYPE == EMBEDDED_DVBC_DEMOD \
        || FRONTEND_DEMOD_TYPE == EMBEDDED_ISDBT_DEMOD || FRONTEND_DEMOD_TYPE == EMBEDDED_DTMB_DEMOD\
        || FRONTEND_S2_DEMOD_TYPE  == EMBEDDED_DVBS_DEMOD || FRONTEND_DEMOD_T2_TYPE  == EMBEDDED_DVBT2_DEMOD)
        {
        ...
        #endif
            {
                ...
                MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_DEMOD, FALSE, TRUE, TRUE);
            #else
                MApi_DMX_FlowSet(eDmxFlow, DMX_FLOW_INPUT_EXT_INPUT1, FALSE, TRUE, TRUE);
            #endif
            }
        }
    #elif (FRONTEND_DEMOD_TYPE == MSTAR_MSB124X_DVBT)
           MS_DEBUG_MSG( printf("FRONTEND_DEMOD_TYPE == MSTAR_MSB124X_ parallel\n"));
...
FUNCTION_RESULT msAPI_Tuner_CheckSignalStrength(WORD *wStrength)
{
    ...
    //printf("msAPI_Tuner_CheckSignalStrength()- eCurDemod=%u\n", eCurDemod);
    ...
    *wStrength = wSignalStrength;
    return E_RESULT_SUCCESS;
        ...
        if (IsS2InUse())
        {
            if( FALSE == msAPI_Demod_S_Get_SignalStrength(&wSignalStrength) )
                return E_RESULT_FAILURE;
        }
        else
#endif
        ...
        else
        {
            ...
            if (DVB_T2_DEMOD == MApp_DTV_Scan_T2_ScanCheckSignal())
            {
                mdev_CofdmGetSignalStrength(&wSignalStrength);
            }
            else
        #endif
