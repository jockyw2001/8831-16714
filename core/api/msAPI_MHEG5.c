        ...
        if (mbxResult != E_MBX_SUCCESS)
        {
            if (MsOS_Timer_DiffTimeFromNow(u32MBXTimeOut) > 5000)
            {
                ...
                switch(mbxResult)
                {
                    ...
                    case E_MBX_ERR_PEER_CPU_NOTREADY:
                        printf("Error: peer cpu not initialized yet, not ready to receive mail message\n", mbxResult);
                        break;
                                              ...
                                              MS_WINDOW_TYPE *pstDisWin)
{
	...
	if( (pstSrcWin->y&1)!=0 || (pstSrcWin->height&1)!=0 )
	{
		...
		if( (ly1&1)!=0 )ly1++;
		if( (y2&1)!=0 )y2--;
		pstDisWin->height = (y2-pstSrcWin->y)*h2/h1+pstDisWin->y;
		pstDisWin->y = (ly1-pstSrcWin->y)*h2/h1+pstDisWin->y;
...
//TMP solution.
            ...
            //Finetune cropping size and display size for garbage line. kris 980617.
            {
                ...
                {
                    tmpDisWidth =  pstXC_SetWin_Info->stCropWin.width * widthScaleRatio / pstXC_SetWin_Info->stCapWin.width;
                    tmpDisHeight = pstXC_SetWin_Info->stCropWin.height * heightScaleRatio / pstXC_SetWin_Info->stCapWin.height;
