...
void msAPI_COPRO_Init(MS_U16 u16BinID, MS_U32 u32_ADR, MS_U32 u32_LEN)
{
 ...
 #elif (CHIP_FAMILY_TYPE == CHIP_FAMILY_NIKON)||(CHIP_FAMILY_TYPE == CHIP_FAMILY_MILAN)
    ...
    #if ENABLE_CI_PLUS
		#if( (CHIP_FAMILY_TYPE == CHIP_FAMILY_NIKON)||(CHIP_FAMILY_TYPE == CHIP_FAMILY_MULAN) ||(CHIP_FAMILY_TYPE == CHIP_FAMILY_MILAN))
    ...
    if(BIN_ID_CODE_VDPLAYER == u16BinID)
    {
        ...
        #elif ((CHIP_FAMILY_TYPE == CHIP_FAMILY_NIKON)||(CHIP_FAMILY_TYPE == CHIP_FAMILY_MULAN)||(CHIP_FAMILY_TYPE == CHIP_FAMILY_MILAN))
...
#if( VD_PLAYER_IS_IN_CROP_CPU )    
    else if(BIN_ID_CODE_VDPLAYER == u16BinID)
    {
        MDrv_MIU_Protect(AEON_MEM_USE_MIU_PROTECT_BLOCK_ID, &u8MIUProtectAeon_ID[0], ALIGN(u32_ADR + ((1<<pageshift) - 1), pageshift), ALIGN(u32_ADREnd, pageshift), ENABLE);
    }
#endif    
...
void msAPI_COPRO_Init_ByAddr(MS_U32 u32_ADR, MS_U32 u32_LEN) // fix me
{
    ...
    MDrv_COPRO_Init_Front();
#if (CHIP_FAMILY_TYPE!=CHIP_FAMILY_S7J) && (CHIP_FAMILY_TYPE!=CHIP_FAMILY_M10) //S7j does not has this client id
    u8MIUProtectAeon_ID[1] = MIU_CLIENT_GE_RW;
