...
{
#if ENABLE_CUST01_CEC
     if(/*((g_u8MenuPageIndex>=MENU_FACTORY)&&(g_u8MenuPageIndex<=MENU_FACTORY_WB_COPY_ALL))  //FactoryMenu On;
        ||g_u8MenuPageIndex==MENU_WATCHTIME_OVERTIME    //+ ken 2009-01-28
        || */ msAPI_Tuner_IsTuningProcessorBusy()         // AutoScaning;
        //||(!DB_HDMI_SETTING.g_bHdmiCecMode) //CEC Disable;
        //||(stGenSetting.g_stFactoryModeSetting.fFacHotKey)    //Hot Key PMode
        ||(IsStorageInUse()))  //  In USB Source
    {
        return TRUE;
    }
    else
