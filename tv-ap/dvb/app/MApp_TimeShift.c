...
static BOOLEAN  MApp_TimeShift_RecordEnable(void)
{
    ...
    #ifdef S3PLUS
    BOOLEAN bRestoreSubtitleSelIdx = FALSE;
    #endif
#endif
    ...
    if ((_enTimeShiftStatus=msAPI_PVR_RecordPathSet(_pstPvrRecordPath)) == E_PVR_API_STATUS_OK)
    {
      ...
      //msAPI_PVR_RecordPathAddPID(_pstPvrRecordPath, g_u16Current_PVR_AudioPID,       MSAPI_DMX_FILTER_TYPE_AUDIO);        //<--from other AP:(03) AUDIO
        ...
        {
            _pstPvrRecordPath->u8DVBSubtitleServiceNum   = 0;
            _pstPvrRecordPath->u8SubtitleMenuNum         = 0;

            _pstPvrRecordPath->u8SubtitleMenuSelectedIdx = 0;
        ...
        else if (SutitleItemSelIdx == 0)
        {
            if (MApp_UiMenuFunc_SelectSubtileLang())
            {
                ...
                _pstPvrRecordPath->u8SubtitleMenuSelectedIdx = 1;
        ...
        #ifdef PVR_8051
        _pstPvrRecordPath->u8SubtitleMenuSelectedIdx = SutitleItemSelIdx;
        _pstPvrRecordPath->u8TTXSubtitleServiceNum = u8TTXSubtitleServiceNum;
        #else
        _pstPvrRecordPath->u8SubtitleMenuSelectedIdx = u8SubtitleItemSelIdx;
        _pstPvrRecordPath->u8TTXSubtitleServiceNum = gu8TTXSubtitleServiceNum;
        #endif
        #endif
        ...
        for (u8i =0; u8i < u8DVBSubtitleServiceNum; u8i++)
        {
            if (u8i == SutitleItemSelIdx-1)
            {
                ...
                memcpy( _pstPvrRecordPath->PVRDVBSubtitleServices[0].StringCodes, DVBSubtitleServices[u8i].StringCodes, 4);
                _pstPvrRecordPath->PVRDVBSubtitleServices[0].u16PID                   = DVBSubtitleServices[u8i].u16PID;
                _pstPvrRecordPath->PVRDVBSubtitleServices[0].u16Ancillary_Page_id     = DVBSubtitleServices[u8i].u16Ancillary_Page_id;
                _pstPvrRecordPath->PVRDVBSubtitleServices[0].u16Composition_Page_id   = DVBSubtitleServices[u8i].u16Composition_Page_id;
                _pstPvrRecordPath->PVRDVBSubtitleServices[0].u8SubtitleType           = DVBSubtitleServices[u8i].u8SubtitleType;
                break;
            }
        }
        #else
        ...
        for (u8i =0; u8i < u8SubtitleMenuNum; u8i++)
        {
            if (u8i == SutitleItemSelIdx-1)
            {
                msAPI_PVR_RecordPathAddPID(_pstPvrRecordPath, SubtitleMenu[0].u16PID, MSAPI_DMX_FILTER_TYPE_SUBTITLE);     //<--from other AP:(05) SUBTITLE
                memcpy( _pstPvrRecordPath->PVRSubtitleMenu[0].StringCodes, SubtitleMenu[u8i].StringCodes, 4);
                _pstPvrRecordPath->PVRSubtitleMenu[0].u16PID                      = SubtitleMenu[u8i].u16PID;
                _pstPvrRecordPath->PVRSubtitleMenu[0].u8Magazine                  = SubtitleMenu[u8i].u8Magazine;
                _pstPvrRecordPath->PVRSubtitleMenu[0].u8Page                      = SubtitleMenu[u8i].u8Page;
                _pstPvrRecordPath->PVRSubtitleMenu[0].u8SubtitleType              = SubtitleMenu[u8i].u8SubtitleType;
                break;
            }
        }
...
static BOOLEAN MApp_TimeShift_Playback_SlowForward(void)
{
    ...
    //printf("MApp_Playback_SlowForward \n");
...
//	else
    ...
    if(u32PlayTime > u32PlayTime_old+50)
    {
        u32PlayTime  = u32PlayTime_old;
    }
    else
    {
        u32PlayTime_old = u32PlayTime;
    }
