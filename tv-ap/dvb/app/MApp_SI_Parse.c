...
static BOOL _MApp_SI_Parse_Restart_Component(void)
{
    ...
    if( IS_DTG_COUNTRY(OSD_COUNTRY_SETTING) &&_u16TempValidAudioIndex && (msAPI_CM_Get_AudioValidIndex() != _u16TempValidAudioIndex))
    {
                    ...
                    switch(u8Tag)
                    {
...

                        {
                            if(u16CurService_ID ==  TSGetU16(&pu8Section[3]))
                            {
                                ...
                                #else //this code should be modify by TTX team
...
#if (ENABLE_SBTVD_BRAZIL_APP == 0)
                                #if ENABLE_SUBTITLE
                                //if((pu8SubtitleDescriptor = MApp_Dmx_FindDescriptorTag(pu8LoopPosition + 5, TAG_SbD, u16TotalDescriptorLength) )!= NULL)
                                {
