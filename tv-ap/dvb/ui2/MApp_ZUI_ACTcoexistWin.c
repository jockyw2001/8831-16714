...
#define MBOX_GWIN_W   472
#define MBOX_GWIN_H   393
...
static void MApp_UiMenu_PAMuteWin_Create(void)
{
    ...
    MApp_ZUI_ACTcoexist_Create(COWIN_ID_PA_MUTE, MUTE_GWIN_PA_X, MUTE_GWIN_PA_Y, MUTE_GWIN_PA_W, MUTE_GWIN_PA_H);
...
#else
            if(MApp_CEC_ARC_Is_Connect() == TRUE)//anvi22
#endif
            {
...
void MApp_ARC_KeyProc_Mute(void)
{
	u8KeyCode = KEY_NULL;
		...
		if(MApp_FreeView_Is_Standby())
		    	return;
	#endif
    	if (MApp_CEC_ARC_GetAmpMute() == FALSE)
    	{
        	MApp_UiMenu_MuteWin_Show();
        	MApp_CEC_ARC_SetAmpMute(TRUE);
    	}
    	else
    	{
	    	#if ENABLE_TTX
