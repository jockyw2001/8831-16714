...
#include "MApp_Key.h"
#endif
#include "MApp_ZUI_Main.h"		//Ray DPW 2017.05.24: To access E_OSD_EMPTY==MApp_ZUI_GetActiveOSD()
#include "ZUI_tables_h.inl"		//Ray DPW 2017.05.24: To access E_OSD_EMPTY==MApp_ZUI_GetActiveOSD()
...
#ifdef sIntoFactOSDKey
static MS_U16 u16FactoryKeyCounter=0;
#endif
static MS_U16 u16MenuKeyCounter = 0;		//Ray DPW 2017.05.24: Menu key counter
extern U8 sFlagSimPowerShutDown;		//Ray DPW 2017.05.24: To access sFlagSimPowerShutDown status
...
BOOLEAN msAPI_GetKeyPad(U8 *u8key,U8 *u8Repstatus)
{
	...
	#ifdef sIntoFactOSDKey
	{
	  ////Ray DPW 2017.05.24: Implement default power on function. After pressing MENU key continuously for ~ 3 sec, it goes to power saving mode
	  ...
	  if((sFlagSimPowerShutDown&0x01)==0x00){
		...
		//Check if MENU key is pressed
		 if(msKeypad_CheckMenuKeyPress()){
			...
			//printf("\nRay first u16MenuKeyCounter=%d",u16MenuKeyCounter);	//Ray DBG 2017.06.05
			//If it's first time get MENU key in no OSD state, check if MENU key is already continuously pressed at before
			if(msKeypad_GetKey(u8key, u8Repstatus)==MSRET_OK){
			    if(*u8Repstatus==0){
				//printf("\nRay first inc u16MenuKeyCounter");	//Ray DBG 2017.06.05
				u16MenuKeyCounter++;		//We start to count MENU key only if MENU key is not pressed continuously at before
			    }
			}
		    }else{
		      //u16MenuKeyCounter++;
			//Ray DPW 2017.06.05: we found that the key detect speed is much faster in HMDI3 port.  So we need to use msKeypad_GetKey which has debounce function to
			//confirm key detection
			if(msKeypad_GetKey(u8key, u8Repstatus)==MSRET_OK){
			    //printf("\nRay u8key=%d",*u8key);	//Ray DBG 2017.06.05
			    if(*u8key==IRKEY_MENU){
				//printf("\nRay inc u16MenuKeyCounter=%d",u16MenuKeyCounter);	//Ray DBG 2017.06.05
				u16MenuKeyCounter++;		//We start to count MENU key only if MENU key is not pressed continuously at before
			    }
			}

			//if(u16MenuKeyCounter==400)
			//Ray DPW 2017.06.05: To use msKeypad_GetKey() function, the u16MenuKeyCounter only need 20 for 3 sec continuous detection
			if(u16MenuKeyCounter==20)
			{
			    //printf("\nRay u16MenuKeyCounter=%d",u16MenuKeyCounter);	//Ray DBG 2017.06.05
			    //printf("\nRay Go to set IRKEY_POWER=%d",IRKEY_POWER);	//Ray DBG 2017.06.05
			    u16MenuKeyCounter++;		//Add one more count to avoid sending IRKEY_POWER twice. Otherwise, it goes to STATE_TOP_STANDBY and no key response.
			    *u8Repstatus = 0;
			    *u8key = IRKEY_POWER;	//If MENU key is pressed continuously for ~3sec, assign key as power key to enter into power saving mode
			    return TRUE;
			}else{
			    *u8Repstatus = 0;
			    *u8key=IRKEY_DUMY;
			    return FALSE;		//If MENU key is started pressed continuously, we won't send MENU key out to display menu
			}
		    }
		}
		else{
		  if(u16MenuKeyCounter>0){
		      u16MenuKeyCounter = 0;			//Ray DPW 2017.05.24: Reset menu key counter as menu key is not pressed continuously
		      //printf("\nRay menu key not pressed. Send MENU key");	//Ray DBG 2017.06.05
		      *u8key = IRKEY_MENU;			//we need to send MENU key to display menu
		      *u8Repstatus = 0;
		      return TRUE;
		  }
		}
	  ...
	  //Ray DPW: Else, it's in power saving mode
	  else{
	      if(msKeypad_CheckMenuKeyPress())
	      {
		  //If user still continuously press MENU key, no more action to send MENU key to turn back to normal mode, until user releases the MENU key
		  if(u16MenuKeyCounter>0){
		      return FALSE;
		  }
	      }else{
	  ...
	  if(msKeypad_CheckFactoryKey())
	  //if(msKeypad_CheckFactoryModeKey())
	  {
	  	*u8Repstatus = 0;
	  	/*if(MApp_ZUI_GetActiveOSD()!=E_OSD_FACTORY_MENU)
	  	{*/
		  	u16FactoryKeyCounter++;
		  	//MsOS_DelayTask(1);
	  		if(u16FactoryKeyCounter>=800)
	  		{				
	  			*u8key = KEY_DIR_FACTORY;
		  		//*u8Repstatus = 0;
		  		u16FactoryKeyCounter = 0;
	  			//printf("MingYuan Print Key u16FactoryKeyCounter %d \n",u16FactoryKeyCounter);
	  			return TRUE;
	  		}
	  		else
	  		{
