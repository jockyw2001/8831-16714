...
U8* EventInfoLine3Left(void)
{
    ...
    au8Section[0]= 0;
    iu8Buffer_i=0;
    static char* locbuff[MSAPI_DMX_SECTION_4K];
    memset(locbuff,0,MSAPI_DMX_SECTION_4K);
    ...
    if (stGenSetting.g_SysSetting.fTimeAutoSync == TIME_AUTO_SYNC_OFF)
    {
        ...
        {
            iu8Buffer_i += snprintf((char *)locbuff,sizeof(locbuff), "%04u/%02u/%02u %02u:%02u",
                stDate.u16Year, stDate.u8Month, stDate.u8Day, stDate.u8Hour, stDate.u8Min);
        }
        else
        {
            iu8Buffer_i += snprintf((char *)locbuff,sizeof(locbuff), "%04u/%02u/%02u %02u:%02u %s",
        ...
        if (IsDTVInUse() && MApp_EpgDB_GetCurEvent(&stEpgEventInfo))
        {
            ...
            if (stGenSetting.g_SysSetting.fTimeFormat == 1) // 12-hour
            {
                if (stDate.u8Hour == 0)
                    ...
                    iu8Buffer_j -= 12;
                iu8Buffer_i += snprintf((char *)au8Section+iu8Buffer_i,sizeof(au8Section), "%02u:%02u%s-",
                        iu8Buffer_j, stDate.u8Min, AMPMText(stDate.u8Hour));

            }
            else
            {
                iu8Buffer_i += snprintf((char *)locbuff+iu8Buffer_i,sizeof(locbuff), "%02u:%02u-",
            ...
            if (stGenSetting.g_SysSetting.fTimeFormat == 1) // 12-hour
            {
                if (stDate.u8Hour == 0)
                    ...
                    iu8Buffer_j -= 12;
                snprintf((char *)locbuff+iu8Buffer_i,sizeof(locbuff), "%02u:%02u%s",
                    iu8Buffer_j, stDate.u8Min, AMPMText(stDate.u8Hour));

            }
            else
            {
                snprintf((char *)locbuff+iu8Buffer_i,sizeof(locbuff), "%02u:%02u",
                        stDate.u8Hour, stDate.u8Min);
            }
        }
    }
    ...
    //strcpy((char *)au8Section,(char *)locbuff);
    strncpy((char *)au8Section,(char *)locbuff, 1024-1);
    return au8Section;
}
