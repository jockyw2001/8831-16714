...
void msAPI_ATV_ResetChannelData(EN_CHANNEL_RESET_SELECTION eChannelResetType)
{
    BYTE i;
    EN_ANT_TYPE bAntennaBackup;

    bAntennaBackup = _bAntenna;

#if ENABLE_SBTVD_BRAZIL_APP
    msAPI_Reset_Tuner_GetNumberOfChBeFound_WhileAutoScan();
#endif

    if( (eChannelResetType & E_CHRESET_AIR) == E_CHRESET_AIR )
    {
        _bAntenna = ANT_AIR;
        ...
        {
            printf("[%bu][%bu] act %bu sea %bu ski %bu\n",
                i, _GetPRIndexTable(i), msAPI_ATV_IsProgramNumberActive(i), msAPI_ATV_IsProgramSearched(i), msAPI_ATV_IsProgramSkipped(i));
        }
        #endif

        _bAntenna = bAntennaBackup;
    }

    if( (eChannelResetType & E_CHRESET_CATV) == E_CHRESET_CATV )
    {
        _bAntenna = ANT_CATV;
        _SaveProgramNumber(DEFAULT_LASTPROGRAM);
...
static BOOLEAN _GetPRTable(BYTE u8Index, BYTE * pu8Buffer, BYTE u8Param)
{
    ...
    else//ANT_CATV
    {
        ...
        case PRDATA_SORT_PARAM:
            ...
            {
                u8Sort = (BYTE)LOWEST_SORTING_PRIORITY;
                memcpy(pu8Buffer,&s8FineTune,sizeof(BYTE));
            }
            return TRUE;
...
#if (ENABLE_SBTVD_BRAZIL_APP || ENABLE_DVB_TAIWAN_APP||(TV_SYSTEM == TV_NTSC))
//------------------------------------------------------------------------------
