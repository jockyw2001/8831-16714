...
BOOLEAN MApp_PVR_SetDiskDriveIndex(U8 u8DriveIndex, BOOLEAN bForceInit)
{
    ...
    if ((msAPI_PVRFS_GetFileSystemType() == PVR_FS_BULKFS) || (bForceInit == TRUE))
    {
        // 0854334
...
/*Fix : URI protect PVR function*/
    ...
    if(msAPI_CI_CardDetect() && (EN_CI_VERSION_CI_PLUS == msAPI_CI_CC_CAM_Mode()))
    {
        ...
        if((( 0x00 != u8URI_EMI )||( EN_CC_CICAM_PIN_CAPABILITIES_CICAM_PIN_ONLY_CACHED == eCicamPinCap )||( EN_CC_CICAM_PIN_CAPABILITIES_BOTH_PIN_CACHED == eCicamPinCap ))
    ...
    switch(_enPVRState)
    {
        ...
        case E_PVR_STATE_WAIT:
        {
            DEBUG_PVR_SM_TIME();
            U8 u8USBState = MApp_MassStorage_DeviceConnectionDetect();
            if (((u8USBState & MS_USB_CONNECTED) || (u8USBState & MS_USB2_CONNECTED))&&(u8DMPStartFlag == FALSE))
            {
    ...
    if( msAPI_Timer_DiffTime_2( u32Time1, MsOS_GetSystemTime() ) > 200 )
    {
