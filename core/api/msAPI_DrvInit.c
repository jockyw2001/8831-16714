...
#include "msAPI_IR.h"
#include "MApp_Init.h"
#include "dvExtAudio.h"		//Ray EXA 2017.04.05
...
#define POWERDOWN_EXEC_DBG(x)       x//x
...
#if (defined(ENABLE_NAND) && (ENABLE_NAND==1))
extern int drvNAND_FLASH_INIT(U32 u32BufAddr);
#endif
...
#ifdef __mips__
  #if ( _BOOT_FROM_51_  == 1 )
      extern void MDrv_BOOT51_SetSPIOffsetForMCU( void );
  #else
...
void msAPI_Power_SetPowerMode2PM51(PM_WakeCfg *PmWakeCfg)
{
    ...
    #if ( _BOOT_FROM_51_  == 1 )
        MDrv_BOOT51_SetSPIOffsetForMCU();
    #else
...
void MDrv_Power_ExecutePowerDown2PM51(PM_WakeCfg *PmWakeCfg, PM_PowerDownCfg *PmPowerDownCfg)
{
    ...
    POWERDOWN_EXEC_DBG(("\33[1;29mMDrv_Power_ExecutePowerDown_N51 \n\33[0m"));
...
void msAPI_Power_PowerDown_EXEC(void)
{
    ...
    if ( DB_HDMI_SETTING.g_bHdmiCecMode )
    {
        ...
        {
            msAPI_CEC_PostMsg_SystemAudioModeRequestTerminationRequest(DIRECTMSG);
            MsOS_DelayTask(200);
        }
...
void msAPI_PowerON_EXEC()
{
  ...
  POWERDOWN_EXEC_DBG(printf(">> msAPI_PowerON_EXEC() \n"));
...
// HPD should be controled after hdmi init
/*
#if (ENABLE_HDMI_HPD_TASK )
  #if (!BLOADER)
    MApi_XC_HPD_Task_Start(TRUE);    //Enable state machine flow
    MApi_XC_HPD_Task(TRUE);        //For state machine go one step forward to set HDMI_HPD low directly
  #endif
#else
    // Set HPD low here to reduce boot time
...
#if (BLOADER)
    DMXBinInfo.B_ID = BIN_ID_CODE_DEMUX_BL;
#else
...
// 0: Dual SAW; 1: external Single SAW; 2:silicon tuner; 3: no saw(VIF); 4: internal single SAW; 5: no SAW(DIF)
    ...
    InitIOExpander(); 	//Ray IOE 2017.02.02: Move IO expander before MApi_XC_Sys_Init( )
...
#ifdef sSP4096Board
    dv_ExtAudioVolumeInit();		//Ray EXA 2017.03.31: Init external audio board with middle volume
#endif
    //InitIOExpander(); //for other chip	//Ray IOE 2017.02.02: Move IO expander before MApi_XC_Sys_Init( )
...
#if 0//(ENABLE_DTV) //DMX error... (Ignoring packet error, continuing...)
    BININFO   DMXBinInfo;
    BOOLEAN   bResult;
    MS_U32    u32DMXBinAddr, u32DMXTmp;
...
#if (BLOADER)
    if(!MDrv_DMA_LoadBin(&DMXBinInfo, u32DMXBinAddr, _PA2VA((POOL_BUFFER_MEMORY_TYPE & MIU1 ? (POOL_BUFFER_ADR | MIU_INTERVAL): (POOL_BUFFER_ADR))), _PA2VA((POOL_BUFFER_MEMORY_TYPE & MIU1) ? (POOL_BUFFER_ADR | MIU_INTERVAL): (POOL_BUFFER_ADR)+BUF_FOR_DECOMPRESS_OFFSET1)))
    {
        printf("\r\n !Load DMXBin Fail!");
    }
#else
    ...
    //MAPI_VID_Init(E_VDEC_CODEC_TYPE_H264,FALSE);
    ...
    vdecCfg.u32CodeAddr        = ((VDEC_AEON_MEMORY_TYPE & MIU1) ? (VDEC_AEON_ADR | MIU_INTERVAL) : (VDEC_AEON_ADR));
    vdecCfg.u32CodeLen         = VDEC_AEON_LEN;
    vdecCfg.u32FramebuffAddr   = VDEC_FRAMEBUFFER_ADR;
    vdecCfg.u32FramebuffLen    = VDEC_FRAMEBUFFER_LEN;
    vdecCfg.u32StreambuffAddr  = VDEC_BITSTREAM_ADR;
    vdecCfg.u32StreambuffLen   = VDEC_BITSTREAM_LEN;
    vdecCfg.u16FirmwareSrcID   = BIN_ID_CODE_MVD3;
    vdecCfg.eVdecType          = E_VDEC_MVD3;
    vdecCfg.u32FirmwareVer     = MVD3_FW_VERSION;

    if (MDrv_VDEC_Init(&vdecCfg) != SUCCESS)
    {
        printf("VDEC Init MVD3 fail!\n");
    }

    vdecCfg.u16FirmwareSrcID   = BIN_ID_CODE_AVCH264;
    vdecCfg.eVdecType          = E_VDEC_SVD;
  #if (DEBUG_H264 == 1)
    vdecCfg.u32FirmwarePara    = TRUE;  //enable ISR for debugging
  #else
    vdecCfg.u32FirmwarePara    = FALSE;
  #endif
    if (MDrv_VDEC_Init(&vdecCfg) != SUCCESS)
    {
        printf("VDEC Init SVD fail!\n");
    }
    */
  #endif // #if (!BLOADER)

  #if (defined(ENABLE_NAND) && (ENABLE_NAND==1))
    drvNAND_FLASH_INIT(((NAND_COPY_BUFFER_MEMORY_TYPE & MIU1) ? (NAND_COPY_BUFFER_ADR | MIU_INTERVAL) : (NAND_COPY_BUFFER_ADR)));
  #endif
