...
//       &&( (UI_INPUT_SOURCE_TYPE == UI_INPUT_SOURCE_DTV)
      ...
      )
    {
        if( MApp_PVR_IsRecording() )
        {
            ...
            MApp_Ui_InputSourcePage_Notify(EN_INPUT_SOURCE_PAGE_NOTIFY_PVR_SOURCE_CHANGE_CHECK_PAGE , NULL, NULL);
            return;
        }
    }
#endif
...
*/
    {
        // Move to MApp_InputSource_SwitchSource()
        /*
    #if ENABLE_DMP
...
#if 0//def ENABLE_GAME
        case EN_FUNC_INPUTSOURCE_LOAD_APP_BIN_FROM_USB:
            if(LoadAppBinFromUSB(para1))
                enResult = EN_FUNC_INPUTSOURCE_STATUS_OK;
            break;

        case EN_FUNC_INPUTSOURCE_GET_SUITABLE_FILE_NUM:
            *p_para2 = MApp_GetSuitableFileNum();
            enResult = EN_FUNC_INPUTSOURCE_STATUS_OK;
            break;

        case EN_FUNC_INPUTSOURCE_GET_LONG_FILE_NAME:
            MApp_GetLongFileName(para1, p_para2);
            break;
#endif
