...
void mhal_combo_HDCP2Initial(void)
{
	...
	MDrv_WriteByteMask(REG_100E62, 0x00, 0xFF); //enable clk
    ...
    // HDMI 2.0 Port1
	//tmp: wilson test; write 74ram for HDCP22 version
	MDrv_WriteByteMask(REG_100B2F, BIT2, BIT2); //HDCP enable for DDC
	MDrv_WriteByteMask(REG_100B33, BIT7, MASKBIT(7:6)); //write enable


	MDrv_Write2ByteMask(REG_100B2E, 0x50, MASKBIT(9:0)); //CPU r/w address (for hdcp_key_sram/74reg)
	MDrv_WriteByteMask(REG_100B32, BIT5, BIT5); //HDCP address load pulse generate
	MDrv_WriteByte(REG_100B30, 0x04); //bit2: HDCP22 version
	MDrv_WriteByteMask(REG_100B32, BIT4, BIT4); // REG_COMBO_DVI_HDCP_DUAL_19[4]: HDCP data write port pulse generate

	//clear RxStatus wirte msg length
	MDrv_WriteByte(REG_100BC6, 0x00);//RIU[REG_1006C6] = 0x00;
	MDrv_WriteByteMask(REG_100BC9, BIT7, BIT7);

        //HDCP 2.2 or HDCP 1.4
        //[1] Enable auto-clear SKE status when receiving ake_init
        //[2] Enable auto-clear SKE status when no hdcp22 capability
        MDrv_WriteByteMask(REG_100B9C, 0x06, 0x06);
#endif
    }
}
...
Bool mhal_combo_HDCP2GetReadDownFlag(BYTE ucPortSelect)
{
    ...
    switch(ucPortSelect)
    {
        ...
        case COMBO_INPUT_SELECT_PORT1:
            bReadDownFlag = ((MDrv_ReadByte(REG_100BCC) & BIT3) ? TRUE : FALSE); // REG_HDCP_P1_66[3]: Read down flag

            if(bReadDownFlag)
            {
                MDrv_WriteByteMask(REG_100BCC, BIT3, BIT3); // REG_HDCP_P1_66[3]: Clear read down flag
            }
        ...
        case COMBO_INPUT_SELECT_PORT2:
        case COMBO_INPUT_SELECT_PORT3:
            bReadDownFlag = ((MDrv_ReadByte(REG_1610CC) & BIT3) ? TRUE : FALSE); // REG_HDCP_DUAL_66[3]: Read down flag
            if(bReadDownFlag)
            {
                MDrv_WriteByteMask(REG_1610CC, BIT3, BIT3); // REG_HDCP_DUAL_66[3]: Clear read down flag
            }
            break;
