...
void MApp_TTX_SetDTV_TTXSystem(U16 pid, BOOLEAN bExitPVR)
{
    ...
    MApp_Dmx_SetFid(MSAPI_DMX_INVALID_FLT_ID, EN_TTX_FID);
        ...
        if (MApp_PVR_IsPlaybacking())
        {
            msAPI_DMX_StartFilter(pid, MSAPI_DMX_FILTER_TYPE_TELETEXT|MSAPI_DMX_FILTER_SOURCE_TYPE_FILE, MApp_Dmx_GetFid(EN_TTX_FID));
        }
        else
#endif
        {
            msAPI_DMX_StartFilter(pid, MSAPI_DMX_FILTER_TYPE_TELETEXT, MApp_Dmx_GetFid(EN_TTX_FID));
        }
...
//** osmans 220408 CNI/VPS specific country and language functions changed so deleted msAPI_CNI_SetCountryLangId() */
    ...
    if( eCountry == E_GERMANY || eCountry == E_AUSTRIA ||    // kk 0331-1 teletext - doesn't set here
        eCountry == E_ITALY || eCountry == E_SWITZERLAND )
