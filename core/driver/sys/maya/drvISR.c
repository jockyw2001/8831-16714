...
static void TimerISR(void)
{
...
#if (ENABLE_SECURITY_R2_HDCP22)
    if(u16HdcpDedicatedTimeMs) u16HdcpDedicatedTimeMs--;
#endif
    ...
    if (gTimerCount0%1000==0)
    {
		...
		if(bLightSensorFlag)
		{
    	if((gTimerCount0%1000==0)&&(bDetectLS_ADC==FALSE))
    	{		
				u16LightSensorCount++;
				if(G_FACTORY_SETTING.stFacLightSensorSetting.u8LS_Delay)
				{
					bDetectLS_ADC = TRUE;
					u16LightSensorCount = 0;
				}	
			}
			if(gTimerCount0%10==0)
			{
				if((u8CheckBklStatus&0x01)==0x00)
				{
				  u8BklPwmSmoothTickCounter++;
				  if(u8BklPwmSmoothTickCounter>=u8BklPwmSmoothTime)
				  {
			  		u8BklPwmSmoothTickCounter = 0;
			  		u8CheckBklStatus = 0x01;
			  	}
			  }	
			}					
		}	
