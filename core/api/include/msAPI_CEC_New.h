...
#define CEC_IR_ENABLE_REPEAT_KEY_PRESS_TIME     200//490//ms //time out, send <User Control Released>
#define CEC_IR_USER_CONTROL_PRESSED_TIME        200//ms
...
#define B_MONITOR_CEC_AUDIO_SYS_PERIOD          2000    // Audio sys polling time  // anviarc
...
typedef struct _MAPI_CEC_INFO_LIST
{
    ...
    BOOLEAN bCecDevicesExisted[E_LA_MAX];

    MS_U16   u16PhysicalAddress[E_LA_MAX];

    MS_U8   u8CecVendorID[E_LA_MAX][3];

    MS_U8   u8CECVersion[E_LA_MAX];

    MS_U8   u8CecDeviceName[E_LA_MAX+1][MAX_CEC_DEVICE_OSD_NAME];

    MsCEC_MSG_POWER_STATUS_PARM enPowerStatus[E_LA_MAX];

    MsCEC_DEVICE_TYPE enDeviceType[E_LA_MAX];
} MSAPI_CEC_INFO_LIST;
...
void msAPI_CEC_PostMsg_SystemAudioModeRequestTerminationRequest(EN_CEC_SEND_MSG enCecSendMsg);
...
BOOLEAN msAPI_CEC_IrdaKeyProcess(U8 ircode);
...
#if 1   // TO DO. should be removed in CEC_NEW_VERSION
