...
#if 0
U16 g_u16CurrentServiceID=0;
WORD g_wCurrentTS_ID,g_wCurrent_ONID;

WORD g_wCurrentTVUserServicePosition,g_wCurrentRadioUserServicePosition;
MEMBER_SERVICETYPE g_eCurrentRealServiceType;
WORD g_wCurrentTVRealServicePosition,g_wCurrentRadioRealServicePosition;
#endif
#endif
    ...
    switch ( u8KeyCode )
    {
        case KEY_CHANNEL_PLUS:
                ...
                )
            {
                ...
                {
                    #if (ENABLE_SBTVD_BRAZIL_CM_APP == 0)
                    dmSetLastWatchedOrdinal();
                    #endif
...
void MApp_ChannelChange_Set_Audio_Decoder_System(WORD wAudType )
{
    ...
    if( wAudType == E_AUDIOSTREAM_AC3 || wAudType == E_AUDIOSTREAM_AC3P || wAudType == E_AUDIOSTREAM_AAC || wAudType == E_AUDIOSTREAM_MPEG4)
    {
    ...
    for(k=0 ; k<5 ; k++)
    {
        for(i=0; i<AudioLangNum; i++)
        {
            ...
            for(j=0; j<MAX_AUD_ISOLANG_NUM; j++)
            {
                ...
                if(aAudioStreamInfo.aISOLangInfo[j].bAudType == 0x00 || u8Nocheck_AudType == 1)
                {
                    if(u8AudLangCodeIdx == enSoundAudioLan || enSoundAudioLan == SI_LANGUAGE_NONE)
                    {
                        switch(k)
                        {
                            case 0:  if(aAudioStreamInfo.wAudType == E_AUDIOSTREAM_MPEG4){
                                        *u8AudLangSelected = i;
                                        *u8LRAudioMode = aAudioStreamInfo.aISOLangInfo[j].bISOLanguageInfo;
                                        return;
                                     }break;
                            case 1:  if(aAudioStreamInfo.wAudType == E_AUDIOSTREAM_AAC){
                                        *u8AudLangSelected = i;
                                        *u8LRAudioMode = aAudioStreamInfo.aISOLangInfo[j].bISOLanguageInfo;
                                        return;
                                     }break;
                            case 2:  if(aAudioStreamInfo.wAudType == E_AUDIOSTREAM_AC3P){
                                        *u8AudLangSelected = i;
                                        *u8LRAudioMode = aAudioStreamInfo.aISOLangInfo[j].bISOLanguageInfo;
                                        return;
                                     }break;
                            case 3:  if(aAudioStreamInfo.wAudType == E_AUDIOSTREAM_AC3){
                                        *u8AudLangSelected = i;
                                        *u8LRAudioMode = aAudioStreamInfo.aISOLangInfo[j].bISOLanguageInfo;
                                        return;
                                     }break;
                            case 4:  if(aAudioStreamInfo.wAudType == E_AUDIOSTREAM_MPEG){
                                        *u8AudLangSelected = i;
                                        *u8LRAudioMode = aAudioStreamInfo.aISOLangInfo[j].bISOLanguageInfo;
                                        return;
                                     }break;
    ...
    for(k=0 ; k<5 ; k++)
    {
        for(i=0; i<AudioLangNum; i++)
        {
            ...
            for(j=0; j<MAX_AUD_ISOLANG_NUM; j++)
            {
                ...
                if(aAudioStreamInfo.aISOLangInfo[j].bAudType == 0x00 || u8Nocheck_AudType == 1 || (bCheckBroadcastMix && aAudioStreamInfo.aISOLangInfo[0].bBroadcastMixedAD))
                {
                    ...
                    (TRUE == aAudioStreamInfo.aISOLangInfo[j].bIsValid))
                    {
                        switch(k)
                        {
                            case 0:  if(aAudioStreamInfo.wAudType == E_AUDIOSTREAM_AC3){
                                        *u8AudLangSelected = i;
                                        *u8LRAudioMode = aAudioStreamInfo.aISOLangInfo[j].bISOLanguageInfo;
                                        return;
                                     }break;
                            case 1:  if(aAudioStreamInfo.wAudType == E_AUDIOSTREAM_AC3P){
                                        *u8AudLangSelected = i;
                                        *u8LRAudioMode = aAudioStreamInfo.aISOLangInfo[j].bISOLanguageInfo;
                                        return;
                                     }break;
                            case 2:  if(aAudioStreamInfo.wAudType == E_AUDIOSTREAM_MPEG4){
                                        *u8AudLangSelected = i;
                                        *u8LRAudioMode = aAudioStreamInfo.aISOLangInfo[j].bISOLanguageInfo;
                                        return;
                                     }break;
                            case 3:  if(aAudioStreamInfo.wAudType == E_AUDIOSTREAM_AAC){
                                        *u8AudLangSelected = i;
                                        *u8LRAudioMode = aAudioStreamInfo.aISOLangInfo[j].bISOLanguageInfo;
                                        return;
                                     }break;
        ...
        for(i = 0; i < AudioLangNum; i++)
        {
            ...
            for(j = 0; j < MAX_AUD_ISOLANG_NUM; j++)
            {
                ...
                if(aAudioStreamInfo.aISOLangInfo[j].bAudType == 0x00)
                {
                    ...
                    #if (ENABLE_SBTVD_BRAZIL_APP==0)
                    else if(IS_DTG_COUNTRY(OSD_COUNTRY_SETTING) && ((u8GetPreferredLang_eng == 0xFF && u8AudLangCodeIdxL == SI_LANGUAGE_ENGLISH)
                           || (u8GetPreferredLang_und == 0xFF && u8AudLangCodeIdxL == SI_LANGUAGE_UND)
                           || (u8GetPreferredLang_qaa == 0xFF && u8AudLangCodeIdxL == SI_LANGUAGE_QAA)))
                    {
    ...
    BYTE cRFChannelNumber, MEMBER_SERVICETYPE bServiceType, WORD wCurrentPosition )
{
    ...
    if( IsS2InUse() )
    {
        ...
        g_stCurS2ProggramId = _astS2ProgramIDTable[CurrentS2idx];
        //printf("\r\nCurrentS2idx=%d [%d]",CurrentS2idx,TP.SatelliteId);

        _GetSatInforByID(g_stCurS2ProggramId.SatelliteId,&sat);
...
void MApp_ChannelChange_EnableChannel_2(SCALER_WIN eWindow)
{
    ...
    if(g_stFactorySetting.stFacMiscSetting.eFactoryExecuteScript== EN_FACTORY_EXECUTE_SCRIPT_ON)
    {
        ...
        {
            msUS_ScriptExecute(EN_SCRIPT_EVENT_CHANNELCHANGE);
        }
    }
#endif
...
//    SHOW_VIDEO_INFO(("MApp_ChannelChange_EnableChannel eWindow=%d",eWindow));
    ...
    if(IsSrcTypeATV(SYS_INPUT_SOURCE_TYPE(eWindow)))
    {
        if ( SYS_INPUT_SOURCE_TYPE(eWindow) != INPUT_SOURCE_TV )
        {
...
//        WORD wAudPID;
    ...
    //#if ENABLE_T_C_COMBO
        MApp_Dmx_Enable_NIT_Other(IsDVBCInUse());
    //#elif ENABLE_DVBC
                    ...
                    if (INVALID_PID != stAdAudioStreamInfo.wAudPID)
                    {
                      ...
                      #endif
                        {
                            if (stGenSetting.g_SoundSetting.bEnableAD)
                            {
                                ...
                                /* set decoder system and reload firmware code */
                                if (stAdAudioStreamInfo.wAudType == E_AUDIOSTREAM_MPEG)
                                {
                                    //printf("mpeg ad\n");
                                    MApi_AUDIO_SetSystem(MSAPI_AUD_DVB2_MPEG);
                                }
                                else if (stAdAudioStreamInfo.wAudType == E_AUDIOSTREAM_AC3)
                                {
                                    //printf("ac3 ad\n");
                                    MApi_AUDIO_SetSystem(MSAPI_AUD_DVB2_AC3);
                                }
                                else if (stAdAudioStreamInfo.wAudType == E_AUDIOSTREAM_AC3P)
                                {
                                    //printf("ac3p ad\n");
                                    MApi_AUDIO_SetSystem(MSAPI_AUD_DVB2_AC3P);
                                }
                                else if (stAdAudioStreamInfo.wAudType == E_AUDIOSTREAM_AAC || stAdAudioStreamInfo.wAudType == E_AUDIOSTREAM_MPEG4)
                                {
                                    //printf("he-aac ad\n");
                                    MApi_AUDIO_SetSystem(MSAPI_AUD_DVB2_AAC);
                                }

                                /* set ad audio PID & start filter */

                                // Marked by coverity_0458
                                //printf("## Channel change, ad audio pid 0x%x, u8AdAudFid %bu\n", stAdAudioStreamInfo.wAudPID, *MApp_Dmx_GetFid(EN_AD_FID));

                                eDMXFLTSTA = msAPI_DMX_StartFilter( stAdAudioStreamInfo.wAudPID, MSAPI_DMX_FILTER_TYPE_AUDIO2, MApp_Dmx_GetFid(EN_AD_FID));
                               // printf("## Channel change, eDMXFLTSTA %bu, ad audio pid 0x%x, u8AdAudFid %bu\n", eDMXFLTSTA, msAPI_DMX_GetFilterPID(*MApp_Dmx_GetFid(EN_AD_FID)), *MApp_Dmx_GetFid(EN_AD_FID));
                              #endif
...
EN_DETECT_SIGNAL_STATE MApp_ChannelChange_DetectSignalStatus(E_UI_INPUT_SOURCE input_type)
{
    ...
    switch(input_type)
    {
        ...
        case UI_INPUT_SOURCE_RGB:    // PC - VGA         3
        {
            ...
            if(ret!=STATE_DETECT_SIGNAL_NOSIGNAL&&ret!=STATE_DETECT_SIGNAL_PC_UNSUPPORT_MODE)
                ret = STATE_DETECT_SIGNAL_OK;
        }
        break;
...
//static Bool bFontLoadF=1;
    ...
    switch ( enChannelChangeState )
    {
        ...
        case STATE_CHANNELCHANGE_MSGBOX_FADING:
        {
            ...
            if ( FALSE==fWaitforMSGBoxFading || msAPI_Timer_DiffTimeFromNow(u32ChannelChangeTimer ) > u32InfoOSD_Duration-3500L) //unit = ms
            {
            #if MHEG5_ENABLE
...
void MApp_ChannelChange_ShowEventInfoBanner(void)
{
    enChannelChangeState = STATE_CHANNELCHANGE_DISPLAY;
...
#if ENABLE_DTV
BOOLEAN MApp_ChannelChange_ChangeSpeciChannel( U16 u16InputValue, U16 wOriginalNetwork_ID, U16 wTransportStream_ID, EN_MHEG_5_TUNE_TYPE bType, BOOLEAN bCheckTsID )
{
  #if MHEG5_ENABLE
    ...
    {
        WORD wAudType = E_AUDIOSTREAM_INVALID;
    ...
    // lcn process
    if ( ( bType == TYPE_TUNE_STREAM_LCN ) || ( bType == TYPE_CHANNEL_CHANGE_LCN ) || ( bType == TYPE_CHANNEL_CHANGE_LCN_QUIETLY )
     #if MHEG5_ENABLE
      #if MHEG5_NDT
    ...
    {
        for ( u16Loop = 0; u16Loop < MAX_DTVPROGRAM; u16Loop++ )
        {
    ...
    if(bType == TYPE_CHANNEL_CHANGE_SER_ID_QUIETLY || bType == TYPE_CHANNEL_CHANGE_LCN_QUIETLY
        || bType == TYPE_TUNE_STREAM_SER_ID || bType == TYPE_TUNE_STREAM_LCN
      #if MHEG5_NDT
