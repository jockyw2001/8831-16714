...
//#if (EPG_THIS_CH==TRUE)
//extern  U32                     u32ThisEPG_SystemTimeUpdate;
//#endif
...
static MS_EPG_EVENT_ITEM        astEventItem[256];
    ...
    for(u8TblIdx = 0; u8TblIdx < MAX_TBL_NUM; u8TblIdx++)
    {
        ...
        for(u8Event = 0; u8Event < pDispVch->au8CntEvent_InTbl[u8TblIdx]; u8Event++)
        {
...
#endif
            {
                ...
                astEventItem[stEventPool.u8TotalNum].u8TblIdx   = u8TblIdx;
                astEventItem[stEventPool.u8TotalNum].u8EventIdx = u8Event;
...
//#if (EPG_THIS_CH==TRUE)
    //u32ThisEPG_SystemTimeUpdate = msAPI_Timer_GetTime0();
//#endif
...
{
/*
    //Update System time in title bar
    if (msAPI_Timer_DiffTimeFromNow(u32ThisEPG_SystemTimeUpdate)> EPGUI_SYSTEMTIME_UPATE_PERIOD)
    {
        MApp_EpgUI_GetandDrawSystemTime();
        u32ThisEPG_SystemTimeUpdate = msAPI_Timer_GetTime0();
    }
*/
 ...
 void MApp_EpgZUI_Get_Event(U8 u8Index)
{
    ...
    MApp_EpgDB_LoadEventTitle(0, pstEvent->u16DbTitlePos, &pSrcEventTitle);
...
#if SUPPORT_LONG_ETT
LPTSTR MApp_EpgZUI_GetExtendText(MS_EPG_EVENT *pEvent)
{
    MS_EPG_EVENT_ETT pSrcEventETT;
    U8 i;
    U8 *puString = pu8EpgString;
    U16 u16ETT_length;
    U16 u16DbEttPos;
    BOOL fIsEttUTF16;
    ...
    MApp_EpgDB_LoadEventETT(0, u16DbEttPos, &pSrcEventETT);
...
static U8* MApp_EpgZUI_Get_Extend_Text_Table(U8 u8Index)
{
    ...
    if ((stEpgDBInfo.u8EttRxOK == MAX_TBL_MASK) || (pstEvent->u16DbEttPos != NULL) )
    {
        ...
        MApp_EpgDB_LoadEventETT(0, pstEvent->u16DbEttPos, &pSrcEventETT);
		//au8EpgUiBuf = &pSrcEventETT.pu8ETT_content;
...
BOOLEAN MApp_ZUI_ACT_HandleEpgKey_ATSC(VIRTUAL_KEY_CODE key)
{
    ...
    switch(key)
    {
        ...
        case VK_PAGE_UP:
            if( MApp_ZUI_API_GetFocus() == HWND_EPG_PROG_TITLE_PANEL )
                return TRUE;
            if (stEventPool.u8TotalNum==0 || u8EpgCurEventIndex == 0)
                return TRUE;
        ...
        case VK_PAGE_DOWN:
            if( MApp_ZUI_API_GetFocus() == HWND_EPG_PROG_TITLE_PANEL )
                return TRUE;
            if (stEventPool.u8TotalNum==0 || u8EpgCurEventIndex == stEventPool.u8TotalNum-1)
                return TRUE;

            if ( (u8EpgCurEventIndex + EVENT_PER_PAGE) >= (stEventPool.u8TotalNum-1))
                u8EpgCurEventIndex = stEventPool.u8TotalNum-1;
            else
                u8EpgCurEventIndex += EVENT_PER_PAGE;
...
//printf("Funs=%s, line=%d, act=%d\n",__FUNCTION__,__LINE__,act);
     switch(act)
    {
        ...
        case EN_EXE_EPG_SHOW_PREV_INFO:
            ...
            if ((u8EpgCurEventIndex >= stEventPool.u8TotalNum-1) ||
    ...
    switch(hwnd)
    {
        ...
        case HWND_EPG_EVENT_INFO_PANEL_ETT_TEXT:
            ...
            if(TempEvent->fIsEitUTF16
                ||((hwnd == HWND_EPG_EVENT_INFO_PANEL_ETT_TEXT) && TempEvent->fIsEttUTF16) /*&& IsDTVInUse() && (stEventPool.u8TotalNum > 0)*/)
            {
#if 0
                printf(">>>");
                for(i=0;i<20;i+=2)
...
#if ENABLE_ATSC_TTS
                if ((hwnd == HWND_EPG_EVENT_INFO_PANEL_ETT_TEXT) && (bEttSpoke == FALSE))
                {
...
LPTSTR MApp_ZUI_ACT_GetEpgDynamicText_ATSC(HWND hwnd)
{
    ...
    U8 *u8TempString = (U8 *)"";
    U8 u8EventIndex = 0;
    U8 u8IsEpgDataReady=((astEpgDbVchMask[0].fIsInSDRAM) || (astEpgDbVchMask[0].u8TblMask != 0x00));
    U8 u8PageBaseIndex = u8EpgCurEventIndex - (u8EpgCurEventIndex%MAX_NUM_OF_EPGLIST_ITEM);
    ...
    switch(hwnd)
    {
        ...
        case HWND_EPG_EVENT_PANEL_EVENT1_TEXT:
            if(IsDTVInUse() && (stEventPool.u8TotalNum > 0) && u8IsEpgDataReady )
            {
                MApp_EpgZUI_Get_Event(u8PageBaseIndex+0);
        ...
        case HWND_EPG_EVENT_PANEL_EVENT2_TEXT:
            if (u8IsEpgDataReady)
            {
                MApp_EpgZUI_Get_Event(u8PageBaseIndex+1);
                u8TempString = ThisEpg_EventTitleText();
            }
            else
            {
        ...
        case HWND_EPG_EVENT_PANEL_EVENT2_TIME_TEXT:
         ...
         case HWND_EPG_EVENT_PANEL_EVENT3_TEXT:
            if (u8IsEpgDataReady)
            {
                MApp_EpgZUI_Get_Event(u8PageBaseIndex+2);
                u8TempString = ThisEpg_EventTitleText();
            }
            else
            {
        ...
        case HWND_EPG_EVENT_PANEL_EVENT4_TEXT:
            if (u8IsEpgDataReady)
            {
                MApp_EpgZUI_Get_Event(u8PageBaseIndex+3);
                u8TempString = ThisEpg_EventTitleText();
            }
            else
            {
        ...
        case HWND_EPG_EVENT_PANEL_EVENT4_TIME_TEXT:
         ...
         case HWND_EPG_EVENT_PANEL_EVENT5_TEXT:
            if (u8IsEpgDataReady)
            {
                MApp_EpgZUI_Get_Event(u8PageBaseIndex+4);
                u8TempString = ThisEpg_EventTitleText();
            }
            else
            {
        ...
        case HWND_EPG_EVENT_PANEL_EVENT5_TIME_TEXT:
         ...
         case HWND_EPG_EVENT_PANEL_EVENT6_TEXT:
            if (u8IsEpgDataReady)
            {
                MApp_EpgZUI_Get_Event(u8PageBaseIndex+5);
                u8TempString = ThisEpg_EventTitleText();
            }
            else
            {
        ...
        case HWND_EPG_EVENT_PANEL_EVENT6_TIME_TEXT:
         ...
         case HWND_EPG_EVENT_PANEL_EVENT7_TEXT:
            if (u8IsEpgDataReady)
            {
                MApp_EpgZUI_Get_Event(u8PageBaseIndex+6);
                u8TempString = ThisEpg_EventTitleText();
            }
            else
            {
        ...
        case HWND_EPG_EVENT_PANEL_EVENT7_TIME_TEXT:
         ...
         case HWND_EPG_EVENT_PANEL_EVENT8_TEXT:
            if (u8IsEpgDataReady)
            {
                MApp_EpgZUI_Get_Event(u8PageBaseIndex+7);
                u8TempString = ThisEpg_EventTitleText();
            }
            else
            {
...
//DTV in use
    ...
    else if((stEventPool.u8TotalNum - u8PageBaseIndex) >=  10)
        return EN_DL_STATE_NORMAL;
    ...
    switch(hwnd)
    {
        ...
        case HWND_EPG_EVENT_PANEL_EVENT8:
            if (_MApp_ZUI_ACT_EpgEventWindowMapToIndex(hwnd) < stEventPool.u8TotalNum)
                return EN_DL_STATE_NORMAL;
            else
                return EN_DL_STATE_HIDDEN;
        default:
