...
static HWND _MApp_ZUI_CTL_DynamicListInitPage(HWND hList, HWND hTarget, BOOLEAN bForward)
{
    ...
    //printf("\nRay: 1st item:%d, last item:%d",windata->hwndFirstItemOfPage, windata->hwndLastItemOfPage);	//Ray DBG 2017.04.13
    //printf("\nRay: total children:%d,visible count:%d",total_children, visible_count);	//Ray DBG 2017.04.13
    ...
    // 4. init first page child positions
    {
        ...
        for (i = page_start_index; i < item_count && i < page_start_index + visible_count; i++)
        {
            ...
            */
            ...
            //printf("\nRay: Call from dyanlist. hwd:%d",visible_list[i-page_start_index]);	//Ray DBG 2017.04.12
        ...
        case MSG_NOTIFY_KEYDOWN:
            {
                ...
                for (count = 0; count < 100; count++)
                {
                    ...
                    if (MApp_ZUI_API_GetParent(next) != hWnd)
                    {
                        ...
                        printf("MApp_ZUI_API_GetParent(next) != hWnd\n");				//Ray DBG 2017.03.06
                        goto DEFAULT;
                    }
                        ...
                        MApp_ZUI_API_IsWindowEnabled(next))
                    {
                        ...
                        /*
                        //Ray OSD 2017.03.07: Check if it's first item of menu. If yes, show focus title icon
                        if(windata->pVarData->hwndFirstShown == next){
                            printf("[]focus 1st item =%u\n", next);
                            MApp_ZUI_API_ShowFocusTitleIcon(windata->pVarData->hwndFirstShown,SW_SHOW);	//show the title icon
                        }
                        //Ray OSD 2017.03.07: Check if it's 2nd item of menu. If yes, hidden the focus title icon
                        if((GETWNDNAVI(windata->pVarData->hwndFirstShown)[NAV_DOWN]) == next) {
                            printf("[]focus 2nd item =%u\n", next);
                            MApp_ZUI_API_ShowFocusTitleIcon(windata->pVarData->hwndFirstShown,SW_HIDE);	//hide the title icon
                        }
			  */
                        //printf("[]focus=%u\n", next);
                        break;
                    ...
                    else
                    {
                        ...
                        printf("[]reinit=%u\n", next);			//Ray DBG 2017.03.06
                        next = _MApp_ZUI_CTL_DynamicListInitPage(hWnd,
                            next,
                            pMsg->wParam == VK_DOWN || pMsg->wParam == VK_RIGHT);

                        //Ray OSD 2017.06.01: When pressing down key, reinit new page and go back to first item, originally it is title. But now since we set first item as next to title, so
                        // 		     we need to go to next item by commenting lines below and jumping back to "for" loop
                        if ((nav == NAV_DOWN) && (MApp_ZUI_API_GetParent(hWnd)== HWND_MENU_PICTURE_PAGE||MApp_ZUI_API_GetParent(hWnd)== HWND_MENU_OPTION_PAGE||MApp_ZUI_API_GetParent(hWnd)== HWND_MENU_SOUND_PAGE)){
                            now = next;		//Keep moving to next item
                        }else{
                          //Press up key doesn't affected as it goes to last item
			  MApp_ZUI_API_SetFocus(next);
			  MApp_ZUI_API_CheckShowFocusTitleIcon(next);	//Ray OSD 2017.03.07: Check if it's needed to show or hide focus title icon
			  break;
                        }
