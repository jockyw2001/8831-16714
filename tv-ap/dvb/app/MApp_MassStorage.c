...
static U16 s_u16CurrentValidDevice = 0;
...
{
    s_u16CurrentValidDevice = 0;
}

U16 MApp_MassStorage_GetValidDevice()
{
    U16 u16Result=0;
    U8 i=0;
    ...
    for(i=0;i<count;i++)
    {
        ...
        if(status==MSD_STATUS_BUSY||status==MSD_STATUS_IDLE||status==MSD_STATUS_INSERTED)
        {
            ...
            {
                //result|=0x10;
                u16Result |= (1<<MSD_CREADER_INDEX_START);
            }
            else
            {
                u16Result |= (1<<i);
...
BOOLEAN MApp_MassStorage_DeviceStatusCheck(void)
{
    ...
    u16ValidDevice = MApp_FS_GetValidDevice();
    if(u16ValidDevice == s_u16CurrentValidDevice)
    {
        return TRUE;
    }
    else
    {
        s_u16CurrentValidDevice = u16ValidDevice;   //update the current valid device
    }
