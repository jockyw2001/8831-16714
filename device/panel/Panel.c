...
#include "Utl.h"
#include "MApp_UiMenuFunc_ATSC.h"		//Ray BKL 2017.02.21: Add for access U16 MApp_UiMenuFunc_GetPictureBacklightValue(void);
#include "EXT_GPIO.h"				//Ray PNL 2017.03.13: To read DIP switch SW3
#include "MApp_RestoreToDefault.h"		//Ray BKL 2017.04.26: To include DFFAULT_DA_PWM_FREQUENCY
...
PANEL_RESOLUTION_TYPE g_PNL_TypeSel             =   PNL_INX_M280DGJ_L30_Vx1_4K2K_60HZ_1Division;		//Ray PNL 2017.03.13: Default set panel index = 0 (Innolux M280DGJ)
...
ST_PANEL_NEW_PANELTYPE stTCON_PNL_WXGA_IPS32_Panasonic_For_Funai=
{
    ...
    78,         //DWORD m_dwPanelDCLK;              //PANEL_DCLK
    60,         //DWORD m_dwPanelMinDCLK;           //PANEL_MIN_DCLK
    LINK_MINILVDS_1CH_3P_6BIT,   //Which extern type exactly, only valid when m_ePanelLinkType==LINK_EXT, otherwise, don't care
};
...
#if (SUPPORT_NEW_PANEL_INTERFACE)
ST_PANEL_NEW_PANELTYPE stPanel_AUO_P550QVN01_0_Vx1_4K2K_60HZ_2Division =
#else
PanelType  tPanel_CSOT_MT5461D01_Vx1_4K2K_60HZ_2Division[] =
#endif
{
    {
        "AUO_P550QVN01_0",//m_pPanelName
        //////////////////////////////////////////////
        // Panel output
        //////////////////////////////////////////////
        0,          //BOOL m_bPanelDither :1;           //PANEL_DITHER          // 8/6 bits panel
        ...
        PANEL_CONNECTOR_SWAP_PORT,//1^PANEL_CONNECTOR_SWAP_PORT,                    // shall swap if
                                                        // (PANEL_SWAP_PORT XOR Board_Connect_Swap) is TRUE
...
//Ray PNL 2017.03.13: Multiple panels panel parameters
//The array index = panel index = DIP switch SW3 Position#4 to Position #1
ST_PANEL_NEW_PANELTYPE* stPanelIndexTbl[] =
{
    &stPanel_INX_M280DGJ_L30_Vx1_4K2K_60HZ_1Division,		//Panel index 0: Innolux M280DGJ-L30 panel
    &stPanel_AUO_P550QVN01_0_Vx1_4K2K_60HZ_2Division,		//Ray DBG 2017.03.13: Panel index 1: 2-division panel for testing
};

//Ray PNL 2017.03.13: Below is the original panel table that we disable
#if 0
ST_PANEL_NEW_PANELTYPE* stPanelIndexTbl[] = //MingYuan Panel Select
{
    ...
    &stPanel_AUO_P550QVN01_0_Vx1_4K2K_60HZ_2Division,
...
#endif //Ray PNL 2017.03.13: #endif of disable original stPanelIndexTbl[]
...
BOOLEAN MApi_PanelParameter_LoadTable(U8 u8PanelIndex)
{
    ...
    stPanelBinTbl.stOldPanelTypeFromUtopia.m_pPanelName = m_bPanelName;

    stPanelBinTbl.stOldPanelTypeFromUtopia.m_bPanelDither =
            stPanelBinStruct.stPanelTbl.m_bPanelDither;

    stPanelBinTbl.stOldPanelTypeFromUtopia.m_ePanelLinkType =
    ...
    stPanelBinTbl.stOldPanelTypeFromUtopia.m_ucPanelAspectRatio =
    ...
    stPanelBinTbl.stOldPanelTypeFromUtopia.m_ucOutTimingMode =
...
//*********************************************************
// Routine: dv_MultiPanelsInit
// Usage: Routine to initialize panel parameters according to DIP switch SW3 setting
//Ray PNL 2017.03.13
//**********************************************************
void dv_MultiPanelsInit(void)
{

#ifdef sSP4096Board
  BYTE ucPanelIndex;

  //Read SW3 position 1 to 4 to get panel index
  ucPanelIndex = MDrv_ExGPIO_Get(SW3_4);
  ucPanelIndex<<=1;
  ucPanelIndex |= MDrv_ExGPIO_Get(SW3_3);
  ucPanelIndex<<=1;
  ucPanelIndex |= MDrv_ExGPIO_Get(SW3_2);
  ucPanelIndex<<=1;
  ucPanelIndex |= MDrv_ExGPIO_Get(SW3_1);
  ucPanelIndex^=0x0f;			//Invert all 4-bit values (1->0 & 0->1), since DIP switch ON = 0, OFF = 1
  ucPanelIndex &=0x0f;

  //Check if panel index exceed max define panel table
  if((ucPanelIndex+1)>PNL_RES_MAX_NUM){
      g_PNL_TypeSel = PNL_INX_M280DGJ_L30_Vx1_4K2K_60HZ_1Division;	//If exceed, we use Innolux M280DGJ panel parameter
  }
  else{
      g_PNL_TypeSel = (PANEL_RESOLUTION_TYPE)ucPanelIndex;	//Assign valid panel index as selected panel
  }

  //Read SW3 position 5 to set panel division: 1 = 2 division, 0 = no division
  stPanelIndexTbl[g_PNL_TypeSel]->stOldPanelTypeFromUtopia.m_bPanelDualPort = (MDrv_ExGPIO_Get(SW3_5)^0x01);	//Toggle value since since DIP switch ON = 0, OFF = 1

  //Read SW3 position 6 to set panel color depth: 1 = 8-bit, 0 = 10-bit color depth
  if(MDrv_ExGPIO_Get(SW3_6)==1){
      stPanelIndexTbl[g_PNL_TypeSel]->enLinkExtType = LINK_VBY1_10BIT_8LANE;
  }else{
      stPanelIndexTbl[g_PNL_TypeSel]->enLinkExtType = LINK_VBY1_8BIT_8LANE;
  }

#else
  //Ray PNL 2017.03.13: For TriView board, we use default setting
  g_PNL_TypeSel = PNL_INX_M280DGJ_L30_Vx1_4K2K_60HZ_1Division;
  //g_PNL_TypeSel = Panel_AUO_P550QVN01_0_Vx1_4K2K_60HZ_2Division;	//Ray DBG 2017.03.13: testing
  //stPanelIndexTbl[g_PNL_TypeSel]->stOldPanelTypeFromUtopia.m_bPanelDualPort = 1;		//Testing of 2-division
...
void MApi_PNL_En(MS_BOOL bPanelOn )
{
    ...
    if(bPanelOn)
    {
        ...
        #endif
				//printf("MingYuan Print MApi_PNL_En On \n");
    }
    else
    {
        ...
        /* enable/disable VCC */
        _msAPI_Scaler_SetPanelVCC(bPanelOn);
        //printf("MingYuan Print MApi_PNL_En Off \n");
    }
...
void MApi_PNL_SetInverter(MS_BOOL bEnable)
{
    ...
    SET_BACKLIGHT_ON(bEnable);			//Ray BKL 2017.02.20: Set backlight on/off system setting as ENABLE/DISABLE
}
...
//*********************************************************
//Ray BKL 2017.02.21:
// Routine: dv_AdjustBacklightPWMDuty
// Usage: Change backlight PWM duty according to PWM freq in order to reflect actual light value
// Input: ucLightVal: Actual light value: 0~255
//**********************************************************
void dv_AdjustBacklightPWMDuty(BYTE ucLightVal)
{
  DWORD sFreq;					//Ray BKL 2017.02.21: Since the period arg of MDrv_PWM_Period( ) is DWORD unit
  DWORD lPeriod;				//Ray BKL 2017.02.21: Since the period arg of MDrv_PWM_Period( ) is DWORD unit
  DWORD  lightDuty;
  //Ray BKL 2017.04.26: If backlight control is D/A, PWM frequency is fixed to DFFAULT_DA_PWM_FREQUENCY 160Hz
  if(GET_BACKLIGHT_CONTROL()==BKL_DA){
      sFreq = DFFAULT_DA_PWM_FREQUENCY;
  }else{
      sFreq = GET_BACKLIGHT_PWM_FREQ();		//Ray BKL 2017.02.21: Get PWM frequency in system parameter
  }
  lPeriod = (DWORD)(12000000)/(sFreq * 255)-1;	//Ray BKL 2017.02.21: Convert frequency to period argument value.  255 is the divider ratio we set in Mst_PwmPortInit()
  lightDuty = (lPeriod*ucLightVal)/255;		//Ray BKL 2017.02.21: Actual Duty = Period * duty ratio = Period * Real light value/Max light value
  MDrv_PWM_DutyCycle(E_PWM_CH1, lightDuty);
}


//*********************************************************
//Ray BKL 2017.02.21:
// Routine: dv_AdjustBacklightPWMFreq
// Usage: Change backlight PWM frequency and then tune the PWM duty accordingly
// Input: sFreq: PWM Frequency in Hz
//**********************************************************
void dv_AdjustBacklightPWMFreq(WORD sFreq)
{
  DWORD lPeriod;
  BYTE  lightVal,realLightValue;
  //Ray BKL 2017.04.26: If backlight control is D/A, PWM frequency is fixed to DFFAULT_DA_PWM_FREQUENCY 160Hz
  if(GET_BACKLIGHT_CONTROL()==BKL_DA){
      sFreq = DFFAULT_DA_PWM_FREQUENCY;
  }
  lPeriod = (DWORD)(12000000)/(sFreq * 255)-1;	//Ray BKL 2017.02.21: Convert frequency to period argument value.  255 is the divider ratio we set in Mst_PwmPortInit()
  MDrv_PWM_Period(E_PWM_CH1, lPeriod);		//Ray BKL 2017.02.21: Adjust new PWM frequency
  lightVal = MApp_UiMenuFunc_GetPictureBacklightValue();		//Ray BKL 2017.02.21: Get backlight value
  realLightValue = msAPI_Mode_PictureBackLightN100toReallyValue(lightVal);		//Ray BKL 2017.02.21: Change from 0~100% to real light value 0~255
  Panel_Backlight_PWM_ADJ(realLightValue);
}
...
void MApi_PNL_SetBackLight(MS_BOOL bEnable)
{
    ...
    {
        U8 u8PWM_Value=0;
        msAPI_Timer_Delayms( g_IPanel.OnTiming(E_APIPNL_POWER_TIMING_2) );
        ...
        //g_u8TransientBacklight = MIN(MAX_BACKLIGHT, MAX_BACKLIGHT);
      	//#if(ENABLE_BACKLIGHT_ADJUST)
				...
				g_u8TransientBacklight = stGenSetting.u8Backlight;
				#else	      	
        g_u8TransientBacklight = ST_PICTURE.u8Backlight;
        #endif
      	//#endif
        u8PWM_Value = msAPI_Mode_PictureBackLightN100toReallyValue( g_u8TransientBacklight );
        //printf("MingYuan Print MApi_PNL_SetBackLight %d \n",u8PWM_Value);
...
BOOLEAN MApi_PNL_Converter_Board_Init(void)
{
    ...
    u8Buf = u8Cmd_2;
    printf("\n\n\n\r### MDrv_IN8903_3D_CMD_DataWrite( %02x, %02x, %02x, %02x, %02x, %02x, %02x)", *u8Buf, *(u8Buf+1), *(u8Buf+2), *(u8Buf+3), *(u8Buf+4), *(u8Buf+5), *(u8Buf+6));
    printf("\n\n\n\r### Converter board init result = %d", bRet);
  #endif
