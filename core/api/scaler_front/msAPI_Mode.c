...
U16 msAPI_Mode_PictureBackLightN100toReallyValue ( U8 u8value )
{
    U8 u8MaxTemp;
    U8 u8MinTemp;
    u8MaxTemp = G_FACTORY_SETTING.stFacLightSensorSetting.u8Backlight_Max;
    u8MinTemp = G_FACTORY_SETTING.stFacLightSensorSetting.u8Backlight_Min;
    //Ray 2017.04.26 BKL: Add if backlight invert is enable
    if((PANEL_DEFAULT_TYPE_SEL==PNL_AUO_P550QVN01_0_Vx1_4K2K_60HZ_2Division)||(GET_BACKLIGHT_INVERT()==ENABLE))	//高電位最亮
    {
    	u8MaxTemp = 255 - u8MaxTemp; //(值反向)
    	u8MinTemp = 255 - u8MinTemp;
    }
//#if(!ENABLE_BACKLIGHT_ADJUST)
    //Ray 2017.04.26 BKL: Add if backlight invert is enable
		if((PANEL_DEFAULT_TYPE_SEL==PNL_AUO_P550QVN01_0_Vx1_4K2K_60HZ_2Division)||(GET_BACKLIGHT_INVERT()==ENABLE))	//高電位最亮
    ...
    stBacklightCurveTbl.stPoint0.ucY = u8MinTemp;

    stBacklightCurveTbl.stPoint1.ucX = 35;
    stBacklightCurveTbl.stPoint1.ucY = ( ( (U16)( u8MinTemp - u8MaxTemp ) * 70/100) + u8MaxTemp );

    stBacklightCurveTbl.stPoint2.ucX = 70;
    stBacklightCurveTbl.stPoint2.ucY = ( ( (U16)( u8MinTemp - u8MaxTemp ) * 35/100) + u8MaxTemp );

    stBacklightCurveTbl.stPoint3.ucX = 100;//150;	//MingYuan Fix Curve
    stBacklightCurveTbl.stPoint3.ucY = (u8MaxTemp);				  	
  	}    
	  else //低電位最亮
		{
    stBacklightCurveTbl.stPoint0.ucX = 0;
    ...
    stBacklightCurveTbl.stPoint1.ucX = 35;
    stBacklightCurveTbl.stPoint1.ucY = ( ( (U16)( u8MaxTemp - u8MinTemp ) * 35/100) + u8MinTemp );

    stBacklightCurveTbl.stPoint2.ucX = 70;
    stBacklightCurveTbl.stPoint2.ucY = ( ( (U16)( u8MaxTemp - u8MinTemp ) * 70/100) + u8MinTemp );
