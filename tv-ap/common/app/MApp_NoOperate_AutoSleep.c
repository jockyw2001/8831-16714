...
#define NO_OPERATE_AUTO_SLEEP_NOTIFY_UI_TIME (60000) // 1 min
...
void MApp_NoOperateAutoSleep_Task(void)
{
    ...
    if( msAPI_Timer_DiffTime_2(_s_u32NoOperateAutoSleep_LastTime, MsOS_GetSystemTime() ) < 4000 )
        return;
    ...
    else
    {
        ...
        {
            if( u32CountDownTime <= NO_OPERATE_AUTO_SLEEP_NOTIFY_UI_TIME ) //  CountDown < 1 min
            {
                printf(" u32CountDownTime = %u\n", u32CountDownTime);
                MApp_UI_NotifySysEvent(E_SYS_EVENT_NO_OPERATE_AUTO_SLEEP_COUNT_DOWN_1_MIN, &u32CountDownTime);
            }
        }
    }
