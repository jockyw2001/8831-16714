...
extern void dv_SerialCmdInterface(void);		//Ray URT 2017.02.03
...
void Cus_UART_AutoAdjWBTool(void)
{
    ...
    switch( g_UartCommand.Buffer[_UART_CMD_INDEX1_] )
    {
        ...
        case 0x42:
            ST_VIDEO.eColorTemp=MS_COLOR_TEMP_MEDIUM;
            ...
            { // MVOP ...
                ...
                if( (mvop_rtn == E_MVOP_OK) && bMvopEn )
                {
                    ...
                    if( E_MVOP_OK == MDrv_MVOP_GetOutputTiming(&mvopTiming) )
                    {
                        ...
                        printf(" HF=%d, VF=%d, interlace=%d, ExpFrameRate=%d\n", mvopTiming.u16H_Freq, mvopTiming.u8Framerate, mvopTiming.bInterlace, mvopTiming.u16ExpFrameRate);
        ...
        case 0x02:
          {
            ...
            // Input sync ...
            {
                ...
                printf("\nu8SyncStatus=%X", sXC_Sync_Status.u8SyncStatus );
                printf(", Interlace=%d", (sXC_Sync_Status.u8SyncStatus&XC_MD_INTERLACE_BIT)?1:0 );

                printf("\nu16Hperiod=%X, H Freq x10=%d", sXC_Sync_Status.u16Hperiod, u16HFreq_x10 );
                printf("\nV total=%d, V Freq x10=%d\n", sXC_Sync_Status.u16Vtotal, MApi_XC_CalculateVFreqx10(u16HFreq_x10, sXC_Sync_Status.u16Vtotal) );
            }

            // Frame lock
            printf("FPLL=%d, FPLL_Locke = %d\n", drvStatus.bEnableFPLL, drvStatus.bFPLL_LOCK);
            printf(" OutputClk=%lX, RealOutputClk=%lX\n", MDrv_Read3Byte(0x10311E), MDrv_Read3Byte(0x103150));
            printf(" InVPeriod=%lX, OutVPeriod=%lX\n", MDrv_Read3Byte(0x103142), MDrv_Read3Byte(0x103146));
...
U8 const _u8AesKey_test[HDCP22_AES_KEY_SIZE] =
{
0x00
};

U8 const  _u8Hdcp22Key_test[HDCP22_KEY_SIZE] =   // hdcp2_key_22_encrypted
{
0x00
};
...
void TestCmd_Atsc(void)
{
    ...
    MApp_ChanProc_GetRfChannelPointer(pCurVirCh, &vRfCh);

    printf(" stTPSetting.u16Frequency=%u\n", vRfCh.stTPSetting.u16Frequency);
    printf(" stTPSetting.fModulation=%u\n", vRfCh.stTPSetting.fModulation);
#else
    printf(" stTPSetting.u16Frequency=%u\n", pCurVirCh->stTPSetting.u16Frequency);
    printf(" stTPSetting.fModulation=%u\n", pCurVirCh->stTPSetting.fModulation);
#endif
...
{
 		#ifdef sCmdSupportAsk
 		if(g_bUartBusyFlag)
 		{
  		g_UartCommand.Buffer[_UART_TVE_CMD_STX_] = UART_TVE_STX_CODE;
	 		g_UartCommand.Buffer[_UART_TVE_CMD_TYPE_] = UART_TVE_RETURN_TYPE;
 			g_UartCommand.Buffer[_UART_TVE_CMD_TXID_] = G_FACTORY_SETTING.stFacOtherSetting.u8MonitorID;
	 		g_UartCommand.Buffer[_UART_TVE_CMD_RXID_] = u8_ReturnRxIdBackup;
    	g_UartCommand.Buffer[_UART_TVE_CMD_CMD1_] = 0x01;
    	g_UartCommand.Buffer[_UART_TVE_CMD_CMD2_] = 0x00;
    	g_UartCommand.Buffer[_UART_TVE_CMD_CMD3_]	= 0xFF; 		
	 		g_UartCommand.Buffer[_UART_TVE_CMD_DATA1_] = 0x00;
 			g_UartCommand.Buffer[_UART_TVE_CMD_DATA2_] = 0x01;
 			g_UartCommand.Buffer[_UART_TVE_CMD_ETX_] = UART_TVE_ETX_CODE; 			
	 		g_bUartBusyFlag = 0;
 			MDrv_UART_TVE_SendReturnCommand();
 		}
		#endif
 		dv_SerialCmdInterface();			//Ray URT 2017.02.03: RS-232 command handler
    //UART_DecodeCommand();					//Ray URT 2017.02.03: Disable original MSTAR UART decode cmd interface
#ifdef ENABLE_RS232_FUNC
    PRI_RS232_DecodeCommand();
#endif
		#ifdef sUseQuickyCmdWay
		if(!g_Uart0CmdTimeTick)
		{
...
//#if (CHIP_FAMILY_TYPE == CHIP_FAMILY_MAYA)
    ...
    //printf("#define ENABLE_TCON_IN8903_PROCESS  TRUE \n");
  #else
    //printf("#define ENABLE_TCON_IN8903_PROCESS  FALSE \n");
  #endif
  #if ENABLE_VX1_CONVERTER_BOARD
    //printf("#define ENABLE_VX1_CONVERTER_BOARD  TRUE \n");
  #else
    //printf("#define ENABLE_VX1_CONVERTER_BOARD  FALSE \n");
  #endif
#endif
