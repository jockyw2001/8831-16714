        ...
        case ST_HEAAC_AUD:
            //AC-3 audio
            {
                ...
                if(pu8Descriptor != NULL)
                {
                    ...
                    DES_DBG(AC3_DESCRIPTOR, printf("[AC3_AD] BsMod = %d, Priority = %d, LandIdx = %d\n", stAC3Descriptor.u8BsMod, stAC3Descriptor.u8Priority, astSoundTrackPMT[u8AudioTrackIndex].u16AudLangIdx));
                }
...
static void MApp_Dmx_PAT_CheckVersionChange(U8 *pu8Section)
{
...
#if (NEW_VIRTUAL_CHANNEL_STRUCTURE == ENABLE)
    if((vRfCh.u16TsId != u16ActualTSid))
#else
    if((pstVirtualCh->u16TsId != u16ActualTSid))
#endif
    {
...
#if (NEW_VIRTUAL_CHANNEL_STRUCTURE == ENABLE)
            if(vRfCh.u8NumOfPatItem  != u8CurPatItem)
#else
            if(pstVirtualCh->u8NumOfPatItem  != u8CurPatItem)
#endif
            {
                 fTableVersionChanged = TRUE;
            }
            #else
            if(((u8NumOfPatItem != u8CurPatItem))&&(u8LastPatItemNo != u8CurPatItem))
            {
    	        DMX_VERSION_DBG(DMX_PATCHECK_VERSION, printf("[PAT VerChg] Tsid [%x->%x], Ver [%bx->%bx], PatNum [%bx->%bx], %bx\r\n", pstVirtualCh->u16TsId, u16ActualTSid, pstVirtualCh->u8PatVersion, u8Version, u8NumOfPatItem, u8CurPatItem, u8LastPatItemNo));
	            u8LastPatItemNo = u8CurPatItem;
	            fTableVersionChanged = TRUE;
            }
            #endif
            ...
            pu8Section += 8;
            {
                ...
                for(iu8Loop_i = 0; iu8Loop_i < u8CurPatItem; iu8Loop_i++)
                {
                    ...
                    if(u16ProgramNumber == ((pu8Section[0] << 8) | pu8Section[1]))
                    {
                        ...
                        {
                            printf("[PAT VerChg] Current PN/pid = \r [New]PN/pid \n",
                                        pstVirtualCh->u16ProgramNumber, pstVirtualCh->u16PmtPid,
                                        ((pu8Section[0] << 8) | pu8Section[1]), (((pu8Section[2] & 0x1f) << 8) | pu8Section[3]));
                            fTableVersionChanged = TRUE;
                        }
                ...
                if( bHasProgramNumberInDB== FALSE)
                {
    ...
    {
#if (NEW_VIRTUAL_CHANNEL_STRUCTURE == ENABLE)
        if(u8Version != vRfCh.u8PatVersion)
            vRfCh.u8PatVersion = u8Version;
#else
        if(u8Version != pstVirtualCh->u8PatVersion)
            pstVirtualCh->u8PatVersion = u8Version;
#endif
    }
...
static void MApp_Dmx_PAT_Parse(MS_PAT_ITEM_ATSC*pastPATItem_func, U8 *pu8Section)
{
    ...
    */
...
#else
        pastVirtualCh[u8ChIndex].u8PatVersion       = u8Version;
        pastVirtualCh[u8ChIndex].u16TsId            = u16ActualTSid;
        pastVirtualCh[u8ChIndex].u16PatSectionNum    = u8CurSectionNum;
#endif
    ...
    for(iu8Loop_i = 0; iu8Loop_i < u8NumOfPatItem; iu8Loop_i++)
    {
        ...
        pastRfCh.u16TsId            = u16ActualTSid;
#else
        pastVirtualCh[iu8Loop_i].u8PatVersion       = u8Version;
        pastVirtualCh[iu8Loop_i].u16TsId            = u16ActualTSid;
#endif
...
void MApp_Dmx_PAT_Cb(U8 u8FID, DMX_EVENT enEvent)
{
    ...
    switch(enStatus)
    {
       ...
       default:
            {
            DMX_DBG(DMX_PAT, printf("[FID=%d] PAT get section unknow error!! \r\n", u8FID));
                break;
            }
    }
...
#endif
                {
                    printf("\033[33m >>>>>>PAT MONITOR VersionChange (%d)>>>>>>>\033[30m\n",__LINE__);
                    fTableVersionChanged = TRUE;
                    u8KeyCode = KEY_EXIT;
                }
            }
...
#else
    pstVirtualCh_func->u8PmtVersion = (pu8Section[5] & 0x3f) >> 1;
#endif
    /*[2]initial variables =================================================*/
    #if (CHECK_CA_DESC_ENABLE == 1)
                ...
                //AC-3 audio
                {
                    ...
                    if(pstVirtualCh_func->u8AudLangNum < MAX_AUD_LANG_NUM_ATSC)
                    {
                        ...
                        if(pu8Descriptor != NULL)
                        {
                            ...
                            DES_DBG(AC3_DESCRIPTOR, printf("[AC3_AD] BsMod = %d, Priority = %d, LandIdx = %d\n", stAC3Descriptor.u8BsMod, stAC3Descriptor.u8Priority, pstVirtualCh_func->astSoundTrack[pstVirtualCh_func->u8AudLangNum].u16AudLangIdx));
...
#else
    for(u8Loop_j = 0; u8Loop_j < u8NumOfPatItem; u8Loop_j++)
    {
        /* Version number */
        pastVirtualCh_func[u8Loop_j].u8VctVersion = u8VctVersion;
    } // end of for
#endif
        ...
        if((pu8LoopPosition[22] << 8 | pu8LoopPosition[23]) == u16ActualTSid)
        {
            ...
            if(u16ProgramNumner != 0 && u16ProgramNumner != 0xFFFF)
            {
                ...
                if(u8Loop_j==u8TotalTableItem && u8TotalTableItem<MAX_VC_PER_PHYSICAL)
                {
                    ...
                    pastRfCh.u16TsId = u16ActualTSid;
#else
                    pastVirtualCh_func[u8Loop_j].u8PatVersion = u8Version;
                    pastVirtualCh_func[u8Loop_j].u8VctVersion = u8VctVersion;
                    pastVirtualCh_func[u8Loop_j].u16TsId = u16ActualTSid;
#endif
...
#endif
#if (NEW_VIRTUAL_CHANNEL_STRUCTURE == ENABLE)
    if (u8Version != vRfCh.u8VctVersion)
#else
    if (u8Version != pstVirtualCh->u8VctVersion)
#endif
    {
...
#if (CHECK_TALBEFIELDS_BEFORE_VERCHANGE)
        {
            ...
            if(pu8Section[6] == pstVirtualCh->stVirtualChNum.u8VCTsctionNumber)
            {
                ...
                if(u8IsVCTFieldChange != 0)
                {
                    ...
                    printf("(%d)(%s) fTableVersionChanged = TRUE~\n", __LINE__, __func__, fTableVersionChanged);
                }
                if(bHasInDB == FALSE)
                {
...
#if (NEW_VIRTUAL_CHANNEL_STRUCTURE == ENABLE)
        if (u8Version != vRfCh.u8VctVersion)
           vRfCh.u8VctVersion = u8Version;
#else
        if (u8Version != pstVirtualCh->u8VctVersion)
            pstVirtualCh->u8VctVersion = u8Version;
#endif
    }
...
void MApp_Dmx_VCT_Monitor(void)
{
    ...
    if (fEnableVctMonitor || (msAPI_Timer_DiffTimeFromNow(u32MonitorVctTimer) > VCT_MONITOR_PERIOD))
    {
        if(u8VctFid == MSAPI_DMX_INVALID_FLT_ID)
        {
            ...
            stSectionMonitor.au8MatchByte[3] = (U8)(RfCh.u16TsId >> 8); //TS ID
            stSectionMonitor.au8MatchByte[4] = (U8)(RfCh.u16TsId); //TS ID
#else
            stSectionMonitor.au8MatchByte[3] = (U8)(pstVirtualCh->u16TsId >> 8); //TS ID
            stSectionMonitor.au8MatchByte[4] = (U8)(pstVirtualCh->u16TsId); //TS ID
#endif
            stSectionMonitor.au8MatchByte[5] = 0x01;
            stSectionMonitor.au8MatchByte[8]    = 0x00;    // protocol_version
...
static BOOLEAN MApp_Dmx_EIT_Check(MS_EIT_PARSE_PARAM_ATSC *pParse, U8 pu8Sec4K[])
{
    ...
    EIT_DBG(EIT_CHECK, ("TblID = %bX, SrcID = %X, VerNum = %d, SN = %d, LSN = %d, Num of Events = %d\n"
        , pParse->u8TblID, pParse->u16SrcID, pParse->u8VerNum, pParse->u8SecNum, pParse->u8LastSecNum, pParse->u8NumOfEvents));
    ...
    for(u8_loop_i = 0; u8_loop_i < pParse->u8NumOfEvents; u8_loop_i++)
    {
        ...
        if(pParse->u8EitSaveIndex[pParse->u8Tbl] < MAX_EVENT_IN_TBL)
        {
            if(MApp_EpgDB_SaveEvent(pParse->u8SrcIdx, pParse->u8Tbl, (u8NumOfEvents + pParse->u8EitSaveIndex[pParse->u8Tbl]), pEvent))
            {
                ...
                MApp_EpgDB_LoadEventTitle(pParse->u8SrcIdx, g_stUiEvent.u16DbTitlePos, &pSrcEventTitle);
...
static void MApp_Dmx_ETT_All_Parse (U8 pu8Sec4K[])
{
    ...
    if (pEvent != NULL)
    {
        if (pEvent->u16DbEttPos == NULL)
        {
            ...
            MApp_EpgDB_LoadEventETT(pParse->u8SrcIdx, g_stUiEvent.u16DbEttPos, &pSrcEventETT);
    ...
    switch(enStatus)
    {
        case MSAPI_DMX_FILTER_STATUS_SECAVAILABLE:
        {
            ...
            else
            {
                ...
                if (stEitParseParam.u8Tbl < MAX_TBL_NUM)
                {
                    if (au8Section[0] == 0xCB)                // EIT
                    {
...
void MApp_Dmx_EIT_Set_Demux(void)
{
    ...
    U16   u16_Ettpid = 0;
    U8    u8_table_id;
    BOOLEAN    bIsEitFidInvalid = 0;
    BOOLEAN    bIsEttFidInvalid = 0;
    //MS_DMX_SECTIONMONITOR_PARA   stSectionMonitor;
    ...
    if(stMGT.version_number != INVALID_VERSION_NUM)
    {
        ...
        #endif
        {
            ...
            if(stEpgDBInfo.au8EttFID[stEpgDBInfo.u8EttCntFID] == MSAPI_DMX_INVALID_FLT_ID)
                bIsEttFidInvalid = 1;

            // Eit Not set filter yet --> set EIT filter
            if((bIsEitFidInvalid==1)&&(u16_pid != 0))
            {
                EIT_DBG(EIT_MONITOR, ("Set Eit-%d filter u16_pid = %x\n", stEpgDBInfo.u8CntFID, u16_pid));
                MApp_Dmx_EIT_Set_DemuxFilter(u16_pid, u8_table_id);
            }
            else    // Already set filter
            {
                //printf("EPG: monitoring EIT at Filter #%d, Pid=%x, Tbl_id-%bx \r\n", stEpgDBInfo.au8FID[stEpgDBInfo.u8CntFID], u16_pid, u8_table_id);
            ...
            // Set Ett0-3 filter when Eit0-3 received
            //if((stEpgDBInfo.u8EitRxOK & BASIC_TBL_MASK) == BASIC_TBL_MASK)
            {
                //Had Received Eit-i && Not set filter Ett-i
                if((GETBIT(stEpgDBInfo.u8EitRxOK,stEpgDBInfo.u8EttCntFID)== 1)&&(GETBIT(stEpgDBInfo.u8EttIsReceiving,stEpgDBInfo.u8EttCntFID) == 0))
                {
                    u16_Ettpid = stMGT.astEttItem[stEpgDBInfo.u8EttCntFID].table_type_PID;
                    u8_table_id = TID_ETS;
                    if (u16_Ettpid == 0)
                    {
                        SETBIT(stEpgDBInfo.u8EttRxOK, stEpgDBInfo.u8EttCntFID);
                ...
                if((bIsEttFidInvalid==1) &&(u16_Ettpid != 0))
                {
                    EIT_DBG(EIT_MONITOR, ("Set Ett-%d filter u16_pid = %x\n", stEpgDBInfo.u8EttCntFID, u16_Ettpid));
                    MApp_Dmx_EIT_Set_DemuxFilter(u16_Ettpid, u8_table_id);
                }
                else   //
                {
                    U8 u8EttCnt = stEpgDBInfo.u8EttCntFID + 1;
                     //Had Received Eit-i && Not set filter Ett-i
                    if((GETBIT(stEpgDBInfo.u8EitRxOK,u8EttCnt) == 1)\
                        &&(GETBIT(stEpgDBInfo.u8EttIsReceiving,u8EttCnt) == 0)\
                        &&(u8EttCnt <(MAX_TBL_NUM-1)))
                    {
                        stEpgDBInfo.u8EttCntFID++;
                    }
                    }
...
static void MApp_Dmx_EIT_For_RRT_Parse(U8 *pu8Section)
{
    ...
    if(version_number != stMGT.astEitItem[0].table_type_version_number)
    {
        ...
        for(iu8Loop_i = 0; iu8Loop_i < MAX_TBL_NUM; iu8Loop_i++)
        {
            memset(&stEitParseParam.u8EitSaveIndex[iu8Loop_i], 0, sizeof(U8)*MAX_TBL_NUM);
        }
...
static void MApp_Dmx_RRT_Parse(MS_REGION5_RATING *pstMgt, U8 *pu8Section)
{
    ...
    */
    ...
    {
#if 0
        strcpy((char *)(pstMgt->u8Region5Name), (char *)(pstMgt->u8Region5Name + 2));
#else
    ...
    for (iu16Loop_i = 0; (iu16Loop_i < u8Num_dimension) && (iu8Buffer_k < REGION5_DIMENSION_NUM); iu16Loop_i++)
    {
        ...
        {
#if 0
            strcpy((char *)(pstMgt->stRegin5Dimension[iu8Buffer_k].u8DimensionName), (char *)(pstMgt->stRegin5Dimension[iu8Buffer_k].u8DimensionName + 1));
#else
        ...
        for( iu8Loop_i = 0;iu8Loop_i < u8Valuesdefined;iu8Loop_i++ )
        {
            ...
            {
                memset(pu8TempRRTRatingDes, 0, REGION5_RATING_DES_MAX_LENGTH);
                //u8RatingValueStrLen=MApp_Dmx_Parse_MSS( pu8TempRRTRatingDes, pu8Section + 1 );
                u8RatingValueStrLen=MApp_Dmx_Parse_MSS( pu8TempRRTRatingDes, pu8Section + 1, REGION5_RATING_DES_MAX_LENGTH, ENABLE);
                //?? double check ATSC_FIX_C
                // Save to DB
            #if(ENABLE_DB_ATSC_RRT5_API)
                MApp_DB_ATSC_Save_RRT_Descriptor(iu16Loop_i, u8RatingValueLoop, pu8TempRRTRatingDes, REGION5_RATING_DES_MAX_LENGTH );
            #else
                msAPI_MIU_Copy(_VA2PA((U32)pu8TempRRTRatingDes)+2, _VA2PA(GET_RRT_DESCRIPTOR_ADDRESS((U8)iu16Loop_i, u8RatingValueLoop)), REGION5_RATING_DES_MAX_LENGTH, MIU_SDRAM2SDRAM);
            #endif
                //?? double check
            }
            else
...
static MS_BOOL MApp_Dmx_RRT_CheckVersionChange(const U8* const _pu8Section)
{
    ...
    U8* pu8Section = ((U8*)u32Section);
    U8 u8AbbrevLength, u8RatingValueLength;
    U8 u8Grad_Scale;
    U8 u8Num_dimension, u8dimensionNameLen, u8Valuesdefined;
    U8* pu8TempRRTRatingDes;
    BOOLEAN bFirstNullStr = TRUE;
    MS_REGION5_RATING pstMgt;

    memcpy(pu8Section,_pu8Section,sizeof(u32Section));
    ...
    if(pstMgt.u8VersionNo != stGenSettingVchipRegion5.u8VersionNo) //check version change
    {
    ...
    for (iu16Loop_i = 0; (iu16Loop_i < u8Num_dimension) && (iu8Buffer_k < REGION5_DIMENSION_NUM); iu16Loop_i++)
    {
        ...
        for( iu8Loop_i = 0;iu8Loop_i < u8Valuesdefined;iu8Loop_i++ )
        {
            ...
            if(pu8TempRRTRatingDes)
            {
                memset(pu8TempRRTRatingDes, 0, REGION5_RATING_DES_MAX_LENGTH);
                //u8RatingValueStrLen=MApp_Dmx_Parse_MSS( pu8TempRRTRatingDes, pu8Section + 1 );
                u8RatingValueStrLen=MApp_Dmx_Parse_MSS( pu8TempRRTRatingDes, pu8Section + 1, REGION5_RATING_DES_MAX_LENGTH,FALSE );
                //?? double check ATSC_FIX_C
                // Save to DB
            #if(ENABLE_DB_ATSC_RRT5_API)
                MApp_DB_ATSC_Save_RRT_Descriptor(iu16Loop_i, u8RatingValueLoop, pu8TempRRTRatingDes, REGION5_RATING_DES_MAX_LENGTH );
...
void MApp_Dmx_EnableTableMonitor_ATSC(void)
{
    ...
    MS_RF_CHANNEL_new RfCh;
    // init version number if RF channel change
    MApp_ChanProc_GetRfChannelPointer(MApp_ChanProc_GetCurChannelPointer(), &RfCh);
    if(!msAPI_Tuner_IsSameRfChannel_ATSC(&(RfCh.stTPSetting)))
#else
    // init version number if RF channel change
    if( !msAPI_Tuner_IsSameRfChannel_ATSC((MS_TP_SETTING_ATSC *)&MApp_ChanProc_GetCurChannelPointer()->stTPSetting) )
#endif
    {
...
void MApp_Dmx_DisableTableMonitor_ATSC(void)
{
    ...
    for (iu8Loop_i=0; iu8Loop_i<MAX_TBL_NUM; iu8Loop_i++)
    {
        if (stEpgDBInfo.au8FID[iu8Loop_i] != MSAPI_DMX_INVALID_FLT_ID)
        {
            msAPI_DMX_CloseFilter(stEpgDBInfo.au8FID[iu8Loop_i]);
            stEpgDBInfo.au8FID[iu8Loop_i] = MSAPI_DMX_INVALID_FLT_ID;
            stEpgDBInfo.au8EttFID[iu8Loop_i] = MSAPI_DMX_INVALID_FLT_ID;
        }
    }
...
{
    /* release all filter */
    for(iu8Loop_i = 0; iu8Loop_i < MSAPI_DMX_MAX_FILTER_NUMBER; iu8Loop_i++)
    {
        msAPI_DMX_CloseFilter(iu8Loop_i);
    }
