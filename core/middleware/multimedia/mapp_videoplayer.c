...
static U16 _gu16PreviewX = 0xffff, _gu16PreviewY, _gu16PreviewWidth, _gu16PreviewHeight;
...
//dafault supported audio is "support all but amr"
...
static BOOLEAN m_bEnableNewMMAudioMode = FALSE;
...
BOOLEAN MApp_VDPlayer_tmp_init(void)
{
    ...
    {
        if(E_MBX_SUCCESS != MDrv_VMBX_Init())
        {
            VDPLAYER_DVD_DBG(printf("E_MBX_CLASS_VDPLAYER - MBX init error\n"));
            return FALSE;
        }

        //E_MBX_CLASS_VDPLAYER use callback function for immediately command
        MDrv_VMBX_UnRegisterMSG(E_MBX_CLASS_VDPLAYER, TRUE);
        ...
        {
             VDPLAYER_DVD_DBG(printf("E_MBX_CLASS_VDPLAYER - MBX register msg error\n"));
             return FALSE;
        }

        //E_MBX_CLASS_VDPLAYER_FS use queue and not immediately
        MDrv_VMBX_UnRegisterMSG(E_MBX_CLASS_VDPLAYER_FS, TRUE);
        if(E_MBX_SUCCESS != MDrv_VMBX_RegisterMSG(E_MBX_CLASS_VDPLAYER_FS, MM_COMMON_MBX_QUEUESIZE))
        {
             VDPLAYER_DVD_DBG(printf("E_MBX_CLASS_VDPLAYER_FS - MBX register msg error\n"));
             return FALSE;
        }

        MDrv_VMBX_Enable(TRUE);
    }
    else
#endif
    {
    ...
    if (msAPI_NPlayer_IsPlaying(E_MPLAYER_MEDIA_MOVIE))
    {
        if(E_MBX_SUCCESS != MSApi_MBX_Init())
        {
            VDPLAYER_DVD_DBG(printf("E_MBX_CLASS_VDPLAYER - MBX init error\n"));
            return FALSE;
        }
...
BOOLEAN MApp_VDPlayer_SetTrickMode(EN_VDP_PLAY_MODE ePlayMode, EN_VDP_PLAY_SPEED ePlaySpeed)
{
    ...
    if (msAPI_NPlayer_IsPlaying(E_MPLAYER_MEDIA_MOVIE))
    {
        ...
        {
            VDPLAYER_DVD_DBG(printf("MApp_VDPlayer_SetTrickMode timeout!\n"));
...
BOOLEAN MApp_VDPlayer_Init(VIDEO_PLAYER_INPUT_SOURCE_TYPE eInSrcType, VIDEO_PLAYER_PLAYING_TYPE eType, FileEntry *pFileEntry)
{
    ...
    if (msAPI_NPlayer_IsPlaying(E_MPLAYER_MEDIA_MOVIE))
    {
        u32VDPlayer_init_timeout = 0xFFFFFFFF;
    }
#endif
...
#endif //#if ((OBA2 == 1) && (ENABLE_BABAO_PLAYER == ENABLE))
    {
        //E_MBX_CLASS_VDPLAYER use callback function for immediately command
        if(E_MBX_SUCCESS != MDrv_VMBX_RegisterMSGWithCallBack(E_MBX_CLASS_VDPLAYER, MM_COMMON_MBX_QUEUESIZE, _MApp_VDPlyer_VMailboxCallback))
        {
    ...
    else
    {
      ...
      #if (MM_1080I_USE_FB)
        _Mapp_VDPlayer_SetShareMemData(E_SHAREMEM_MVD_FRAMEBUFFER_MODE_CHECKING, (U32)DISABLE); //enable FBL checking
      #else
...
#endif //
    _Mapp_VDPlayer_SetShareMemData(E_SHAREMEM_ENABLE_NEW_AUDIO_MM_MODE, m_bEnableNewMMAudioMode);
...
#if (OBA2)
    // Send filename includes full path to dvd library.
    {
        U16 u16FullName[128];
        U8 *u8FullName = (U8 *)u16FullName;

        MApp_FileBrowser_GetFileEntryFullpath(pFileEntry, (char *)u8FullName);
        sprintf((char *)u8FullName, "%s/%s", u8FullName, (char *)&u16LongFileName[0]);
        MApp_DVD_Setup(DVD_SETUP_TYPE_FILENAME, u8FullName);
    }
#else
...
BOOLEAN MApp_VDPlayer_ChangeProgam(U8 u8ProgramIdx)
{
    ...
    if (msAPI_NPlayer_IsPlaying(E_MPLAYER_MEDIA_MOVIE))
    {
        ...
        do
        {
            VDPLAYER_DBG(printf("0.wait MApp_VDPlayer_ChangeProgam coprocessor ack ing... \n"));
            MApp_VDPlayer_MailBoxHandler();
        }
        ...
        {
            VDPLAYER_DBG(printf("0.MApp_VDPlayer_ChangeProgam timeout!\n"));
...
static void _MApp_VDPlayer_ProcessBlockingRead(void)
{
    ...
    if(read_size != 0) //need continue read in next loop
    {
        ...
        {
            LongLong ltell, llen;
            unsigned long long u64Tell, u64Len;
            ltell = msAPI_DataStreamIO_Tell_LongLong(_gu32gStreamHdl);
            llen = msAPI_DataStreamIO_Length_LongLong(_gu32gStreamHdl);
            u64Tell = ((unsigned long long)ltell.Hi<<32)+ltell.Lo;
            u64Len = ((unsigned long long)llen.Hi<<32)+llen.Lo;

            if ((u64Tell >= u64Len) ||
                (msAPI_Timer_DiffTimeFromNow(_gu32BlockReadTimer) > 60000))
            {
                VDPLAYER_DVD_DBG(printf("ProcessBlockingRead[%d]\n",msAPI_Timer_DiffTimeFromNow(_gu32BlockReadTimer)));
...
static void _MApp_VDPlayer_ScaleScalerWindows(VDPLAYER_ZOOM_FACT eZoom, BOOLEAN bIsFirstTimeSetup)
{
    ...
    if (msAPI_NPlayer_IsPlaying(E_MPLAYER_MEDIA_MOVIE))
    {
        if ((ST_VIDEO.eAspectRatio == EN_AspectRatio_Original || ST_VIDEO.eAspectRatio == EN_AspectRatio_14X9) && ((g_sDispWindow.width == 0) || (g_sDispWindow.height == 0)))
        {
            ...
            else if (ST_VIDEO.eAspectRatio == EN_AspectRatio_14X9 )
            {
                ...
                if (g_IPanel.Width()*u32ParHeight > g_IPanel.Height()*u32ParWidth)
                {
                    stDstWin.width = g_IPanel.Height()*u32ParWidth/u32ParHeight;
                    stDstWin.height = g_IPanel.Height();
        ...
        else if (eZoom != VDPLAYER_ZOOM_FACT_NORMAL)
        {
        ...
        U8 u8TSStream)
{
    ...
    UNUSED(u8TSStream);
    g_bIsTSRadioOnly = FALSE;
    g_enVDPlayerVideoType = (EN_VDPLAYER_VIDEO_TYPE)u8VideoType;

    if (u8TSStream && (u8VideoType == E_VDPLAYER_VIDEO_TS))
    {
        printf("TS Radio\n");
        g_bIsTSRadioOnly=TRUE;
#if(ENABLE_MM_XC_FBL_MODE)
        MApi_XC_EnableFrameBufferLess(DISABLE);
#endif
...
*/
          ...
          #else // #if (MJPEG_HARDWARE_MODE_ENABLED)
            {
                ...
                if (msAPI_NPlayer_IsPlaying(E_MPLAYER_MEDIA_MOVIE))
                {
                    if ((ST_VIDEO.eAspectRatio == EN_AspectRatio_Original || ST_VIDEO.eAspectRatio == EN_AspectRatio_14X9) && ((g_sDispWindow.width == 0) || (g_sDispWindow.height == 0)))
                    {
                        if (ST_VIDEO.eAspectRatio == EN_AspectRatio_Original)
                        {
                            if ((u16DisplayWidth >= stMVOPTiming.u16Width) && (u16DisplayHeight >= stMVOPTiming.u16Height))
                            {
                                u16DisplayX = (u16DisplayWidth-stMVOPTiming.u16Width)/2;
                                u16DisplayY = (u16DisplayHeight-stMVOPTiming.u16Height)/2;
                                u16DisplayWidth = stMVOPTiming.u16Width;
                                u16DisplayHeight = stMVOPTiming.u16Height;
                            }
                        }
                        else if (ST_VIDEO.eAspectRatio == EN_AspectRatio_14X9)
                        {
                            ...
                            if (u16DisplayWidth*(U16)u32ParHeight > u16DisplayHeight*(U16)u32ParWidth)
                            {
                                temp = u16DisplayWidth;
                                u16DisplayWidth = u16DisplayHeight*(U16)u32ParWidth/(U16)u32ParHeight;
                                u16DisplayX = (temp-u16DisplayWidth)/2;
                                u16DisplayY = 0;
                            }
                            else
                            {
                                temp = u16DisplayHeight;
                                u16DisplayHeight = u16DisplayWidth*(U16)u32ParHeight/(U16)u32ParWidth;
                                u16DisplayX = 0;
                                u16DisplayY = (temp - u16DisplayHeight)/2;
                            }
                        }
                    }
                    else if ((g_sDispWindow.width != 0) && (g_sDispWindow.height != 0))
                    {
                        u16DisplayX = g_sDispWindow.x;
                        u16DisplayY = g_sDispWindow.y;
                        u16DisplayWidth  = g_sDispWindow.width;
                        u16DisplayHeight = g_sDispWindow.height;
                    }
                }
                else
            #endif
    ...
    MApi_XC_Mux_OnOffPeriodicHandler(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW), ENABLE);

 #if (ENABLE_VE)
    //config and enable VE output
    {
        //U8 u8TurnOffDestination = ENABLE;
        MApi_XC_Mux_TriggerPathSyncEvent(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW), NULL);
        //Skip on here to avoid garbage, turn it on with the on of main window screen
        //MApi_XC_Mux_TriggerDestOnOffEvent(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW),&u8TurnOffDestination);
    }
#endif
...
void MApp_VDPlayer_MailBoxHandler(void)
{
    ...
    static BOOLEAN bMuteAudioAMoment = FALSE;
    MBX_MSGQ_Status Mstatus;
    MBX_Msg MB_Message;
#if (!FILE_SYSTEM_LONGLONG_API_ENABLE)
    U32 u32Offset;
...
//                        u32Addr = (u32Addr & 0x7FFFFFFF);
                        ...
                        if (msAPI_NPlayer_IsPlaying(E_MPLAYER_MEDIA_MOVIE))
                        {
                            ...
                            if (_bStreamSeeked && (read_size != 0) && (u64Tell < u64Len))
                            {
                                _gu32ResidualReadSize = read_size;
                    ...
                    case MB_VDPLAYER_AUDIO_INIT:
                        ...
                        if(bMuteAudioAMoment)
                        {
                            bMuteAudioAMoment = FALSE;
                            // unmute audio when the video is on screen
                            msAPI_AUD_AdjustAudioFactor(E_ADJUST_AUDIOMUTE, E_AUDIO_INTERNAL_1_MUTEON, E_AUDIOMUTESOURCE_ACTIVESOURCE);
                        }
                        MApi_AUDIO_SetMpegInfo(Audio_MPEG_infoType_MMFileSize, FILE_SIZE_2KB, 0);  //set default to 2K
                        switch (tmpvar)
                        {
                            case E_VDP_CODEC_ID_WMAV1:
                            case E_VDP_CODEC_ID_WMAV2:
                              ...
                              #if (CHIP_FAMILY_TYPE == CHIP_FAMILY_S7J || CHIP_FAMILY_TYPE == CHIP_FAMILY_S7LD) //after other project add, delete this line.
                                msAPI_WMA_SetASFParm(WMA_PARAMTYPE_MAX_PACKET_SIZE, _MApp_VDPlayer_GetShareMemData(E_SHAREMEM_WMA_PKT_SIZE));
                              #endif
                                break;
                            ...
                            case E_VDP_CODEC_ID_PCM_ALAW:
                            {
                                ...
                                {
                                    MApi_AUDIO_XPCM_Param(G711_u_LAW, u8CannelNumber - 1, u16SampleRate, u16BitsPerSample,0, tmpvar);
                                }
                                else if(tmpvartemp==E_VDP_CODEC_ID_PCM_ALAW)
                                {
                                    MApi_AUDIO_XPCM_Param(G711_A_LAW, u8CannelNumber - 1, u16SampleRate, u16BitsPerSample,0, tmpvar);
                                }
                                else
                                {
                            ...
                            case E_VDP_CODEC_ID_COOK:
                            {
                                ...
                                u16SampleRate=_MApp_VDPlayer_GetShareMemData(E_SHAREMEM_RM_LBR_SAMPLE_RATE);
                                ...
                                VDPLAYER_DBG(printf("u16SampleRate %u\n",u16SampleRate));
                                for(i=0;u16CodecNum>i;i++ )
                                {
                                  #if 0
                                    VDPLAYER_DBG(printf("u16CannelsA[%bd] %u\n",i,u16CannelsA[i]));
                                    VDPLAYER_DBG(printf("u16RegionsA[%bd] %u\n",i,u16RegionsA[i]));
                                    VDPLAYER_DBG(printf("u16cplStartA[%bd] %u\n",i,u16cplStartA[i]));
                                    VDPLAYER_DBG(printf("u16cplQbitsA[%bd] %u\n",i,u16cplQbitsA[i]));
                                    VDPLAYER_DBG(printf("u16FrameSizeA[%bd] %u\n",i,u16FrameSizeA[i]));
                                  #else // Modified by coverity_0442
...
bool MApp_VDPlayer_MovieThumbnail_SetMVOP(void)
{
    ...
    stVidStatus.u16FrameRate = 60000; //MApp_VDPlayer_GetInfo(pItem,E_VDPLAYER_INFO_FRM_PER_SEC);
    ...
    if (stVidStatus.u16HorSize == 0 || stVidStatus.u16HorSize>1920)
    {
        return FALSE;
    }

    if (stVidStatus.u16VerSize == 0 || stVidStatus.u16VerSize>1088)
    {
        return FALSE;
    }
...
*/
    ...
    msAPI_GE_SetDC_CSC_FMT(GFX_YUV_RGB2YUV_255, GFX_YUV_OUT_255, GFX_YUV_IN_255, GFX_YUV_YVYU, GFX_YUV_YUYV);
...
#if SUPPORTED_PLAYABLE_AUDIO_TRACK_CHECK==1
//this api allows customer to set supported audio format, and must be called before MApp_VDPlayer_Init()
//ex. support all audio but no WMA -- E_VDP_AUDIO_SUPPORT_ALL | E_VDP_AUDIO_NO_SUPPORT_WMA
//ex. support MP3 and AAC only -- E_VDP_AUDIO_SUPPORT_NONE & (~E_VDP_AUDIO_NO_SUPPORT_MPG) & (~E_VDP_AUDIO_NO_SUPPORT_AAC)
void MApp_VDPlayer_SetSupportedAudio(U32 u32SupportedFormat)
{
    u32VDPlayerAudioSupportedFormat = u32SupportedFormat;
    //_Mapp_VDPlayer_SetShareMemData(E_SHAREMEM_SUPPORTED_AUDIO, u32VDPlayerAudioSupportedFormat);
}

//call this api after MApp_VDPlayer_Init() to get real supported audio
//coprocessor will check IPAuthority during init and fill the real supported audio back into sharmemory
U32 MApp_VDPlayer_GetSupportedAudio(void)
{
    U32 value;

    //value = u32VDPlayerAudioSupportedFormat;
    value = _MApp_VDPlayer_GetShareMemData(E_SHAREMEM_SUPPORTED_AUDIO);
...
BOOLEAN MApp_VDPlayer_GetMovieAudioCodecSupported(void)
{
    ...
    else
    {
        ...
        {
            return TRUE; //no audio track case, like HT3.1 I03 which is no audio track
        }
        else
        {
            switch (u32AudioCodecID)
