...
//--------------------- sTriviewSource_DTV_HDMI_DVI_VGA_YPbPr_AV_SV ----------------------------------
	/*#if ENABLE_S2
    {UI_INPUT_SOURCE_S2,  	HWND_SOURCE_INPUT_ITEM_S2},
	#endif
	#if (ENABLE_ATSC)
    {UI_INPUT_SOURCE_ATSC,	HWND_SOURCE_INPUT_ITEM_ATSC},
	#else
    {UI_INPUT_SOURCE_ATV,   HWND_SOURCE_INPUT_ITEM_ATV},
	#endif
	#if ENABLE_DVBT
    {UI_INPUT_SOURCE_DVBT,	HWND_SOURCE_INPUT_ITEM_DVBT},
	#endif
	#if ENABLE_DVBC
    {UI_INPUT_SOURCE_DVBC,  HWND_SOURCE_INPUT_ITEM_DVBC},
	#endif
	#if ENABLE_ISDBT
    {UI_INPUT_SOURCE_ISDBT, HWND_SOURCE_INPUT_ITEM_ISDB},
	#endif
	#if ENABLE_DTMB
    {UI_INPUT_SOURCE_DTMB,  HWND_SOURCE_INPUT_ITEM_DTMB},
	#endif*/
	#if (INPUT_HDMI_VIDEO_COUNT >= 1)
    {UI_INPUT_SOURCE_HDMI,        HWND_SOURCE_INPUT_ITEM_HDMI1},
	#endif
	#if (INPUT_HDMI_VIDEO_COUNT >= 2)
    {UI_INPUT_SOURCE_HDMI2,				HWND_SOURCE_INPUT_ITEM_HDMI2},
	#endif
	#if (INPUT_HDMI_VIDEO_COUNT >= 3)
    {UI_INPUT_SOURCE_HDMI3,				HWND_SOURCE_INPUT_ITEM_HDMI3},
	#endif
	#if (INPUT_HDMI_VIDEO_COUNT >= 4)
    {UI_INPUT_SOURCE_HDMI4,				HWND_SOURCE_INPUT_ITEM_HDMI4},
	#endif	
	#if	(INPUT_VGA_COUNT >= 1)
    {UI_INPUT_SOURCE_RGB,					HWND_SOURCE_INPUT_ITEM_PC},
  #endif  	
	#if (!ENABLE_SBTVD_BRAZIL_APP)
	#if (INPUT_SCART_VIDEO_COUNT >= 1)
    {UI_INPUT_SOURCE_SCART, HWND_SOURCE_INPUT_ITEM_SCART1},
	#endif
	#if (INPUT_SCART_VIDEO_COUNT >= 2)
    {UI_INPUT_SOURCE_SCART2,HWND_SOURCE_INPUT_ITEM_SCART2},
	#endif
	#endif
	#if (INPUT_YPBPR_VIDEO_COUNT>=1)
    {UI_INPUT_SOURCE_COMPONENT,		HWND_SOURCE_INPUT_ITEM_COMPONENT1},
	#endif
	#if (INPUT_YPBPR_VIDEO_COUNT >= 2)
    {UI_INPUT_SOURCE_COMPONENT2,	HWND_SOURCE_INPUT_ITEM_COMPONENT2},
	#endif
	#if (INPUT_AV_VIDEO_COUNT >= 1)
    {UI_INPUT_SOURCE_AV,          HWND_SOURCE_INPUT_ITEM_AV1},
	#endif
	#if (INPUT_AV_VIDEO_COUNT >= 2)
    {UI_INPUT_SOURCE_AV2,         HWND_SOURCE_INPUT_ITEM_AV2},
	#endif
	#if (INPUT_AV_VIDEO_COUNT >= 3)
    {UI_INPUT_SOURCE_AV3,         HWND_SOURCE_INPUT_ITEM_AV3},
	#endif
	#if (INPUT_SV_VIDEO_COUNT >= 1)
    {UI_INPUT_SOURCE_SVIDEO,			HWND_SOURCE_INPUT_ITEM_SVIDEO1},
	#endif
	#if (INPUT_SV_VIDEO_COUNT >= 2)
    {UI_INPUT_SOURCE_SVIDEO2,			HWND_SOURCE_INPUT_ITEM_SVIDEO2},
	#endif
//---------------------------------------------------------------------------------
#else
	#if ENABLE_S2
...
U16 _ZUI_ACT_GetInputSourceStringID(HWND hwnd)
{
    ...
    u16TmpID = _MApp_ZUI_ACT_InputSourceWindowMapToId(hwnd);
    printf("_ZUI_ACT_GetInputSourceStringID hwnd:%u, u16TmpID\n", hwnd, u16TmpID);
    switch(u16TmpID)
    {
/********************* < PC > *********************/
...
LPTSTR MApp_ZUI_ACT_GetInputSourceDynamicText(HWND hwnd)
{
    ...
    switch(hwnd)
    {
        ...
        case HWND_SOURCE_INPUT_ITEM_HDMI4:
         {
	    ...
	    #elif defined(sHDMI3StrToDVI)			//Ray HDM 2017.03.29: Change the HDMI3 to DVI and HDMI4 to HDMI3 strings
            if(hwnd == HWND_SOURCE_INPUT_ITEM_HDMI1)
                u16TempID = en_str_HDMI1;
            else if(hwnd == HWND_SOURCE_INPUT_ITEM_HDMI2)
                u16TempID = en_str_HDMI2;
            ...
            else if(hwnd == HWND_SOURCE_INPUT_ITEM_HDMI3)
            {
                ...
                {
                    u16Value = (((g_stMenuCecDeviceList.wPhyAddr[i] & 0xF000) >> 12) & 0x000F);
		    #if	defined(sHDMI3StrToDVI)
                    //Ray HDM 2017.03.29: For SP-4096 board, HDMI3 is DVI port and thus has no CEC function

		    #elif defined(sHDMI2StrToDVI)
    		  	      	//#ifdef sHDMI2StrToDVI
                    if(u16Value == 2)
                    {
                        u16StrBuf[curStringLen] = CHAR_SPACE;
		    ...
		    #elif  defined(sHDMI3StrToDVI)		//Ray HDM 2017.03.29: HDMI4 is changed as HDMI3
                    if(u16Value == 3)
                    {
                        u16StrBuf[curStringLen] = CHAR_SPACE;
                        curStringLen +=1;
...
BOOLEAN MApp_ZUI_ACT_HandleInputSourceKey(VIRTUAL_KEY_CODE key)
{
    ...
    switch(key)
    {
        ...
        case VK_EXIT:
            ...
            return TRUE;
        case VK_POWER:
	    printf("\nRay: input source go to power saving state");
...
#ifdef sIntoFactOSDKey
void MApp_ZUI_ACT_ExecuteFactoryOSDHotKey(void)
{
  //printf("MingYuan enMainMenuState %d \n",enMainMenuState);
  //printf("MingYuan _enTargetInputSourceState %d \n",_enTargetInputSourceState);
...
static E_UI_INPUT_SOURCE _MApp_ZUI_ACT_GetSourceType(void)
{
    ...
    if(MApp_ZUI_GetActiveOSD() == E_OSD_MAIN_MENU && MApp_ZUI_API_IsSuccessor(HWND_MENU_APP_PAGE, MApp_ZUI_API_GetFocus()))
    {
          ...
          s16Index -=1;			//Ray OSD 2017.03.02: Since we add one more App title item. We need to move back one step to keep USB Media Player at original position
          source_type = MApp_ZUI_ACT_GetAppItemSource(s16Index);
          //printf("\nMingYuan MApp_ZUI_ACT_GetAppItemSource = %d \n",source_type);
    }
    else
    {
          source_type = _MApp_ZUI_ACT_InputSourceWindowMapToId(MApp_ZUI_API_GetFocus());
          //printf("MingYuan _MApp_ZUI_ACT_InputSourceWindowMapToId = %d \n",source_type);
    }
...
#if ENABLE_ATSC_TTS
            {
                U16 u16TempID = _ZUI_ACT_GetInputSourceStringID(MApp_ZUI_API_GetFocus());
                printf("Src change to :%u\n", u16TempID);
                MApp_TTSControlSetInputStringID(u16TempID);
...
*/
        ...
        switch(act)
        {
            case EN_EXE_GAME_SELECT:
                ...
                if(cur>=HWND_SOURCE_GAME_1&&cur<=HWND_SOURCE_GAME_16)
                    bExeResult = MApp_FuncExec_InputSourcePage(EN_FUNC_INPUTSOURCE_LOAD_APP_BIN_FROM_USB, (U8)(cur-HWND_SOURCE_GAME_1), NULL);;
            break;
            default:
                break;
        }
    ...
    LPTSTR MApp_ZUI_ACT_GetGameBrowserDynamicText(HWND hwnd)
    {
        ...
        MApp_FuncExec_InputSourcePage(EN_FUNC_INPUTSOURCE_GET_SUITABLE_FILE_NUM, NULL, &u8Item);
        if(u8Item==0)
            return CHAR_BUFFER;
        ...
        switch(hwnd)
        {
            ...
            case HWND_SOURCE_GAME_16:
                ...
                {
                    MApp_FuncExec_InputSourcePage(EN_FUNC_INPUTSOURCE_GET_LONG_FILE_NAME, (U8)(hwnd-HWND_SOURCE_GAME_1), (U8 *)CHAR_BUFFER);
                }
            break;
            ...
            case VK_SELECT:
                {
                    ...
                    if(cur>=HWND_SOURCE_GAME_1&&cur<=HWND_SOURCE_GAME_16)
                        MApp_FuncExec_InputSourcePage(EN_FUNC_INPUTSOURCE_LOAD_APP_BIN_FROM_USB, (U8)(cur-HWND_SOURCE_GAME_1), NULL);;
                    return TRUE;
                }
            default:
                break;
