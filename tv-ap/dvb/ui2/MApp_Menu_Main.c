    ...
    switch(enMainMenuState)
    {
        ...
        case STATE_MENU_WAIT:
            ...
            else if(MApp_ZUI_GetActiveOSD() == E_OSD_MAIN_MENU)
            {
                ...
                {
                    if( msAPI_Timer_DiffTime_2(s_u32LastRefreshTime, msAPI_Timer_GetTime0()) > 100 )
                    {
        ...
        case STATE_MENU_GOTO_STANDBY:
            //printf("\nRay: Main menu to standby");		//Ray DBG 2017.06.05
            MApp_ZUI_ACT_ShutdownOSD();
            enMainMenuState = STATE_MENU_INIT;			//Ray DPW 2017.06.06: Prevent to go to standby continuously
            u8KeyCode = KEY_POWER;
        ...
        case STATE_MENU_GOTO_VOLUME_INFO:			//Ray VOL 2017.03.09: Exit current main menu and prepare show volume info page
            MApp_ZUI_ACT_ShutdownOSD();
            enMainMenuState = STATE_MENU_INIT;
            enRetVal = EXIT_GOTO_VOLUME;
            break;
