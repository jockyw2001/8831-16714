...
// current event info
#if (ATSC_CC != ATV_CC)
static MS_EPG_EVENT current_event;
...
static void MApp_ClosedCaption_Switch_Service_Mode(U8 mode)
{
    ...
    stGenSetting.g_CaptionSetting.u8CaptionServiceMode = mode;
...
void MApp_ClosedCaption_GetUserCmd(void)
{
    ...
    CC_DBINFO_CMD_1(printf("\n...[%s, %d] %d...\n", __FUNCTION__, __LINE__, stGenSetting.g_CaptionSetting.u8CaptionServiceMode));

    if ( stGenSetting.g_CaptionSetting.u8CaptionServiceMode == CS_OFF )
    {
    ...
    if(IsDTVInUse())
    {
        if ( (stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV == CS_CC1) || (stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV == CS_CC2) ||
             (stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV == CS_TEXT1) || (stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV == CS_TEXT2))
        {
             u8Field = 0;
        }
        else if (stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV != CS_OFF)
        {
            u8Field = 1;
        }
        ...
        {
            CC_DBAPP(printf("\nEvent not changed"));
            if (stGenSetting.g_CaptionSetting.u8CaptionServiceMode >= CS_SERVICE1)
            {
                if (u8CC708AvailableType & ( 1 << (stGenSetting.g_CaptionSetting.u8CaptionServiceMode - CS_SERVICE1)))
                {
                    u8CurrentCaptionServiceMode = stGenSetting.g_CaptionSetting.u8CaptionServiceMode;
                }
                else if ((u8CC608AvailableType & (1 << u8Field)) && (stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV != CS_OFF))
                {
                    stGenSetting.g_CaptionSetting.u8CaptionServiceMode = stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV;
                    u8CurrentCaptionServiceMode = stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV;
                }
                else
                {
                    u8CurrentCaptionServiceMode = stGenSetting.g_CaptionSetting.u8CaptionServiceMode;
        ...
        {
            CC_DBAPP(printf("\nEvent changed: %d", stGenSetting.g_CaptionSetting.u8CaptionServiceMode));
            u8CurrentCaptionServiceMode = stGenSetting.g_CaptionSetting.u8CaptionServiceMode;
            u8CC608AvailableType = 0;
            u8CC708AvailableType = 0;
        }
        ...
        if(TRUE == current_event.fHasCCInfo)
        {
            if ((u8CurrentCaptionServiceMode >= CS_SERVICE1)
                ...
                #if (PARSING_CC_WHENMUTE)
                &&(stGenSetting.g_CaptionSetting.u8CCMode == CAPTIONMENU_MODE_ON ||CAPTIONMENU_MODE_CC_WHEN_MUTE))
                #else
    ...
    if ( ( stGenSetting.g_CaptionSetting.u8CaptionServiceMode >= CS_CC1 ) && ( stGenSetting.g_CaptionSetting.u8CaptionServiceMode <= CS_TEXT4 ) )
    {
        fEnableClosedCaptionFunc = TRUE;
        if ( ( stGenSetting.g_CaptionSetting.u8CaptionServiceMode == CS_CC1 ) || ( stGenSetting.g_CaptionSetting.u8CaptionServiceMode == CS_CC2 )  \
            || ( stGenSetting.g_CaptionSetting.u8CaptionServiceMode == CS_TEXT1 ) || ( stGenSetting.g_CaptionSetting.u8CaptionServiceMode == CS_TEXT2 ) )
        {
...
#if (ATSC_CC == DTV_CC)
    else if ( ( stGenSetting.g_CaptionSetting.u8CaptionServiceMode >= CS_SERVICE1 ) && ( stGenSetting.g_CaptionSetting.u8CaptionServiceMode <= CS_SERVICE6 ) )
    {
        fEnableClosedCaptionFunc = TRUE;
...
void MApp_ClosedCaption_Init(void)
{
    ...
    if (fEnableClosedCaptionFunc == TRUE)
    {
      ...
      //  msAPI_CC_SetGwinID(CC_USING_GOP_ID, CC_USING_GWIN_ID);
        ...
        MApp_CC_CtrlParser(CC_SELECTOR_608_TIMEOUT_CAPITON, 16000);
        MApp_CC_CtrlParser(CC_SELECTOR_708_TIMEOUT_CAPTION, 20000);
        MApp_CC_CtrlParser(CC_SELECTOR_608_TIMEOUT_TEXT, 16000);
        //MApp_CC_CtrlParser(CC_SELECTOR_CC_CHECK_PERIOD, CC_CHECK_PERIOD_TIME);
        MApp_CC_CtrlParser(CC_SELECTOR_TRANSLUCENT, 0x30);
        MApp_CC_CtrlParser(CC_SELECTOR_ATV_AXIS_XPOS, 275);
        MApp_CC_CtrlParser(CC_SELECTOR_608_SCROLL_STEP, TRUE);
        //MApp_CC_CtrlParser(CC_SELECTOR_TEXT_ROWNO, 8);
        MApp_CC_CtrlParser(CC_SELECTOR_INDENT_GAP, 4); //must be even & higher than 2
        MApp_CC_CtrlParser(CC_SELECTOR_DUMP_DBG_MSG, FALSE);
        //MApp_CC_CtrlParser(CC_SELECTOR_DTV_AXIS_YPOS, 110);
        //MApp_CC_CtrlParser(CC_SELECTOR_ROW_TOPRSVHEIGHT , 3);
        //MApp_CC_CtrlParser(CC_SELECTOR_ROW_BTMRSVHEIGHT , 3);

        #if (KOREAN_CC_ENABLE)
        if(stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV == CS_CC3)
            MApp_CC_CtrlParser(CC_SELECTOR_ATV_AXIS_YPOS, 250);
        else
#if ((CHIP_FAMILY_TYPE != CHIP_FAMILY_WHISKY)||(MEMORY_MAP > MMAP_32MB))
            MApp_CC_CtrlParser(CC_SELECTOR_ATV_AXIS_YPOS, 114);
#endif
        #else
        MApp_CC_CtrlParser(CC_SELECTOR_ATV_AXIS_YPOS, 114);
        #endif
        ...
        msAPI_CC_SetOsdSetting(&stGenSetting.g_CaptionSetting);
        ...
        if(IsDTVInUse())
        {
            ...
            {
                if (u8CurrentCaptionServiceMode != stGenSetting.g_CaptionSetting.u8CaptionServiceMode)
                    u8CurrentCaptionServiceMode = stGenSetting.g_CaptionSetting.u8CaptionServiceMode;
                u8StopSwitchServiceMode = 0;
                u8CC708AvailableType = 0;
                u8CC608AvailableType = 0;
...
#if (ATSC_READY) // Sleep function not ready
  #if  (ATSC_CC == DTV_CC)
                ...
                if(IsDTVInUse())
                {
                        ...
                        && !MApp_VChip_GetCurVChipBlockStatus())
                    {
                        ...
                        if (u8PrgChg == PRG_CHANGE_BY_EVENT)
                        {
                            u8CC708AvailableType = msAPI_CC_DigitalCC708_AvailableType();
                            u8CC608AvailableType = msAPI_CC_CC608_AvailableType();
                            if (fCC708 && (u8CC708AvailableType & ( 1 << (stGenSetting.g_CaptionSetting.u8CaptionServiceMode - CS_SERVICE1))))
                            {
                                if ((u8CurrentCaptionServiceMode != stGenSetting.g_CaptionSetting.u8CaptionServiceModeDTV) &&
                                    (stGenSetting.g_CaptionSetting.u8CaptionServiceModeDTV != CS_OFF))
                                {
                                    // Mode Changed
                                    MApp_ClosedCaption_Switch_Service_Mode(stGenSetting.g_CaptionSetting.u8CaptionServiceModeDTV);
                                    CC_DBAPP(printf("\nA === ======  \r\n"));
                                }
                            }
                            g_u32CCAutoSwichStartTime = msAPI_Timer_GetTime0();
                            CC_DBAPP(printf("\nPg changed: %d: %bu:%bu %d", g_u32CCAutoSwichStartTime, fCC708, u8CC708AvailableType, u8CC608AvailableType));
                            u8StopSwitchServiceMode = 0;
                        }
                        ...
                        if (u32TimeDiff < AUTOSWITCH_CCDETECTTIME)
                        {
                            ...
                            {
                                if ((u32TimeDiff > AUTOSWITCH_CCTIMEOUT) && (u8CurrentCaptionServiceMode >= CS_SERVICE1) && ((!fCC708) ||(!(u8CC708AvailableType & ( 1 << (stGenSetting.g_CaptionSetting.u8CaptionServiceMode - CS_SERVICE1))))))
                                {
                                    MApp_ClosedCaption_Switch_Service_Mode(stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV);
                                }

                                if (((u8CC708AvailableType & ( 1 << (stGenSetting.g_CaptionSetting.u8CaptionServiceModeDTV - CS_SERVICE1))))&&(fCC708))
                                {
                                    if (u8CurrentCaptionServiceMode < CS_SERVICE1)
                                    {
                                        MApp_ClosedCaption_Switch_Service_Mode(stGenSetting.g_CaptionSetting.u8CaptionServiceModeDTV);
...
void MApp_CC_Initialize(void)
{
    ...
    MApp_CC_CtrlParser(CC_SELECTOR_CC_CHECK_PERIOD, 0);
    MApp_CC_CtrlParser(CC_SELECTOR_TRANSLUCENT, 0x30);
    msAPI_CC_SetInputData(CC_INPUT_USING157, FALSE);
...
U32 MApp_CC_CtrlParser(U32 selector, U32 value)
{
    ...
    switch (selector)
    {
        ...
        case CC_SELECTOR_SERVICE:
            ...
            if(IsDTVInUse())
            {
                stGenSetting.g_CaptionSetting.u8CaptionServiceModeDTV = value;
            }
            else
            {
                stGenSetting.g_CaptionSetting.u8CaptionServiceModeATV = value;
            }

            stGenSetting.g_CaptionSetting.u8CaptionServiceMode = value;
...
#else
        msAPI_CC_ResetOsdSetting(&stCCVarSetting);  // temporary
        stGenSetting.g_CaptionSetting.u8CaptionServiceMode = CS_SERVICE1;
        MApp_ClosedCaption_GetUserCmd();
#endif
...
#else
        msAPI_CC_ResetOsdSetting(&stCCVarSetting);
        stGenSetting.g_CaptionSetting.u8CaptionServiceMode = CS_CC1;
        MApp_ClosedCaption_GetUserCmd();
#endif
