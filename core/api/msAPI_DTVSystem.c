...
#if(ASTRA_HD_ENABLE)
static DVBS_SI_SGT_BASIC_SERVICE_LIST_INFO _astServiceListInfo[MAX_MUX_NUMBER];
...
WORD msAPI_CM_GetNextProgramPosition(MEMBER_SERVICETYPE bServiceType, WORD wBasePosition, BOOLEAN bIncludeSkipped, CHANNEL_LIST_TYPE bList_type, MEMBER_SERVICETYPE *bNewServiceType)
{
    ...
    if(IS_LCN_RANK_COUNTRY(OSD_COUNTRY_SETTING))
    {
        ...
        for(wOrder=wOrder+1; wOrder < u16ProgramIndexTableArraySize; wOrder++)
        {
            ...
            &&( FALSE == m_astDTVProgramIndexTable[wOrder].bIsDelete))
            {
                break;
            }
        }
    ...
    *pu8HpLp = u8HpLp;
    ...
    if(IsS2InUse())
    {
        for(i=0;i<MAX_DTV_S2_MUX_NUMBER;i++)
        {
            if ((_astS2ProgramIDTable[i].wTransportStream_ID == wTS_ID) && (_astS2ProgramIDTable[i].wOriginalNetwork_ID == wON_ID))
            {
                *pu8HpLp=_astS2ProgramIDTable[i].cHpLp;
                return TRUE;
            }
        }
        return FALSE;
...
BYTE msAPI_CM_GetHpLp(MEMBER_SERVICETYPE bServiceType, WORD wPosition)
{
    ...
    if( FALSE == msAPI_DtvSys_Get_IDTable_ByServiceAndPos(CUR_DB_SEL(), bServiceType, wPosition, &u8HpLp, E_PRDATA_HPLP) )
    {
        u8HpLp = 0;
    }
    ...
    {
        cIDIndex = m_astS2ProgramIndexTable[wOrder].cIDIndex;
        return _astS2ProgramIDTable[cIDIndex].cHpLp;
    }
#endif
    cIDIndex = m_astDTVProgramIndexTable[wOrder].cIDIndex;
    ...
    for(i=0; i < MAX_AUD_LANG_NUM; i++)
    {
        if( (TRUE == IsAudioPIDValid(stAudInfo[i].wAudPID)) &&
            ...
            ((E_AUDIOSTREAM_MPEG == stAudInfo[i].wAudType) ||
             ...
             (E_AUDIOSTREAM_DRA == stAudInfo[i].wAudType)) )
        {
            cAudioStreamCount++;
...
BOOLEAN msAPI_CM_GetAudioStreamInfo(MEMBER_SERVICETYPE bServiceType, WORD wPosition, AUD_INFO * pstAudioStreamInfo, BYTE cOrdinal)
{
    ...
    if( (TRUE == IsAudioPIDValid(stAudInfo[cOrdinal].wAudPID)) &&
        ...
        ((E_AUDIOSTREAM_MPEG == stAudInfo[cOrdinal].wAudType) ||
         ...
         (E_AUDIOSTREAM_DRA == stAudInfo[cOrdinal].wAudType) ) )
    {
...
BOOLEAN msAPI_CM_Dvb_FavorProgram(MEMBER_SERVICETYPE bServiceType, WORD wPosition, BOOLEAN bFavorite)
{
    CHANNEL_ATTRIBUTE stCHAttribute;
    WORD wOrder,wPRIndex;
    ...
    for( wPosition1 = 0; wPosition1 < wProgramCount; wPosition1++ )
    {
        ...
        for( wOrder2 = 0; wOrder2 < u16ProgramIndexTableArraySize; wOrder2++ )
        {
               ...
               (pstProgramIndexTable[wOrder1].wLCN == pstProgramIndexTable[wOrder2].wLCN) )
            {
                ...
                else if ( (IS_NORDIC_COUNTRY( m_eCountry )||OSD_COUNTRY_SETTING == OSD_COUNTRY_THAILAND) &&(pstProgramIndexTable[wOrder2].bServiceTypePrio != pstProgramIndexTable[wOrder1].bServiceTypePrio ))
                {
...
static void DistinguishSDAndHDLCNDuplicate(MEMBER_SERVICETYPE bServiceType)
{
    ...
    for( wPosition1 = 0; wPosition1 < wProgramCount; wPosition1++ )
    {
        ...
        if(bGotSameService == TRUE)//for no the same SD service
        {
            /*
...
static void AssignNewLCN(MEMBER_SERVICETYPE bServiceType, E_LCN_ASSIGNMENT_TYPE eAssignmentType, WORD wBeginFromLCN, WORD wEndToLCN, WORD wBeginOfNewLCN, WORD wEndOfNewLCN, WORD wNewAssignableLCN)
{
    ...
    case E_LCN_DUPLICATE:
        for(wPosition=0; wPosition < wProgramCount ; wPosition++)
        {
            ...
            if(IsS2InUse())
            {
                if( ((E_LCN_DUPLICATE|E_LCN_PRESENT) == m_astS2ProgramIndexTable[wOrder].eLCNAssignmentType) &&
                    ((wBeginFromLCN <= m_astS2ProgramIndexTable[wOrder].wLCN) ||
                     (m_astS2ProgramIndexTable[wOrder].wLCN <= wEndToLCN)) )
                {
                    if( TRUE == bIsDeletionRequired )
                    {
                        m_astS2ProgramIndexTable[wOrder].bIsDelete = TRUE;
                    ...
                    {
                        m_astS2ProgramIndexTable[wOrder].wLCN = wNewAssignableLCN;
                        SetProgramTable(m_astS2ProgramIndexTable[wOrder].wPRIndex, (BYTE *)&(wNewAssignableLCN), E_DATA_LCN);
                        m_astS2ProgramIndexTable[wOrder].eLCNAssignmentType &= ~(E_LCN_DUPLICATE|E_LCN_PRESENT);
                        wNewAssignableLCN++;
                        if( wNewAssignableLCN > wEndOfNewLCN )
                        {
                            break;
    ...
    case E_LCN_ABSENT:
        for(wPosition=0; wPosition < wProgramCount ; wPosition++)
        {
            ...
            if(IsS2InUse())
            {
                if( E_LCN_ABSENT == m_astS2ProgramIndexTable[wOrder].eLCNAssignmentType )
                {
                    if( TRUE == bIsDeletionRequired )
                    {
                        m_astS2ProgramIndexTable[wOrder].bIsDelete = TRUE;
    ...
    case E_LCN_LAST:
        ...
        if(IsS2InUse())
        {
...
#endif
                wOrder = ConvertPositionToOrder(bServiceType, wPosition);
                if( E_LCN_LAST & m_astS2ProgramIndexTable[wOrder].eLCNAssignmentType )
                {
                    if(m_astS2ProgramIndexTable[wOrder].wLCN > wMaxLCN)
                    {
                        wMaxLCN = m_astS2ProgramIndexTable[wOrder].wLCN;
                    }
                }
            }
            ...
            for(wPosition=0; wPosition < wProgramCount ; wPosition++)
            {
                ...
                if( ((E_LCN_DUPLICATE|E_LCN_PRESENT|E_LCN_LAST) == m_astS2ProgramIndexTable[wOrder].eLCNAssignmentType) &&
                    ((wBeginFromLCN <= m_astS2ProgramIndexTable[wOrder].wLCN) ||
                     (m_astS2ProgramIndexTable[wOrder].wLCN <= wEndToLCN)) )
                {
                    if( TRUE == bIsDeletionRequired )
                    ...
                    {
                        m_astS2ProgramIndexTable[wOrder].wLCN = NewAssignLastLCN;
                        SetProgramTable(m_astS2ProgramIndexTable[wOrder].wPRIndex, (BYTE *)&(NewAssignLastLCN), E_DATA_LCN);
                        NewAssignLastLCN++;
            ...
            while(1)
            {
                ...
                for(wPosition=0; wPosition < wProgramCount ; wPosition++)
                {
                    ...
                    if( E_LCN_LAST & m_astS2ProgramIndexTable[wOrder].eLCNAssignmentType )
                    {
                        if(m_astS2ProgramIndexTable[wOrder].wLCN < wMinimalLCN)
                        {
                            wMinimalLCN=m_astS2ProgramIndexTable[wOrder].wLCN;
                ...
                {
                    m_astS2ProgramIndexTable[wOrderMinimalLCN].eLCNAssignmentType &= ~E_LCN_LAST;
                    m_astS2ProgramIndexTable[wOrderMinimalLCN].wLCN = wNewAssignableLCN;
                    SetProgramTable(m_astS2ProgramIndexTable[wOrderMinimalLCN].wPRIndex, (BYTE *)&(wNewAssignableLCN), E_DATA_LCN);
                    wNewAssignableLCN++;
                    if(wNewAssignableLCN > wEndOfNewLCN )
                    {
                        break;
...
static void AssignNewLCNforLCNSorting(WORD wBeginFromLCN, WORD wEndToLCN, WORD wBeginOfNewLCN, WORD wEndOfNewLCN, WORD wLastLCN)
{
    ...
    WORD wMinOrder,wMinSID,wCurrentSID,wOrder1,wOrder2;
    BOOLEAN bIsDeletionRequired;
    ...
    for( wOrder1 = 0; wOrder1 < u16ProgramIndexTableArraySize; wOrder1++ )
    {
        ...
        if((OSD_COUNTRY_SETTING == OSD_COUNTRY_UNITED_ARAB_EMIRATES)&&(eAssignmentType == E_LCN_ZERO))
        {
        ...
        if((eAssignmentType == (E_LCN_PRESENT|E_LCN_DUPLICATE))||(eAssignmentType == E_LCN_ABSENT)||(eAssignmentType == E_LCN_ZERO))
        {
            ...
            else
            {
                for(wOrder2=wOrder1+1; wOrder2 < u16ProgramIndexTableArraySize; wOrder2++)
                {
                    ...
                    if((eAssignmentType == (E_LCN_PRESENT|E_LCN_DUPLICATE))||(eAssignmentType == E_LCN_ABSENT)||(eAssignmentType == 32))
                    {
...
static WORD GetLastLCNByRange(MEMBER_SERVICETYPE bServiceType, WORD wStart, WORD wEND)
{
    ...
    if(IsS2InUse())
    {
        for(wPosition=0; wPosition < wProgramCount; wPosition++)
        {
            ...
            {
                wOrder = ConvertPositionToOrder(bServiceType, wPosition);
                if((m_astS2ProgramIndexTable[wOrder].wLCN<wStart)||(m_astS2ProgramIndexTable[wOrder].wLCN>wEND))
                {
                    continue;
                }
                if(wLastLCN < m_astS2ProgramIndexTable[wOrder].wLCN )
                {
...
void ArrangeChannelNumberAfterSort(MEMBER_SERVICETYPE eServiceType)
{
    ...
    if (eServiceType == E_SERVICETYPE_DTV)
    {
        ...
        for(s32Order=wOrderOfTV; s32Order < MAX_DTV_S2_PROGRAM; s32Order++)
        {
            ...
            msAPI_Timer_ResetWDT();
        #endif
            if(TRUE == IsProgramEntityActive(m_astS2ProgramIndexTable[s32Order].wPRIndex))
            {
                m_astS2ProgramIndexTable[s32Order].wLCN = (WORD)s32Order +1;
                SetProgramTable(m_astS2ProgramIndexTable[s32Order].wPRIndex, (BYTE *)&(m_astS2ProgramIndexTable[s32Order].wLCN), E_DATA_LCN);
            }
            else if(FALSE == IsProgramEntityActive(m_astS2ProgramIndexTable[s32Order].wPRIndex))
            {
    ...
    else if (eServiceType == E_SERVICETYPE_RADIO)
    {
        ...
        for(s32Order=wOrderOfRadio; s32Order >=S2_FIRST_ORDER_OF_TV; s32Order--)
        {
            ...
            if(TRUE == IsProgramEntityActive(m_astS2ProgramIndexTable[s32Order].wPRIndex))
            {
                m_astS2ProgramIndexTable[s32Order].wLCN= wOrderOfRadio-(WORD)s32Order +1;
                SetProgramTable(m_astS2ProgramIndexTable[s32Order].wPRIndex, (BYTE *)&(m_astS2ProgramIndexTable[s32Order].wLCN), E_DATA_LCN);
            }
            else if(FALSE == IsProgramEntityActive(m_astS2ProgramIndexTable[s32Order].wPRIndex))
            {
    ...
    else if (eServiceType == E_SERVICETYPE_DATA)
    {
        ...
        for(s32Order=wOrderOfData; s32Order >=S2_FIRST_ORDER_OF_TV; s32Order--)
        {
            ...
            if(TRUE == IsProgramEntityActive(m_astS2ProgramIndexTable[s32Order].wPRIndex))
            {
                m_astS2ProgramIndexTable[s32Order].wLCN= wOrderOfData-(WORD)s32Order+1;
                SetProgramTable(m_astS2ProgramIndexTable[s32Order].wPRIndex, (BYTE *)&(m_astS2ProgramIndexTable[s32Order].wLCN), E_DATA_LCN);
            }
            else if(FALSE == IsProgramEntityActive(m_astS2ProgramIndexTable[s32Order].wPRIndex))
            {
    ...
    {
        m_astS2ProgramIndexTable[s32Order].wLCN = DEFAULT_ORDER;
        return;
    }
}
#endif
...
static void ArrangeLogicalChannelNumber(MEMBER_SERVICETYPE bServiceType, E_MEMBER_LCN_TYPE eLcnType, BOOLEAN bSkipDupSrvRemove)
{
    ...
    case E_SINGAPORE:
        ...
        if (eCountry == E_FRANCE || eCountry == E_NETHERLANDS)
        {
            DistinguishLCNDuplicate(bServiceType);
            DistinguishSDAndHDLCNDuplicate(bServiceType);
        }
...
static void ArrangeLogicalChannelNumberforLCNSorting(E_MEMBER_LCN_TYPE eLcnType)
{
    ...
    for(eServiceType =E_SERVICETYPE_DTV; eServiceType<E_SERVICETYPE_UNITED_TV; eServiceType++)
    {
        ...
        RemoveSameService(eServiceType);//for Nordig Spec
        ...
        switch(OSD_COUNTRY_SETTING)
        {
            case OSD_COUNTRY_UNITED_ARAB_EMIRATES:
                DistinguishNotNativeON_ID(eServiceType,UAE_ONID);
            break;
    ...
    if((wLastLCN == 0)&&(OSD_COUNTRY_SETTING == OSD_COUNTRY_MALAYSIA))
    {
        u16LcnStart =1;
    }
    ...
    else if(OSD_COUNTRY_SETTING == OSD_COUNTRY_MALAYSIA || (OSD_COUNTRY_SETTING == OSD_COUNTRY_UNITED_ARAB_EMIRATES))
    {
        u16MaxLcn = 999;
    }
    else
...
/// @return BOOLEAN: Function execution result
//****************************************************************************
static BYTE RemoveMismatchedProgram(MEMBER_SERVICETYPE bServiceType, WORD cRFChannelNumber, BYTE cPlp_ID, BYTE cHpLp, WORD *pwService_ID, MEMBER_SERVICETYPE *peService_Type, BYTE cCountOfServiceID, BYTE sat, WORD freq)
//static BYTE RemoveMismatchedProgram(MEMBER_SERVICETYPE bServiceType, WORD cRFChannelNumber, BYTE cPlp_ID, BYTE cHpLp, WORD *pwService_ID, MEMBER_SERVICETYPE *peService_Type, BYTE cCountOfServiceID)
{
    // Unused warning...
    ...
    DTVPROGRAMID_M *pMuxTable = _astDTVProgramIDTable;
    DTVPROGRAMINDEX * pstDTVProgramIndexTable = m_astDTVProgramIndexTable;
    u16MaxMuxNum = msAPI_CM_GetMuxMaxNumber();

  #if DVB_T_C_DIFF_DB
    if( IsDVBCInUse() )
    {
        pMuxTable = _astDTVCProgramIDTable;
    }
    else
  #endif
  #if (ENABLE_S2)
    ...
    {
        pMuxTable = _astS2ProgramIDTable;
        u16MaxMuxNum = MAX_DTV_S2_MUX_NUMBER;
        pstDTVProgramIndexTable = m_astS2ProgramIndexTable;
    }
  #endif
#endif
    ...
    if( IsS2InUse() ) // DVBS
    {
        ...
        for(wIDIndex=0; wIDIndex < u16MaxMuxNum; wIDIndex++)
        {
            if( (sat == pMuxTable[wIDIndex].SatelliteId)
              &&(freq == pMuxTable[wIDIndex].u32S2Frequency)&&(wIDIndex == cRFChannelNumber))
            {
              //printf(">>>>> cIDIndex = 0x%x\n", cIDIndex);
              break;
            }
        }
    }
    ...
    for(i=wProgramCount; i > 0; i--)
    {
        ...
        if(IsS2InUse())
        {
            wIDIndex = m_astS2ProgramIndexTable[wOrder].cIDIndex;
        }
        else
      #endif
        {
        ...
        if( IsS2InUse() )
        {
            //printf("r\n_____default__%d_%d___comp_%d_%d___",pMuxTable[cIDIndex].SatelliteId,pMuxTable[cIDIndex].u32S2Frequency,sat,freq);
            if( (sat== pMuxTable[wIDIndex].SatelliteId)
            ...
            #else
                wServiceId = m_astS2ProgramIndexTable[wOrder].wService_ID;
            #endif
...
BYTE msAPI_CM_RemoveMismatchedProgram(WORD cRFChannelNumber,WORD wTSID, BYTE cPLP_ID,BYTE cHpLp, WORD *pwService_ID, MEMBER_SERVICETYPE *peService_Type, BYTE cCountOfServiceID,BYTE sat,WORD freq)
//BYTE msAPI_CM_RemoveMismatchedProgram(BYTE cRFChannelNumber, WORD wTSID, BYTE cPLP_ID, BYTE cHpLp, WORD *pwService_ID, MEMBER_SERVICETYPE *peService_Type, BYTE cCountOfServiceID)
{
    ...
    if( IsS2InUse() )
    {
        cRemovedProgramCount += RemoveMismatchedProgram(E_SERVICETYPE_DTV, wPhysicalChannelNumber, cPLP_ID, cHpLp, pwService_ID, peService_Type, cCountOfServiceID,sat,freq);
        cRemovedProgramCount += RemoveMismatchedProgram(E_SERVICETYPE_DATA, wPhysicalChannelNumber, cPLP_ID, cHpLp, pwService_ID, peService_Type, cCountOfServiceID,sat,freq);
        cRemovedProgramCount += RemoveMismatchedProgram(E_SERVICETYPE_RADIO, wPhysicalChannelNumber, cPLP_ID, cHpLp, pwService_ID, peService_Type, cCountOfServiceID,sat,freq);
    }
    else
#endif
    {
        cRemovedProgramCount += RemoveMismatchedProgram(E_SERVICETYPE_DTV, cRFChannelNumber, cPLP_ID, cHpLp, pwService_ID, peService_Type, cCountOfServiceID,sat,freq);
        cRemovedProgramCount += RemoveMismatchedProgram(E_SERVICETYPE_DATA, cRFChannelNumber, cPLP_ID, cHpLp, pwService_ID, peService_Type, cCountOfServiceID,sat,freq);
        cRemovedProgramCount += RemoveMismatchedProgram(E_SERVICETYPE_RADIO, cRFChannelNumber, cPLP_ID, cHpLp, pwService_ID, peService_Type, cCountOfServiceID,sat,freq);
    }
...
WORD ConvertOrderToPosition_DBSel(TypDtvDbSel eDtvDbSel, WORD wOrder)
{
    ...
    if(IS_LCN_RANK_COUNTRY(OSD_COUNTRY_SETTING))
    {
        ...
        #else
            switch(m_astDTVProgramIndexTable[wOrder1].bServiceType)
        #endif
            {
    ...
    #else
        bNewServiceType = (MEMBER_SERVICETYPE)m_astDTVProgramIndexTable[wOrder].bServiceType;
    #endif
...
//BOOLEAN msAPI_CM_AddProgramIDTable(DTVPROGRAMID *pstDtvIDTable, BOOLEAN bSave, BYTE *cIDIndex)
{
    ...
    if(IsS2InUse())
    {
        maxidnum = MAX_DTV_S2_MUX_NUMBER;
        pProgramIDTable=_astS2ProgramIDTable;
    }
    else
    {
        maxidnum= MAX_MUX_NUMBER;
        ...
        if(IsS2InUse())
        {
                ...
                IsIDEntityActive_DBSel(eDtvDbSel, wIDIndex))
            {
                if(cNetworkIndex != pProgramIDTable[wIDIndex].cNetWorkIndex)
                {
                    ...
                    //if((cNetworkIndex < MAX_NETWOEK_NUMBER)&&(FALSE == SetIDTable(wIDIndex,(BYTE *)&cNetworkIndex,E_DATA_NETWORK_INDEX)))
                    if( (cNetworkIndex < u16NetWorkTableArraySize)
...
#endif
        {
            ...
            {   // DVBT
                if( /*pstDtvIDTable->wTransportStream_ID == pProgramIDTable[wIDIndex].wTransportStream_ID &&
                                         pstDtvIDTable->wOriginalNetwork_ID == pProgramIDTable[wIDIndex].wOriginalNetwork_ID &&*/
                    pstDtvIDTable->cRFChannelNumber == pProgramIDTable[wIDIndex].cRFChannelNumber &&
    ...
    #else
        #if (ENABLE_T_C_COMBO)
            if(  (!IsDVBCInUse()||(pstDtvIDTable->u32Frequency == pProgramIDTable[wIDIndex].u32Frequency||
                   IsFreqInsideOffsetRange(pstDtvIDTable->u32Frequency, pProgramIDTable[wIDIndex].u32Frequency))
                 )
               &&
                //(pstDtvIDTable->wTransportStream_ID == pProgramIDTable[wIDIndex].wTransportStream_ID) &&
                //(pstDtvIDTable->wOriginalNetwork_ID == pProgramIDTable[wIDIndex].wOriginalNetwork_ID) &&
                 ( IsDVBCInUse() ||((pstDtvIDTable->cPLPID == pProgramIDTable[wIDIndex].cPLPID) &&
                    (pstDtvIDTable->cHpLp== pProgramIDTable[wIDIndex].cHpLp) &&
                    (pstDtvIDTable->cRFChannelNumber == pProgramIDTable[wIDIndex].cRFChannelNumber))
                 )
              )
    ...
    #endif
            {
                ...
                if(cNetworkIndex != pProgramIDTable[wIDIndex].cNetWorkIndex)//TS change case
                {
                    //if((pProgramIDTable[wIDIndex].cNetWorkIndex < MAX_NETWOEK_NUMBER))  //&&(FALSE == SetIDTable(cIDIndex,(BYTE *)&cNetworkIndex,E_DATA_NETWORK_INDEX)))
                    if((pProgramIDTable[wIDIndex].cNetWorkIndex < u16NetWorkTableArraySize))
                    {
                        cNetworkIndex = pProgramIDTable[wIDIndex].cNetWorkIndex;// = cNetworkIndex;
            ...
            #if ( /*ENABLE_T_C_COMBO ||*/ ENABLE_DVBC)
                if (pstDtvIDTable->u32Frequency != pProgramIDTable[wIDIndex].u32Frequency
                  #if 1//(ENABLE_T_C_COMBO)
                    && IsDVBCInUse()
                  #endif
    ...
    //if(wIDIndex >= msAPI_CM_GetMuxMaxNumber())//search free mux
    ...
    #else
        #if (ENABLE_T_C_COMBO)
            if((IsDVBCInUse() && (INVALID_FREQUENCY == pProgramIDTable[wIDIndex].u32Frequency)) ||
                (!IsDVBCInUse() && (INVALID_PHYSICAL_CHANNEL_NUMBER == pProgramIDTable[wIDIndex].cRFChannelNumber )))
        #elif ENABLE_DVBC
            if(INVALID_FREQUENCY == pProgramIDTable[wIDIndex].u32Frequency)
        #else
            if(INVALID_PHYSICAL_CHANNEL_NUMBER == pProgramIDTable[wIDIndex].cRFChannelNumber )
...
BOOLEAN UpdateOrderOfProgramTable(MEMBER_SERVICETYPE bServiceType)
{
    ...
    if(IsS2InUse())
    {
        for(i=0; i < wProgramCount; i++)
        {
            ...
            if( (bServiceType == m_astS2ProgramIndexTable[wOrder].bServiceType) &&
            (TRUE == IsProgramEntityActive(m_astS2ProgramIndexTable[wOrder].wPRIndex)) )
            {
                GetProgramTable(m_astS2ProgramIndexTable[wOrder].wPRIndex, (BYTE *)&wSavedOrder, E_DATA_ORDER);
                if( wOrder != wSavedOrder )
                {
                    wSavedOrder = wOrder;
                    SetProgramTable(m_astS2ProgramIndexTable[wOrder].wPRIndex, (BYTE *)&wSavedOrder, E_DATA_ORDER);
                }
            ...
            else
            {
                ...
                m_astS2ProgramIndexTable[wDeletedOrder].bServiceType = E_SERVICETYPE_INVALID;
                m_astS2ProgramIndexTable[wDeletedOrder].wPRIndex = INVALID_PRINDEX;

                ActiveProgramEntity_DBSel(eCurDtvDbSel, m_astS2ProgramIndexTable[wDeletedOrder].wPRIndex, FALSE);
...
BOOLEAN SortProgram(MEMBER_SERVICETYPE bServiceType)
{
  ...
  #if ENABLE_S2
    DTVPROGRAMINDEX* pProgramInfo=m_astDTVProgramIndexTable;
    if (IsS2InUse())
    {
        pProgramInfo= m_astS2ProgramIndexTable;
    }
  #endif
    ...
    for(wPosition1=0; wPosition1 < wProgramCount; wPosition1++)
    {
        ...
        for(wPosition2=wPosition1+1; wPosition2 < wProgramCount; wPosition2++)
        {
            ...
            #if 1//ENABLE_S2
                #if (ENABLE_T_C_COMBO)
                    if((IsDVBCInUse() && ( pMuxTable[pProgramInfo[wOrder2].cIDIndex].u32Frequency<
                        pMuxTable[pProgramInfo[wOrder1].cIDIndex].u32Frequency)) ||
                        (!IsDVBCInUse()&&( pMuxTable[pProgramInfo[wOrder2].cIDIndex].cRFChannelNumber <
                        pMuxTable[pProgramInfo[wOrder1].cIDIndex].cRFChannelNumber )))
                #elif ENABLE_DVBC
                    if( pMuxTable[pProgramInfo[wOrder2].cIDIndex].u32Frequency<
                        pMuxTable[pProgramInfo[wOrder1].cIDIndex].u32Frequency)
            ...
            #else
                #if (ENABLE_T_C_COMBO)
                    if((IsDVBCInUse() && ( pMuxTable[m_astDTVProgramIndexTable[wOrder2].cIDIndex].u32Frequency<
                        pMuxTable[m_astDTVProgramIndexTable[wOrder1].cIDIndex].u32Frequency)) ||
                        (!IsDVBCInUse()&&( pMuxTable[m_astDTVProgramIndexTable[wOrder2].cIDIndex].cRFChannelNumber <
...
static BOOLEAN MoveProgram(WORD wFromOrder, WORD wToOrder)
{
    ...
    if(IsS2InUse())
    {
        if(wFromOrder > wToOrder)
        {
            ...
            {
                m_astS2ProgramIndexTable[wOrder] = m_astS2ProgramIndexTable[wOrder-1];
                            wPRIndex = m_astS2ProgramIndexTable[wOrder].wPRIndex;
                            GetProgramTable(wPRIndex, (BYTE *)&wOriinalLCN, E_DATA_TS_LCN);
                            if ((m_astS2ProgramIndexTable[wOrder].wLCN > 1) &&
                                (m_astS2ProgramIndexTable[wOrder].wLCN < INVALID_LCN_MAX_NUMBER)&&
                                (wOriinalLCN > INVALID_LCN_MAX_NUMBER))
                                {
                                    m_astS2ProgramIndexTable[wOrder].wLCN = m_astS2ProgramIndexTable[wOrder].wLCN-1;
                                }
            }
...
#endif
    {
        switch(eMember)
        {
            ...
            case E_DATA_SYMB_RATE:
                return SetNVRAM(u32CurProgIdRamDiskAddr + offsetof(DTVPROGRAMID_M, u32SymbRate),
                                pcBuffer, sizeof(U32));
...
BOOLEAN GetIDTable_DBSel(TypDtvDbSel eDtvDbSel, WORD bIDIndex, BYTE * pcBuffer,PROGRAMDATA_MEMBER eMember)
{
...
#endif
    {
        switch(eMember)
        {
            ...
            case E_DATA_SYMB_RATE:
                    return GetNVRAM(u32CurProgIdRamDiskAddr + offsetof(DTVPROGRAMID_M, u32SymbRate),
                            pcBuffer, sizeof(U32));
...
BOOLEAN FillProgramIDWithDefault(DTVPROGRAMID_M * pstDTVProgramID)
{
    ...
    pstDTVProgramID->u8Polarity = 0xff;
...
void UpdateProgramCount_DBSel(TypDtvDbSel eDtvDbSel, MEMBER_SERVICETYPE bServiceType)
{
    ...
    if (IsS2InUse())
    {
        m_awS2ProgramCount[ConvertServiceTypeToPosition(bServiceType)][EXCLUDE_NOT_VISIBLE_AND_DELETED] = CountProgram(bServiceType, EXCLUDE_NOT_VISIBLE_AND_DELETED);
        m_awS2ProgramCount[ConvertServiceTypeToPosition(bServiceType)][INCLUDE_NOT_VISIBLE_EXCLUDE_DELETED] = CountProgram(bServiceType, INCLUDE_NOT_VISIBLE_EXCLUDE_DELETED);
        m_awS2ProgramCount[ConvertServiceTypeToPosition(bServiceType)][INCLUDE_ALL] = CountProgram(bServiceType, INCLUDE_ALL);
    }
    else
#endif
    {
    ...
    if(IsS2InUse())
    {
        for(wOrder=0; wOrder < MAX_DTV_S2_PROGRAM; wOrder++)
        {
            if( bServiceType == m_astS2ProgramIndexTable[wOrder].bServiceType )
            {
                if( eCountOption == EXCLUDE_NOT_VISIBLE_AND_DELETED )
                {
                    if( (FALSE == m_astS2ProgramIndexTable[wOrder].bVisibleServiceFlag) ||
                    (TRUE == m_astS2ProgramIndexTable[wOrder].bIsDelete) )
                    {
                        continue;
                    }
                }
                ...
                {
                    if( TRUE == m_astS2ProgramIndexTable[wOrder].bIsDelete )
                    {
                        continue;
                    }
                }
        ...
        if( MAX_DTV_S2_PROGRAM <= wProgramCount )
        {
            return MAX_DTV_S2_PROGRAM;
        }
    }
    else
#endif
...
BOOLEAN msAPI_CM_InitDTVDataManager(void)
{
    ...
    // What is this ???
#if(ASTRA_HD_ENABLE)
    memset(_astServiceListInfo, 0, sizeof(DVBS_SI_SGT_BASIC_SERVICE_LIST_INFO)*MAX_MUX_NUMBER);
#endif  //#if(ASTRA_HD_ENABLE)
...
BOOLEAN msAPI_CM_S2SetsearchPosition(WORD wPosition)
{
    ...
    else
    {
        ...
        {
            if(m_astS2ProgramIndexTable[tempOrder].wPRIndex == wPosition)
            {
                wOrder = tempOrder;
                break;
            }
    ...
    msAPI_CM_SetCurrentServiceType((MEMBER_SERVICETYPE)m_astS2ProgramIndexTable[wOrder].bServiceType);

    if(m_astS2ProgramIndexTable[wOrder].bServiceType==E_SERVICETYPE_DTV)
    {
    ...
    else if(m_astS2ProgramIndexTable[wOrder].bServiceType==E_SERVICETYPE_RADIO)
    {
...
WORD msAPI_CM_CountFindProgramByName(MEMBER_SERVICETYPE bServiceType,BYTE *bProgramName)
{
    ...
    U8 u8SpecFlag=0;//Èç¹ûµÚÒ»¸ö×Ö·ûÊÇ\x5£¬ÄÇÃ´ËµÃ÷ÊÇÌØÊâµÄ·ûºÅ
...
BOOLEAN msAPI_CM_SaveServiceListInfo(const U8 *pu8Dest, U16 u16ServiceListID, BOOLEAN bListInFlag)
{
   int i = 0;
...
BOOLEAN msAPI_CM_GetExistServiceListInfo( const U16 u16ServiceListID, BOOLEAN bListInFlag)
{
    ...
    if(bListInFlag)
    {
        for(i = 0; i < MAX_MUX_NUMBER; i++)
        {
            if(_astServiceListInfo[i].iSUsed == TRUE)
            {
             if((_astServiceListInfo[i].u16ServiceListID == u16ServiceListID)  && (u16ServiceListID != 0) )
             {
                        ret = TRUE;
...
U16 msAPI_CM_GetCurrentServiceListID(void)
{
    ...
    for(i = 0; i < MAX_MUX_NUMBER; i++)
    {
        if(_astServiceListInfo[i].iSUsed == TRUE)
        {
             ...
             {
                       return _astServiceListInfo[i].u16ServiceListID;
             }
        }
    }
