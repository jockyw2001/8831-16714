...
#ifdef PVR_UTOPIA
#ifdef MIPS_CHAKRA
static U8 xdata * _pu8MetadataForWrite;
static U8 xdata * _pu8MetadataForRead;
#else
...
*/
    ...
    if (pstPvrRecordPath->bRecordAll == FALSE)
    {
        if (DMX_FILTER_STATUS_OK!= MApi_DMX_Pvr_Start(FALSE))  {ASSERT(0);}
    }
    else
    {
...
enPvrApiStatus msAPI_PVR_RecordChangeFile(U16 *pu16RecordFileName, _msAPI_PVR_RecordPath * pstPvrRecordPath)
{
  ...
  //printf("msAPI_PVR_RecordFileChange\n");
    ...
    pstPvrRecordPath->u32RecordedPeriod          = 0;
    pstPvrRecordPath->u32PausedTime              = 0;
    pstPvrRecordPath->u32PausedPeriod            = 0;
...
/***********************************************************************************************/
U32 msAPI_PVR_PlaybackGetTotalTimeByTimeStamp(U8 u8FileHandle)
{
    LongLong fileLen;
...
/***********************************************************************************************/
U32 msAPI_PVR_PlaybackGetPosTimeByTimeStamp(U8 u8FileHandle,LongLong filePos)
{
	//LongLong CurfileLen;
    U32 u32ReadLength,u32Totaltime;
    U32 u32ReadPointer = PVR_TRICK_MODE_ADR;
    ...
    if(enUpdateMode == E_METADATA_UPDATE_FILE)
    {
        ...
        if (_PvrRecordMeataData.u16RecordedAvgRate == 0)
        {
            if(pstPvrPlaybackPath->enDirection != E_PLAYBACK_PATH_DIRECTION_FASTBACKWARD)
            {
                U32 u32Totaltime = 0;
                LongLong fileLen;
                u32Totaltime = msAPI_PVR_PlaybackGetTotalTimeByTimeStamp(pstPvrPlaybackPath->hReadFile);
                if (u32Totaltime >= 10)
                {
  ...
  //printf("timestamp nop; playback setbuf->setspeed; cmdq nop\n");
    ...
    #if 1
    {
        ...
        msAPI_PVRFS_FileRead(pstPvrPlaybackPath->hReadFile, msAPI_MIU_XData2SDRAMAddr((U16)u8PacketHeader), 8);
            ...
            msAPI_PVRFS_FileRead(pstPvrPlaybackPath->hReadFile, (U32)u8PacketHeader, 8);
            #else
            msAPI_PVRFS_FileRead(pstPvrPlaybackPath->hReadFile, msAPI_MIU_XData2SDRAMAddr((U32)u8PacketHeader), 8);
            #endif
        #else
        //[for AES decrypt
...
static U32 GetLinearFilePos(_msAPI_PVR_PlaybackPath * pstPvrPlaybackPath ,U32 u16JumpSecond,U32 u32FileValidPosStrKB,U32 u32FileValidPosEndKB)
{
	...
	else
	{
	    ...
	    {
	        mid = (low + high) / 2;
	        mid_time = msAPI_PVR_PlaybackGetPosTimeByTimeStamp(pstPvrPlaybackPath->hReadFile,msAPI_PVR_KToLongLong(mid*PVR_TS_PACKET_SIZE));
...
enPvrApiStatus msAPI_PVR_PlaybackPathJumpForward(_msAPI_PVR_PlaybackPath * pstPvrPlaybackPath, U32 u16JumpSecond, enPvrJumpBufferMode enBufferMode)
{
	...
	mid_time = msAPI_PVR_PlaybackGetPosTimeByTimeStamp(pstPvrPlaybackPath->hReadFile,msAPI_PVR_KToLongLong(u32JumpKByte));
