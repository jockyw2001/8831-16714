...
#if 1//defined (MSOS_TYPE_LINUX) || defined (MSOS_TYPE_ECOS)
#include <math.h>
#endif
/*@ </Include> @*/
...
MAPI_BOOL MSB1233C_HW_init(void)
{
    ...
    DBG_DEMOD_LOAD_I2C(printf("[msb1233c]MIU wait test done, u8_tmp=0x%x,bRet=0x%x\n", u8_tmp, bRet));
    while((u8_tmp & 0x80) != 0x80)
    {
        if(u8_timeout++ > 200)
        {
            ERR_DEMOD_MSB(printf("[msb1233c][err]MIU self test NOT finished...\n"));
            return MAPI_FALSE;
        }
        // 10us delay
        usleep(10);
    ...
    if(u8_tmp == 0x40)
    {
        ERR_DEMOD_MSB(printf("@msb1233c, error, MIU self test fail !!!!\n"));
        bRet = false;
    }
    else
    {
        DBG_DEMOD_LOAD_I2C(printf("@msb1233c, MIU self test successful.\n"));
    }
...
//wangfan@2012/09/11 Remark: Disable WatchDog, cause this Initialization may cost a long time and reset WatchDog repeatedly
#if (WATCH_DOG == ENABLE )
...
MAPI_U8 DTV_DVBT_DSPReg_CRC(void)
{
    ...
    for (idx = 0; idx<((MAPI_U8)MSB1233C_DVBT_PARAM_LEN - (MAPI_U8)T_OPMODE_RFAGC_EN - 1); idx++)
    {
...
#define T2_TS_SERIAL_VAL        0x00	//0x01
...
#define CCI                     0x00
#define SERIAL_TS               0x01 //0: parallel 1:serial
#define TS_OUT_INV              0x01 //
#define TS_DATA_SWAP            0x00
...
MAPI_BOOL ExtendCmd(MAPI_U8 SubCmd, MAPI_U32 u32Param1, MAPI_U32 u32Param2, void *pvParam3)
{
    ...
    if(SubCmd == 0x80)  // temp solution, to turn on/off IF AGC
    {
        if(u32Param1 == 1)          // turn on
        {
            ...
            DBG_DEMOD_MSB(printf("MSB1233C: Turn on IF AGC: %02x\n", u8Data));
        }
        else if(u32Param1 == 0)     // turn off
        {
            ...
            u8Data &= (0xFF-0x10);
            u8Status &= WriteReg(0x0A00 + (0x18) * 2, u8Data);
            DBG_DEMOD_MSB(printf("MSB1233C: Turn off IF AGC: %02x\n", u8Data));
        }
    }
        ...
        switch(m_enCurrentDemodulator_Type)
        {
            case E_DEVICE_DEMOD_DVB_T2:
            {
                ...
                if(bCheckPass)
                {
                    ...
                    bDoReset = MAPI_FALSE;
                    *((EN_MSB_1233C_LOCK_STATUS *)pvParam3) = E_DEMOD_LOCK;
                }
                else if((u32NowTime - u32ChkScanTimeStart < u32Timeout)
                        || (u32NowTime - u32LockTimeStartDVBT2 < u32LockTimeout))
                {
                    FECLock = MAPI_FALSE;
                    *((EN_MSB_1233C_LOCK_STATUS *)pvParam3) = E_DEMOD_CHECKING;
                }
                else
                {
                    FECLock = MAPI_FALSE;
                    *((EN_MSB_1233C_LOCK_STATUS *)pvParam3) = E_DEMOD_UNLOCK;
                }
            }
            break;
...
EN_FRONTEND_SIGNAL_CONDITION DTV_GetSNR(void)
{
	...
	switch (m_enCurrentDemodulator_Type)
	{
		case E_DEVICE_DEMOD_DVB_T2:
		{
			if(FECLock == FALSE) g_msb1233c_fSNR = (float)0.0;
			else
			{
				float fsnr1 = 0.0;
				float fsnr2 = 0.0;
...
//				fsnr1 = 10.0*Log10Approx((float)snr_out/fSNRDivider[snr_ave_num]);
...
//					fsnr2 = snr_calib_slope*fsnr1+10.0*Log10Approx(snr_calib_bias);
					fsnr2 = snr_calib_slope*fsnr1+10.0*log10f(snr_calib_bias);
				}
				else
					fsnr2 = fsnr1;

				g_msb1233c_fSNR = fsnr1;
			...
			g_msb1233c_fSNR = g_msb1233c_fSNR + snr_poly;

			if (g_msb1233c_fSNR>25)
			eSignalCondition = E_FE_SIGNAL_STRONG;
			else if (g_msb1233c_fSNR>20)
			eSignalCondition = E_FE_SIGNAL_MODERATE;
			else if (g_msb1233c_fSNR>15)
...
MAPI_BOOL DTV_DVB_T_Get_TPS_Parameter(MAPI_U16 * pu16TPS_parameter, E_SIGNAL_TYPE eSignalType)
{
    ...
    if(MSB1231_Lock(COFDM_FEC_LOCK_T2_123X) == MAPI_TRUE)
    {
...
MAPI_U16 DTV_GetSignalQuality(void)
{
    ...
    switch(m_enCurrentDemodulator_Type)
    {
        case E_DEVICE_DEMOD_DVB_T2:
        {
            ...
            //DBG_GET_SIGNAL(printf("MSB1233C Data BER = %8.3e \n ", fber));
...
*/
            ...
            cn_rec = fSNR;
            cn_ref = fDVBT2_SQI_CNref[u8Modulation][u8CodeRate];
...
//            DBG_GET_SIGNAL(printf(">>> MSB1233C SNR=%f\n",fSNR);)
//            DBG_GET_SIGNAL(printf(">>> MSB1233C SQI=%d\n",SQI);)

/*
            printf(">>> MSB1233C Data BER = %ld, fBER_SQI=%d\n ", (MAPI_U32)(fber*100000000), (MAPI_U16)(fBER_SQI*100));
			printf(">>> MSB1233C cn_rec = %d, cn_ref = %d, cn_rel = %d\n",(MAPI_U16)(cn_rec*100), (MAPI_U16)(cn_ref*100), (MAPI_U16)(cn_rel*100));
			printf(">>> MSB1233C SQI=%d\n",SQI);
*/
...
S_MSB_123X_RFAGC_SSI FT2125_RFAGC_SSI[] =
{
    {-19.40,    0x1B},
    {-20.40,    0x1C},
    {-21.40,    0x1D},
...
S_MSB_123X_IFAGC_SSI FT2125_IFAGC_SSI[] =
{
    {-48,   0x3A},
    {-49,   0x3B},
    {-50,   0x3C},
...
{
#if 0//(FRONTEND_TUNER_TYPE == NUTUNE_FK1602_TUNER)
    ...
    ErrorCode = tmbslTDA18273GetPowerLevel(pObj->tUnitTDA18273, &PowerLevel_val);
    if(ErrorCode == TM_OK)
    ...
    MAPI_U8  reg = 0;
    //S_MSB_123X_IFAGC_SSI     *ifagc_ssi;
...
{
    EN_MSB_1233C_LOCK_STATUS eLockStatus = E_DEMOD_CHECKING;
...
MS_BOOL DTV_GetMSB1233C_Ready(void)
{
    return stMapiDemodulatorFun.bMSB123X_Ready;
}
...
MAPI_BOOL MSB123x_DJB_RST(void)
{
    ...
    if(!DTV_GetMSB1233C_Ready() && (DVB_T2_DEMOD != MApp_DTV_Scan_T2_ScanCheckSignal()))
	{
		  return bRet;
	}

    if ( MSB1231_Lock(COFDM_P1_LOCK_HISTORY_123X) == TRUE )
    {
