...
#define LOG_FILE_MEM_ADDR_START(GroupIdx)   (MIU0_END_ADR + (GroupIdx*LOG_FILE_MEM_ADDR_SIZE(GroupIdx)))
...
void Debug_Log_MemWriteByte(U32 u32MemAddr, U8 u8Data )
{
    U32 u32VirAddr;
...
void Debug_Log_PutCharInMem(U8 u8GroupIdx, char ch)
{
    ...
    Debug_Log_MemWriteByte( g_LogFile_u32CurMemAddr[u8GroupIdx], ch );
...
int Debug_Log_MemPrintf(U8 u8GroupIdx, const char *fmt, ...)
{
    ...
    for (i = 0; i < iret; i++)
    {
        ...
        {
            Debug_Log_PutCharInMem(u8GroupIdx, '\r');
        }

        Debug_Log_PutCharInMem(u8GroupIdx, ch);
    }
...
U8 msDebug_OpenUSBPort(void)
{
    ...
    //printf_2(" msDebug_UartToUSB_OpenUSBPort()\n",0);
...
#define FATAL_ERR_MSG_DELAY     2000
...
void msDebug_FatalErrorHandler(U32 u32Id, U32 u32Para)
{
    ...
    printf("\nFatal Error (%u,%u) happen at %u\n", u32Id, u32Para, MsOS_GetSystemTime());
...
void msDebug_PrintChgSrcTime(char* pStr)
{
    ...
    printf(" at %u(%u)", g_Debug_u32Time_ChangeSrc, MsOS_GetSystemTime());
