...
#define NEWCROPMSG(x)                   //x//x
#define MSG(x)                          //x//x
#define DEBUG_SET_WIN(x)                //x//x
#define DEBUG_CAP_WIN(x)                //x//x
#define DEBUG_ASPECT_RATIO(x)           //x//x
#define DEBUG_SCALER_FLOW(x)            //x//x
#define DEBUG_SCALER_INPUT_TIMING(x)    //x//x
#define DEBUG_OVERSCAN(x)               //x//x
...
// Over Scan
//------------------------------------------------------------------------------
#ifdef MSD8220_ReferCodeBase //OverScan
#define OVERSCAN_DEFAULT_H              0 // 1.0%	
#define OVERSCAN_DEFAULT_V              0 // 1.0%
#else
...
void MApp_Scaler_3D_SetPanelTiming_ByResolution(EN_4K2K_OUTPUT_RESOLUTION_TYPE eResType, EN_3D_TYPE eBackEnd3DType)
{
    ...
    if(enNewPanelIndex != g_PNL_TypeSel)
    {
        ...
        if(devPanel_IsVx1(g_PNL_TypeSel))
        {
            ...
            //printf("MingYuan Print MApp_Scaler_3D_SetPanelTiming_ByResolution Vby1 \n");
        }
...
void MApp_Scaler_SetTiming(INPUT_SOURCE_TYPE_t enInputSourceType, SCALER_WIN eWindow)
{
    ...
    else if( IsSrcTypeATV(enInputSourceType) || IsSrcTypeDigitalVD(enInputSourceType) )
    {
        ...
        if( bVdInputIs50Hz ) // VD 50Hz
        {
            ...
            {
                //printf("\nWarning: u16InputVFreq=%u is abnormal!\n", stTimingInfo.u16InputVFreq);
                stTimingInfo.u16InputVFreq = 500;
            }
        }
        else
    ...
    if(IsSrcTypeHDMI(enInputSourceType))
    {
        ...
        if (MApi_XC_GetHdmiSyncMode() == HDMI_SYNC_DE)
        {
            ...
            {                       
            	                            // HDMI mode
            	  #ifdef MSD8220_ReferCodeBase  //OverScan
                pstXC_SetWin_Info->stCapWin.x = 0;
                pstXC_SetWin_Info->stCapWin.y = 0;
            	  #else                          
                                       ...
                                       MS_WINDOW_TYPE *pstDstWin)
{
    ...
    if(enVideoScreen ==VIDEOSCREEN_ORIGIN)
    {
        ...
        if(IsDTVInUse())
        {
            if( gstVidStatus.u8AspectRate == ASP_16TO9)  //16:9
            {
                enVideoScreen = VIDEOSCREEN_WIDE2;
            }
            else if(gstVidStatus.u8AspectRate == ASP_4TO3) //4:3
            {
                enVideoScreen = VIDEOSCREEN_LETTERBOX;
            }
            else
            {
                if((pstCropWin->width > 800) && (pstCropWin->height > 620))
                {
                    enVideoScreen = VIDEOSCREEN_WIDE2;
                }
                else
        ...
        if( u8Enable ) // Disable BlueScreen
        {
            ...
            else if( IsAnalogSourceInUse() )//( IsHDMIInUse() )
            {
                ...
                else // 50/60P
                {
                    ...
                    // This is a patch!! // Don't why need mute so long
...
BOOLEAN MApp_Scaler_Check_PointToPoint_Mode(PQ_WIN eWindow)
{
    ...
    MSG(printf("[MDrv_PQ_Check_PointToPoint_Mode] %u\n",bRet);)
...
BOOL MApp_Scaler_Is_AspectRatioSupported_ByUI(EN_MENU_AspectRatio eUiAspectRatio)
{
    ...
    switch( eUiAspectRatio )
    {
	    ...
	    return FALSE;			//Ray ART 2017.05.23: Disable Original (Auto)
            {
            #if (ENABLE_3D_PROCESS)
                if( MApi_XC_Get_3D_Output_Mode() != E_XC_3D_OUTPUT_MODE_NONE)
                    return FALSE;
        ...
        case EN_AspectRatio_Zoom1:
        case EN_AspectRatio_Zoom2:
	    return FALSE;			//Ray ART 2017.05.23: Disable Zoom1 and 2
        ...
        case EN_AspectRatio_Panorama:
	    return FALSE;			//Ray ART 2017.05.23: Disable Panorama
            {
