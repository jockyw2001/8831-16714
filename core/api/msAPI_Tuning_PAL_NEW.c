...
#define AUTO_TUNING_FLOW_DEBUG      FALSE//FALSE//TRUE
...
void msAPI_Tuner_TuningProcessor(eAFTSTEP eState)
{
    ...
    switch ( _eCurrentTuningState )
    {
        ...
        case AFT_GOTO_CHECK_VDLOCK:
            //TUNING_FLOW_MSG(printf(" AFT_GOTO_CHECK_VDLOCK \n"));
            u32VifVDLockCheckStart=msAPI_Timer_GetTime0();
        ...
        case AFT_CHECK_VDLOCK:
            //TUNING_FLOW_MSG(printf(" AFT_CHECK_VDLOCK \n"));
        ...
        case AFT_DETECTVIDEO:
        {
            ...
            //TUNING_FLOW_MSG(printf(" AFT_DETECTVIDEO \n"));
               ...
               || (u16StatusLoop & (VD_CHECK_STATUS_RDY|VD_HSYNC_LOCKED)) )
            {
                //printf(" if( L && VD Lock )\n");
                if( !(_bIsLSearch)
                    &&(((msAPI_AVD_CheckLockAudioCarrier()==TRUE)&&(_u16TunerPLL <= (_u16PreviousStationTunerPLL+ (TUNER_PLL_PLUS_6p5MHz))))||msAPI_AVD_CheckFalseChannel())
                   )
                {
            ...
            #else
            {
                ...
                if( bIsAutoTune )
                {
                    ...
                    {
                        printf(" eVdStd_Stable=%u\n", eVdStd_Stable);
                        printf(" eVdStd_Current=%u\n", eVdStd_Current);
                    }
                    */
            ...
            {
                //printf("[VD not ready %u]", MsOS_GetSystemTime());
                /*if( msAPI_Timer_DiffTime_2(s_Tuning_u32DetectVideoStartTime, MsOS_GetSystemTime()) > 400 )
                {
                    printf("[VD not ready %u]", msAPI_Timer_DiffTime_2(s_Tuning_u32DetectVideoStartTime, MsOS_GetSystemTime()));
                }*/
        ...
        case AFT_SHOWTIME2:
          ...
          #if(ENABLE_AUDIO_AUTO_DETECT)
            if ( (msAPI_Timer_DiffTime(u32DetectAudioStart, msAPI_Timer_GetTime0()))
                  > ATVSCAN_AUDIO_WAIT_TIME2)
          #endif//(ENABLE_AUDIO_AUTO_DETECT)
            {
              #if(ENABLE_AUDIO_AUTO_DETECT)
        ...
        case AFT_DETECTAUDIO:
        {
            ...
            AUDIOSTANDARD_TYPE eAudioStd = msAPI_AUD_GetResultOfAutoStandardDetection();
            ...
            {
                _eCurrentTuningState = AFT_CHECK_CNI;
                printf(" Audio: [%u:%u] \n", eAudioStd, msAPI_AUD_GetAudioStandard());
            }
            break;
        }
        ...
        case AFT_CHECK_FALSESIGNAL:
            ...
            #if 1 // Setup next tuner freq and audio system
                if ((_bIsLSearch)&&(u8SoundSystem == E_AUDIOSTANDARD_L))
                {
        ...
        case AFT_MEMORIZEPRDATA:
            ...
            case E_AUTO_SEARCH_TYPE_ALLWAYDOWN:
                ...
                {
                    msAPI_ATV_SetAudioStandard(msAPI_ATV_GetCurrentProgramNumber(), E_AUDIOSTANDARD_M);
                }
                else
              #endif
                {
            ...
            #endif
                printf(" prog=%d,  Audio STD = %d \n",msAPI_ATV_GetCurrentProgramNumber(), msAPI_ATV_GetAudioStandard(msAPI_ATV_GetCurrentProgramNumber()));
                break;
            ...
            case E_AUTO_SEARCH_TYPE_ONETODOWN:
                ...
                printf("\r\n prog=%d,  Audio STD = %d",msAPI_ATV_GetCurrentProgramNumber(), msAPI_ATV_GetAudioStandard(msAPI_ATV_GetCurrentProgramNumber()));
                break;
...
#if(ENABLE_S2)
BOOL  msAPI_Get_CurrentTuningState(void) //ryan add
{
     if (_eCurrentTuningState==AFT_IDLE)
         return TRUE;
     else
