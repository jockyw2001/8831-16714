...
BOOLEAN MApp_ZUI_ACT_ExecutePvrAction(U16 act)
{
    switch(act)
    {
        ...
        case EN_EXE_INC_TIME_SHIFT_DISK_SIZE:
            ...
            {
            U8 maxSize_GB = MIN((msAPI_PVRFS_GetTotalDiskSizeSetToUsedMB()/1024), TIME_SHIFT_MAX_DISK_SIZE_GB);

            if(msAPI_PVRFS_GetTotalDiskSizeSetToUsedMB()%1024 != 0 && maxSize_GB < TIME_SHIFT_MAX_DISK_SIZE_GB)
            {
                maxSize_GB = maxSize_GB + 1;//support timeshift for all flash memory
            }
            if(g_u16TimeShiftDiskSize_GB == 0)
            {
               g_u16TimeShiftDiskSize_GB = maxSize_GB;
            }
            if(EN_EXE_INC_TIME_SHIFT_DISK_SIZE == act)
            {
                if(g_u16TimeShiftDiskSize_GB < maxSize_GB)
                {
                    g_u16TimeShiftDiskSize_GB++;
                }
                else if(g_u16TimeShiftDiskSize_GB == maxSize_GB)
                {
            ...
            {
                if(g_u16TimeShiftDiskSize_GB >1 && g_u16TimeShiftDiskSize_GB <= maxSize_GB)
                {
                    g_u16TimeShiftDiskSize_GB--;
                }
            ...
            else if(g_u16TimeShiftDiskSize_GB == 0)
            {
    #if (ENABLE_BULK_FILE_SYSTEM)
                MApp_PVR_TimeShiftSetRecordLengthMB(((msAPI_PVRFS_GetTotalDiskSizeSetToUsedMB()/64)*64));
    #endif
            }
            else
            {
    ...
    switch(hwnd)
    {
        ...
        case HWND_PVR_TIME_SHIFT_OPTION:
        {
            ...
            if(g_PVRCheckFSStatus >= EN_PVR_CHKSTS_CHECK_TMSHFT_SIZE)
            {
                ...
                {
#if 0 //fix pvr
                    snprintf((char*)CHAR_BUFFER2, 4, "All" );
                    FS_ASCII2Unicode((U8*)CHAR_BUFFER2);
                    return (LPTSTR) CHAR_BUFFER2;
#else
                    snprintf((char*)CHAR_BUFFER4, 4, "All" );
                    FS_ASCII2Unicode((U8*)CHAR_BUFFER4);
...
#if 0 //fix pvr
                    __MApp_UlongToString(TimeShiftSize_GB, CHAR_BUFFER2, digit_TimeShiftDiskSize);
                    CHAR_BUFFER2[digit_TimeShiftDiskSize] = CHAR_SPACE;
                    MApp_ZUI_API_Strcpy(CHAR_BUFFER2+digit_TimeShiftDiskSize+1, MApp_ZUI_API_GetString(en_str_GB));
                    return (LPTSTR) CHAR_BUFFER2;
#else
...
#if 0 //fix pvr
                    __MApp_UlongToString(TimeShiftSize_MB, CHAR_BUFFER2, digit_TimeShiftDiskSize);
                    CHAR_BUFFER2[digit_TimeShiftDiskSize] = CHAR_SPACE;
                    MApp_ZUI_API_Strcpy(CHAR_BUFFER2+digit_TimeShiftDiskSize+1, MApp_ZUI_API_GetString(en_str_MB));
                    return (LPTSTR) CHAR_BUFFER2;
#else
        ...
        case HWND_PVR_TIME_SHIFT_SIZE_SD_TIME_TXT:
            ...
            if(msAPI_BKFS_GetTotalDiskSizeSetToUsedMB()%1024 != 0 && maxSize_GB < TIME_SHIFT_MAX_DISK_SIZE_GB)
            {
                maxSize_GB = maxSize_GB + 1;//support timeshift for all flash memory
            }
            ...
            {
                timeShiftTime_Mins = maxSize_GB * TIME_SHIFT_SIZE_TO_TIME_RATIO_SD;
            }
            else
            {
        ...
        case HWND_PVR_TIME_SHIFT_SIZE_HD_TIME_TXT:
            ...
            {
                timeShiftTime_Mins = maxSize_GB * TIME_SHIFT_SIZE_TO_TIME_RATIO_HD;
            }
            else
            {
