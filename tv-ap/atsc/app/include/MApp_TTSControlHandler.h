...
#define TTS_MEM_SYNTHESIS_AUDIO_ADR      TTS_MEM_ADR //    (TTS_MEM_TEXT_ADR + TTS_MEM_TEXT_LEN)
#define TTS_MEM_SYNTHESIS_AUDIO_LEN      TTS_MEM_LEN //      (TTS_MEM_LEN - TTS_MEM_TEXT_LEN)

#define TTS_MUTE_BGM                    (1)

#define TTS_TEXT_BUF_LEN                (100) //bytes

#define STEREO_20MS_DATA_BYTES          (1280)

#define TTS_STRING_BUFFER_LENGTH        (1024)

#define TTS_AUDIO_PLAY_DELAYMS          (18)
#define TTS_ADUIO_PLAY_REMAINS_DELAYMS  (18)

#define TTS_ENGINE_ACK_TIME             (10000)

#define AUDIO_DSP_BUFFER_SIZE           (0x8000)

#define TTS_GEN_BLOCK_NUMBER            (8)

#define TTS_CHECK_OUTPUT_TIME           FALSE


#ifdef MAPP_TTSCONTROLHANDLER_H
#define INTERFACE
#else
...
typedef struct
{
    MB_TTS_ENGINE_CMD_IO    enTTSCmd;
    EN_TTS_TEXT_TYPE    enTextType;
    U8 u8Data1;
    U8 u8Data2;
} stTTSMBXData;
...
typedef enum
{
    STATE_TTS_INIT,
    STATE_TTS_TEXT_IN,
    STATE_TTS_TEXT_ID_IN,
    STATE_TTS_LONG_TEXT_IN,
    STATE_TTS_AUDIO_PLAY_INIT,
    STATE_TTS_AUDIO_PLAY,
    STATE_TTS_LONG_TEXT_AUDIO_PLAY,
    STATE_TTS_FINISH,
} EN_TTS_STATE;

INTERFACE void MApp_TTSControlHandler_Init(void);
INTERFACE void MApp_TTSControlHandler_Reset(void);
INTERFACE U8*  MApp_TTSControlGetInputText(void);
INTERFACE void MApp_TTSControlSay(U8* text, U16 text_len, U8* waveout, U32 waveout_len);
INTERFACE void MApp_TTSAudioPlayInit(void);
INTERFACE void MApp_TTSAudioPlay(void);
INTERFACE void MApp_TTSContorlStopAndFlush(void);
INTERFACE void MApp_TTSControlGetCapabilities(void);
INTERFACE EN_LANGUAGE MApp_TTSControlGetLanguage(void);
INTERFACE U16 MApp_TTSControlGetPitch(void);
INTERFACE void MApp_TTSControlGetRate(void);
INTERFACE void MApp_TTSControlSetInputText(U16* str, U16 len);
INTERFACE void MApp_TTSControlSetLanguage(EN_LANGUAGE lang);
INTERFACE void MApp_TTSControlSetPitch(U16 u16Pitch);
INTERFACE EN_RET MApp_TTSControlHandler_Main(void);
INTERFACE U8 MApp_TTSControlIsTextIn(void);
INTERFACE BOOLEAN MApp_TTSControlIsOn(void);
INTERFACE void MApp_TTSControlSetOn(BOOLEAN bOnOff);
INTERFACE void MApp_TTSControlSetTextAndSay(U16* str, U16 len);
INTERFACE void MApp_TTSControlSayLongText(void);
INTERFACE U8 MApp_TTSControlSplitLongText(U8* str, U16 len);
INTERFACE void MApp_TTSControlSetInputStringID(U16 u16ID);
INTERFACE void MApp_TTSControlSetInputHWndID(U16 u16ID);
