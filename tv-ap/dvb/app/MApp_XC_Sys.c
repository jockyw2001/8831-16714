...
//Ray ORI 2017.05.05: Image orientation init which is called when power up
//Input GET_IMG_ORIENTATION(): MIRROR_NORMAL,MIRROR_H_ONLY,MIRROR_V_ONLY,MIRROR_HV,
void dv_ImageOrientationInit(void)
{
  BYTE HFlip;
  BYTE VFlip;
  ...
  switch(GET_IMG_ORIENTATION()){
    ...
    {
	    MApi_XC_EnableMirrorMode(FALSE);
	    MDrv_MVOP_SetVOPMirrorMode(HFlip, E_VOPMIRROR_HORIZONTALL);
	    MDrv_MVOP_SetVOPMirrorMode(VFlip, E_VOPMIRROR_VERTICAL);
#if(ENABLE_VE)
    ...
    {
	    //MApi_XC_EnableMirrorMode(TRUE);
	     MApi_XC_EnableMirrorMode2(GET_IMG_ORIENTATION());
    }
    // GOP Mirror
    if(VFlip==1){
	MApi_GOP_GWIN_SetHMirror(ENABLE);
    }else{
	MApi_GOP_GWIN_SetHMirror(DISABLE);	//Text H Mirror only enables when V-flip is enabled
    }
    MApi_GOP_GWIN_SetVMirror(VFlip);

}


//Ray ORI 2017.05.05: Image Orientation setting
//Input GET_IMG_ORIENTATION(): MIRROR_NORMAL,MIRROR_H_ONLY,MIRROR_V_ONLY,MIRROR_HV,
void dv_SetImageOrientation(void)
{
...
BOOLEAN MApp_SetDesignImageMirror(void)
{
    ...
    if(stGenSetting.g_SysSetting.g_MirrorEnable)
	{
		...
		{
			//MApi_XC_EnableMirrorMode(TRUE);
			 MApi_XC_EnableMirrorMode2(MIRROR_H_ONLY);		//Ray ORI 2017.05.05: Test
		}
		// GOP Mirror
		MApi_GOP_GWIN_SetHMirror(TRUE);
		MApi_GOP_GWIN_SetVMirror(FALSE);		//Ray ORI 2017.05.05: Test
		//MApi_GOP_GWIN_SetVMirror(TRUE);
	}
    else
    {
