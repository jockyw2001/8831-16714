...
void _msAPI_VID_SetMemoryLayout(VDEC_InitParam *pInitParam)
{
    ...
    pInitParam->SysConfig.u32FrameBufAddr = VDEC_FRAMEBUFFER_ADR | MIU_INTERVAL;
    pInitParam->SysConfig.u32FrameBufSize = VDEC_FRAMEBUFFER_LEN;
    pInitParam->SysConfig.u32BitstreamBufAddr = VDEC_BITSTREAM_ADR | MIU_INTERVAL;
...
#ifdef VDEC_FRAMEBUFFER_DTV_SD_LEN
#if (VDEC_FRAMEBUFFER_DTV_SD_LEN != 0)
  ...
  #if 0//(defined SUPPORT_MPEG2_SD_ONLY || (defined AUSTRALIA && (MEMORY_MAP <= MMAP_64MB)))
    pInitParam->SysConfig.u32FrameBufAddr = VDEC_FRAMEBUFFER_SD_DTV_ADR;
    pInitParam->SysConfig.u32FrameBufSize = VDEC_FRAMEBUFFER_SD_DTV_LEN;
    pInitParam->SysConfig.u32BitstreamBufAddr = VDEC_BITSTREAM_SD_DTV_ADR;
    pInitParam->SysConfig.u32BitstreamBufSize = VDEC_BITSTREAM_SD_DTV_LEN;
  #endif
...
U16 msAPI_VID_Get_VdecBinId(VDEC_CodecType eCodecType)
{
    ...
    printf("msAPI_VID_Get_VdecBinId(eCodecType=%u)\n", eCodecType);
...
#else
    {
        ...
        #endif
          #if( CHIP_SUPPORT_HEVC && ((CHIP_FAMILY_TYPE==CHIP_FAMILY_NASA)||(CHIP_FAMILY_TYPE==CHIP_FAMILY_WHISKY)) )//IMPLEMENT_DTV_H265
            if(eCodecType == E_VDEC_CODEC_TYPE_HEVC)
            {
...
#if 1
/*
( 1||(CHIP_FAMILY_TYPE == CHIP_FAMILY_EULER) || \
...
//        SHOW_VIDEO_ERR(("MAPI_VID_Init not suuport this codec type(%d)",eCodecType))
    ...
    if  (msAPI_VID_GetCodecType() != E_VDEC_CODEC_TYPE_MPEG2)
    {
        SHOW_VIDEO_INFO(("MAPI_VID_Init not suuport this codec type(%d)\n",msAPI_VID_GetCodecType()));
        return FALSE;
    }
#endif
    ...
    if((CHIP_FAMILY_TYPE == CHIP_FAMILY_NASA) && (initParam.eCodecType  == E_VDEC_CODEC_TYPE_AVS))
    {
    ...
    if(MApi_XC_IsCurrentFrameBufferLessMode() && g_bApplyMVOPCrop)
    {
        ...
        (msAPI_VID_GetCodecType() == E_VDEC_CODEC_TYPE_VC1_ADV))
        {
            ...
            dc_param.u16StripSize = 720;
               ...
               ||(msAPI_VID_GetCodecType() == E_VDEC_CODEC_TYPE_AVS)
            #if( CHIP_SUPPORT_HEVC )
               ||(msAPI_VID_GetCodecType() == E_VDEC_CODEC_TYPE_HEVC)
            #endif
               )
...
{
    U32 u32FBSize_SD, u32WxH, u32MemUsage = 0;
    VDEC_Result     ret;
    ...
    ret = MApi_VDEC_GetControl(VDEC_USER_CMD_GET_FB_USAGE_MEM,&u32MemUsage);
    if (E_VDEC_OK != ret)
    {
        printf("WARNING: AVC_FB_USAGE_MEM, ret = %d\n",ret);
        return bNeedFBL;
    }
    DEBUG_DTV_FBL(printf(" VDEC_FB_USAGE_MEM = 0x%lx\n", u32MemUsage););


#ifdef VDEC_FRAMEBUFFER_SD_ADR
    u32FBSize_SD = VDEC_FRAMEBUFFER_SD_LEN;
#else
    u32FBSize_SD = VDEC_FRAMEBUFFER_LEN;
#endif
    DEBUG_DTV_FBL( printf("VDEC_u32FBSize=%u\n", u32FBSize_SD); );
    ...
    {
        DEBUG_DTV_FBL( printf("VDEC use mem > VDEC_FRAMEBUFFER_LEN ==> FBL\n"); );
        bNeedFBL = TRUE;
        return bNeedFBL;
    }
