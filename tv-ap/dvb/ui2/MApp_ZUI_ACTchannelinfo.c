...
LPTSTR _MApp_ZUI_GetInputSourceResolution(void);	//Ray OSD 2017.04.19
LPTSTR _MApp_ZUI_ACT_GetInputResolutionString(void);	//Ray SRC 2017.05.15
LPTSTR _MApp_ZUI_ACT_GetInputHorFreqString(void);	//Ray SRC 2017.05.15
LPTSTR _MApp_ZUI_ACT_GetInputVerFreqString(void);	//Ray SRC 20
...
//Ray SRC 2017.05.15:
//*************************************************************************
//Function name:        _MApp_ZUI_ACT_GetInputResolutionString
//Input parameter:    	None
//Return parameter:     String of input resolution
//Description:          Get input resolution in string format. e.g. 3840x2160
//*************************************************************************
LPTSTR _MApp_ZUI_ACT_GetInputResolutionString(void)
{
  U16 u16H, u16V;
  LPTSTR str = CHAR_BUFFER; 	//iu8Buffer_i = 0;
  SCALER_WIN eWindow = MAIN_WINDOW; 	//Fix to main window as there is no sub window in SP-4096
  ...
  else if ( IsHDMIInUse() )
  {
      ...
      *str++ = CHAR_x;
      __MApp_UlongToString( ( U32 )u16V, str, MApp_GetNoOfDigit( u16V ) );
      str += MApp_ZUI_API_Strlen(str);
      //Ray OSD 2017.05.15: if it's interlace signal, add "i" at the end
  ...
  else if (IsDTVInUse())
   {
       ...
       {
	   //Ray OSD 2017.05.15: Add horizontal resolution in DTV display
	  __MApp_UlongToString( ( U32 )u16H, str, MApp_GetNoOfDigit( u16H ) );
	  str += MApp_ZUI_API_Strlen(str);
	  *str++ = CHAR_x;
           __MApp_UlongToString( ( U32 )u16V, str, MApp_GetNoOfDigit( u16V ) );
...
//Ray SRC 2017.05.15:
//*************************************************************************
//Function name:        _MApp_ZUI_ACT_GetInputHorFreqString
//Input parameter:    	None
//Return parameter:     String of input horizontal frequency
//Description:          Get input horizontal frequency in string format. e.g. 135kHz
//*************************************************************************
LPTSTR _MApp_ZUI_ACT_GetInputHorFreqString(void)
{
  //from case CUR_INPUT_SOURCE_VIDEO_RES:
  U16 u16HFreq;			//Ray OSD 2017.04.19: Horizontal freq of input source
  SCALER_WIN eWindow = MAIN_WINDOW; 	//Fix to main window as there is no sub window in SP-4096
  LPTSTR str = CHAR_BUFFER; //iu8Buffer_i = 0;
  ...
  if ( IsVgaInUse() )
  {
	  ...
	  u16HFreq = (MApi_XC_Getu16HorizontalFrequency(MApp_PCMode_Get_Mode_Idx(eWindow))+5)/10; 	//Ray OSD 2017.04.19: Get horizontal freq (+5 is for round off before /10)

	  ////Ray OSD 2017.04.19: Add H:xxkHz to display horizontal freq
	  __MApp_UlongToString( ( U32 )u16HFreq, str, MApp_GetNoOfDigit( u16HFreq ) );
	  str += MApp_ZUI_API_Strlen(str); //iu8Buffer_i = __strlen( p_string_buffer );
	  *str++ = CHAR_k;
	  *str++ = CHAR_H;
	  *str++ = CHAR_z;
  ...
  else if (IsDTVInUse())
  {
      ...
      if(MApp_IsSrcHasSignal(eWindow) && bRet )//if(u16V > 0)
      {
	  u16HFreq = (MApi_XC_PCMonitor_Get_HFreqx10(eWindow)+5)/10;		//(+5 is for round off before /10)
	  ////Ray OSD 2017.04.19: Add H:xxkHz to display horizontal freq
	  __MApp_UlongToString( ( U32 )u16HFreq, str, MApp_GetNoOfDigit( u16HFreq ) );
	  str += MApp_ZUI_API_Strlen(str); //iu8Buffer_i = __strlen( p_string_buffer );
	  *str++ = CHAR_k;
...
//Ray SRC 2017.05.15:
//*************************************************************************
//Function name:        _MApp_ZUI_ACT_GetInputVerFreqString
//Input parameter:    	None
//Return parameter:     String of input vertical frequency
//Description:          Get input vertical frequency in string format. e.g. 60Hz
//*************************************************************************
LPTSTR _MApp_ZUI_ACT_GetInputVerFreqString(void)
{
  //from case CUR_INPUT_SOURCE_VIDEO_RES:
  U16 u16F;
  SCALER_WIN eWindow = MAIN_WINDOW; 	//Fix to main window as there is no sub window in SP-4096
...
static LPTSTR _MApp_ZUI_ACT_GetInputSourceResolution(SCALER_WIN eWindow)
{
    ...
    else if ( IsVgaInUse() )
    {
        ...
        else
        {
            ...
            *str++ = CHAR_SPACE;
            ////
            ////Ray OSD 2017.04.19: Add V:xxHz to display vertical freq
            *str++ = CHAR_V;
            *str++ = CHAR_COLON;
            ////
            __MApp_UlongToString( ( U32 )u16F, str, MApp_GetNoOfDigit( u16F ) );
    ...
    {
	//Ray OSD 2017.06.02: Change to display resolution in form of 3840x2160
	  if (MApp_IsSrcHasSignal(eWindow) == FALSE)
	      return NULL;
	...
	//Ray OSD 2016.06.02: Comment the original code of displaying resolution, H and V frequencies
	/*
        if (MApp_IsSrcHasSignal(eWindow) == FALSE)
            return NULL;
        ...
        *str++ = CHAR_H;
        *str++ = CHAR_COLON;
        u16HFreq = (MApi_XC_PCMonitor_Get_HFreqx10(eWindow)+5)/10;		//Get H Freq, (+5 is for round off before /10)
        __MApp_UlongToString( ( U32 )u16HFreq, str, MApp_GetNoOfDigit( u16HFreq ) );
        str += MApp_ZUI_API_Strlen(str); //iu8Buffer_i = __strlen( p_string_buffer );
        *str++ = CHAR_k;
        *str++ = CHAR_H;
...
//Ray OSD 2017.04.19: Enable other c program to get the input source resolution string
LPTSTR _MApp_ZUI_GetInputSourceResolution()
{
...
static U16 _MApp_ZUI_ACT_GetEventInfoGenreStringID(void)
{
    ...
    switch ( astEventInfo[bNextEvent].u8Content_nibble_level1 )
    {
        case 0x1:
...
LPTSTR _GetSatelliteNameTxt(void)
{
    ...
    TP_temp = _astS2ProgramIDTable[CurrentS2idx];
//  printf("\r\nCurrentS2idx=%d [%d]",CurrentS2idx,TP_temp.SatelliteId);
    _GetSatInforByID(TP_temp.SatelliteId,&sat);

    if(TRUE == _GetSatInforByID(TP_temp.SatelliteId,&sat) && CurrentS2idx < MAX_DTV_S2_MUX_NUMBER)
    {
            if(TP_temp.SatelliteId == 1)
            {
...
LPTSTR MApp_ZUI_ACT_GetChannelInfoDynamicText(HWND hwnd)
{
    ...
    */
                ...
                if (enFrotEndLockStatus != FRONTEND_UNKNOWN)
                {
                    ...
                    if (MApp_GetSignalStatus() == SIGNAL_LOCK)
                    {
                        ...
                        //if (bdrawAudioflag)
                        {
                            ...
                            else if (E_AUDIOSTREAM_INVALID != stAudioStreamInfo.wAudType)
                            {
        ...
        case HWND_CHINFO_POP_RIGHT_CH_SOUND_SYS_TEXT:
                ...
                )//Has signal
            {
                ...
                switch(eAudioStandard)
                {
                    ...
                    #if ( ENABLE_ATV_CHINA_APP|ENABLE_DTMB_CHINA_APP || ENABLE_DVB_TAIWAN_APP || ENABLE_SBTVD_BRAZIL_APP || ENABLE_DVBC_PLUS_DTMB_CHINA_APP || (TV_SYSTEM == TV_NTSC) )
        ...
        //#endif
            ...
            if ( IsDTVInUse() && _MApp_ZUI_ACT_IsCurrentSourceAvailable() )
            {
                u16TempID = _MApp_ZUI_ACT_GetLanguageStringID(
                    (EN_LANGUAGE)stGenSetting.g_SoundSetting.enSoundAudioLan1, FALSE);
            }
         #endif
        break;
        ...
        case HWND_CHINFO_POP_LEFT_TV_VIDEO_RES_TEXT:
          ...
          #endif
            {
        	//printf("\nRay: resolution 1, hwnd:%d",hwnd);		//Ray OSD 2017.04.19: HWND_CHINFO_PIP_SRC1_VIDEO_RES_TEXT is the hwnd to display resolution when press INFO key
