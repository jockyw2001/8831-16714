...
int oad_prepare(void)
{
    ...
    MApp_OAD_GetInfo_BL( &wOad_PID, &(stTempTP.u32Frequency), (U8 *)&(stTempTP.enBandWidth),&u8CountryID);
...
int oad_check()
{
    ...
    while(1)
    {
        ...
        {
            OAD_DEBUG( printf("Error> Exit oad_check\n") );
...
void main_loop(void)
{
    ...
    MApp_BL_DisplaySystem(0);
    {
        ...
        MApp_OAD_GetInfo_BL( &wOad_PID_Orig, &(stOrigTP.u32Frequency), (U8 *)&(stOrigTP.enBandWidth),&u8Country);
    ...
    while(1)
    {
        ...
        {
            MS_TP_SETTING stTempTP;
            MApp_OAD_GetInfo_BL( &wOad_PID, &(stTempTP.u32Frequency), (U8 *)&(stTempTP.enBandWidth), &u8Country);
            DBG_PRINT( printf("OAD upgrade start, PID : 0x%x !!\n", wOad_PID) );
            DBG_PRINT( printf("OAD upgrade start, freq: %lu !!\n", stTempTP.u32Frequency) );
