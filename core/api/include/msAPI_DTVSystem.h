...
//=================================================================
    ...
    #if( ENABLE_ISDBT && ENABLE_SBTVD_BRAZIL_APP )// ISDBT
...
typedef struct __attribute__ ((__packed__))
{
    ...
    BYTE wAudType ;            ///<  0x01: MPEG, 0x02: AC-3, 0x03: MPEG4_AUD
    WORD wAudPID  ;           ///< Audio PID
...
} DTVPROGRAMID_M; // For channel manager&DB
...
#if 1 // Refine for Reduce struct size
typedef struct
{
    U16 wService_ID;
    //-------------------------------------------------
...
#define IS_LCN_RANK_COUNTRY(c)            ( (c==E_MALAYSIA)      ||  \
                                          (c==E_INDONESIA)      ||  \
                                          (c==E_UNITED_ARAB_EMIRATES))
...
#if ENABLE_S2_CHANNEL_FAV_NEWSTYLE
INTERFACE BOOLEAN msAPI_CM_FavorProgram(MEMBER_SERVICETYPE bServiceType, WORD wPosition, BOOLEAN bFavorite, E_FAVORITE_TYPE eFavorType);
...
#if(ASTRA_HD_ENABLE)
INTERFACE BOOLEAN msAPI_CM_SaveServiceListInfo(const U8 *pu8Dest, U16 u16ServiceListID, BOOLEAN bListInFlag);
