...
#define EXT_GPIO_C		//Ray IOE 2017.02.27
#include "EXT_GPIO.h"		//Ray IOE 2017.02.27
...
#if (I2C_IO_EXPANDER_TYPE == I2C_IO_EXPANDER_PCF8575) //Ray IOE 2017.02.02: add init state I2C_IO_EXPANDER_PCF8575
    #define I2C_IO_EXPANDER_P0_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P1_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P2_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P3_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P4_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P5_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P6_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P7_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P8_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P9_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P10_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P11_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P12_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P13_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P14_IS_GPIO GPIO_OUT_HIGH
    #define I2C_IO_EXPANDER_P15_IS_GPIO GPIO_OUT_HIGH

    #define I2C_IO_EXPANDER_ADDR 0x4f
#endif
...
#if (I2C_IO_EXPANDER_TYPE == I2C_IO_EXPANDER_PCF8575) 		//Ray IOE 2017.02.02: add for I2C_IO_EXPANDER_PCF8575
...
#else					//Ray IOE 2017.02.02: else of (I2C_IO_EXPANDER_TYPE == I2C_IO_EXPANDER_PCF8575)
...
#endif			//Ray IOE 2017.02.02: end of (I2C_IO_EXPANDER_TYPE == I2C_IO_EXPANDER_PCF8575)
...
#elif (I2C_IO_EXPANDER_TYPE == I2C_IO_EXPANDER_PCF8575) 		//Ray IOE 2017.02.02: add for I2C_IO_EXPANDER_PCF8575
//-----------------------------------------------------------------
// PCF8575
// Remote 8-bit I/O expander for I2C-bus
//-----------------------------------------------------------------

static U8 PCF8574_u8OUT0 = I2C_IO_OUT0;
static U8 PCF8574_u8OUT1 = I2C_IO_OUT1;

void MDrv_ExGPIO_Init(void)  //dc:
{
//    U8 u8Loop;

//    for (u8Loop = 3; u8Loop > 0; u8Loop--)
//        if (MDrv_IIC_WriteBytes(I2C_IO_EXPANDER_ADDR, 0, 0, 2, &PCF8574_u8OUT) != FALSE)
//            break;
//    ASSERT(u8Loop);

    U8 u8Loop;
    U8 u8Data[2];
    u8Data[0] = (PCF8574_u8OUT0) & 0xFF;
    u8Data[1] = (PCF8574_u8OUT1) & 0xFF;
    for (u8Loop = 3; u8Loop > 0; u8Loop--)
		if (MDrv_IIC_WriteBytes(I2C_IO_EXPANDER_ADDR, 0, 0, 2, u8Data)!= FALSE)
            break;
    ASSERT(u8Loop);
...
void MDrv_ExGPIO_Set(U16 u16BitMask, BOOLEAN bHigh)
{
    U8 u8Loop;
    U8 u8Data[2];
	U16 PCF8574_u16OUT;
	...
	PCF8574_u16OUT = ((((U16)u8Data[0])<<8)|u8Data[1]);

    if (bHigh)
        PCF8574_u16OUT |= u16BitMask;
    else
        PCF8574_u16OUT &= ~u16BitMask;
...
//Ray IOE 2017.02.27:
//*********************************************************
// Routine: MDrv_ExGPIO_Get
// Usage: Read one pin of external I/O expander
// Input: u16BitMask: The pin position to be read. For example: JC3_11_12
// Output: 1 = pin is set high; 0 = pin is set low
//**********************************************************
BYTE MDrv_ExGPIO_Get(U16 u16BitMask)
{
//    U8 u8Loop;
//    U8 u8Input;
...
//    return (WORD)(u8Input & u8BitMask);
    BYTE returnData = 0;
    U8 u8Data[2]= {0};
    MDrv_IIC_ReadBytes(I2C_IO_EXPANDER_ADDR, 0, 0, 2, u8Data);
    //printf("\nRay: Read IO expander high:%x,low:%x",u8Data[1],u8Data[0]);	//Ray DBG 2017.03.30
    //Ray IOE 2017.02.27: Return 1 if bit is set
    //Ray IOE 2017.03.30: First read byte u8Data[0] is P07~P00 states, 2nd read byte u8Data[1] is P17~P10 states
    if((((((U16)u8Data[1])<<8)|u8Data[0]) & u16BitMask)>0)
    {
	returnData = 1;
    }
