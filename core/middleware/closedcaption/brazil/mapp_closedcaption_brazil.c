...
void MApp_ClosedCaption_Init(void)
{
   ...
   // MApi_GOP_GWIN_SwitchGOP(CC_USING_GOP_ID);    // Following up the OSD
   // msAPI_CC_SetGwinID(CC_USING_GOP_ID, CC_USING_GWIN_ID);
    ...
    #if (BRAZIL_CC&&(GOP_GWIN_RB2_MEMORY_TYPE&MIU1))
    MApi_GOP_MIUSel(CC_USING_GOP_ID, E_GOP_SEL_MIU1);
    MApi_GOP_GWIN_SetDoubleHeap(GOP_GWIN_RB2_ADR | MIU_INTERVAL, GOP_GWIN_RB2_LEN);
    #endif
    #if (BRAZIL_CC&&GOP_GWIN_RB2_ADR)
    MApi_GOP_MIUSel(CC_USING_GOP_ID, E_GOP_SEL_MIU1);
    MApi_GOP_GWIN_SetDoubleHeap(GOP_GWIN_RB2_ADR | MIU_INTERVAL, GOP_GWIN_RB2_LEN);
    #endif
    ...
    if(IsDTVInUse())
    {
        //msAPI_CC_SetCCFont(&Font_CC[0]);//Font_CC[0], brazil cc font table
        ...
        MApp_CC_CtrlParser(CC_SELECTOR_ENABLESVS, TRUE);
        MApp_CC_CtrlParser(CC_SELECTOR_ENABLESHS, FALSE);
        CC_BrazilDecoderFuns.EnableDumpRawData(FALSE);

       msAPI_CC_Brazil_SetMisInfo(ENABLE);
       //msAPI_CC_Brazil_SetPalette();
       //msAPI_CC_Brazil_SetPalette();//bruce.gao@0628,workaround mode ,avoid charactor transparrent.
       //msAPI_CC_Brazil_Init();
    ...
    {
        //msAPI_CC_SetCCFont(&Font_CC[1]);//Font_CC[1], atv cc font table
        ...
        for (i = 0; i < 4; i++)
        {
        printf("[%s][%d]MApi_GOP_GWIN_DeleteFB,id=%d\n",__FUNCTION__,__LINE__,i);
            MApi_GOP_GWIN_DeleteFB(i);
        }
        #endif
