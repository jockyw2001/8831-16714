...
#include "MApp_XC_PQ.h"	//MingYuan Add
...
#endif
//#if (IR_TYPE_SEL==IR_TYPE_TRIVIEW_STD)  //MingYuan USB
...
#if 1//(AUDIO_MODE_USE_EQUALIZER==ENABLE)	//MingYuan EQ
...
void MApp_ZUI_ACT_ExecuteUSBHotKey(void) //¥¼§¹
{ 
  #if ENABLE_DMP
  if((UI_INPUT_SOURCE_TYPE!=UI_INPUT_SOURCE_DMP))
  {
    ...
    MApp_ZUI_CTL_DynamicListSetItemFocus(HWND_MENU_APP_PAGE_LIST,HWND_MENU_APP_DMP);	  
	  MApp_ZUI_ACT_ExecuteWndAction(EN_EXE_EXECUTE_APP);	
...
U16 MApp_UiMenuFunc_DecIncATSCLanguage_Cycle(BOOLEAN action, U16 u16Value)
{
    ...
    if(action == TRUE)
    {
        if(u16Value == LANGUAGE_ENGLISH)
            ...
            u16Value = LANGUAGE_GERMAN;
        else if(u16Value == LANGUAGE_GERMAN)
            u16Value = LANGUAGE_ITALIAN;    
        else if(u16Value == LANGUAGE_ITALIAN)
            u16Value = LANGUAGE_PORTUGUESE;
        else if(u16Value == LANGUAGE_PORTUGUESE)
            u16Value = LANGUAGE_RUSSIAN;
        else if(u16Value == LANGUAGE_RUSSIAN)
            u16Value = LANGUAGE_CHINESE;
        else if(u16Value == LANGUAGE_CHINESE)
    ...
    else if(action == FALSE)
    {
        if(u16Value == LANGUAGE_CHINESE)
            ...
            u16Value = LANGUAGE_ENGLISH;
        else if(u16Value == LANGUAGE_ENGLISH)
            u16Value = LANGUAGE_CHINESE; 
        else
            u16Value = LANGUAGE_ENGLISH;
    }	
...
U16 MApp_UiMenuFunc_GetPictureBacklightValue(void)
{
        ...
        return stGenSetting.u8Backlight;        
			#else	 	
...
{						
	#ifdef sAddBacklightPara									
  stGenSetting.u8Backlight = MApp_UiMenuFunc_DecIncValue(bEnable, stGenSetting.u8Backlight, 0, 100, 1);
  Panel_Backlight_PWM_ADJ(msAPI_Mode_PictureBackLightN100toReallyValue(stGenSetting.u8Backlight));            
  MApp_WriteDatabase(RM_GEN_BACKLIGHT_ADDRESS, (U8*)&(stGenSetting.u8Backlight), 1);    
	#else		
  ST_PICTURE.u8Backlight = MApp_UiMenuFunc_DecIncValue(bEnable, ST_PICTURE.u8Backlight, 0, 100, 1);
...
void Panel_Backlight_PWM_WithSmooth(void)
{
	U16 u16BklValue;
	u16BklValue = u16TargetBlPwm;
	if(u16TargetBlPwm!=u16CurrentBlPwm)
	{
		if(u16CurrentBlPwm<u16BklValue)
			u16CurrentBlPwm++;
		else if(u16CurrentBlPwm>u16BklValue)
			u16CurrentBlPwm--;
   	Panel_Backlight_PWM_ADJ(msAPI_Mode_PictureBackLightN100toReallyValue(u16CurrentBlPwm)); 
	}	
	else
	{
		Panel_Backlight_PWM_ADJ(msAPI_Mode_PictureBackLightN100toReallyValue(u16BklValue)); 
	}	
}
//============================================================================
void Panel_Backlight_PWM_WithLS(U16 u16BklValue)
{
	U16 sAdcStep;
	//U16 sPwmLightV;
	if(bLightSensorFlag)
	{
		...
		{				
			sAdcStep = (((u16CurrentLightSensorValue - G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Min)*100)/(G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Max - G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Min));
			if(sAdcStep>100)	
				sAdcStep = 100;
			u16TargetBlPwm = sAdcStep;
		}
		else
		{			
			u16TargetBlPwm = u16BklValue;
		}
	}
	else
	{	
...
BOOLEAN MApp_UiMenuFunc_AdjAudioMode(BOOLEAN action)
{
    ...
    else
    {
        ...
        if(!MApp_Aud_GetUserMuteStatus())
            MApi_AUDIO_SetMute( AUD_DSP_NA, AUD_DSP_NA, AUD_DSP_NA, AUD_DSP_NA, AUD_AMP_UNMUTE );
        msAPI_Timer_Delayms( 100 ); // to avoid mts key POP noise
        MApi_AUDIO_SetMute( AUD_DSP_UNMUTE, AUD_DSP_UNMUTE, AUD_DSP_NA, AUD_DSP_UNMUTE, AUD_AMP_NA );
        #endif
    }
...
{
    stGenSetting.g_SysSetting.fTimeAutoSync=MApp_UiMenuFunc_DecIncValue_Cycle(action, stGenSetting.g_SysSetting.fTimeAutoSync, TIME_AUTO_SYNC_OFF, TIME_AUTO_SYNC_ITEM_NUM-1, 1);

    if ((stGenSetting.g_SysSetting.fTimeAutoSync==TIME_AUTO_SYNC_ON) && g_u32StreamTime)
...
BOOLEAN MApp_UiMenuFunc_AdjustAudioLanguage( BOOLEAN action)
{
    ...
    (EN_DAP_AUDIO_LANGUAGE)MApp_UiMenuFunc_DecIncValue_Cycle( action, stGenSetting.g_SoundSetting.enDAPAudioLanguage, DAP_SOUND_LANGUAGE_ENGLISH, DAP_SOUND_LANGUAGE_FRENCH, 1 );
...
void MApp_UiMenuFunc_SaveCurrentRegion5Ratings(void)
{
    ...
    #else
        if(strcmp((const char*)stGenSettingVchipRegion5.u8Region5Name, (const char*)stEZ_VchipRegion5.u8Region5Name)==0)
        {
    ...
    memcpy(stEZ_VchipRegion5.u8Region5Name, stGenSettingVchipRegion5.u8Region5Name, sizeof(stEZ_VchipRegion5.u8Region5Name));
    ...
    for(u8Loop2=0; u8Loop2< stGenSettingVchipRegion5.u8NoDimension; u8Loop2++)
    {
...
void MApp_ReturnResolution(U8 *strBuffer)
{
...
#endif
    {
        ...
        if(!(msAPI_AVD_Get_LatchStatus() & VD_HSYNC_LOCKED))
        {
           ...
           printf("mvideo_vd_get_videostatus() is %x \n",msAPI_AVD_Get_LatchStatus());
