...
static E_SCREEN_MUTE_STATUS s_eScreenMuteStatus[2] = {E_SCREEN_MUTE_INIT,E_SCREEN_MUTE_INIT};
...
static E_MUX_INPUTPORT eHdmiPortMappingResult[INPUT_HDMI_VIDEO_COUNT];

#if (ENABLE_HDMI_4K_2K||ENABLE_EDID_SWITCH) // 4k X 2k
U8 g_XC_Sys_u8EdidSel = EDID_2p0;
#else
U8 g_XC_Sys_u8EdidSel = EDID_1p4;
#endif
...
/*static*/ void _MApi_XC_Sys_Init_Panel(void)
{
    ...
    dv_MultiPanelsInit();			//Ray PNL 2017.03.13: Init multiple panel parameters
    ...
    else if(devPanel_IsVx1(g_PNL_TypeSel))
    {
        ...
        //printf("MingYuan Print _MApi_XC_Sys_Init_OD Vby1 \n");
    }
  #endif
    else // LVDS output
    {
...
void _MApi_XC_Sys_Init_PQ(XC_INITDATA *pstXC_InitData)
{
    ...
    {
        MApi_BDMA_Copy(BinInfo.B_FAddr, (U32)((PQ_MAIN_MEMORY_TYPE & MIU1) ? (PQ_MAIN_ADR | MIU_INTERVAL) : (PQ_MAIN_ADR)), BinInfo.B_Len, MIU_FLASH2SDRAM);
        sXC_PQ_InitData.u8PQBinCnt = 1;
        sXC_PQ_InitData.stPQBinInfo[0].u8PQID = (MS_U8) PQ_BIN_STD_MAIN;
        sXC_PQ_InitData.stPQBinInfo[0].PQBin_PhyAddr = (MS_PHYADDR)((PQ_MAIN_MEMORY_TYPE & MIU1) ? (PQ_MAIN_ADR | MIU_INTERVAL) : (PQ_MAIN_ADR));
        ...
        {
            MApi_BDMA_Copy(BinInfo.B_FAddr, (U32)((PQ_TEXT_MAIN_MEMORY_TYPE & MIU1) ? (PQ_TEXT_MAIN_ADR | MIU_INTERVAL) : (PQ_TEXT_MAIN_ADR)), BinInfo.B_Len, MIU_FLASH2SDRAM);
            sXC_PQ_InitData.u8PQTextBinCnt = 1;
            sXC_PQ_InitData.stPQTextBinInfo[0].u8PQID = (MS_U8)PQ_TEXT_BIN_STD_MAIN;
            sXC_PQ_InitData.stPQTextBinInfo[0].PQBin_PhyAddr = (MS_PHYADDR)((PQ_TEXT_MAIN_MEMORY_TYPE & MIU1) ? (PQ_TEXT_MAIN_ADR | MIU_INTERVAL) : (PQ_TEXT_MAIN_ADR));
        }
    }
    ...
    {
        MApi_BDMA_Copy(BinInfo.B_FAddr, (U32)((PQ_MAIN_EX_MEMORY_TYPE & MIU1) ? (PQ_MAIN_ADR | MIU_INTERVAL) : (PQ_MAIN_EX_ADR)), BinInfo.B_Len, MIU_FLASH2SDRAM);
        sXC_PQ_InitData.u8PQBinCnt = 2;
        sXC_PQ_InitData.stPQBinInfo[1].u8PQID = (MS_U8) PQ_BIN_EXT_MAIN;
        sXC_PQ_InitData.stPQBinInfo[1].PQBin_PhyAddr = (MS_PHYADDR)((PQ_MAIN_EX_MEMORY_TYPE & MIU1) ? (PQ_MAIN_EX_ADR | MIU_INTERVAL) : (PQ_MAIN_EX_ADR));
    }
...
void _MApi_XC_HDMI_PortMapping_Init(void)
{
    ...
    for(u8DummyIdx=0; u8DummyIdx<INPUT_HDMI_VIDEO_COUNT; u8DummyIdx++)
    {
        ...
        _MApi_XC_Sys_InputSource_InputPort_Mapping((INPUT_SOURCE_TYPE_t)(INPUT_SOURCE_HDMI+u8DummyIdx), &eInputPort, &u8NoUse);
...
#if( ENABLE_CEC ) // If CEC on, must tuen on HPD early

  #if (ENABLE_HDMI_HPD_TASK )
      MApi_XC_HPD_Task_Start(TRUE);   //Enable state machine flow
      MApi_XC_HPD_Task();     //For state machine go one step forward to set HDMI_HPD high directly

  #else //if ( !ENABLE_HDMI_HPD_TASK )
...
#if (ENABLE_DDC_RAM)
static U8 EDID_VGA[128]= // need modity VGA EDID content!
...
{  /*    00   01   02   03   04   05   06   07   08   09   0A   0B   0C   0D   0E   0F
       -------------------------------------------------------------------------------*/          
,
...
#define EDID_PA_POS_2P0     0xA1    //0xA1, 0xA2 is physical address


#if (ENABLE_DOLBY_BULLETIN24)
static U8 EDID_DDP_2p0_DVI_0[256]=
{  /*    00   01   02   03   04   05   06   07   08   09   0A   0B   0C   0D   0E   0F
       -------------------------------------------------------------------------------*/     
0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x36,0x74,0x30,0x00,0x01,0x00,0x00,0x00,
0x0A,0x16,0x01,0x03,0x80,0x73,0x41,0x78,0x0A,0xCF,0x74,0xA3,0x57,0x4C,0xB0,0x23,
...
0x53,0x74,0x61,0x72,0x20,0x44,0x65,0x6D,0x6F,0x0A,0x20,0x20,0x00,0x00,0x00,0xFD,
0x00,0x32,0x4B,0x1E,0x50,0x17,0x00,0x0A,0x20,0x20,0x20,0x20,0x20,0x20,0x01,0xF1,
0x02,0x03,0x31,0xF2,0x4D,0x01,0x03,0x04,0x05,0x07,0x90,0x12,0x13,0x14,0x16,0x9F,
0x20,0x22,0x29,0x09,0x07,0x07,0x11,0x17,0x50,0x55,0x17,0x00,0x83,0x01,0x00,0x00,
0x70,0x03,0x0C,0x00,0x10,0x00,0xB8,0x44,0x20,0xA0,0x82,0x01,0x02,0x03,0x04,0x01,
0x41,0x8C,0x0A,0xD0,0x8A,0x20,0xE0,0x2D,0x10,0x10,0x3E,0x96,0x00,0xC4,0x8E,0x21,
...
0x2C,0x25,0x80,0xC4,0x8E,0x21,0x00,0x00,0x9E,0x00,0x00,0x00,0x00,0x00,0x00,0x23,
};

#define EDID_DDP_PA_POS     0xA4
#define EDID_DDP_SAD_POS    0x99
...
0x53,0x74,0x61,0x72,0x20,0x44,0x65,0x6D,0x6F,0x0A,0x20,0x20,0x00,0x00,0x00,0xFD,
0x00,0x32,0x4B,0x1E,0x50,0x17,0x00,0x0A,0x20,0x20,0x20,0x20,0x20,0x20,0x01,0xF1,
0x02,0x03,0x2C,0xF2,0x4D,0x01,0x03,0x04,0x05,0x07,0x90,0x12,0x13,0x14,0x16,0x9F,
0x20,0x22,0x26,0x09,0x07,0x07,0x11,0x17,0x50,0x83,0x01,0x00,0x00,0x6E,0x03,0x0C,
0x00,0x10,0x00,0xB8,0x2D,0x20,0xC0,0x04,0x01,0x41,0x00,0x00,0x8C,0x0A,0xD0,0x8A,
...
0x21,0x00,0x00,0x9E,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x18,
};
...
/*80*/ 0x02,0x03,0x2F,0xF2,0x4D,0x01,0x03,0x04,0x05,0x07,0x90,0x12,0x13,0x14,0x16,0x9F,
/*90*/ 0x20,0x22,0x29,0x09,0x07,0x07,0x11,0x17,0x50,0x55,0x17,0x00,0x83,0x01,0x00,0x00,
/*A0*/ 0x6E,0x03,0x0C,0x00,0x10,0x00,0xB8,0x2D,0x20,0xC0,0x04,0x01,0x41,0x00,0x00,0x8C,
/*B0*/ 0x0A,0xD0,0x8A,0x20,0xE0,0x2D,0x10,0x10,0x3E,0x96,0x00,0xC4,0x8E,0x21,0x00,0x00,
/*C0*/ 0x18,0x8C,0x0A,0xD0,0x90,0x20,0x40,0x31,0x20,0x0C,0x40,0x55,0x00,0xC4,0x8E,0x21,
/*D0*/ 0x00,0x00,0x18,0x01,0x1D,0x00,0xBC,0x52,0xD0,0x1E,0x20,0xB8,0x28,0x55,0x40,0xC4,
/*E0*/ 0x8E,0x21,0x00,0x00,0x1E,0x01,0x1D,0x80,0xD0,0x72,0x1C,0x16,0x20,0x10,0x2C,0x25,
/*F0*/ 0x80,0xC4,0x8E,0x21,0x00,0x00,0x9E,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xA6,
};
...
/*60*/ 0x53,0x74,0x61,0x72,0x20,0x44,0x65,0x6D,0x6F,0x0A,0x20,0x20,0x00,0x00,0x00,0xFD,
/*70*/ 0x00,0x32,0x4B,0x1E,0x50,0x17,0x00,0x0A,0x20,0x20,0x20,0x20,0x20,0x20,0x01,0xF1,
/*80*/ 0x02,0x03,0x28,0xF2,0x4D,0x01,0x03,0x04,0x05,0x07,0x90,0x12,0x13,0x14,0x16,0x9F,
/*90*/ 0x20,0x22,0x29,0x09,0x07,0x07,0x11,0x17,0x50,0x55,0x17,0x00,0x83,0x01,0x00,0x00,
/*A0*/ 0x67,0x03,0x0C,0x00,0x10,0x00,0xB8,0x2D,0x8C,0x0A,0xD0,0x8A,0x20,0xE0,0x2D,0x10,
/*B0*/ 0x10,0x3E,0x96,0x00,0xC4,0x8E,0x21,0x00,0x00,0x18,0x8C,0x0A,0xD0,0x90,0x20,0x40,
/*C0*/ 0x31,0x20,0x0C,0x40,0x55,0x00,0xC4,0x8E,0x21,0x00,0x00,0x18,0x01,0x1D,0x00,0xBC,
/*D0*/ 0x52,0xD0,0x1E,0x20,0xB8,0x28,0x55,0x40,0xC4,0x8E,0x21,0x00,0x00,0x1E,0x01,0x1D,
/*E0*/ 0x80,0xD0,0x72,0x1C,0x16,0x20,0x10,0x2C,0x25,0x80,0xC4,0x8E,0x21,0x00,0x00,0x9E,
/*F0*/ 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0xDA,
};

#define EDID_DDP_PA_POS     0xA4
#define EDID_DDP_SAD_POS    0x99
#endif // #if (ENABLE_DOLBY_BULLETIN24)

#endif // #if (ENABLE_3D_PROCESS)


#elif (ENABLE_3D_PROCESS) // 3D Enable
static U8 EDID_DVI_0[256]=
{  /*    00   01   02   03   04   05   06   07   08   09   0A   0B   0C   0D   0E   0F
       -------------------------------------------------------------------------------*/
/*00*/ 0x00,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x00,0x36,0x74,0x30,0x00,0x01,0x00,0x00,0x00,
...
#endif // #if (INPUT_HDMI_VIDEO_COUNT >= 1)
...
void _MApi_XC_DDCRam_Init(void)//sample code for DDC RAM, need remove later
{
    XC_DDCRAM_PROG_INFO stDDCRamInfo;
#if (INPUT_HDMI_VIDEO_COUNT >= 2)
    U8 EDID_DVI_1[256] = {0};
    ...
    if( g_XC_Sys_u8EdidSel == EDID_2p0 )
    {
        printf("Load EDID 2\n");
    }
    else
...
#if (INPUT_HDMI_VIDEO_COUNT >= 1)
   #if ENABLE_EDID_SWITCH
    //if(stGenSetting.g_SysSetting.u8EDID_select == EDID_2p0)
    if( g_XC_Sys_u8EdidSel == EDID_2p0 )
    {
        _MApi_XC_DDCRam_Init_Hdmi(EDID_2p0_DVI_0, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI1_MUX), 0x10, EDID_PA_POS_2P0);
    }
    else
   #endif
    {
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_0, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI1_MUX), 0x10, EDID_PA_POS);
    }
#endif

#if (INPUT_HDMI_VIDEO_COUNT >= 2)
    //copy EDID from DVI0 and change physical address/checksum
 #if ENABLE_EDID_SWITCH
    //if (stGenSetting.g_SysSetting.u8EDID_select == EDID_2p0)
    if( g_XC_Sys_u8EdidSel == EDID_2p0 )
    {
        memcpy((void*)EDID_DVI_1, (void*)EDID_2p0_DVI_0, 256);
        EDID_DVI_1[EDID_PA_POS_2P0] = 0x20;
        EDID_DVI_1[0xFF] -= (EDID_DVI_1[EDID_PA_POS_2P0]-EDID_2p0_DVI_0[EDID_PA_POS_2P0]);   //checksum difference
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_1, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI2_MUX), 0x20, EDID_PA_POS_2P0);
    }
    else
  #endif
    {
        memcpy((void*)EDID_DVI_1, (void*)EDID_DVI_0, 256);
        EDID_DVI_1[EDID_PA_POS] = 0x20;
        EDID_DVI_1[0xFF] -= (EDID_DVI_1[EDID_PA_POS]-EDID_DVI_0[EDID_PA_POS]);   //checksum difference
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_1, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI2_MUX), 0x20, EDID_PA_POS);
    }
#endif
   ...
   //if(stGenSetting.g_SysSetting.u8EDID_select == EDID_2p0)
    ...
    {
        //copy EDID from DVI0 and change physical address/checksum
        memcpy((void*)EDID_DVI_2, (void*)EDID_2p0_DVI_0, 256);
        EDID_DVI_2[EDID_PA_POS_2P0] = 0x30;
        EDID_DVI_2[0xFF] -= -(EDID_DVI_2[EDID_PA_POS_2P0]-EDID_2p0_DVI_0[EDID_PA_POS_2P0]);   //checksum difference
    #if (MS_BOARD_TYPE_SEL == BD_MST084B_10AHB_15043)
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_2, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI4_MUX), 0x40, EDID_PA_POS_2P0);
    #else
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_2, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI3_MUX), 0x30, EDID_PA_POS_2P0);
    #endif
    }
   else
  #endif
    {
        //copy EDID from DVI0 and change physical address/checksum
        memcpy((void*)EDID_DVI_2, (void*)EDID_DVI_0, 256);
        EDID_DVI_2[EDID_PA_POS] = 0x30;
        EDID_DVI_2[0xFF] -= (EDID_DVI_2[EDID_PA_POS]-EDID_DVI_0[EDID_PA_POS]);   //checksum difference
    #if (MS_BOARD_TYPE_SEL == BD_MST084B_10AHB_15043)
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_2, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI4_MUX), 0x40, EDID_PA_POS);
    #else
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_2, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI3_MUX), 0x30, EDID_PA_POS);
    #endif
    }
#endif
    ...
    {
        //copy EDID from DVI0 and change physical address/checksum
        memcpy((void*)EDID_DVI_3, (void*)EDID_2p0_DVI_0, 256);
        EDID_DVI_3[EDID_PA_POS_2P0] = 0x40;
        EDID_DVI_3[0xFF] -= (EDID_DVI_3[EDID_PA_POS_2P0]-EDID_2p0_DVI_0[EDID_PA_POS_2P0]);   //checksum difference
    #if (MS_BOARD_TYPE_SEL == BD_MST084B_10AHB_15043)
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_3, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI3_MUX), 0x30, EDID_PA_POS_2P0);
    #else
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_3, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI4_MUX), 0x40, EDID_PA_POS_2P0);
    #endif
    }
    else
  #endif
    {
        //copy EDID from DVI0 and change physical address/checksum
        memcpy((void*)EDID_DVI_3, (void*)EDID_DVI_0, 256);
        EDID_DVI_3[EDID_PA_POS] = 0x40;
        EDID_DVI_3[0xFF] -= (EDID_DVI_3[EDID_PA_POS]-EDID_DVI_0[EDID_PA_POS]);   //checksum difference
    #if (MS_BOARD_TYPE_SEL == BD_MST084B_10AHB_15043)
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_3, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI3_MUX), 0x30, EDID_PA_POS);
    #else
        _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_3, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI4_MUX), 0x40, EDID_PA_POS);
    #endif
   }
#endif
}
...
U32 g_XC_SYS_u32OPVSyncCount = 0;
...
// init DLC
    ...
    //CEC
  #if 0//(ENABLE_CEC)    //Move CEC init to later in init
    msAPI_CEC_Init();
  #endif
...
void MApi_XC_HDMI_PostInit(void)
{
#if (INPUT_HDMI_VIDEO_COUNT > 0)
...
#endif
{
    ...
    if (bEnable )
    {
        ...
        //MApi_XC_W2BYTEMSK(REG_SC_BK01_42_L, 0x2255, 0xFFFF );
        //MApi_XC_W2BYTEMSK(REG_SC_BK01_40_L, 0x00, 0x03 );
...
void msAPI_Scaler_SetFreezeScreen( BOOLEAN bEnable, U16 u16ScreenUnMuteTime, SCALER_WIN eWindow)
{
    ...
    if (bEnable )
    {
        ...
        {   // sws check
            MSG(printf("Enable FreezeScreen return\n"));
            return;
        }
        ...
        MSG(printf("Enable FreezeScreen(%u)\n", msAPI_Timer_GetTime0()));
    ...
    else
    {
        if(stSystemInfo[eWindow].u8PanelPowerStatus & PANEL_POWER_FREEZESCREEN)
        {
            MSG(printf("Disable FreezeScreen(%u)\n", msAPI_Timer_GetTime0()));
        ...
        {
            MSG(printf("Disable FreezeScreen return\n"));
        }
    }
}
#endif

E_SCREEN_MUTE_STATUS msAPI_Scaler_GetScreenMute( SCALER_WIN eWindow)
{
...
{
    U16 u16MuteStatus = s_eScreenMuteStatus[eWindow];

    /*if( u16MuteStatus )
    {
        printf("VideoMute[%u]=0x%X", eWindow, u16MuteStatus);

        if( u16MuteStatus&E_SCREEN_MUTE_TEMPORARY )
            printf(" Temp,");
        if( u16MuteStatus&E_SCREEN_MUTE_RATING )
            printf(" Rating,");
        if( u16MuteStatus&E_SCREEN_MUTE_FREERUN )
            printf(" Freerun,");
        if( u16MuteStatus&E_SCREEN_MUTE_CHANNEL )
            printf(" Channel,");
        if( u16MuteStatus&E_SCREEN_MUTE_BLOCK )
            printf(" Block,");
        if( u16MuteStatus&E_SCREEN_MUTE_MHEG5 )
            printf(" MHEG5,");
        if( u16MuteStatus&E_SCREEN_MUTE_HDMI_AVMUTE )
            printf(" HDMI_AVMute,");
        if( u16MuteStatus&E_SCREEN_MUTE_BY_UI )
            printf(" UI,");
        if( u16MuteStatus&E_SCREEN_MUTE_BY_SCALER_FPLL )
            printf(" SCALER_FPLL,");

    #if( ENABLE_ATSC )
        if( u16MuteStatus&E_SCREEN_MUTE_BY_VCHIP )
            printf(" VChip,");
    #endif

    #if( ENABLE_EWS)
        if( u16MuteStatus&E_SCREEN_MUTE_BY_EWS)
            printf(" EWS,");
    #endif

        if( u16MuteStatus&E_SCREEN_MUTE_BY_VDEC)
            printf(" VDEC,");
    ...
    else // Disable Mute
    {
        ...
        if (eScreenMuteTmp != s_eScreenMuteStatus[eWindow])
        {
            if( s_eScreenMuteStatus[eWindow] == E_SCREEN_MUTE_INIT  )
            {
                ...
                #if 1//( ENABLE_LOG_CHANGE_CHANNEL )
...
//    printf("\r\n INPUT_SOURCE_HDMI = %d", INPUT_SOURCE_HDMI);
    ...
    if( IsSrcTypeHDMI(enInputSourceType) )
    {
    ...
    #if ENABLE_EDID_SWITCH
        {
            ...
            MApi_XC_Sys_Reload_Edid(enInputSourceType, EDID_2p0);
        }
        //else
    #else
        {
...
void msAPI_Scaler_SetCVBSMute( BOOLEAN bEn , E_VE_MUTE_STATUS eMuteStatus , INPUT_SOURCE_TYPE_t enInputPortType, E_DEST_TYPE OutputPortType) //SCR 071123
{
    ...
    DEBUG_CVBS_OUT_MUTE(printf("msAPI_Scaler_SetCVBSMute(bEn=%u, MuteState=%u, Input=%u, Output=%u, s_eVEMuteStatus[%u]=%u\n", bEn, eMuteStatus, enInputPortType, OutputPortType, u8PortID, s_eVEMuteStatus[u8PortID]));
...
void msAPI_Scaler_SetAFDToVEWSS(BOOLEAN bEn, U8 u8AFD, U8 u8SrcARC)
{
    ...
    //printf("SetAFDToVEWSS=%bd, AFD=%bu, SCRARC=%bu\n", bEn, u8AFD, u8SrcARC);
    if(bEn)
    {
        switch(u8AFD)
        {
...
void MApi_XC_Sys_DLC_DBC_OnOff(MS_BOOL bEnable)
{
    ...
    {
        if( (IsVgaInUse() == FALSE) && ( (MApi_XC_HDMI_GetHdmiType(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)) == E_HDMI_STATUS_DVI)== FALSE ) )
        {
...
void MApi_XC_Sys_Periodic_Handler(SCALER_WIN eWindow, MS_BOOL bRealTimeMonitorOnly)
{
    ...
    if ( (msAPI_Timer_DiffTimeFromNow(u32TimerCounter)>16) ) // updated by Bruce.Liu 20150831
    {
    ...
    if(u8Operat_flag)
    {
...
#if ENABLE_DLC
        {
            ...
            if( !g_bHistogramReady )
            {
                if( MApi_XC_DLC_GetHistogramHandler(eWindow) == TRUE )
                {
        ...
        #if 0//ENABLE_DCC, Daten FixMe
        #if ENABLE_DMP
        if (!IsStorageInUse())
        #endif  // ENABLE_DMP
            msAPI_DCC_Handler(stDlcInfo.u8MaxHistogram , stDlcInfo.u8AvgHistogram);
        #endif
...
#if 0
void MApi_XC_HDMI_SetHotPlug_ALL(MS_BOOL bEnable)
{
    MApi_XC_HDMI_SetHotPlug(bEnable, INPUT_PORT_DVI0);
    MApi_XC_HDMI_SetHotPlug(bEnable, INPUT_PORT_DVI1);
    MApi_XC_HDMI_SetHotPlug(bEnable, INPUT_PORT_DVI2);
    MApi_XC_HDMI_SetHotPlug(bEnable, INPUT_PORT_DVI3);
}

void MApi_XC_HDMI_SetDVIClkPullLow_ALL(MS_BOOL bEnable)
{
    MApi_XC_HDMI_SetDVIClkPullLow(bEnable, INPUT_PORT_DVI0);
    MApi_XC_HDMI_SetDVIClkPullLow(bEnable, INPUT_PORT_DVI1);
    MApi_XC_HDMI_SetDVIClkPullLow(bEnable, INPUT_PORT_DVI2);
    MApi_XC_HDMI_SetDVIClkPullLow(bEnable, INPUT_PORT_DVI3);
}
#endif
...
typedef enum
{
    ...
    E_HPD_STATE__SET_HPD_DISABLE, // HotPlug=0, delay(300), DviClk=low
    E_HPD_STATE__SET_HPD_DISABLE_DELAY,
...
static EnuHpdState s_Hdmi_Hpd_State = E_HPD_STATE__NONE;
static U32 s_u32Hdmi_Hpd_DelayStartTime;

// for IMMESWITCH, HDP should be pull low/high at least once
static BOOLEAN bIMMESWITCH_InitHPD = FALSE;
static BOOLEAN bIsHPDInit=TRUE;

#define HDMI_HDP_TASK_DBG(x)    //x

void MApi_XC_HPD_Task_Start(BOOLEAN bIsInitStage)
{
    XC_INITMISC sXC_Init_Misc;
    bIsHPDInit = bIsInitStage;


    HDMI_HDP_TASK_DBG(printf(" MApi_XC_HPD_Task_Start(%u) at %u\n", bIsInitStage, MsOS_GetSystemTime()));
    ...
    /*
    always turn on HDP for CEC to access EDID
    */
#if 0
    if ( IsSrcTypeHDMI(stSystemInfo[MAIN_WINDOW].enInputSourceType) )
    {
        // HDMI -> do HPD
        ;
    }
    else if (DB_HDMI_SETTING.g_bHdmiCecMode == ENABLE)
    {
        // non HDMI -> if CEC option is on -> do HDP to access EDID
        ;
    }
    else
    {
    ...
    if( bIsHPDInit) // For Power on init case
    {
        s_Hdmi_Hpd_State = E_HPD_STATE__SET_HPD_ENABLE;
    }
    else // For switch source case
    {
        MApi_XC_GetMISCStatus(&sXC_Init_Misc);

        if (  (sXC_Init_Misc.u32MISC_A & E_XC_INIT_MISC_A_IMMESWITCH)
           && (bIMMESWITCH_InitHPD)
           )
        {
            return;
        ...
        if(s_Hdmi_Hpd_State!= E_HPD_STATE__HPD_OK)
        {
            U8 u8DummyIdx = 0;
            for(u8DummyIdx=0; u8DummyIdx<INPUT_HDMI_VIDEO_COUNT; u8DummyIdx++)
            {
...
void MApi_XC_HPD_Task(void)
{
    ...
    //printf("MApi_XC_HPD_Task(bIsInitStage=%u)\n", bIsInitStage );

    switch( s_Hdmi_Hpd_State )
    {
        ...
        case E_HPD_STATE__SET_HPD_DISABLE: // HotPlug=0, delay(300), DviClk=low
...
#endif
            HDMI_HDP_TASK_DBG(printf("\n (1) E_HPD_STATE__SET_HPD_DISABLE at %u\n", MsOS_GetSystemTime());)
            if(bIsHPDInit)
            {
                for(u8DummyIdx=0; u8DummyIdx<INPUT_HDMI_VIDEO_COUNT; u8DummyIdx++)
                {
            ...
            {
                //printf("[E_HPD_STATE__SET_HPD_DISABLE] SYS_INPUT_SOURCE_TYPE=%d\n", SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW));
                if(IsSrcTypeHDMI(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)))
                {
                    MApi_XC_HDMI_SetHotPlug(FALSE, eHdmiPortMappingResult[SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)-INPUT_SOURCE_HDMI]);
                }
            }
            s_u32Hdmi_Hpd_DelayStartTime = MsOS_GetSystemTime();

            s_Hdmi_Hpd_State = E_HPD_STATE__SET_HPD_DISABLE_DELAY;
            break;

        case E_HPD_STATE__SET_HPD_DISABLE_DELAY: // Delay 300 ms
            if( MsOS_Timer_DiffTimeFromNow(s_u32Hdmi_Hpd_DelayStartTime) > 250 )
            {
                HDMI_HDP_TASK_DBG(printf("\n (2) E_HPD_STATE__SET_HPD_DISABLE_DELAY at %u\n", MsOS_GetSystemTime()));

              #if(INPUT_HDMI_VIDEO_COUNT>=2)  //when only one HDMI port , don't change HDMI clk to speed up source switch
                if(bIsHPDInit)
                {
                    for(u8DummyIdx=0; u8DummyIdx<INPUT_HDMI_VIDEO_COUNT; u8DummyIdx++)
                    {
                ...
                else
                {
                    if(IsSrcTypeHDMI(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)))
                    {
                        MApi_XC_HDMI_SetDVIClkPullLow(TRUE, eHdmiPortMappingResult[SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)-INPUT_SOURCE_HDMI]);   // DVI clock pull low
                    }
                }
              #endif
                if(bIsHPDInit)
        ...
        case E_HPD_STATE__RESET_HDMI:
            HDMI_HDP_TASK_DBG(printf("\n (3) E_HPD_STATE__RESET_HDMI at %u\n", MsOS_GetSystemTime()));
        ...
        case E_HPD_STATE__HPD_DELAY:
            if( MsOS_Timer_DiffTimeFromNow(s_u32Hdmi_Hpd_DelayStartTime) > HDMI_HPD_SOURCE_SWITCH_DELAY )
            {
                HDMI_HDP_TASK_DBG(printf("\n (4) E_HPD_STATE__HPD_DELAY at %u\n", MsOS_GetSystemTime()));
                s_Hdmi_Hpd_State = E_HPD_STATE__SET_HPD_ENABLE;
            }
            break;

        case E_HPD_STATE__SET_HPD_ENABLE: // DviClk=High, delay(300), HotPlug=1
            HDMI_HDP_TASK_DBG(printf("\n (5) E_HPD_STATE__SET_HPD_ENABLE at %u\n", MsOS_GetSystemTime()));
          #if(INPUT_HDMI_VIDEO_COUNT>=2) //when only one HDMI port , don't change HDMI clk to speed up source switch
            if(bIsHPDInit)
            {
            ...
            {
                //printf("[E_HPD_STATE__SET_HPD_ENABLE] SYS_INPUT_SOURCE_TYPE=%d\n", SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW));

                if(IsSrcTypeHDMI(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)))
                {
                    MApi_XC_HDMI_SetDVIClkPullLow(FALSE, eHdmiPortMappingResult[SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)-INPUT_SOURCE_HDMI]);      // DVI clock pull high
                }
        ...
        case E_HPD_STATE__SET_HPD_ENABLE_DELAY:
            if( MsOS_Timer_DiffTimeFromNow(s_u32Hdmi_Hpd_DelayStartTime) > 250 )
            {
                HDMI_HDP_TASK_DBG(printf("\n (6) E_HPD_STATE__SET_HPD_ENABLE_DELAY at %u\n", MsOS_GetSystemTime()));

              #if 0
                MApi_XC_HDMI_SetHotPlug_ALL(bHotPlugPullLow);
              #else
              if(bIsHPDInit)
              {
                for(u8DummyIdx=0; u8DummyIdx<INPUT_HDMI_VIDEO_COUNT; u8DummyIdx++)
              ...
              else
              {
                  if(IsSrcTypeHDMI(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)))
                  {
                      MApi_XC_HDMI_SetHotPlug(TRUE, eHdmiPortMappingResult[SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW)-INPUT_SOURCE_HDMI]);
                  }
              }
              #endif

                s_Hdmi_Hpd_State = E_HPD_STATE__HPD_OK;
                HDMI_HDP_TASK_DBG(printf(" E_HPD_STATE__HPD_OK at %u\n", MsOS_GetSystemTime()));
...
{
    if ( (s_Hdmi_Hpd_State == E_HPD_STATE__NONE)
       ||(s_Hdmi_Hpd_State == E_HPD_STATE__HPD_OK) )
    {
        return FALSE;
    }
...
//=====================================================================================
    ...
    if( s_XC_u16CurVT == s_XC_u16LastVT )
    {
        ...
        if( s_XC_bVTStable == FALSE )
        {
            ...
            {
                s_XC_bVTStable = TRUE;
                printf("[XC] VT=%u stable at %u,%u\n", s_XC_u16CurVT, g_Debug_u32Time_ChangeChannel, s_XC_Sys_Task_LastTime);
            }
        }
    }
    else
    {
        //if( s_XC_bVTStable )
        {
            ...
            printf("[XC] VT=%u->%u unstable at %u,%u\n", s_XC_u16LastVT, s_XC_u16CurVT, g_Debug_u32Time_ChangeChannel, s_XC_Sys_Task_LastTime);
        }
...
//#endif
    ...
    #endif
        DEBUG_XC_FB( printf(" u32MainSize=%u\n", u32MainSize); );
        // Set to driver
...
void MApi_XC_UpdateEDID(BOOLEAN bIsType2Edid, U8* pu8DdpSAD)
{
    ...
    if (bIsType2Edid == TRUE)
    {
        U8 u8NewChecksum = 0;
        u8PAPos = EDID_DDP_PA_POS;
      #if ENABLE_EDID_SWITCH
        //if(stGenSetting.g_SysSetting.u8EDID_select == EDID_2p0)
        if( g_XC_Sys_u8EdidSel == EDID_2p0 )
        {
            memcpy((void*)u8Edid0, (void*)EDID_DDP_2p0_DVI_0, 256);
            //calculate new checksum
            u8NewChecksum = EDID_DDP_2p0_DVI_0[0xFF];
            u8NewChecksum += (EDID_DDP_2p0_DVI_0[EDID_DDP_SAD_POS]+EDID_DDP_2p0_DVI_0[EDID_DDP_SAD_POS+1]+EDID_DDP_2p0_DVI_0[EDID_DDP_SAD_POS+2]);
        }
        else
      #endif
        {
            memcpy((void*)u8Edid0, (void*)EDID_DDP_DVI_0, 256);
            //calculate new checksum
            u8NewChecksum = EDID_DDP_DVI_0[0xFF];
            u8NewChecksum += (EDID_DDP_DVI_0[EDID_DDP_SAD_POS]+EDID_DDP_DVI_0[EDID_DDP_SAD_POS+1]+EDID_DDP_DVI_0[EDID_DDP_SAD_POS+2]);
        }
        u8NewChecksum -= (pu8DdpSAD[0]+pu8DdpSAD[1]+pu8DdpSAD[2]);
        //replace new SAD and checksum
        u8Edid0[EDID_DDP_SAD_POS] = pu8DdpSAD[0];
        u8Edid0[EDID_DDP_SAD_POS+1] = pu8DdpSAD[1];
        u8Edid0[EDID_DDP_SAD_POS+2] = pu8DdpSAD[2];
        u8Edid0[0xFF] = u8NewChecksum;
    }
    else
    {
    #if ENABLE_EDID_SWITCH
        ...
        if( g_XC_Sys_u8EdidSel == EDID_2p0 )
        {
            u8PAPos = EDID_PA_POS_2P0;
            memcpy((void*)u8Edid0, (void*)EDID_2p0_DVI_0, 256);
        }
        else
    #endif
        {
            u8PAPos = EDID_PA_POS;
            memcpy((void*)u8Edid0, (void*)EDID_DVI_0, 256);
        }
    }
...
#if (INPUT_HDMI_VIDEO_COUNT >= 1)
  #if ENABLE_DYNAMIC_EDID
    stEDIDConfig.bSAD = DEFAULT;        // DEFAULT,SUPPORT,NOT_SUPPORT
    ...
    stEDIDConfig.b4K2K = DEFAULT;       // DEFAULT,SUPPORT,NOT_SUPPORT
    stEDIDConfig.b3D = DEFAULT;         // DEFAULT,SUPPORT,NOT_SUPPORT
    stEDIDConfig.u8PAPort = EDID_HDMI_PORT1;
    msApi_Dynamic_EDID_main(u8Edid0, EDID_DVI, &stEDIDConfig);
    _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI1_MUX), stEDIDConfig.u8PAPort, stEDIDConfig.u8PAOut);
  #else
    _MApi_XC_DDCRam_Init_Hdmi(u8Edid0, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI1_MUX), 0x10, u8PAPos);
  #endif
#endif
    ...
    stEDIDConfig.b4K2K = DEFAULT;       // DEFAULT,SUPPORT,NOT_SUPPORT
    stEDIDConfig.b3D = DEFAULT;         // DEFAULT,SUPPORT,NOT_SUPPORT
    stEDIDConfig.u8PAPort = EDID_HDMI_PORT2;
    msApi_Dynamic_EDID_main(u8Edid0, EDID_DVI_1, &stEDIDConfig);
    _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_1, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI2_MUX), stEDIDConfig.u8PAPort, stEDIDConfig.u8PAOut);
  #else
    //copy EDID from DVI0 and change physical address/checksum
    memcpy((void*)EDID_DVI_1, (void*)u8Edid0, 256);
    EDID_DVI_1[u8PAPos] = 0x20;
    EDID_DVI_1[0xFF] = u8Edid0[0xFF]-0x10;   //checksum difference is 0x20-0x10
    _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_1, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI2_MUX), 0x20, u8PAPos);
  #endif
#endif
    ...
    stEDIDConfig.b4K2K = DEFAULT;       // DEFAULT,SUPPORT,NOT_SUPPORT
    stEDIDConfig.b3D = DEFAULT;         // DEFAULT,SUPPORT,NOT_SUPPORT
    stEDIDConfig.u8PAPort = EDID_HDMI_PORT3;
    msApi_Dynamic_EDID_main(u8Edid0, EDID_DVI_2, &stEDIDConfig);
    _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_2, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI3_MUX), stEDIDConfig.u8PAPort, stEDIDConfig.u8PAOut);
  #else
    //copy EDID from DVI0 and change physical address/checksum
    memcpy((void*)EDID_DVI_2, (void*)u8Edid0, 256);
    EDID_DVI_2[u8PAPos] = 0x30;
    EDID_DVI_2[0xFF] = u8Edid0[0xFF]-0x20;   //checksum difference is 0x30-0x10
    _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_2, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI3_MUX), 0x30, u8PAPos);
  #endif
#endif
    ...
    stEDIDConfig.b3D = DEFAULT;         // DEFAULT,SUPPORT,NOT_SUPPORT
    stEDIDConfig.u8PAPort = EDID_HDMI_PORT3;
    msApi_Dynamic_EDID_main(u8Edid0, EDID_DVI_3, &stEDIDConfig);
    _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_3, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI4_MUX), stEDIDConfig.u8PAPort, stEDIDConfig.u8PAOut);
  #else
    //copy EDID from DVI0 and change physical address/checksum
    memcpy((void*)EDID_DVI_3, (void*)u8Edid0, 256);
    EDID_DVI_3[u8PAPos] = 0x40;
    EDID_DVI_3[0xFF] = u8Edid0[0xFF]-0x30;   //checksum difference is 0x40-0x10
    _MApi_XC_DDCRam_Init_Hdmi(EDID_DVI_3, _MApi_XC_DDC_GetHDMIDDCProgType(INPUT_HDMI4_MUX), 0x40, u8PAPos);
  #endif
#endif

//Hot plug
  #if (ENABLE_HDMI_HPD_TASK )
    MApi_XC_HPD_Task_Start(FALSE);   //Enable state machine flow
...
void MApi_XC_Sys_Set_EdidSel( U8 u8EdidSel )
{
    g_XC_Sys_u8EdidSel = u8EdidSel;
}

void MApi_XC_Sys_Reload_Edid(INPUT_SOURCE_TYPE_t enInputSourceType, U8 u8Edid)
{
    //PRINT_CURRENT_LINE();
    ...
    //stGenSetting.g_SysSetting.u8EDID_select = u8Edid;
    MApi_XC_Sys_Set_EdidSel(u8Edid);
		
		#if (ENABLE_DDC_RAM) //MingYuan Add
    _MApi_XC_DDCRam_Init();
		#endif
...
void MApi_XC_Sys_ResetDVIPort(INPUT_SOURCE_TYPE_t enINPUT_SOURCE_TYPE)
{
    ...
    _MApi_XC_Sys_InputSource_InputPort_Mapping(enINPUT_SOURCE_TYPE, &eInputPort, &u8NoUse);

// NASA is not support MDrv_DVI_Software_Reset()
#if( CHIP_FAMILY_TYPE != CHIP_FAMILY_NASA )
