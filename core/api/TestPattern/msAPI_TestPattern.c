...
static void _DrawBMP_Init(void)
{
    ...
    DEBUG_TEST_PATTERN(printf(" u16MemOffset=0x%X\n", u16MemOffset););
    ...
    //printf("s_TestPattern_pu8DstBuf=0x%X\n", s_TestPattern_pu8DstBuf);
    //printf("s_TestPattern_pu8DstBuf_NonCache=0x%X\n", s_TestPattern_pu8DstBuf_NonCache);
...
//        MS_U8  u8DefaultPhase;          ///<Obsolete
    ...
    //memory format
    MDrv_WriteByte(BK_SELECT_00, REG_BANK_SCMI);
    MDrv_Write2Byte( L_BK_SCMI(0x02), 0x00);
    ...
    //Vertical scaling
    MDrv_WriteByte(BK_SELECT_00, REG_BANK_HVSP);
    MDrv_WriteByteMask(H_BK_HVSP(0x0A), 0x00, BIT0);
    ...
    //YUV 444->422
    MDrv_WriteByte(BK_SELECT_00, REG_BANK_HVSP);
    MDrv_WriteByteMask(L_BK_HVSP(0x0C), 0x00, BIT7);
    ...
    MDrv_WriteByte(L_BK_HVSP(0x6E), 0x00);

    //  3X3 ByPass = RGB
    MDrv_WriteByte(BK_SELECT_00, REG_BANK_VOP);
    MDrv_WriteByte(0x102F5E, 0x00);

    // Gamma LUT Off
    MDrv_WriteByte(BK_SELECT_00, REG_BANK_VOP);
    MDrv_WriteByteMask(0x102FA0, 0x00, BIT0 );
...
void _msAPI_TestPattern_CopyLine( U16 u16SrcLineIdx, U16 u16TgtLineIdx, U16 u16LineCount )
{
...
BOOL msAPI_TestPattern_Draw_Rect_2(U16 u16XStart, U16 u16YStart, U16 u16XSize, U16 u16YSize )
{
    U16 i;
    ...
    if( (u16XStart == 0) && (u16XSize >= s_TestPattern_u16PanelWidth ) )
    {
        ...
        {
            _msAPI_TestPattern_CopyLine( u16YStart, u16YStart + i, 1 );
        }
    }
    else
    {
...
BOOL msAPI_TestPattern_Draw_Checkbox(void)
{
    ...
    for( u16Y = 0; (u16Y + 0) < s_TestPattern_u16MemVLen; u16Y += 100 )
    {
        if( bFlag )
            u16X = 100;
        else
            u16X = 0;
        for( ; (u16X + 0) < s_TestPattern_u16MemOffset; u16X += 200 )
        {
            msAPI_TestPattern_Draw_Rect_2(u16X, u16Y, 100, 100);
        }
...
BOOL msAPI_TestPattern_Draw_RGB(void)
{
    ...
    // R Bar
    u16YSize[0] = s_TestPattern_u16PanelHigh/3;
    msAPI_TestPattern_Set_CurColor(0xFF, 0, 0);
    msAPI_TestPattern_Draw_Rect_2(0, 0, s_TestPattern_u16PanelWidth, u16YSize[0]);

    // G Bar
    u16YSize[1] = s_TestPattern_u16PanelHigh/3;
    msAPI_TestPattern_Set_CurColor(0, 0xFF, 0);
    msAPI_TestPattern_Draw_Rect_2(0, u16YSize[0], s_TestPattern_u16PanelWidth, u16YSize[1]);

    // B Bar
    u16YSize[3] = s_TestPattern_u16PanelHigh - u16YSize[0] - u16YSize[1];
    msAPI_TestPattern_Set_CurColor(0, 0, 0xFF);
    msAPI_TestPattern_Draw_Rect_2(0, u16YSize[0]+u16YSize[1],
                                s_TestPattern_u16PanelWidth, u16YSize[3]);
...
BOOL msAPI_TestPattern_Draw_Stripe(BOOL bIsVertical)
{
    ...
    if( bIsVertical )
    {
        ...
        {
            _msAPI_TestPattern_CopyLine( 0, u16Y, 1 );
        }
    }
    else
    {
        ...
        {
            _msAPI_TestPattern_CopyLine( 0, u16Y, 2 );
        }
    }
...
#endif
    {
        ...
        switch(u8TPNo)
        {
            ...
            case E_TEST_PATTERN_ID_RECT_2:
                msAPI_TestPattern_Set_CurColor(0xFF, 0xFF, 0xFF);
                bResult = msAPI_TestPattern_Draw_Rect_2(480, 270, 960, 540);
                break;
