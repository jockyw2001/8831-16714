...
//                         SCL              SDA             SPEED
    ...
    #define MIIC_BUS_DDCD0  PAD_DDCDA_CLK,  PAD_DDCDA_DAT,  I2C_SPEED_MAPPING_100K		//Ray EDD 2017.02.24: Change DDCD0 SCL, SDA and SPEED definition
    #define MIIC_BUS_DDCD1  PAD_DDCDB_CLK,  PAD_DDCDB_DAT,  I2C_SPEED_MAPPING_100K		//Ray EDD 2017.02.27: Define DDCD1 SCL, SDA and SPEED
    #define MIIC_BUS_DDCD2  PAD_DDCDC_CLK,  PAD_DDCDC_DAT,  I2C_SPEED_MAPPING_100K		//Ray EDD 2017.02.27: Define DDCD2 SCL, SDA and SPEED
    #define MIIC_BUS_DDCD3  PAD_DDCDD_CLK,  PAD_DDCDD_DAT,  I2C_SPEED_MAPPING_100K		//Ray EDD 2017.02.27: Define DDCD3 SCL, SDA and SPEED
...
I2C_Bus g_I2CBus[] =
{
    ...
    { I2C_BUS( MIIC_BUS_DDCD1 ) },	//Ray EDD 2017.02.27: Add MIIC_BUS_DDC1, DDC2 and DDC3 for HDMI2, HDMI3 and DVI respectively
    { I2C_BUS( MIIC_BUS_DDCD2 ) },
    { I2C_BUS( MIIC_BUS_DDCD3 ) },
...
SWI2C_BusCfg g_I2CBus_Temp[] =
{
    ...
    {MIIC_BUS_DDCA0},
    {MIIC_BUS_DDCD0},
    {MIIC_BUS_DDCD1},			//Ray EDD 2017.03.16: Add MIIC_BUS_DDC1, DDC2 and DDC3 for HDMI2, HDMI3 and DVI respectively
    {MIIC_BUS_DDCD2},
    {MIIC_BUS_DDCD3},
};
...
//Ray EDD 2017.03.28
extern void GPIO_DDCDA_ENABLE(U8 enable);
extern void GPIO_DDCDB_ENABLE(U8 enable);
extern void GPIO_DDCDC_ENABLE(U8 enable);
extern void GPIO_DDCDD_ENABLE(U8 enable);
...
void IIC_UseBus( U8 u8BusChn )
{
    if ( u8BusChn < COUNTOF( g_I2CBus ) )
    {
        ...
        //printf("\nRay: IIC_UseBus:%d",u8BusSel);		//Ray DBG 2017.02.24
        switch ( u8BusChn )
        {
        ...
        case E_I2C_BUS_DDCD0:
          GPIO_DDCDA_ENABLE(0);			//Ray EDD 2017.03.28: To change port from DDCDA to GPIO for IIC access
            //   GPIO_DDCD0_ENABLE(1);
            break;
        case E_I2C_BUS_DDCD1:
	    GPIO_DDCDB_ENABLE(0);		//Ray EDD 2017.03.28: To change port from DDCDB to GPIO for IIC access
            break;
        case E_I2C_BUS_DDCD2:
	    GPIO_DDCDC_ENABLE(0);		//Ray EDD 2017.03.28: To change port from DDCDC to GPIO for IIC access
            // GPIO_DDCD2_ENABLE(1);
            break;
        case E_I2C_BUS_DDCD3:
	    GPIO_DDCDD_ENABLE(0);		//Ray EDD 2017.03.28: To change port from DDCDD to GPIO for IIC access
            // GPIO_DDCD2_ENABLE(1);
            break;
        default:
            break;
...
void IIC_UnuseBus( void )
{
    ...
    case E_I2C_BUS_DDCA0:
        ...
        break;
    case E_I2C_BUS_DDCD0:
      GPIO_DDCDA_ENABLE(1);			//Ray EDD 2017.03.28: To change port from GPIO back to DDCDA after IIC access
        //GPIO_DDCD0_ENABLE(0);
        break;
    case E_I2C_BUS_DDCD1:
	GPIO_DDCDB_ENABLE(1);			//Ray EDD 2017.03.28: To change port from GPIO back to DDCDB after IIC access
        //GPIO_DDCD1_ENABLE(0);
        break;
    case E_I2C_BUS_DDCD2:
      GPIO_DDCDC_ENABLE(1);			//Ray EDD 2017.03.28: To change port from GPIO back to DDCDC after IIC access
        //GPIO_DDCD2_ENABLE(0);
        break;
    case E_I2C_BUS_DDCD3:
      GPIO_DDCDD_ENABLE(1);			//Ray EDD 2017.03.28: To change port from GPIO back to DDCDD after IIC access
        //GPIO_DDCD2_ENABLE(0);
        break;
    default:
        break;
...
void MDrv_IIC_Init()
{
    ...
    //Ray EDD 2017.03.16: Init all EDID IIC buses to input port
    BYTE i;
    for(i=E_I2C_BUS_DDCD0;i<=E_I2C_BUS_DDCD3;i++)
    {
      IIC_UseBus(i);
      pin_scl_set_input();
      pin_sda_set_input();
      IIC_UnuseBus();			//Ray EDD 2017.03.28: release the IIC bus back to DDC
    }
...
BOOLEAN MDrv_IIC_WriteBytes(U16 u16BusNumSlaveID, U8 AddrCnt, U8* pu8addr, U16 u16size, U8* pBuf)
{
    ...
    //Ray DBG 2017.02.13: Add debug message on check if any IIC write happen
        //printf("Ray:MDrv_IIC_WriteBytes Write write ID:%x,write first byte:%x\n,write size:%d",u16BusNumSlaveID,pBuf[0],u16size);
        //printf("Ray: USE_SW_I2C_HIGHSPEED is %d",USE_SW_I2C_HIGHSPEED);
    ...
    while (u8Dummy--)
    {
        if (IIC_AccessStart(LOBYTE(u16BusNumSlaveID), SWIIC_WRITE) == FALSE)
        {
        	...
        	printf("\nRay: IIC_AccessStart fail");	//Ray DBG 2017.02.24
                goto fail;
            }
        }
        ...
        while (AddrCnt)
        {
            ...
            {
        	printf("\nRay: IIC_SendByte fail at addrCnt:%d",AddrCnt);	//Ray DBG 2017.02.24
                goto fail;
            }
            pu8addr++;
        }
        while (u16size) // loop of writting data
        {
            ...
            {
        	printf("\nRay: IIC_SendByte fail at u16size:%d",u16size);	//Ray DBG 2017.02.24
                goto fail;
            }
