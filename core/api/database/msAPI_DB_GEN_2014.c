...
BOOL _msAPI_DB_GEN_Set_UnitStatus(U8 u8UnitIdx, U8 u8UnitStatus)
{
    ...
    {
        //printf("\nError: Set_UnitStatus: Verify faild! unit=%u, status=0x%X\n", u8UnitIdx, u8UnitStatus);
        return FALSE;
    }
...
void _msAPI_DB_GEN_InitUnit(U8 u8UnitIdxStart, U8 u8UnitNum )
{
    U8 i, u8UnitIdx;
    BOOL bResult = TRUE;
...
BOOL _msAPI_DB_GEN_Read_Header(U8 u8UnitIdx, Stru_DB_Gen_Header* pstDB_Gen_Header, BOOL bCheckMagicId)
{
    BOOL bResult = TRUE;
...
U8 msAPI_DB_GEN_FindLastGood(void)
{
    ...
    for( u8UnitIdx = 0; u8UnitIdx < s_stDB_GEN_Info.u8FlashUnitNumTotal; u8UnitIdx += 1 )
    {
        ...
        if( u8CurUnitStatus == DB_GEN_UNIT_STATUS_GOOD )
        {
            ...
            DEBUG_FIND_LAST(printf(" WT=%u ", stDB_Gen_Header_Cur.u32WriteTime););
...
void msAPI_DB_GEN_Task(void)
{
    ...
    switch( s_stDB_GEN_Info.eState )
    {
        ...
        case E_DB_GEN_STATE__WAIT_ERASE_DONE:
            ...
            // Set unit to empty
            _msAPI_DB_GEN_InitUnit(s_u8LastEraseUnitIdxStart, s_u8LastEraseUnitNum);
