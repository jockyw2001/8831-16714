...
#if (ENABLE_THAI_OSD)
    Font[FONT_THAI_0].fHandle = msAPI_Font_LoadFlashFontToSDRAM(BIN_ID_FONT_THAI_35);
    FontHandleIndex[Font[FONT_THAI_0].fHandle] = FONT_THAI_0;
    Font[FONT_THAI_1].fHandle = msAPI_Font_LoadFlashFontToSDRAM(BIN_ID_FONT_THAI_30);
    FontHandleIndex[Font[FONT_THAI_1].fHandle] = FONT_THAI_1;
    Font[FONT_THAI_2].fHandle = msAPI_Font_LoadFlashFontToSDRAM(BIN_ID_FONT_THAI_26);
...
BOOLEAN MApp_LoadFontInit_TTF(U16 u16FontBinID, BOOLEAN bUnloadFirst)
{
    ...
    if (SUCCESS == MDrv_DMA_LoadBin(&BinInfo,
                            _PA2VA((TTF_BUFFER_ADR & MIU1) ? (TTF_BUFFER_ADR | MIU_INTERVAL) : (TTF_BUFFER_ADR)),
