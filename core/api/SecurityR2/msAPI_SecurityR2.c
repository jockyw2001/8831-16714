...
//    U8 u8MIUProtectSecuR2_Run_ID[3] = { MIU_CLIENT_SECURE_R2_RW, MIU_CLIENT_AESDMA_RW, 0xFF};
    ...
    printf("%s(u16BinID=0x%X, u32_ADR=0x%X, u32_LEN=0x%X)\n", __func__, u16BinID, u32_ADR, u32_LEN);
    printf("BinInfo.B_FAddr= 0x%X\n",BinInfo.B_FAddr);
    printf("BinInfo.B_Len= 0x%X\n",BinInfo.B_Len);
    printf("BinInfo.B_IsComp= 0x%X\n",BinInfo.B_IsComp);
...
static void msAPI_SecurityR2_LoadBin(void)
{
    ...
    msAPI_SECU_R2_LoadBin(BIN_ID_CODE_SECU_R2_64MB, SECURITY_R2_ADR, SECURITY_R2_LEN);
#else
...
bank:0x100E
16bits
doyle.huang(Doyle Huang (¶À¨ó·ì)) 17:18:45
­n°O±o ©¹¥k shift 16bit
>>16
0x5A*2
#endif
...
//Maya 222B  PAD_GPIO8/PAD_GPIO9  connect I2S_OUT_BCK/I2S_OUT_MCK for Audio output
    ...
    //printf("SecurityR2.bin adr=%x\n" , MDrv_Read4Byte(HDCP22_MBX_CONFIRM_ADR));
    //printf("len=%x\n" ,MDrv_Read4Byte(HDCP22_MBX_CONFIRM_LEN));

    if((SECURITY_R2_ADR!=u32ConfirmAdr) || (u32ConfirmLen!=u32ConfirmLen))
    {
        MApp_Fatal_Error("MMAP.h SECURITY_R2_ADR/Len is not match with SecurityR2.bin ", __FUNCTION__);
    }
    ...
    //printf("MMAP.h SECURITY_R2_ADR=%x\n", SECURITY_R2_ADR>>16);
...
void msAPI_HDCP22_SendKeyToSecurityR2(void)
{
    ...
    pkey=(U8 *)(HDCP_KEY_BUFFER_ADR|BIT31|0x8000);
    memset(pkey, 0x00, HDCP22_AES_KEY_SIZE+HDCP22_KEY_SIZE+1);  // extra one byte to inform security R2 use internal efuse key or not
    ...
    {
        *(pkey+i+HDCP22_AES_KEY_SIZE)=_u8Hdcp22Key[i];
    }

    // last byte
    *(pkey+HDCP22_AES_KEY_SIZE+HDCP22_AES_KEY_SIZE)=su8UseEfuseKey;
    ...
    for(i=0; i<HDCP22_KEY_SIZE; i++)
    {
        printf("%x ", *(pkey+i+HDCP22_AES_KEY_SIZE));
        if(i%16==15) printf("\n");
    }
//while(1);
    printf("\n");
    ...
    printf("su8UseEfuseKey=%x ", *(pkey+HDCP22_AES_KEY_SIZE+HDCP22_AES_KEY_SIZE));
