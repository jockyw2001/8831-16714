...
static LPTSTR _MApp_ZUI_ACT_GetAudioLangItemText(U8 u8index)
{
    ...
    if ( IsDTVInUse() )
    {
        ...
        U16 u16TempID=_MApp_ZUI_ACT_GetLanguageStringID(MApp_GetLanguageBySILanguage((EN_SI_LANGUAGE)stAudInfo.aISOLangInfo[0].bISOLangIndex), FALSE);
        ...
        case HWND_AUDLANG_ITEM9_AUDIO_TYPE2_ICON:
            ...
            if(IsAtscInUse() || IsATVInUse())
            {
                 ...
                 else if( MApp_ZUI_ACT_GetAudioFormatStatus(u8Index, STATE_AUDIO_AC3_TYPE))
                 {
					 ...
					 if(MApi_AUDIO_GetAC3PInfo(Audio_AC3P_infoType_AC3Type) == 1) // Means it is AC3 only
					 {
						 if (ds_type == DS_FOCUS)
						 {
					 ...
					 else if(MApi_AUDIO_GetAC3PInfo(Audio_AC3P_infoType_AC3Type) == 2) // Means it is AC3 PLUS
					 {
						 if (ds_type == DS_FOCUS)
						 {
