...
static void _MApp_ChannelList_Init(U8 u8ServiceType)
{
    ...
    printf("UI_MApp_ChannelList_Init: u8ServiceType=%bu\n", (U8)u8ServiceType);
...
BOOLEAN MApp_ZUI_ACT_ExecuteChannelListAction(U16 act)
{
    ...
    {
        pChListData->u16CurCh = 0;
        pChListData->u16TotalChCount = msAPI_CHPROC_CM_CountProgram((MEMBER_SERVICETYPE)E_SERVICETYPE_DTV, E_PROGACESS_INCLUDE_VISIBLE_ONLY);
        _MApp_ChannelList_Handler(pChListData->u16CurCh, 0, pChListData->u16TotalChCount, CHANNEL_LIST_COMMAND_INIT);
        gu8VerChange = FALSE;
    }
#endif
       ...
       case HWND_MENU_CHLIST_SOFTKEYBOARD_TITLE_M:
        ...
        case HWND_OP_MENU_ITEM9_DELIVERY_SYS_TYPE:
        {
          if(_MApp_ZUI_ACT_IsOpProfileMenuMode())
          {
            ...
            U8 i = 0;
            g_wProgramPosition = _u8OpMenuCurrentPage*CHANNEL_LIST_PAGE_NUM + u8VisIndex;
...
//Channel number
                MApp_UiMenuFunc_ConvertProgramNumber2Str(&MApp_ChanProc_GetChannelPointer(VIR_CH_IDX(g_wProgramPosition+1))->stVirtualChNum, au8Section);
                MApp_ChanProc_RefreshChannelBuffer(MAIN_LIST);
...
//Channel name
                ...
                pu8String = MApp_ChanProc_GetChannelPointer(VIR_CH_IDX(g_wProgramPosition+1))->acServiceName;
...
#endif
            {
                ...
                #endif
               {
                ...
                else
                {
                    ...
                    #if ENABLE_S2//ENABLE_QSORT_SERVICE
                        else if(_eChannelListMode == MODE_CHLIST_S2||_eChannelListMode == MODE_CHLIST_FIND_BYNAME)
                        {
                            MEMBER_SERVICETYPE bServiceType;
                            U16 u16Number = 0;
                            msAPI_CHPROC_CM_GetAttributeOfOrdinal((MEMBER_SERVICETYPE)pChListData->u8ServiceType, g_wProgramPosition, ( BYTE *)&bServiceType, E_SERVICE_ATTR_TYPE, E_PROGACESS_INCLUDE_VISIBLE_ONLY);
                            ...
                            else if (bServiceType == E_SERVICETYPE_RADIO)
                            {
                                ...
                                *str++ = CHAR_SPACE; //p_string_buffer[u8_idx++] = CHAR_SPACE;
                            #if(ENABLE_S2_PREDIT_SWITCH_LIST)
                                g_wProgramPosition = g_wProgramPosition - DTVcount;
                                MApp_CharTable_GetServiceNameToUCS2(bServiceType, g_wProgramPosition, str, KEEP_CONTROL_CODE_NONE);
                            #else
                            ...
                            else if (bServiceType == E_SERVICETYPE_DATA)
                            {
                                ...
                                *str++ = CHAR_SPACE; //p_string_buffer[u8_idx++] = CHAR_SPACE;
                            #if(ENABLE_S2_PREDIT_SWITCH_LIST)
                                g_wProgramPosition = g_wProgramPosition - DTVcount - Radiocount;
                                MApp_CharTable_GetServiceNameToUCS2(bServiceType, g_wProgramPosition, str, KEEP_CONTROL_CODE_NONE);
                            #else
...
U16 MApp_ZUI_ACT_GetChannelListDynamicBitmap(HWND hwnd, DRAWSTYLE_TYPE ds_type)
{
    ...
    switch(hwnd)
    {
        ...
        if (IsAtscInUse() || IsATVInUse())
        {
            ...
            U32 u32ShowDTVIcon = FALSE;
            U8 u8ShowRadioIcon = FALSE;
            ...
            ptmpstVirtualCh = MApp_ChanProc_GetChannelPointer(VIR_CH_IDX(g_wProgramPosition+1));
        #if 0//(NEW_VIRTUAL_CHANNEL_STRUCTURE == ENABLE)
            MApp_ChanProc_GetRfChannelPointer(ptmpstVirtualCh, &pRf_Ch);
            if ( pRf_Ch.stTPSetting.fModulation == DEMOD_ATSC_MODE_NTSC )
        #else
            //if ( ptmpstVirtualCh->stTPSetting.fModulation == DEMOD_ATSC_MODE_NTSC )
            if ( MApp_ChanProc_Is_ChannelAtv(ptmpstVirtualCh) )
        #endif
                u32ShowDTVIcon = SW_HIDE;
            else
                u32ShowDTVIcon = SW_SHOW;


            if(u32ShowDTVIcon == SW_SHOW)
            {
                if((ptmpstVirtualCh->u16VideoPid == 0) && (ptmpstVirtualCh->astSoundTrack[0].u16AudioPid !=0))
                    u8ShowRadioIcon = SW_SHOW;
            }
        ...
        case HWND_CHLIST_ITEM9_FAV:
            ...
            if (IsAtscInUse() || IsATVInUse())
            {
                ...
                if(IS_FAV_CH_ATSC(g_wProgramPosition+1))
                   u32ShowFlagFav = SW_SHOW;
                else
                   u32ShowFlagFav = SW_HIDE;
