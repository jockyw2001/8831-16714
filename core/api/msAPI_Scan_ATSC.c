    ...
    switch(enNTSCScanState)
    {
        case STATE_NTSC_SCAN_INIT:
            ...
            msAPI_IFDM_SetVIF_Notch();
            {
                ...
                while (msAPI_Timer_GetTime0()-u32StartTime < VD_STABLE_TIME)
                {
...
EN_SCAN_RESULT msAPI_VSB_Scan(MS_TP_SETTING_ATSC *pstTPSetting)
{
    ...
    switch(enVSBScanState)
    {
        ...
        case STATE_VSB_CHECK_FSYNC_LOCK:
            DEBUG_ATSC_SCAN_FLOW(printf("case STATE_VSB_CHECK_FSYNC_LOCK:\n"););
            {
               if(msAPI_Timer_DiffTimeFromNow(Check_demod_fsyc_time) > DEMOD_FIRST_CHECK_LOCK_DELAY)
               {
                   if(msAPI_VSB_Check_FsyncLock())
                   {
                       Result = LOCK;
                       DEBUG_ATSC_SCAN_FLOW(printf("msAPI_VSB_Check_FsyncLock() -> LOCK\n"););
                   }
                   else
                   {
                       if((msAPI_Timer_DiffTimeFromNow(Check_demod_fsyc_time) > DEMOD_FIRST_CHECK_LOCK_PERIOD))
                       {
                           Result = UNLOCK;
                           DEBUG_ATSC_SCAN_FLOW(printf("msAPI_VSB_Check_FsyncLock() -> UNLOCK\n"););
                       }
                       else
                       {
                           Result = CHECKING;
