...
#if (ENABLE_ATSC_1000_CHANNEL)//(NEW_VIRTUAL_CHANNEL_STRUCTURE == ENABLE)
...
typedef struct
{
    ...
    U8  u8Region5Name[36];   // The max Region name length is 32. 36 is : 2(to Record Actual length) +32 (Max Region Name Length)+2(End of string bytes(NULL Bytes))
...
typedef struct
{
    U8  u8VersionNo;
    U8  u8NoDimension;
    U8  u8Region5Name[33];   // The max Region name length is 32.
