...
#define _BIT31 (1L<<31)
    ...
    u32CertKeyAddr = SECURITY_R2_ADR + KEY_BANK__CERT_OFFSET + _BIT31;
    //memcpy( au8certDat, (U8 *)u32CertKeyAddr, sizeof(au8certDat));

    printf("u32CertKeyAddr=%x\n", u32CertKeyAddr);
...
#if HDCP22_INTERNAL_CEERTRX_KEY // get key from DRAM(descrypted by SecurityR2)
    msAPI_MIU_ReadMemBytes(u32CertKeyAddr, au8certDat, CERTRX_SIZE );
    memcpy(sg_pu8HDCP22Data, au8certDat, CERTRX_SIZE);
    memcpy((void *)((ULONG)sg_pu8HDCP22Data|BIT31), au8certDat, CERTRX_SIZE);
#else
    msAPI_MIU_ReadMemBytes(u32CertKeyAddr, sg_pu8HDCP22Data, CERTRX_SIZE );
...
//    memcpy((void *)((ULONG)sg_pu8HDCP22Data|BIT31), u32CertKeyAddr, CERTRX_SIZE);
    ...
    for ( u16i = 0; u16i < CERTRX_SIZE; u16i++ )
    {
        printf("0x%02X ", * ((U8 *)(u32CertKeyAddr+u16i)));
        if ( (u16i+1)%16 == 0)
            printf("\r\n");
    }
    printf("\r\n");
...
{
   // static BOOL s_bHDCP_Initial_Done = FALSE;
  #if  1 // (CustomerType == CustomerType_LG_Unicorn)
    static BOOL s_bAES_DeCode_Done = FALSE;
    ...
    //ToDo Refine these code!!!
  #if 1//(CustomerType == CustomerType_LG_Unicorn)
    if (1) // Cus_LGE_HDCP_Key_Info_Status() == E_HDCP_KEY_INFO_RECEIVE_VALID_DONE)
    {

//PRINT_SECURITY("if(1)");
        if(1)  // mdrv_combo_HDCP2RomCodeDone())  // daniel@ wait ROM code interrupt
        {
            static U8 once;
            if(!once)
            {
            ...
            // no DP in U11
            //mapi_DP_SetHDCP2CallBackFunction(mdrv_combo_HDCP2RxEventProc, tCOMBO_HDCP2_CERTRX_TABLE);
            s_bAES_DeCode_Done = TRUE;
            //printf("\n****************AES_DeCode_Done = TRUE\n");
        }

#if 0
        if (s_bAES_DeCode_Done == TRUE)
        {
            if (Cus_LGE_HDCP_AES_Key_DeCodeDone())
            {
                mapi_combo_KeyInitial();

                s_bHDCP_Initial_Done = TRUE;
                //printf("\n****************HDCP_Initial_Done = TRUE\n");
            }
        }
#endif
    }
