...
U8 MApp_FileBrowser_GetCurrentLongName(U32 MIUAddr,U8 charLen)
{
    ...
    else
    {
        ...
        return msAPI_FCtrl_EntryGetLongNameByFileEntry(fileBrowserEnv[u8CurrentEnvHandle].u8EnvId,
                    &fileBrowserEnv[u8CurrentEnvHandle].fileEntry, MIUAddr,charLen, &bShortNameOnly);
#else
...
#if OBA2
    {
        BOOL pbShortNameOnly;
        len = msAPI_FCtrl_EntryGetLongNameByFileEntry(fileBrowserEnv[u8CurrentEnvHandle].u8EnvId,
                    &fileBrowserEnv[u8CurrentEnvHandle].fileEntry, (U32)(pString),128, &isLongNameOnly);
    }
#else
...
{
#if OBA2
    printf("currentEntry: EnvironmentID=%u, Length=%lu, pathname:%s\n",(U8)fileBrowserEnv[u8CurrentEnvHandle].fileEntry.EnvironmentID, (U32)fileBrowserEnv[u8CurrentEnvHandle].fileEntry.EntryFileLength, (U32)fileBrowserEnv[u8CurrentEnvHandle].fileEntry.fullname);
#else
...
void MApp_FileBrowser_GetFileEntryFullpath(FileEntry *fileEntry, char *fullpath)
{
#if OBA2
    char *tmp = NULL;

    strcpy(fullpath, fileEntry->fullname);
