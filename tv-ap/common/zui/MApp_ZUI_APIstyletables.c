...
void _MApp_ZUI_API_DefaultOnPaint(HWND hWnd, PAINT_PARAM * param, BOOLEAN bDrawText)
{
    ...
    if (bDrawText)
    {
        ...
        else if (param->bIsFocus) //the same focus group
        {
            ...
            if (focused == MApp_ZUI_API_GetParent(hWnd))
            {
                if ((GETFOCUSDRAWSTYLE(hWnd)->component == CP_TEXT_OUT) || (GETFOCUSDRAWSTYLE(hWnd)->component ==  CP_TEXT_OUT_DYNAMIC))
                {
                    DRAW_TEXT_OUT * paramTextOut = (DRAW_TEXT_OUT*)&_Zui_TextOut_List[GETFOCUSDRAWSTYLE(hWnd)->u16Index];
                    MApp_TTSControlSetInputStringID(paramTextOut->StringID);
