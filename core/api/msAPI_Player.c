...
{
    if (g_wCurrent_AudioType != E_PLAY_AUDIO_TYPE_INVALID)
    {
        #if (defined(S3PLUS) || defined(S4LITE))
        msAPI_Player_AudioInit((enPlayAudioType)g_wCurrent_AudioType, E_PLAY_SOURCE_LIVE, bEnableAD, E_PLAY_AUDIO_TYPE_INVALID);
        #else
        msAPI_Player_AudioInit((enPlayAudioType)g_wCurrent_AudioType, E_PLAY_SOURCE_LIVE, bEnableAD, (enPlayAudioType)g_enCurrent_AudioDescriptorType);
        #endif
    }
}
...
{
    #ifdef PVR_8051
    msAPI_Scaler_SetFreezeImg(TRUE);
    #else
    #ifdef PVR_UTOPIA
    MApi_XC_FreezeImg(TRUE, MAIN_WINDOW);
    #else
    mvideo_sc_set_freezeimg(TRUE, MAIN_WINDOW);
    #endif
    #endif
}
...
{
    #ifdef PVR_8051
    msAPI_Scaler_SetFreezeImg(FALSE);
    #else
    #ifdef PVR_UTOPIA
        if(IsDTVInUse())
        {
    ...
    #else
    mvideo_sc_set_freezeimg(FALSE, MAIN_WINDOW);
    #endif
    #endif
}
...
void msAPI_Player_ScalerMute(U32 u32MuteTime)
{
    ...
    #else
    msAPI_Scaler_GenerateBlackVideo(TRUE);
    msAPI_Scaler_SetScreenMute(E_SCREEN_MUTE_TEMPORARY, ENABLE, u32MuteTime);
    #endif
}
