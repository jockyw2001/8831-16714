...
#if ((BRAZIL_CC )|| (ATSC_CC == ATV_CC)|| ENABLE_SBTVD_BRAZIL_APP)
//#include "msAPI_vbi.h" //remove by jany
...
#include "apiXC_Dlc.h"
#include "Utl.h"
#if (ENABLE_VD_PACH_IN_CHINA||ENABLE_DVB_TAIWAN_APP || ENABLE_SBTVD_BRAZIL_APP || (TV_SYSTEM == TV_NTSC) )
...
#endif*/
#ifdef DEBUG_VIDEO
    #define debugVideoPrint(a,b)    debugPrint(a,b)
#else
...
#endif
{
    ...
    //debugVideoPrint("Set Video Standard:0x%x", eDetectedStandard);

    //printf("msAPI_AVD_SetVideoStandard(%u), bVdInputIs50Hz=%u\n", eDetectedStandard, bVDInputIs50Hz);
    ...
    if( eDetectedStandard == E_VIDEOSTANDARD_NOTSTANDARD )
    {
        ...
        printf("VD: SetStd: No sync, 50Hz=%u => Use Std=%s\n", bVDInputIs50Hz, msAPI_AVD_Get_StdString(eStandard) );
    }
    else
    {
        /*#if(sPowerDown_Function==sSimple_PowerDownFun)
...
// please call every 5 ms
#define VD_CHECK_STD_PERIOD     5 // ms
...
#define VD_CHECK_STD_STABLE_THRESHOLD_CVBS              (40/VD_CHECK_STD_PERIOD) // 40ms
#define VD_CHECK_STD_STABLE_THRESHOLD_ATV_CUR_NO_STD    (40/VD_CHECK_STD_PERIOD) // 40ms
#define VD_CHECK_STD_STABLE_THRESHOLD_ATV_AUTO_SCAN     (40/VD_CHECK_STD_PERIOD) // 40ms
#define VD_CHECK_STD_STABLE_THRESHOLD_ATV_NORMAL        (40/VD_CHECK_STD_PERIOD) // 40ms
...
static void msAPI_AVD_CheckVideoStandard(void)
{
    ...
    //printf("msAPI_AVD_CheckVideoStandard %d\r\n",eDetectedStandardCurrent);
...
#if(ENABLE_ATSC)
    if( IsATVInUse() && msAPI_AVD_IsSyncLocked() )
    {
        eDetectedStandardCurrent = E_VIDEOSTANDARD_NTSC_M;
        // Clear 50Hz flag
        u16Status &= (~VD_VSYNC_50HZ);
    }
#endif
    ...
    switch (m_eStandardDetectionState)
    {
        case E_DETECTION_START:
            ...
            m_eCurrentVideoStandard = E_VIDEOSTANDARD_NOTSTANDARD;
            DEBUG_VD_CHECK_STD( printf(" m_eFSMDetectedStandard=%u\n", m_eFSMDetectedStandard ); );
            break;
        ...
        case E_VD_STATE__UNSTABLE:
            ...
            if( m_eFSMDetectedStandard == eDetectedStandardCurrent )
            {
                ...
                if (eDetectedStandardCurrent==E_VIDEOSTANDARD_NOTSTANDARD)
                {
                    ...
                    u16DebounceValue = (msAPI_Tuner_IsTuningProcessorBusy()) ? NO_SIGNAL_DETECT_THRESHOLD_SCAN : NO_SIGNAL_DETECT_THRESHOLD;
                }
                else
                {
                ...
                if( u8StdDetStableCnt >= u16DebounceValue )
                {
                  ...
                  #if (ENABLE_SBTVD_BRAZIL_APP)//ISDB Open PAL_NC CC
        ...
        case E_VD_STATE__STABLE: // signal is stable
            ...
            if ( m_eFSMDetectedStandard != eDetectedStandardCurrent)
            {
                ...
                //printf(" [VD %u] ", eDetectedStandardCurrent);
...
static void msAPI_AVD_CheckAspectRatio(void)
{
    ...
    {
        if( (m_eVideoSource == E_INPUT_SOURCE_ATV && m_bIsAutoAV2Active == TRUE) || ( m_eVideoSource == E_INPUT_SOURCE_SCART2) )
        {
            cCurrentScart2ID = GetScart2IDLevel();
        ...
        else if( (m_eVideoSource == E_INPUT_SOURCE_ATV && m_bIsAutoAV1Active == TRUE)|| (m_eVideoSource == E_INPUT_SOURCE_SCART1) )
        {
            cCurrentScart1ID = GetScart1IDLevel();
