    ...
    switch(act)
    {
        ...
        case EN_EXE_SET_TARGET_REGION_RIGHT:
            if(pInfo != NULL)
            {
                ...
                {
                    if (pInfo->pCountryInfo[u8TargetRegionCountryIdx].u16PrimaryRegionNum >0)
                    {
                ...
                {
                     if (pInfo->pCountryInfo[u8TargetRegionCountryIdx].pPrimaryRegionInfo[u16PrimaryRegionInx].u16SecondaryRegionNum >0)
                    {
                ...
                {
                    if (pInfo->pCountryInfo[u8TargetRegionCountryIdx].pPrimaryRegionInfo[u16PrimaryRegionInx].pSecondaryRegionInfo[u16SecondaryRegionInx].u16TertiaryRegionNum >0)
                    {
...
LPTSTR MApp_ZUI_ACT_GetAutoTuningDynamicText(HWND hwnd)
{
    ...
    switch(hwnd)
    {
        ...
        case HWND_OAD_TUNE_SCAN_PROGRESS_CH_UHF:
            ...
            #else
            return MApp_ZUI_API_GetU16String(MApp_OAD_GetOadRfChannel());
            #endif
    #endif
          ...
          #endif
        {
            ...
            #endif
                {
                    if ( ( (21 <= pstScanMenuSetting->u8RFChannelNumber)
                         &&(pstScanMenuSetting->u8RFChannelNumber <= 69 )
                  #if ENABLE_DVB
                       &&(MApp_TopStateMachine_GetTopState() != STATE_TOP_DTV_SCAN)
                  #endif
                        )
                       ||( (21 <= MApp_Scan_Get_CurRFChannel())
                         &&(MApp_Scan_Get_CurRFChannel() <= 69)
                  #if ENABLE_DVB
                       &&(MApp_TopStateMachine_GetTopState() == STATE_TOP_DTV_SCAN)
                  #endif
                       ))
