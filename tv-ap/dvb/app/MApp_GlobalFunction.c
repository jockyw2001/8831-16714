...
void MApp_ConvertSeconds2StTime ( U32 u32SystemTime, ST_TIME *pstTime )
{
    ...
    //printf("MApp_ConvertSeconds2StTime(u32SystemTime=%u)\n", u32SystemTime);
...
{
    //printf("MApp_SetLocalSystemTime(t=%u)\n", u32LocalSystemTime);
    DEBUG_WAKE_UP( printf("MApp_SetLocalSystemTime(u32LocalSystemTime=%u)\n", u32LocalSystemTime); );
...
#ifdef sNewAudioMuteStyle
//------------------------------------------------
void MApp_Audio_MuteOnOff(BOOL bMute)
{
	if(bMute)
	{
		MUTE_On();
...
//------------------------------------------------
void MApp_Audio_MuteOnOff_ByEarPhone(BOOL bMute)
{
	if(bMute)
	{
		MUTE_On();
...
{
    return u16AudioStreamType;
}

void MApp_ReMapAudioLanguage(void)
{
    ...
    for(iu8Loop_i = 0; iu8Loop_i< pstVirtualCh->u8AudLangNum; iu8Loop_i++)
    {
        if((pstVirtualCh->astSoundTrack[iu8Loop_i].u8AudLangIdx != SOUND_AUDIO_LAN_ENGLISH)&&
            (pstVirtualCh->astSoundTrack[iu8Loop_i].u8AudLangIdx != SOUND_AUDIO_LAN_FRENCH)&&
            (pstVirtualCh->astSoundTrack[iu8Loop_i].u8AudLangIdx != SOUND_AUDIO_LAN_SPANISH)&&
            (pstVirtualCh->astSoundTrack[iu8Loop_i].u8AudLangIdx != SOUND_AUDIO_LAN_KOREA)&&
            (pstVirtualCh->astSoundTrack[iu8Loop_i].u8AudLangIdx != SOUND_AUDIO_LAN_JAPAN)&&
            (pstVirtualCh->astSoundTrack[iu8Loop_i].u8AudLangIdx != SOUND_AUDIO_LAN_CHINESE)
        )
        {
               memcpy( &astgSoundTrack[iu8Loop_j],&pstVirtualCh->astSoundTrack[iu8Loop_i],sizeof( MS_SOUND_TRACK_ATSC ) );
               iu8Loop_j++;
...
{
    PRINT_CURRENT_LINE();
    //printf("MApp_MuteAvByLock(bEnableMute=%u)\n", bEnableMute);
...
BOOL MApp_Is_InAutoScan_ATSC(void)
{
    ...
    {
        if( MApp_TopStateMachine_GetTopState() == STATE_TOP_SCAN )
        {
            return TRUE;
        }
    }
...
//---------------------------------------------------
U16 MApp_UiMenuFunc_FactoryGetLSNiceADCReso(void)
{
	U16 TempVal;
	TempVal = ((G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Max-G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Min)/100);
	return TempVal;
}
//---------------------------------------------------
U16 MApp_UiMenuFunc_FactoryGetLSNiceAdjustStepMin(void)
{
	U16 TempVal;
	if((G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Min/10)<=0)
		TempVal = 1;
	else	
		TempVal = (G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Min/10);
	if(TempVal>=255)	
		TempVal = 255;
	return TempVal;	
}
//---------------------------------------------------
U16 MApp_UiMenuFunc_FactoryGetLSNiceAdjustStepMax(void)
{
	U16 TempVal;
	if(((65535-G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Max)/10)<=0)
		TempVal = 1;
	else	
		TempVal = ((65535-G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Max)/10);
	if(TempVal>=255)	
		TempVal = 255;		
	return TempVal;	
}
//---------------------------------------------------
U16 MApp_UiMenuFunc_FactoryGetLSNiceLimitMin(void)
{
	U16 TempVal;
	TempVal = (G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Max - 1000);
  return TempVal;
}
//---------------------------------------------------
U16 MApp_UiMenuFunc_FactoryGetLSNiceLimitMax(void)
{
	U16 TempVal;
	TempVal = (G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Min + 1000);
  return TempVal;	
}
//---------------------------------------------------
void LightSensorChip_Init(void)
{
#if(SENSOR_CHIP==TSL2571_SENSOR)	
	MDrv_IIC_WriteByte(TSL2571_DEVICE_ADDRESS,TSL2571_ENABLE ,0x02);
	MDrv_IIC_WriteByte(TSL2571_DEVICE_ADDRESS,TSL2571_ATIME ,0xC0);//0xDB);
	MDrv_IIC_WriteByte(TSL2571_DEVICE_ADDRESS,TSL2571_CONTROL ,0x02);
	MDrv_IIC_WriteByte(TSL2571_DEVICE_ADDRESS,TSL2571_ENABLE ,0x03);
	MsOS_DelayTask(200);
#endif
}
//===============================================================
BOOLEAN LightSensorChip_CheckDevice(void)
{
	U8 uLsb;
	#if(SENSOR_CHIP==TSL2571_SENSOR)	
	MDrv_IIC_ReadByte( TSL2571_DEVICE_ADDRESS, TSL2571_CONTROL, &uLsb );	
	//printf("uLsb %d \n",uLsb);
	if(uLsb==0x02)	
	#endif
	{
...
//===============================================================
U16 LightSensorChip_GetADC(void)
{
	U16 sGetVal;
	U8 uMsb,uLsb;
	#if(SENSOR_CHIP==TSL2571_SENSOR)
	MDrv_IIC_ReadByte( TSL2571_DEVICE_ADDRESS, TSL2571_C1DATA, &uLsb );
	MDrv_IIC_ReadByte( TSL2571_DEVICE_ADDRESS, TSL2571_C1DATAH, &uMsb );	
	#endif
	sGetVal = ((uMsb<<8)|uLsb);
	return sGetVal;
}
//===============================================================
BOOLEAN LightSensor_Init(void)
{
	BOOLEAN sGetVal;
	LightSensorChip_Init();
	sGetVal = LightSensorChip_CheckDevice();
	return sGetVal;
}
//===============================================================
BOOLEAN CheckLS_Change(U8 sCheckTick)
{
	U16 sGetVal;
	U16 sCheckDiffV;
	sCheckDiffV = ((G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Max - G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Min)/MApp_UiMenuFunc_FactoryGetLSNiceADCReso());
	sGetVal = LightSensorChip_GetADC();
	if(sGetVal<=G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Min)
		sGetVal = G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Min;
	if(sGetVal>=G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Max)
		sGetVal = G_FACTORY_SETTING.stFacLightSensorSetting.u16LS_Max;
	if(abs(sGetVal - u16CurrentLightSensorValue)>=sCheckDiffV)
	{
		if(u8LSCheckCounter>=sCheckTick)
		{
			u16CurrentLightSensorValue = sGetVal;
			u8LSCheckCounter = 0;
			bDetectLS_ADC = FALSE;								
			return TRUE;
		}
