...
BOOLEAN msAPI_Tuner_Wait(U32 WaitTiming)
{
    ...
    #if (FRONTEND_IF_DEMODE_TYPE  == MSTAR_INTERN_VIF)
    {
        ...
        while (msAPI_Timer_GetTime0()-u32StartTime < WaitTiming)
        {
...
BOOLEAN msAPI_NTSC_GetFreq_Offset(MS_TP_SETTING_ATSC *pstModifyTPSetting)
{
    ...
    return FALSE;
	 ...
	 U8 count=0;
        ...
        {
            ///extern BOOLEAN fScanTypeIsAuto;
            extern BOOLEAN MApp_Scan_GetVDLockStatus();
...
// + Return    : BOOLEAN
//------------------------------------------------------*/
BOOLEAN Set_Tuner_Freq(U16 u16Frequency, EN_DEMOD_ATSC_MODE enModulation)
{
    U32 freq = (U32)(u16Frequency)*FREQ_STEP;


    DEBUG_API_TUNER_FREQ_ATSC(printf("Set_Tuner_Freq(u16Frequency=%u, enModulation=%u)\n", u16Frequency, enModulation););
    DEBUG_API_TUNER_FREQ_ATSC( printf(" => freq=%u\n", freq); );
    ...
    else if(enModulation == DEMOD_ATSC_MODE_8VSB)
    {
        ...
        devDigitalTuner_SetFreq(freq+1750,E_RF_CH_BAND_6MHz);
...
{
    DEBUG_API_TUNER_FREQ_ATSC( printf("msAPI_Tuner_SetFreq(u16Frequency=%u, enModulation=%u)\n", u16Frequency, enModulation); );
    ...
    msAPI_Tuner_PaThruI2C(ENABLE);
        ...
        // Config Tuner
    if(FALSE == Set_Tuner_Freq(u16Frequency, enModulation))
    {
