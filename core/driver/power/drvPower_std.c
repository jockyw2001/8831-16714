...
void MDrv_Power_Standby_CheckWakeupDevice(void)
{
    ...
    DEBUG_PM_WAKEUP( printf("stPowerGenSetting.wPM_WakeUpDevice=0x%X\n", stPowerGenSetting.wPM_WakeUpDevice ); );
    ...
    if( IsHDMIInUse() )
    {
        ...
        { // Power down by User or AutoSleep, => Need wakeup by DVI
        #if( ENABLE_PM_WAKEUP_DVI_PORT_SEPERATE )
  ...
  #if (ENABLE_CEC) // Wakeup from PM condition: CEC on and auto TV on
    ...
    //DEBUG_PM_WAKEUP( printf("2 stPowerGenSetting.wPM_WakeUpDevice=0x%X\n", stPowerGenSetting.wPM_WakeUpDevice  ); );
    DEBUG_PM_WAKEUP( MDrv_Power_PrintWakeUpFlag(stPowerGenSetting.wPM_WakeUpDevice); );
}
