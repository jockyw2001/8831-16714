...
#define CAPTURE_AUDIO_BUFFER_END            (CAPTURE_AUDIO_BUFFER_ADR+CAPTURE_AUDIO_BUFFER_LEN)
...
typedef struct
{
    U8 u8ValidDevice, u8TotalDriveNum;
...
typedef struct
{
    ...
    U32 u32PrePCMOffset;        /// Record the last time posistion which DSP write to memory
...
static U16  m_u16DeviceValidStatus;
...
#if(STORE_SUBTITLE_ENTRY_TO_SUBTITLE_BUFFER || OLD_BGM) //fix compile error
...
static BOOLEAN _MApp_MPlayer_MapDriveToPhysicalDevice(void)
{
    ...
    U8 u8PartitionIdx;
    U8 u8DriveNum;
    U8 u8IndexEnd = MSD_USB_2_INDEX_END;
...
#if ENABLE_MPLAYER_SORT
    #if ENABLE_MPLAYER_SORT_EX
        //In ENABLE_MPLAYER_SORT_EX, playlist sort by physical index.
        m_u16CurDirectoryPhy[m_u16CurDirectoryDepth] = _MApp_MPlayer_LookupPhysicalIndex(u16FileIdx);
    #else
...
#if (ENABLE_OGG)
    else if((pFileBufPtr[0] == 'O') && (pFileBufPtr[1] == 'g') && (pFileBufPtr[2] == 'g') && (pFileBufPtr[3] == 'S'))
    {
        //ogg file
...
#endif
#if (ENABLE_AMR)
    else if(!memcmp(pFileBufPtr,"#!AMR-WB\n",9))
    {
        //awb file
...
#endif
#if (ENABLE_FLAC)
    else if((pFileBufPtr[0] == 'f') && (pFileBufPtr[1] == 'L') && (pFileBufPtr[2] == 'a') && (pFileBufPtr[3] == 'C'))
    {
...
#endif
#if (ENABLE_AC3)
    else if(((pFileBufPtr[0] == 0x0B) && (pFileBufPtr[1] == 0x77))  || ((pFileBufPtr[0] == 0x77) && (pFileBufPtr[1] == 0x0B)))
    {
...
static BOOLEAN _MApp_MPlayer_ReCalcDeviceDriveInfo(void)
{
    ...
    u16DeviceValidMask = BMASK((MSD_USB_INDEX_START+MSD_USB_NUM-1):MSD_USB_INDEX_START);
    //printf("USB1: u16DeviceValidMask=%X\n", u16DeviceValidMask);
    m_MPlayerDeviceStatus[0].u16LunValid = m_u16DeviceValidStatus & (u16DeviceValidMask); // daniel_check
...
/*
  #if (ENABLE_USB)
    m_MPlayerDeviceStatus[u8DeviceCnt].u16LunValid = m_u16DeviceValidStatus & 0xF0;
  #else
    m_MPlayerDeviceStatus[u8DeviceCnt].u16LunValid = m_u16DeviceValidStatus & 0xF;
  #endif
*/

  u16DeviceValidMask = BMASK((MSD_USB_2_INDEX_START+MSD_USB_2_NUM-1):MSD_USB_2_INDEX_START);
  //printf("USB2: u16DeviceValidMask=%X\n", u16DeviceValidMask);
  m_MPlayerDeviceStatus[u8DeviceCnt].u16LunValid = m_u16DeviceValidStatus & (u16DeviceValidMask); // daniel_check
    ...
    if(m_MPlayerDeviceStatus[u8DeviceCnt].u16LunValid)
    {
        ...
        //m_MPlayerDeviceStatus[u8DeviceCnt].u16LunValid = (m_u16DeviceValidStatus & 0xF0)>>4;
        //if(m_MPlayerDeviceStatus[u8DeviceCnt].u16LunValid)
...
static BOOLEAN _MApp_MPlayer_AddCurFileToPlayingList(void)
{
    ...
    #if ENABLE_MPLAYER_SORT_EX
        //In ENABLE_MPLAYER_SORT_EX, playlist sort by physical index.
        m_u16CurDirectoryPhy[m_u16CurDirectoryDepth] = _MApp_MPlayer_LookupPhysicalIndex(m_u16CurFileIdx);
    #else
...
void _MApp_MPlayerPrintFileName(void)
{
    U16 *pu16FileName=NULL;
    U8 strLen;
    pu16FileName = (U16*)msAPI_Memory_Allocate(SUBTITLE_LYRIC_LONG_FILENAME_SIZE,
    ...
    U8 u8i;
    for(u8i=0; u8i<strLen; u8i++)
    {
        printf("%s", pu16FileName+u8i);
    }
    printf("\n");
}
...
//    m_u16DeviceConnectStatus = MApp_MassStorage_DeviceConnectionDetect();
...
#endif
    {
        ...
        m_u16DeviceValidStatus = MApp_MassStorage_GetValidDevice();
...
enumMPlayerRet MApp_MPlayer_ConnectDrive(U16 u16DriveIdx)
{
    ...
    for(i=0; i<MApp_MassStorage_GetTotalDriveNum(); i++)
    {
        ...
        {
        #if ENABLE_USB
            U8 u8IndexEnd = MSD_USB_INDEX_END;
            if(m_DriveInfo[i].u8Lun < u8IndexEnd)
            {
                m_u8CurDeviceIdx = E_MPLAYER_USB0;
                bFound = TRUE;
                break;
...
//    }
    ...
    if(m_eMoviePreviewState == E_MOVIE_PREVIEW_RUNNING || m_eMoviePreviewState == E_MOVIE_PREVIEW_1ST_FRAME_DECODE)
    {
        msAPI_Scaler_SetBlueScreen(ENABLE, E_XC_FREE_RUN_COLOR_BLACK, DEFAULT_SCREEN_UNMUTE_TIME, MAIN_WINDOW);
      #if ENABLE_DVD
    ...
    if(m_eMoviePreviewState == E_MOVIE_PREVIEW_1ST_FRAME_DECODE)
    {
        if(bMute)
        {
...
#endif
        MApp_VDPlayer_Stop();
        MApi_XC_Mux_OnOffPeriodicHandler(SYS_INPUT_SOURCE_TYPE(MAIN_WINDOW), DISABLE);
//        if(m_u8ExternalSubtitleFileHandle != FCTRL_INVALID_FILE_HANDLE)
#if (ENABLE_SUBTITLE_DMP)
...
//******************************************************************************
U8 *MApp_MPlayer_QueryMusicInfo(EN_MP3_INFO eInfo,U8 *u8Length)
{
    if(m_MPlayerInfo[E_MPLAYER_BACKGROUND_PLAYER].eMediaSubType == E_MPLAYER_SUBTYPE_MP3)
    {
        if(u8Length != NULL)
            *u8Length = MApp_Music_GetInfoStringLength(eInfo);
        return MApp_Music_GetInfoString(eInfo);
    }
#if (ENABLE_AAC)
    else if(m_MPlayerInfo[E_MPLAYER_BACKGROUND_PLAYER].eMediaSubType == E_MPLAYER_SUBTYPE_AAC)
    {
        if(u8Length != NULL)
            *u8Length = MApp_Music_GetInfoStringLength(eInfo);
        return MApp_Music_GetInfoString(eInfo);
...
#endif
#if (ENABLE_WAV)
    else if(m_MPlayerInfo[E_MPLAYER_BACKGROUND_PLAYER].eMediaSubType == E_MPLAYER_SUBTYPE_WAV)
    {
        if(u8Length != NULL)
            *u8Length = MApp_Music_GetInfoStringLength(eInfo);
        return MApp_Music_GetInfoString(eInfo);
...
#endif
#if (ENABLE_OGG)
    else if(m_MPlayerInfo[E_MPLAYER_BACKGROUND_PLAYER].eMediaSubType == E_MPLAYER_SUBTYPE_OGG)
    {
        if(u8Length != NULL)
            *u8Length = MApp_Music_GetInfoStringLength(eInfo);
        return MApp_Music_GetInfoString(eInfo);
...
#endif
#if (ENABLE_WMA)
    else if(m_MPlayerInfo[E_MPLAYER_BACKGROUND_PLAYER].eMediaSubType == E_MPLAYER_SUBTYPE_WMA)
    {
...
#endif //#if (ENABLE_WMA)
#if (ENABLE_NON_PARSING_AUDIO_DATA_INPUT)
    else if(m_MPlayerInfo[E_MPLAYER_BACKGROUND_PLAYER].eMediaSubType == E_MPLAYER_SUBTYPE_LOAS)
    {
        if(u8Length != NULL)
            *u8Length = MApp_Music_GetInfoStringLength(eInfo);
        return MApp_Music_GetInfoString(eInfo);
...
#endif //#if (ENABLE_NON_PARSING_AUDIO_DATA_INPUT)
#if (ENABLE_FLAC)
    else if(m_MPlayerInfo[E_MPLAYER_BACKGROUND_PLAYER].eMediaSubType == E_MPLAYER_SUBTYPE_FLAC)
    {
        if(u8Length != NULL)
            *u8Length = MApp_Music_GetInfoStringLength(eInfo);
        return MApp_Music_GetInfoString(eInfo);
...
#endif
#if (ENABLE_AC3)
    else if(m_MPlayerInfo[E_MPLAYER_BACKGROUND_PLAYER].eMediaSubType == E_MPLAYER_SUBTYPE_AC3)
    {
        if(u8Length != NULL)
            *u8Length = MApp_Music_GetInfoStringLength(eInfo);
        return MApp_Music_GetInfoString(eInfo);
...
#if (ENABLE_AMR)
    else if(m_MPlayerInfo[E_MPLAYER_BACKGROUND_PLAYER].eMediaSubType == E_MPLAYER_SUBTYPE_AMR)
    {
        if(u8Length != NULL)
            *u8Length = MApp_Music_GetInfoStringLength(eInfo);
        return MApp_Music_GetInfoString(eInfo);
    }
    else if(m_MPlayerInfo[E_MPLAYER_BACKGROUND_PLAYER].eMediaSubType == E_MPLAYER_SUBTYPE_AWB)
    {
        if(u8Length != NULL)
            *u8Length = MApp_Music_GetInfoStringLength(eInfo);
        return MApp_Music_GetInfoString(eInfo);
...
void MApp_MPlayer_CheckUSBStatus(void)
{
    switch(m_MPlayerUSBDetect.eCurrentState)
    {
        ...
        case STATE_USB_DETECT_GET_DEVICE_AND_DRIVE:
            m_MPlayerUSBDetect.u8ValidDevice = MApp_MassStorage_GetValidDevice();
        ...
        case STATE_USB_DETECT_CHECK_PORT_CHANGED:
            {
                ...
                if((m_u16DeviceValidStatus != m_MPlayerUSBDetect.u8ValidDevice) ||
                   (m_MPlayerUSBDetect.u8TotalDriveNum != m_u8TotalDriveNum) ||
                   (bPort1Changed || bPort2Changed))
                {
                    m_u16DeviceValidStatus = m_MPlayerUSBDetect.u8ValidDevice;
...
void MApp_MPlayer_CheckUSBStatus(void)
{
    ...
    {
        u16ValidDevice = m_u16DeviceValidStatus;
        u8TotalDriveNum = m_u8TotalDriveNum;
    }

    //if((m_u16DeviceValidStatus != u8ValidDevice) ||
    //   (u8TotalDriveNum != m_u8TotalDriveNum))
    bPort1Changed = msAPI_MSDCtrl_IsPortChanged(MSD_PORT_1);
    bPort2Changed = msAPI_MSDCtrl_IsPortChanged(MSD_PORT_2);
    //printf("device and port remember ok. m_u16DeviceValidStatus=%bx,  m_u8TotalDriveNum=%bx  \n ",m_u16DeviceValidStatus, m_u8TotalDriveNum);
    if((m_u16DeviceValidStatus != u16ValidDevice) ||
       (u8TotalDriveNum != m_u8TotalDriveNum) ||
    (bPort1Changed || bPort2Changed))
    {
        ...
        if(u8NewDeviceIndex != 0xFF && (1 << u8NewDeviceIndex & u16ValidDevice))
        {
    ...
    #endif
        m_u16DeviceValidStatus = u16ValidDevice;
...
//                    if(eType < VIDEO_PLAYER_PLAYING_TYPE_NUM)
                    ...
                    #endif // #if (ENABLE_MOVIE_PREVIEW_1ST_FRAME)
                    }
                    m_eMovieMainExit = EXIT_VDPLAYER_NULL;
                }
            }
                ...
                ))
        {
            ...
            if(m_MPlayerInfo[eMPlayerIdx].eKnlFlag1 & E_MPLAYER_KNL_FLG1_PHOTO_PLAY)
            {
                ...
                if(m_u8DecodedPics && !(m_MPlayerInfo[eMPlayerIdx].eMediaPlayingStatus & E_MPLAYER_PHOTO_STATE_PAUSE))
                {
                    ...
                    #if (ENABLE_3D_PROCESS && (!ENABLE_4K2K))
                        // This MPO in 3D mode
...
BOOLEAN MApp_MPlayer_MusicMetaData_Probe_ByPlayListIndex(U16 u16Index,ST_MPLAYER_PROBE_INFO *pstTempMplayerProbeInfo,EN_MUSIC_INFO eMusicInfoType)
{
    ...
    MPLAYER_DBG(printf("u16BitRate:%d  u16SampleRate:%d \n",pstTempMplayerProbeInfo->u16BitRate,pstTempMplayerProbeInfo->u16SampleRate));
...
#define SHOWN_VOLUME_NAME_MAX_LEN        10
U8 MApp_MPlayer_GetVolumName(U8 u8DriveID,U8* pVolumStrBuf)
{
    U8 u8Length = 0;
    ...
    if(MApp_MPlayer_QueryTotalDriveNum()>0)
    {
        ...
        if(u8Length>0)
        {
            ...
            if(u8Length < (SHOWN_VOLUME_NAME_MAX_LEN-1))
            {
                for(u16ItemIdx=u8Length;u16ItemIdx<SHOWN_VOLUME_NAME_MAX_LEN;u16ItemIdx++)
                {
                    strBuf[u16ItemIdx] = 0x20;
                }
            }
            memcpy(pVolumStrBuf, strBuf, (SHOWN_VOLUME_NAME_MAX_LEN+SHOWN_VOLUME_NAME_MAX_LEN));
        }

        //add if to display the volume label centeredly
        if(u8Length > SHOWN_VOLUME_NAME_MAX_LEN)
            u8Length = SHOWN_VOLUME_NAME_MAX_LEN;
    }
...
enumCaptureRet MApp_MPlayer_CaptureAudioBegin(ST_AUDIO_CAPTURE_SET_INFO *pCaptureAudioinfo)
{
    ...
    m_AudioCapture.u32SampleRate = MApi_AUDIO_GetSampleRate();
    ...
    MApi_AUDIO_SetCommAudioInfo(Audio_Comm_infoType_Set_PCM_Capture_DDR_Info, CAPTURE_AUDIO_BUFFER_ADR, CAPTURE_AUDIO_BUFFER_LEN);
    MApi_SND_ProcessEnable(Sound_ENABL_Type_PCM_Capture, TRUE);
    m_AudioCapture.u32PrePCMOffset = (U32)MApi_AUDIO_GetCommAudioInfo(Audio_Comm_infoType_Get_PCM_Capture_Wptr);

    if(m_AudioCapture.u32PrePCMOffset >= CAPTURE_AUDIO_BUFFER_ADR && m_AudioCapture.u32PrePCMOffset <= CAPTURE_AUDIO_BUFFER_END)
    {
        eRet = E_CAPTURE_RET_OK;
    ...
    {
        eRet = E_CAPTURE_RET_FAIL;
        CAPTURE_ERR(printf("[Audio Capture] PCM address (0x%x) from DSP is wrong at capture begin.\n",m_AudioCapture.u32PrePCMOffset));
    }
...
enumCaptureRet MApp_MPlayer_CaptureAudioTask()
{
    ...
    u32CurrentPCMOffset = (U32)MApi_AUDIO_GetCommAudioInfo(Audio_Comm_infoType_Get_PCM_Capture_Wptr);
    if(u32CurrentPCMOffset >= CAPTURE_AUDIO_BUFFER_ADR && u32CurrentPCMOffset <= CAPTURE_AUDIO_BUFFER_END)
    {
    ...
    if(u32ResidualPCMSize >= CAPTURE_AUDIO_READ_LEN)
    {
        ...
        {
            u8Factor = CAPTURE_AUDIO_MAX_READ_FACTOR;
        }
        ...
        if((m_AudioCapture.u32PrePCMOffset+u32DesiredReadBytes) > CAPTURE_AUDIO_BUFFER_END)
        {
            u32Timer = msAPI_Timer_GetTime0();
            u32RemainBytes = msAPI_FCtrl_FileWrite(m_AudioCapture.u8WriteFileHandle, m_AudioCapture.u32PrePCMOffset, (CAPTURE_AUDIO_BUFFER_END-m_AudioCapture.u32PrePCMOffset));
            CAPTURE_PFM(printf("\033[15;31m[Audio Capture Peformace] Write size: 0x%x. Cost time : %d\n\033[0m",(CAPTURE_AUDIO_BUFFER_END-m_AudioCapture.u32PrePCMOffset),msAPI_Timer_DiffTimeFromNow(u32Timer)));
            u32DesiredReadBytes -= (CAPTURE_AUDIO_BUFFER_END-m_AudioCapture.u32PrePCMOffset);
            m_AudioCapture.u32DataChunkSize += (CAPTURE_AUDIO_BUFFER_END-m_AudioCapture.u32PrePCMOffset);
            //PCM Offset should return to the head of the buffer
            m_AudioCapture.u32PrePCMOffset = CAPTURE_AUDIO_BUFFER_ADR;
        }

        u32Timer = msAPI_Timer_GetTime0();
        u32RemainBytes += msAPI_FCtrl_FileWrite(m_AudioCapture.u8WriteFileHandle, m_AudioCapture.u32PrePCMOffset, u32DesiredReadBytes);
        ...
        if(_Mapp_MPlayer_WAVHeaderGenerator(u8Header, sizeof(u8Header), &m_AudioCapture))
        {
            ...
            CAPTURE_PFM(printf("\033[15;31m[Audio Capture Peformace] Header Update Write size: 0x%x. Cost time : %d\n\033[0m",CAPTURE_AUDIO_WAV_HEADER_LEN,msAPI_Timer_DiffTimeFromNow(u32Timer)));
        }
        else
        {
...
enumCaptureRet MApp_MPlayer_CaptureAudioStop(void)
{
    if(MApp_MPlayer_IsCapturingAudio())
    {
        ...
        MApi_SND_ProcessEnable(Sound_ENABL_Type_PCM_Capture, FALSE);
