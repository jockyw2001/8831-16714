...
{
#if ( ENABLE_DVB_TAIWAN_APP || ENABLE_SBTVD_BRAZIL_APP || (TV_SYSTEM == TV_NTSC) )
    msAPI_ATV_ResetChannelData(E_CHRESET_ALL);
#else
...
void msAPI_ATV_SetAudioStandard(BYTE u8cProgramNumber, AUDIOSTANDARD_TYPE eStandard)
{
    ...
    //printf("msAPI_ATV_SetAudioStandard_(u8cProgramNumber=%u, eStandard=%u)\n", u8cProgramNumber, eStandard);
...
#if (ENABLE_SBTVD_BRAZIL_APP || ENABLE_DVB_TAIWAN_APP)
...
#else
BOOLEAN msAPI_ATV_SetAudioMode(AUDIOSOURCE_TYPE eSource, AUDIOMODE_TYPE eAudioMode)
#endif
{
    ATV_MISC Misc;
...
void msAPI_ATV_GetMediumAndChannelNumber(BYTE u8ProgramNumber, MEDIUM * peMedium, BYTE * pcChannelNumber)
{
    ...
    if ( u8ProgramNumber >= MAX_ATVPROGRAM )
    {
        if ( peMedium != NULL )
        {
...
void msAPI_ATV_SortProgram(BYTE u8Start, BYTE u8Stop)
{
    ...
    if ( u8Start >= u8Stop || u8Stop >= MAX_ATVPROGRAM )
    {
        return;
    }
...
BYTE msAPI_ATV_GetFirstProgramNumber(BOOLEAN bIncludeSkipped)
{
    ...
    for (u8ProgramNumber = ATV_FIRST_PR_NUM; u8ProgramNumber < MAX_ATVPROGRAM; u8ProgramNumber++)
    {
...
BYTE msAPI_ATV_GetActiveProgramCount(void)
{
    ...
    for ( i = ATV_FIRST_PR_NUM; i < MAX_ATVPROGRAM; i++ )
    {
        if ( _IsIndexActive(i) )
        {
            ++count;
...
BYTE msAPI_ATV_GetPreviousFavoriteProgramNumber(BYTE u8BaseProgramNumber)
{
    ...
    do
    {
        ...
        {
            u8ProgramNumber = MAX_ATVPROGRAM -1;
        }
...
BYTE msAPI_ATV_ConvertOrdinalNumberToProgramNumber(WORD u16OrdinalNumber)
{
    ...
    if (u16OrdinalNumber < MAX_ATVPROGRAM)
    {
        for ( i = ATV_FIRST_PR_NUM; i < MAX_ATVPROGRAM; i++ )
        {
            if ( _IsIndexActive(i) )
...
BYTE msAPI_ATV_GetPrevProgramNumber(BYTE u8BaseProgramNumber, BOOLEAN bIncludeSkipped)
{
    ...
    do
    {
        if (u8ProgramNumber == ATV_FIRST_PR_NUM)
        {
            u8ProgramNumber = MAX_ATVPROGRAM;
        }
...
{
    if ( _u8CurrentProgramNumber >= (MAX_ATVPROGRAM - 1) )
    {
        return FALSE;
    }
...
static void _CorrectDuplication(void)
{
    BYTE u8DuplicationMap[MAX_PRTABLEMAP];
    BYTE i;
    BYTE u8PRIndex;
...
static BOOLEAN _SetPRTable(BYTE u8Index, BYTE * pu8Buffer, BYTE u8Param)
{
    ...
    if ( TRUE != _IsPREntityActive(u8PRIndex) )
    {
        ...
        if (u8NewPREntity >= MAX_ATVPROGRAM || u8NewPREntity == PR_NOTHING)
        {
            _CorrectDuplication();
...
static BOOLEAN _CopyPRTable(BYTE u8Index)
{
    ...
    if ( u8Index == (MAX_ATVPROGRAM-1) )
    {
        return TRUE;
    }

    u8PRIndex = _GetPRIndexTable((MAX_ATVPROGRAM-1));
    ...
    for ( i = (MAX_ATVPROGRAM-1); i > u8Index; i-- )
    {
...
static BOOLEAN _DeletePRTable(BYTE u8Index)
{
    ...
    for ( i = u8Index; i < MAX_ATVPROGRAM-1; i++ )
    {
        u8PRIndex = _GetPRIndexTable(i+1);

        _SetPRIndexTable(i, u8PRIndex);
    }
    _SetPRIndexTable(MAX_ATVPROGRAM-1, PR_NOTHING);
...
static BOOLEAN _MovePRTable(BYTE u8From, BYTE u8To)
{
    ...
    if ( u8From >= MAX_ATVPROGRAM || u8To >= MAX_ATVPROGRAM )
    {
        return FALSE;
    }
...
static BOOLEAN _SwapPRTable(BYTE u8Index1, BYTE u8Index2)
{
    ...
    if ( u8Index1 >= MAX_ATVPROGRAM || u8Index2 >= MAX_ATVPROGRAM )
    {
        return FALSE;
    }
